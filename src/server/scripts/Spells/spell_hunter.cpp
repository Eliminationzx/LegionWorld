...
class areatrigger_at_steel_trap : public AreaTriggerScript
{
    ...
    struct areatrigger_at_steel_trapAI : AreaTriggerAI
    {
        ...
        int32 CallSpecialFunction(uint32 /*Num*/)
        {
            return triggerTimer;
        }
            ...
            void HandleOnCast()
            {
                ...
                if (!caster->IsWithinMeleeRange(target))
                {
                    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(130392)) // Blink Strikes
                    {
                        ...
                        caster->_AddCreatureSpellCooldown(130393, time(NULL) + spellInfo->Effects[EFFECT_3]->BasePoints);
                    }
                }
            }
            void Register() override
...
class spell_hun_t19_bm : public SpellScriptLoader
{
    ...
    class spell_hun_t19_bm_SpellScript : public SpellScript
    {
        ...
        void RemoveInvalidTargets(std::list<WorldObject*>& targets)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                bool hasDireFrenzy = caster->HasSpell(DireFrenzy);
                ...
                for (auto itr : caster->m_Controlled)
                {
                    ...
                    {
                        if (uint32 createdSpellId = itr->GetUInt32Value(UNIT_FIELD_CREATED_BY_SPELL))
                        {
    ...
    public:
        ...
        class spell_hun_windburst_SpellScript : public SpellScript
        {
            ...
            void HandleAfterCast()
            {
                ...
                {
                    uint32 count = 0;
                    caster->CastSpell(caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), 223114, true);
                    ...
                    for (int i = 1; i < maxCount; i++)
                    {
                        ...
                        caster->CastSpell(x, y, z, 226872, true);
                    }
                }
            }
...
class spell_hun_flare : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_flare_AuraScript : public AuraScript
        {
            ...
            void Register() override
            {
                DoCalcMaxDuration += AuraCalcMaxDurationFn(spell_hun_flare_AuraScript::CalculateMaxDuration);
            }
        };
...
class spell_hun_trailblazer : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_trailblazer_AuraScript : public AuraScript
        {
            ...
            {
                Unit* caster = GetCaster();
                if (!caster || caster->isInCombat() || caster->HasAura(231390))
                {
                    update = 0;
                    return;
                }
                ...
                if (update >= aurEff->GetAmount())
                {
                    update = 0;
                    caster->CastSpell(caster, 231390, true);
                }
            }

            void Register() override
            {
                OnEffectUpdate += AuraEffectUpdateFn(spell_hun_trailblazer_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_DUMMY);
            }
        };
