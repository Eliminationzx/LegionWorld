        ...
        class spell_monk_zen_flight_check_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckTarget()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    {
                        case 571:
                            if (!_player->HasSpell(54197))
                                canFly = false;
                            break;
                        case 870:
                            if (!_player->HasSpell(115913))
                                canFly = false;
                            break;
                        case 1116:
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (caster->HasAura(196738))  // Elusive Dance
                    {
                        ...
                        if (caster->HasAura(124273))
                            basepoints = 15;
                        if (basepoints)
                            caster->CastCustomSpell(caster, 196739, &basepoints, NULL, NULL, true);
                    }
                    ...
                    if (AuraEffect const* aurEff = caster->GetAuraEffect(124255, EFFECT_1))
                    {
                        if (AuraEffect const* gaiPlins = caster->GetAuraEffect(208837, EFFECT_0)) // Gai Plin's Imperial Brew
                        {
                            ...
                            caster->CastCustomSpell(caster, 208840, &bp0, NULL, NULL, true);
                        }
                        ...
                        if (AuraEffect const* hotTrub = caster->GetAuraEffect(202126, EFFECT_0)) // Hot Trub (Honor Talent)
                        {
                            ...
                            caster->CastCustomSpell(caster, 202127, &bp0, NULL, NULL, true);
                        }
                    }
                    ...
                    if (AuraEffect* aurEff2 = caster->GetAuraEffect(238129, EFFECT_2)) // Quick Sip
                    {
                        if (Aura* aura = caster->GetAura(215479))
                            aura->SetDuration(aura->GetDuration() + aurEff2->GetAmount() * IN_MILLISECONDS);
                        else
                            caster->CastSpellDuration(caster, 215479, true, aurEff2->GetAmount() * IN_MILLISECONDS);
                    }
                }
            }
...
// Ironskin Brew - 115308, for aura 215479
class spell_monk_ironskin_brew_aura : public SpellScriptLoader
{
    public:
        spell_monk_ironskin_brew_aura() : SpellScriptLoader("spell_monk_ironskin_brew_aura") { }

        class spell_monk_ironskin_brew_aura_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_monk_ironskin_brew_aura_SpellScript);
            ...
            void HandleOnHit()
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (Aura* aura = caster->GetAura(215479))
                        aura->SetDuration(aura->GetDuration() + aura->GetMaxDuration());
                    else
            ...
            void Register() override
            {
                OnHit += SpellHitFn(spell_monk_ironskin_brew_aura_SpellScript::HandleOnHit);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_monk_ironskin_brew_aura_SpellScript();
        }
};
        ...
        class spell_monk_renewing_mist_AuraScript : public AuraScript
        {
            ...
            bool JumpOnTarget(Unit* caster, Player* target, Unit* monk, AuraEffect const* aurEff)
            {
                if (!target->HasAura(119611, monk->GetGUID()))
                {
                    if (Aura* aura = GetAura())
                    {
                        if (Aura* jumpAura = monk->AddAura(119611, target))
                        {
                            jumpAura->SetMaxDuration(aura->GetDuration());
                            jumpAura->SetDuration(aura->GetDuration());
                        }
                        aura->Remove();
                    }
...
class spell_monk_sotw : public SpellScriptLoader
{
    public:
        spell_monk_sotw() : SpellScriptLoader("spell_monk_sotw") { }

        class spell_monk_sotw_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_monk_sotw_SpellScript);
            ...
            void HandleOnHit()
            {
                if (Player* _plr = GetCaster()->ToPlayer())
                {
                    if (_plr->GetSelectedUnit() != GetHitUnit())
                    {
                        ...
                        if (count) // custom blizz hotfix 16.09.2016
                            if (count >= 10)
                                damage = damage / count;
                            else if (count >= 5)
                                damage = damage * 0.2f;
            ...
            void Register() override
            {
                OnHit += SpellHitFn(spell_monk_sotw_SpellScript::HandleOnHit);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_monk_sotw_SpellScript();
        }
};
...
void AddSC_monk_spell_scripts()
{
    ...
    new spell_monk_ironskin_brew_aura();
