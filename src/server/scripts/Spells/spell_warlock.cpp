        ...
        class spell_warl_demonic_gateway_cast_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                if(Unit* caster = GetCaster())
                {
                    ...
                    {
                        if (caster->GetMap()->GetId() == 529 && caster->GetAreaId() == 3424)
                        {
                            if ((pos->GetPositionX() <= 1199.0f && pos->GetPositionY() <= 1199.0f && pos->GetPositionZ() >= -51.0f) || (pos->GetPositionZ() >= 13.9f))
                                return SPELL_FAILED_NOT_HERE;
                        }
                        else if (caster->GetMap()->GetId() == 529 && caster->GetAreaId() == 3422)
        ...
        class spell_warl_hand_of_guldan_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* caster = GetCaster())
                    if (Unit* target = GetHitUnit())
                    {
                        caster->CastCustomSpell(target, 86040, &bp, NULL, NULL, true);
                        caster->CastCustomSpell(target, 104317, &bp, NULL, NULL, true);
                    }
            }
...
class spell_warl_imp_firebolt_basic : public SpellScriptLoader
{
    public:
        spell_warl_imp_firebolt_basic() : SpellScriptLoader("spell_warl_imp_firebolt_basic") { }

        class spell_warl_imp_firebolt_basic_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_warl_imp_firebolt_basic_SpellScript);
            ...
            void Register() override
            {
                OnEffectHitTarget += SpellEffectFn(spell_warl_imp_firebolt_basic_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_warl_imp_firebolt_basic_SpellScript();
        }
};
    ...
    public:
        ...
        class spell_warl_demonic_empowerment_AuraScript : public AuraScript
        {
            ...
            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                AuraEffect* aurEff = caster->GetAuraEffect(238145, EFFECT_0); // Thal'kiel's Ascendance
                if (!aurEff || !roll_chance_i(aurEff->GetAmount()))
                    return;
            ...
            void Register() override
            {
                AfterEffectApply += AuraEffectApplyFn(spell_warl_demonic_empowerment_AuraScript::OnApply, EFFECT_1, SPELL_AURA_MELEE_SLOW, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            }
        };
...
class spell_warl_soulshatter : public SpellScriptLoader
{
    public:
        ...
        class spell_warl_soulshatter_SpellScript : public SpellScript
        {
            ...
            {
                if (Unit* target = GetHitUnit())
                    SetHitDamage(target->CountPctFromMaxHealth(GetSpellInfo()->Effects[EFFECT_1]->CalcValue(GetCaster())));
            }

            void Register() override
            {
                OnEffectLaunchTarget += SpellEffectFn(spell_warl_soulshatter_SpellScript::HandleOnHit, EFFECT_1, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };
...
class spell_warl_incinerate : public SpellScriptLoader
{
    public:
        ...
        class spell_warl_incinerate_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                OnEffectLaunchTarget += SpellEffectFn(spell_warl_incinerate_SpellScript::HandleOnHit, EFFECT_1, SPELL_EFFECT_SCHOOL_DAMAGE);
                OnEffectHitTarget += SpellEffectFn(spell_warl_incinerate_SpellScript::HandleHit, EFFECT_1, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };
...
void AddSC_warlock_spell_scripts()
{
    ...
    new spell_warl_imp_firebolt_basic();
