...
class spell_dh_blade_dance : public SpellScriptLoader
{
    public:
        ...
        class spell_dh_blade_dance_SpellScript : public SpellScript
        {
                ...
                if (AuraEffect* aurEf = caster->GetAuraEffect(206416, EFFECT_0)) // First Blood
                {
                    if (Unit* target = GetHitUnit())
                    {
                        if (target->GetGUID() == caster->GetSelection())
                            SetHitDamage(GetHitDamage() + int32(CalculatePct(GetHitDamage(), aurEf->GetAmount())));
                    }
                }
            }
            ...
            void Register() override
            {
                OnHit += SpellHitFn(spell_dh_blade_dance_SpellScript::HandleOnHit);
            }
        };
            ...
            if (Unit* caster = GetCaster())
            {
                if (WorldLocation const* loc = GetExplTargetDest())
                {
                    switch (GetSpellInfo()->Id)
                    {
                        ...
                        case 209980:
                        {
                            caster->CastSpell(loc, caster->HasAura(212613) ? 226263 : 226370, true);
                            return;
                        }
                        case 209651:
                        {
                            caster->CastSpell(loc, caster->HasAura(212613) ? 226264 : 226259, true);
                            return;
                        }
                    }
                }
...
class areatrigger_rage_of_the_illidari : public AreaTriggerScript
{
    ...
    struct areatrigger_rage_of_the_illidariAI : AreaTriggerAI
    {
        ...
        int32 CallSpecialFunction(uint32 damage) override
        {
            savedDamage += damage;
            return 0;
        }
