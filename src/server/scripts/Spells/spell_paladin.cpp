    ...
    public:
        ...
        class spell_pal_holy_shield_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, float& absorbAmount)
            {
                if (Unit* caster = GetCaster())
                {
                    absorbAmount = dmgInfo.GetDamage() * 0.4f; // when blocking is absorbed 40% dmg
        ...
        class spell_pal_divine_intervention_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* /*AuraEffect**/, DamageInfo& dmgInfo, float& absorbAmount)
            {
                ...
                if (Unit* target = GetTarget())
                {
                    ...
                    if (target->ToPlayer()->HasSpellCooldown(642) || target->HasAura(SPELL_FORBEARANCE))
                        return;
...
class spell_pal_last_defender : public SpellScriptLoader
{
    public:
        ...
        class spell_pal_last_defender_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_pal_last_defender_AuraScript);
            ...
            void CalculateAmountAbsorb(AuraEffect const* aurEff, float & amount, bool & canBeRecalculated)
            {
                // Set absorbtion amount to unlimited
                amount = -1;
                absorbPct = int32((1 - pow(0.97, GetAura()->GetCustomData())) * 100);
            }

            void CalculateAmount(AuraEffect const* aurEff, float & amount, bool & canBeRecalculated)
            {
                amount = int32((1 - pow(0.97, GetAura()->GetCustomData())) * 100);
            }
            ...
            void Register() override
            {
                 DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_last_defender_AuraScript::CalculateAmountAbsorb, EFFECT_3, SPELL_AURA_SCHOOL_ABSORB);
                 DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_last_defender_AuraScript::CalculateAmount, EFFECT_4, SPELL_AURA_MOD_DAMAGE_DONE);
                 OnEffectAbsorb += AuraEffectAbsorbFn(spell_pal_last_defender_AuraScript::Absorb, EFFECT_3, SPELL_AURA_SCHOOL_ABSORB);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_pal_last_defender_AuraScript();
        }
};
...
class spell_pal_devotion_aura : public SpellScriptLoader
{
    public:
        ...
        class spell_pal_devotion_aura_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_pal_devotion_aura_AuraScript);
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, float & amount, bool & canBeRecalculated)
            {
                ...
                if (!caster->HasAura(31821) && GetAura()->GetCustomData())
                    amount = RoundingFloatValue(amount / GetAura()->GetCustomData());

                if (ObjectGuid areaTriggerGuid = GetAura()->GetSpellAreaTrigger())
                    if(AreaTrigger* areaTrigger = ObjectAccessor::GetAreaTrigger(*caster, areaTriggerGuid))
                    {
                        ...
                        {
                            if(Unit* unit = ObjectAccessor::GetUnit(*caster, *iter))
                                if (AuraEffect* aurEff = unit->GetAuraEffect(210320, EFFECT_0))
            ...
            void Register() override
            {
                 DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_devotion_aura_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_CREATE_AREATRIGGER);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_pal_devotion_aura_AuraScript();
        }
};
    ...
    public:
        ...
        class spell_pal_judgment_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (AuraEffect* aurEff = caster->GetAuraEffect(231657, EFFECT_0))
                {
...
class spell_pal_blessing_of_the_ashbringer : public SpellScriptLoader
{
    ...
    class spell_pal_blessing_of_the_ashbringer_AuraScript : public AuraScript
    {
        ...
        int32 CallSpecialFunction(uint32 /*Num*/) override
        {
            if (!timer)
                timer = BOTATimer;
    ...
    public:
        ...
        class spell_pal_blessing_of_sacrifice_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* aurEff, DamageInfo & dmgInfo, float & absorbAmount)
            {
                ...
                if (Unit* caster = GetCaster())
                    caster->CastCustomSpell(caster, 199450, &bp0, NULL, NULL, true, NULL, aurEff);
            }
        ...
        class spell_pal_unbreakable_will_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    caster->GetAuraEffectsByMechanic(1100379686, mechanicAuras);
            ...
            void Register() override
            {
                OnEffectRemove += AuraEffectRemoveFn(spell_pal_unbreakable_will_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };
    ...
    public:
        ...
        class spell_pal_the_light_saves_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                Unit* target = GetHitUnit();
                Unit* caster = GetCaster();
                if(!target || !caster || target->HasAura(211426))
                    return;

                if (AuraEffect const* aurEff = target->GetAuraEffect(200423, EFFECT_0)) // The Light Saves
                {
                    SetHitHeal(GetHitHeal() + CalculatePct(GetHitHeal(), aurEff->GetAmount()));
                    target->CastSpell(target, 211426, true);
                    aurEff->GetBase()->Remove();
                }
                else if (AuraEffect const* aurEff = caster->GetAuraEffect(200421, EFFECT_0)) // The Light Saves
                {
                    if (target->HealthBelowPct(aurEff->GetAmount()))
                        if(target->HasAura(53563, caster->GetGUID()) || target->HasAura(156910, caster->GetGUID()) || target->HasAura(200025, caster->GetGUID()))
                            target->CastSpell(target, 200423, true);
                }
            }
