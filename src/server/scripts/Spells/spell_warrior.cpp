    ...
    public:
        ...
        class spell_warr_ignore_pain_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* aurEff, float & amount, bool & /*canBeRecalculated*/)
            {
                ...
                if (GetCaster()->HasAura(202574))
                    powerCost *= 2;

                int32 percAdd = (powerCost - 200) * 0.1675f + 33.0f;
                if (Player* plr = GetCaster()->ToPlayer())
                {
                    int32 absorbData = int32(GetSpellInfo()->Effects[EFFECT_0]->BonusCoefficientFromAP * plr->GetTotalAttackPowerValue(BASE_ATTACK));

                    if (plr->HasAura(202561)) // Never Surrender
                        percAdd += (100 - int32(plr->GetHealthPct()));
                    if (AuraEffect* aurEf = plr->GetAuraEffect(203581, EFFECT_0)) // Dragon Scales
                    {
                        percAdd += aurEf->GetAmount();
                        aurEf->GetBase()->Remove();
                    }
                    if (AuraEffect* aurEf = plr->GetAuraEffect(202095, EFFECT_1)) // Indomitable
                        percAdd += aurEf->GetAmount();

                    if (AuraEffect* aurEf3 = plr->GetAuraEffect(254339, EFFECT_0)) // Item - Warrior T21 Protection 4P Bonus - Blocked Attacks Buff Ignore Pain
                    {
                        absorbData += aurEf3->GetAmount();
                        aurEf3->GetBase()->Remove();
                    }

                    amount += CalculatePct(absorbData, percAdd);
                    amount += aurEff->GetOldBaseAmount();

                    if (amount > plr->GetMaxHealth())
                        amount = plr->GetMaxHealth();
                }
            }

            void Absorb(AuraEffect* /*AuraEffect**/, DamageInfo& dmgInfo, float& absorbAmount)
            {
                absorbAmount = CalculatePct(dmgInfo.GetDamage(), GetSpellInfo()->Effects[EFFECT_1]->CalcValue(GetCaster()));
            }

            void Register() override
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warr_ignore_pain_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
                OnEffectAbsorb += AuraEffectAbsorbFn(spell_warr_ignore_pain_AuraScript::Absorb, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
            }
        };
...
class spell_war_t21_prot_4p : public SpellScriptLoader
{
    public:
        spell_war_t21_prot_4p() : SpellScriptLoader("spell_war_t21_prot_4p") { }

        class spell_war_t21_prot_4p_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_war_t21_prot_4p_AuraScript);
            ...
            void Register() override
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_war_t21_prot_4p_AuraScript::CalculateDummy, EFFECT_0, SPELL_AURA_DUMMY);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_war_t21_prot_4p_AuraScript();
        }
};
