    ...
    public:
        ...
        struct boss_murozondAI : public BossAI
        {
            ...
            void CompleteEncounter()
            {
                if (instance)
                {
                    ...
                    if (auto pGo = ObjectAccessor::GetGameObject(*me, instance->GetGuidData(DATA_HOURGLASS)))
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_NOZDORMU_INTRO:
                            if (auto pNozdormu = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_NOZDORMU)))
                        ...
                        case EVENT_DESPAWN:
                            ...
                            instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_SANDS_OF_THE_HOURGLASS);
                            if (auto pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
                                pTarget->Kill(me);
                            break;
                        default:
                            break;
        ...
        private:
            ...
            void ActivateMirrors()
            {
                ...
                {
                    if (auto pMirror = (*itr)->ToCreature())
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_NOZDORMU_OUTRO_4:
                            if (auto pGo = me->FindNearestGameObject(HOURGLASS_OF_TIME, 300.0f))
...
class go_murozond_hourglass_of_time : public GameObjectScript
{
    public:
        ...
        bool OnGossipHello(Player* pPlayer, GameObject* pGo)
        {
            ...
            if (auto pMurozond = ObjectAccessor::GetCreature(*pGo, instance->GetGuidData(DATA_MUROZOND)))
            {
