        ...
        struct boss_echo_of_tyrandeAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (instance->GetData(DATA_TYRANDE_EVENT) == DONE)
                {
                    ...
                    if (auto pStalker = me->SummonCreature(NPC_STALKER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f))
                    {
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            if (auto pPlayer = me->FindNearestPlayer(500.0f))
                            {
                                Position pos;
                                ...
                                if (auto pCreature = me->SummonCreature(entry, pos))
            ...
            void MovementInform(uint32 type, uint32 data)
            {
                ...
                if (data == POINT_MOONLANCE)
                {
                    if (me->GetEntry() == NPC_MOONLANCE_1)
                    {
                        ...
                        if (auto pLance1 = me->SummonCreature(NPC_MOONLANCE_2_1, pos1_1, TEMPSUMMON_TIMED_DESPAWN, 30000))
                            pLance1->GetMotionMaster()->MovePoint(POINT_MOONLANCE, pos1_2);
                        if (auto pLance2 = me->SummonCreature(NPC_MOONLANCE_2_2, pos2_1, TEMPSUMMON_TIMED_DESPAWN, 30000))
                            pLance2->GetMotionMaster()->MovePoint(POINT_MOONLANCE, pos2_2);
                        if (auto pLance3 = me->SummonCreature(NPC_MOONLANCE_2_3, pos3_1, TEMPSUMMON_TIMED_DESPAWN, 30000))
    ...
    public:
        ...
        bool OnTrigger(Player* pPlayer, const AreaTriggerEntry* /*pAt*/, bool /*enter*/)
        {
            ...
            if (InstanceScript* instance = pPlayer->GetInstanceScript())
            {
                ...
                {
                    if (auto pTyrande = ObjectAccessor::GetCreature(*pPlayer, instance->GetGuidData(DATA_ECHO_OF_TYRANDE)))
                    {
