                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SPAWN_GHOUL:
                        {
                            ...
                            for (uint8 i = 1; i < 8; ++i)
                            {
                                if (auto pGhoul = me->SummonCreature(NPC_RISEN_GHOUL, ghoulPos[i]))
                                {
                                    if (_prev)
                                    {
    ...
    public:
        ...
        struct npc_echo_of_sylvanas_risen_ghoulAI : public Scripted_NoMovementAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                if (auto pTarget = ObjectAccessor::GetCreature(*me, _guid))
                    pTarget->RemoveAura(SPELL_WRACKING_PAIN_ANY);
                _guid.Clear();
                if (auto pSylvanas = me->FindNearestCreature(NPC_ECHO_OF_SYLVANAS, 300.0f))
