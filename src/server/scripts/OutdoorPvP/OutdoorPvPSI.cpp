...
void OutdoorPvPSI::HandlePlayerEnterZone(ObjectGuid guid, uint32 zone)
{
...
void OutdoorPvPSI::HandlePlayerLeaveZone(ObjectGuid guid, uint32 zone)
{
    Player* player = ObjectAccessor::GetObjectInMap(guid, m_map, (Player*)nullptr);
    if (!player)
        return;
        ...
        case ALLIANCE:
            {
                ...
                if (atEntry)
                {
                    ...
                    if (player->GetDistance(atEntry->Pos.X, atEntry->Pos.Y, atEntry->Pos.Z) > 5.0f + atEntry->Radius)
                    {
                        ...
                        if (!go->Create(sObjectMgr->GetGenerator<HighGuid::GameObject>()->Generate(), SI_SILITHYST_MOUND, map, player->GetPhaseMask(), *player, G3D::Quat(), 100, GO_STATE_READY))
                        {
                            delete go;
                            return true;
                        }
