...
void OutdoorPvPRG::HandleKill(Player* killer, Unit* killed)
{
    ...
    if (!killed->IsPlayer() && killed->GetEntry() != 111136)
        return;
    ...
    if (killed->GetEntry() == 111136)
        m_canSummon = true;

    // killed->CastSpell(killed, 218880, true);
}
...
bool OutdoorPvPRG::Update(uint32 diff)
{
    ...
    if (!m_playersInArea.empty() && m_playersInArea.size() < 3)
    {
        if (m_summonTimer >= 30000)
        {
            if (TempSummon* summon = m_map->SummonCreature(111136, summon_pos[urand(0, 2)], NULL, 20000))
            {
