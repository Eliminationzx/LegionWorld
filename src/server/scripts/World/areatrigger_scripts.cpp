...
class areatrigger_at_test_script : public AreaTriggerScript
{
    public:
        areatrigger_at_test_script() : AreaTriggerScript("areatrigger_at_test_script") { }

    struct areatrigger_at_test_scriptAI : AreaTriggerAI
    {
        InstanceScript* _instance;

        areatrigger_at_test_scriptAI(AreaTrigger* areatrigger) : AreaTriggerAI(areatrigger)
        {
            _instance = areatrigger->GetInstanceScript();
        }
        ...
        {
            startPos = *pos;
            float angle = float(_instance ? _instance->GetData(1) : M_PI/2);
            float x = 0.0f;
            float y = 0.0f;
            at->GetNearPoint2D(x, y, at->_range, angle);
            endPos.m_positionX = x;
            endPos.m_positionY = y;
            return false;
        }
    };
    ...
    {
        return new areatrigger_at_test_scriptAI(areatrigger);
    }
};
...
void AddSC_areatrigger_scripts()
{
    ...
    new areatrigger_at_test_script();
