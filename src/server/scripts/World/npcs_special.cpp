...
public:
    ...
    struct npc_storm_earth_and_fireAI : ScriptedAI
    {
        ...
        void ComonOnHome() override
        {
            ...
            switch (me->GetEntry())
            {
                case 69792: num = 3; break; // Earth
                case 69791: num = 4; break; // Fire
                default:
                    return;
            }
            ...
            if (me->isAlive())
            {
                me->CastSpell(owner, 196860, true);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!comeonhome)
            {
                ...
                canSeeCheck += diff;
                if (canSeeCheck >= 1000 && me->getVictim())
                {
                    if (!me->canSeeOrDetect(me->getVictim()) && !comeonhome)
                    {
                        ComonOnHome();
                        return;
                    }
...
public:
    ...
    struct npc_slg_generic_mop_large_aoiAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (!validMap || !who || !who->IsPlayer() || me->GetDistance(who) > 5.0f)
                return;

            switch (me->GetAreaId())
            {
                case 7384: //Soul Area
                    uint8 DATA_ARCHMAGE_SOL = 3;
