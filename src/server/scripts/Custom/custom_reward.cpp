...
public:
    ...
    uint32 ItemDel(Item* _item, Player* player, uint32 count)
    {
        ...
        if(!result)
        {
            ...
            chH.PSendSysMessage(20021, sObjectMgr->GetItemTemplate(_item->GetEntry())->Name->Str[player->GetSession()->GetSessionDbLocaleIndex()]);
        }
        return count;
    }
    ...
    void ItemIfExistDel(Item* _item, Player* player, int16 slot, SQLTransaction& trans)
    {
        ...
        {
            if (proto->Quality == ITEM_QUALITY_LEGENDARY && proto->RequiredExpansion == EXPANSION_LEGION)
                needDelete = true;
        ...
        if (needDelete)
        {
            ...
            {
                ChatHandler chH = ChatHandler(player);
                chH.PSendSysMessage(20020, sObjectMgr->GetItemTemplate(_item->GetEntry())->Name->Str[player->GetSession()->GetSessionDbLocaleIndex()]);
