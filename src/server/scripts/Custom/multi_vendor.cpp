                ...
                switch(action)
                {
                    case CHANGE_FACTION: // 64
                    {
                        ...
                        if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                        {
                            if (player->DestroyDonateTokenCount(priceFaction))
                            {
                                ...
                                sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u %s", SERVICE_CHANGE_FACTION,  priceFaction, player->GetInfoForDonate().c_str());
                            }
                            else
                            {
                                player->CLOSE_GOSSIP_MENU();
                    ...
                    case CHANGE_RACE: // 128
                    {
                        ...
                        if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                        {
                            if (player->DestroyDonateTokenCount(priceRace))
                            {
                                ...
                                trans->Append(stmt);
                                LoginDatabase.CommitTransaction(trans);
                                sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u %s", SERVICE_CHANGE_RACE,  priceRace, player->GetInfoForDonate().c_str());
                            }
                            else
                            {
                                player->CLOSE_GOSSIP_MENU();
                    ...
                    case CHANGE_GENDER: // 8
                    {
                        ...
                        if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                        {
                            if (player->DestroyDonateTokenCount(priceGender))
                            {
                                ...
                                trans->Append(stmt);
                                LoginDatabase.CommitTransaction(trans);
                                sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u %s", SERVICE_CHANGE_GENDER,  priceGender, player->GetInfoForDonate().c_str());
                            }
                            else
                            {
                                player->CLOSE_GOSSIP_MENU();
                    ...
                    case CHANGE_NAME: // 1
                    {
                        ...
                        { 
                            if (player->DestroyDonateTokenCount(priceName))
                            {
                                ...
                                trans->Append(stmt);
                                LoginDatabase.CommitTransaction(trans);
                                sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u %s", SERVICE_CHANGE_NAME,  priceName, player->GetInfoForDonate().c_str());
                            }
                            else
                            {
                                player->CLOSE_GOSSIP_MENU();
                    ...
                    case ADD_BONUS:
                    {
                        ...
                        if (sObjectMgr->GetPricesForAddBonus())
                        {
                            ...
                            ItemTemplate const* pProto = sObjectMgr->GetItemTemplate((pItem ? pItem->GetEntry() : NULL));
                            for (auto itr = price->begin(); itr != price->end(); ++itr)
                            {
                                ...
                                switch (bonus)
                                {
                                    case 42: // Скорость передвижения
                                        ...
                                        (pProto ? pProto->Name->Str[localeConstant] : "Item not found"));
                            ...
                            if (pItem)
                            {
                                ...
                                {
                                    //char namebonus[500], question[500];
                                    sprintf(namebonus, sObjectMgr->GetTrinityString(20070, localeConstant), (pProto ? pProto->Name->Str[localeConstant] : "Item not found"));
                                    player->ADD_GOSSIP_ITEM(1, namebonus, GOSSIP_SENDER_MAIN, 3001); // exit
                                    sprintf(question, sObjectMgr->GetTrinityString(20071, localeConstant),	(pProto ? pProto->Name->Str[localeConstant] : "Item not found"));
                                    do
                                    {
                    ...
                    case 3006:
                    {
                        ...
                        {
                            if (proto->Class != ITEM_CLASS_ARMOR || proto->Quality == ITEM_QUALITY_LEGENDARY)
                            {
                                player->CLOSE_GOSSIP_MENU();
                                player->SendEquipError(EQUIP_ERR_NOT_EQUIPPABLE);
                                return false;
                            }
                            if (proto->RequiredLevel < 101)
                            {
                                player->CLOSE_GOSSIP_MENU();
                        ...
                        if (player->HasDonateToken(tcost))
                        {
                            if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                            {
                                if (player->DestroyDonateTokenCount(tcost))
                                {
                                    ...
                                    trans->Append(stmt);
                                    LoginDatabase.CommitTransaction(trans);
                                    sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u, itemguid = %u, itemid = %u, bonus = %u,  %s", SERVICE_SELL_ADD_BONUS,  tcost, pItem->GetGUIDLow(), pItem->GetEntry(), IdBonus, player->GetInfoForDonate().c_str());
                                }
                                else
                                {
                                    player->CLOSE_GOSSIP_MENU();
                    ...
                    case RENAME_GUILD:
                    {
                        ...
                        if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                        {
                            if (player->DestroyDonateTokenCount(cost))
                            {
                                ...
                                trans->Append(stmt);
                                LoginDatabase.CommitTransaction(trans);
                                sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u, buy_levels = %u,  %s", SERVICE_GUILD_RENAME, cost, 1, player->GetInfoForDonate().c_str());
                ...
                if (player->HasDonateToken(efirendu))
                {
                    if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                    {
                        if (player->DestroyDonateTokenCount(efirendu))
                        {
                            ...
                            trans->Append(stmt);
                            LoginDatabase.CommitTransaction(trans);
                            sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u, buy_levels = %u,  %s", SERVICE_LEVEL_UP, efirendu, (ucode - level), player->GetInfoForDonate().c_str());
                            //  LoginDatabase.PExecute("INSERT INTO `store_history` (`realm`, `account`, `bnet_account`, `char_guid`, `char_level`, `product`, `count`, `token`) VALUES (%u, %u, %u, %u, %u, %u, %u, %u);", realm.Id.Realm, player->GetSession()->GetAccountId(), player->GetSession()->GetBattlenetAccountId(), player->GetGUID().GetCounter(), level, -1, ucode, efirendu);
                        }
                        else
                        {
            ...
            if (player->HasDonateToken(tokens))
            {
                if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                {
                    if (player->DestroyDonateTokenCount(tokens))
                    {
                        ...
                        trans->Append(stmt);
                        LoginDatabase.CommitTransaction(trans);
                        sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost =  %u, buy_golds = " SI64FMTD ",  %s", SERVICE_SELL_GOLD, tokens, ucode * 1000, player->GetInfoForDonate().c_str());
                    }
                    else
                    {
                        player->CLOSE_GOSSIP_MENU();
                    ...
                    if (player->HasDonateToken(efirendu))
                    {
                        if (!sWorld->getBoolConfig(CONFIG_DONATE_ON_TESTS))
                        {
                            if (player->DestroyDonateTokenCount(efirendu))
                            {
                                ...
                                trans->Append(stmt);
                                LoginDatabase.CommitTransaction(trans);
                                sLog->outDebug(LOG_FILTER_DONATE, "[Service] Type: %d, cost = %u, buy_art_levels = %u,  %s, art = %s", SERVICE_SELL_ART_XP, efirendu, countbuy, player->GetInfoForDonate().c_str(), outArt);
                            }
                            else
                            {
                                player->CLOSE_GOSSIP_MENU();
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)  override
    {
        QueryResult result = LoginDatabase.PQuery("SELECT `s_h`.`item_guid`, `s_h`.`item`, `s_h`.`count`, `s_h`.`token` FROM `store_history` AS s_h JOIN `store_product_realms` AS s_p_r ON `s_p_r`.`product` = `s_h`.`product` JOIN `store_products` AS s_p ON `s_p`.`id` = `s_h`.`product` JOIN `store_category_realms` AS s_c_r ON `s_c_r`.`category` = `s_p`.`category` WHERE `s_h`.`char_guid` = '%u' AND `s_h`.`status` IN ('0', '6') AND `s_h`.`item` > '0' AND `s_h`.`realm` = '%u' AND (`s_c_r`.`return` = '1' OR `s_p_r`.`return` = '1') AND `s_p_r`.`realm` = '%u' AND `s_c_r`.`realm` = '%u'", player->GetGUIDLow(), realm.Id.Realm, realm.Id.Realm, realm.Id.Realm);
        if (!result)
        {
            LocaleConstant localeConstant = player->GetSession()->GetSessionDbLocaleIndex();
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    char printinfo[500];
                    sprintf(printinfo, sObjectMgr->GetTrinityString(20043, localeConstant), pProto->Name->Str[localeConstant], token);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        if (action > 0)
        {
            QueryResult result = LoginDatabase.PQuery("SELECT `s_h`.`char_guid`, `s_h`.`item`, `s_h`.`token`, `s_h`.`count` FROM `store_history` AS s_h JOIN `store_product_realms` AS s_p_r ON `s_p_r`.`product` = `s_h`.`product` JOIN `store_products` AS s_p ON `s_p`.`id` = `s_h`.`product` JOIN `store_category_realms` AS s_c_r ON `s_c_r`.`category` = `s_p`.`category` WHERE `s_h`.`item_guid` = '%u' AND `s_h`.`status` IN ('0', '6') AND `s_h`.`realm` = '%u' AND`s_h`.`item` > 0 AND (`s_c_r`.`return` = '1' OR `s_p_r`.`return` = '1') AND `s_p_r`.`realm` = '%u' AND `s_c_r`.`realm` = '%u'", action, realm.Id.Realm, realm.Id.Realm, realm.Id.Realm);
            if (!result)
            {
            ...
            if (Item *item = GetItemByGuid(ObjectGuid::Create<HighGuid::Item>(action), player))
            {
                ...
                {
                    sLog->outDebug(LOG_FILTER_DONATE, "[Status] Return item  guid = %u, entry = %u, return_tokens = %u %s", item_guid, item_entry, uint32(efircount * 0.7), player->GetInfoForDonate().c_str());
                    { // status
                    ...
                    if (player->HasDonateToken(infoAdd->cost) && player->DestroyDonateTokenCount(infoAdd->cost))
                    {
                        ...
                        switch(type)
                        {
                            case DONATE_TYPE_TITLE:
                            {
                                ...
                                sLog->outDebug(LOG_FILTER_DONATE, "[Buy] Title entry = %u, cost = %u, %s", infoAdd->action, infoAdd->cost, player->GetInfoForDonate().c_str());
                                break;
                            }
                            case DONATE_TYPE_MORPH:
                            {
                                ChatHandler(player).PSendSysMessage("You can use your morph with macro: .donate morph use %u", infoAdd->action);
                                sLog->outDebug(LOG_FILTER_DONATE, "[Buy] Morph entry = %u, cost = %u, %s", infoAdd->action, infoAdd->cost, player->GetInfoForDonate().c_str());
                                break;
                            }
                            default:
                                break;
