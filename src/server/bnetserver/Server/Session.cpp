...
void Battlenet::Session::Start()
{
    ...
    sLog->outTrace(LOG_FILTER_BATTLENET, "%s Accepted connection", GetClientInfo().c_str());
    ...
    if (result)
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "%s tries to log in using banned IP!", GetClientInfo().c_str());
        CloseSocket();
        return;
    }
...
bool Battlenet::Session::Update()
{
    ...
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s BattlenetSocket::Update() failture", GetClientInfo().c_str());
        return false;
    }
...
void Battlenet::Session::AsyncWrite(MessageBuffer* packet)
{
    if (!IsOpen())
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s Battlenet::Session::AsyncWrite !IsOpen()", GetClientInfo().c_str());
        return;
    }
...
uint32 Battlenet::Session::HandleLogon(authentication::v1::LogonRequest const* logonRequest)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "[Battlenet::LogonRequest] %s attempted to log in with game other than WoW (using %s)!", GetClientInfo().c_str(), logonRequest->program().c_str());
        return ERROR_BAD_PROGRAM;
    }*/

    if (logonRequest->platform() != "Win" && logonRequest->platform() != "Wn64" && logonRequest->platform() != "Mc64")
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "[Battlenet::LogonRequest] %s attempted to log in from an unsupported platform (using %s)!", GetClientInfo().c_str(), logonRequest->platform().c_str());
        return ERROR_BAD_PLATFORM;
    }

    if (GetLocaleByName(logonRequest->locale()) == LOCALE_enUS && logonRequest->locale() != "enUS")
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "[Battlenet::LogonRequest] %s attempted to log in with unsupported locale (using %s)!", GetClientInfo().c_str(), logonRequest->locale().c_str());
...
uint32 Battlenet::Session::HandleVerifyWebCredentials(authentication::v1::VerifyWebCredentialsRequest const* verifyWebCredentialsRequest)
{
    ...
    if (!_accountInfo)
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s Battlenet::Session::HandleVerifyWebCredentials ERROR_DENIED", GetClientInfo().c_str());
        return ERROR_DENIED;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "[Session::HandleVerifyWebCredentials] Account '%s' is locked to IP - '%s' is logging in from '%s'",
    ...
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "[Session::HandleVerifyWebCredentials] Account '%s' is not locked to ip", _accountInfo->Login.c_str());
        if (_accountInfo->LockCountry.empty() || _accountInfo->LockCountry == "00")
            sLog->outDebug(LOG_FILTER_BATTLENET, "[Session::HandleVerifyWebCredentials] Account '%s' is not locked to country", _accountInfo->Login.c_str());
        else if (!_accountInfo->LockCountry.empty() && !_ipCountry.empty())
        {
            sLog->outDebug(LOG_FILTER_BATTLENET, "[Session::HandleVerifyWebCredentials] Account '%s' is locked to country: '%s' Player country is '%s'",
    ...
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "[Session::HandleVerifyWebCredentials] Account '%s' is not activate", _accountInfo->Login.c_str());
    ...
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "%s [Session::HandleVerifyWebCredentials] Banned account %s tried to login!", GetClientInfo().c_str(), _accountInfo->Login.c_str());
        return ERROR_GAME_ACCOUNT_BANNED;
    }
    else if (_accountInfo->IsSuspended)
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "%s [Session::HandleVerifyWebCredentials] Temporarily banned account %s tried to login!", GetClientInfo().c_str(), _accountInfo->Login.c_str());
...
uint32 Battlenet::Session::HandleGetAccountState(account::v1::GetAccountStateRequest const* request, account::v1::GetAccountStateResponse* response)
{
    if (!_authed)
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s Battlenet::Session::HandleGetAccountState ERROR_DENIED", GetClientInfo().c_str());
        return ERROR_DENIED;
    }
...
uint32 Battlenet::Session::HandleProcessClientRequest(game_utilities::v1::ClientRequest const* request, game_utilities::v1::ClientResponse* response)
{
    if (!_authed)
    {
        sLog->outError(LOG_FILTER_BATTLENET, "%s sent ClientRequest with ERROR_DENIED", GetClientInfo().c_str());
        return ERROR_DENIED;
    }
    ...
    if (!command)
    {
        sLog->outError(LOG_FILTER_BATTLENET, "%s sent ClientRequest with no command.", GetClientInfo().c_str());
    ...
    {
        sLog->outError(LOG_FILTER_BATTLENET, "%s sent ClientRequest with unknown command %s.", GetClientInfo().c_str(), command->name().c_str());
...
uint32 Battlenet::Session::HandleGetAllValuesForAttribute(game_utilities::v1::GetAllValuesForAttributeRequest const* request, game_utilities::v1::GetAllValuesForAttributeResponse* response)
{
    if (!_authed)
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s Battlenet::Session::HandleGetAllValuesForAttribute ERROR_DENIED", GetClientInfo().c_str());
        return ERROR_DENIED;
    }
...
void Battlenet::Session::HandshakeHandler(boost::system::error_code const& error)
{
    if (error)
    {
        sLog->outError(LOG_FILTER_BATTLENET, "%s SSL Handshake failed %s", GetClientInfo().c_str(), error.message().c_str());
        CloseSocket();
        return;
    }
...
inline bool PartialProcessPacket(Battlenet::Session* session, MessageBuffer& inputBuffer)
{
    ...
    if (buffer.GetRemainingSpace() > 0)
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s PartialProcessPacket", session->GetClientInfo().c_str());
    ...
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s PartialProcessPacket processMethod", session->GetClientInfo().c_str());
        session->CloseSocket();
        return false;
    }
...
void Battlenet::Session::ReadHandler()
{
    if (!IsOpen())
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "%s ReadHandler() !IsOpen()", GetClientInfo().c_str());
        return;
    }
