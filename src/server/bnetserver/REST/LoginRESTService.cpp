...
bool LoginRESTService::Start(boost::asio::io_service& ioService)
{
    ...
    {
        sLog->outError(LOG_FILTER_BATTLENET, "REST Specified login service port (%d) out of allowed range (1-65535), defaulting to 8081", _port);
        _port = 8081;
    }
    ...
    if (endPoint == end || ec)
    {
        sLog->outError(LOG_FILTER_BATTLENET, "REST Could not resolve LoginREST.ExternalAddress %s", configuredAddress.c_str());
        return false;
    }
...
void LoginRESTService::Run()
{
    ...
    {
        sLog->outError(LOG_FILTER_BATTLENET, "REST Couldn't bind to %s:%d", _bindIP.c_str(), _port);
        return;
    }

    sLog->outInfo(LOG_FILTER_BATTLENET, "REST Login service bound to http://%s:%d", _bindIP.c_str(), _port);
    ...
    while (!_stopped)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_BATTLENET, "REST Failed SSL handshake from IP=%s", address.to_string().c_str());
            continue;
        }

        sLog->outDebug(LOG_FILTER_BATTLENET, "REST Accepted connection from IP=%s", address.to_string().c_str());
    ...
    sLog->outInfo(LOG_FILTER_BATTLENET, "REST Login service exiting...");
}
...
int32 LoginRESTService::HandleGet(soap* soapClient)
{
    ...
    sLog->outDebug(LOG_FILTER_BATTLENET, "REST [%s:%d] Handling GET request path=\"%s\"", ip_address.c_str(), soapClient->port, soapClient->path);

    static std::string const expectedPath = "/bnetserver/login/";
    if (strstr(soapClient->path, expectedPath.c_str()) != &soapClient->path[0])
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "REST [%s:%d] Handling GET 404", ip_address.c_str(), soapClient->port);
        return 404;
    }
...
int32 LoginRESTService::HandlePost(soap* soapClient)
{
    ...
    sLog->outDebug(LOG_FILTER_BATTLENET, "REST [%s:%d] Handling POST request path=\"%s\"", ip_address.c_str(), soapClient->port, soapClient->path);

    static std::string const expectedPath = "/bnetserver/login/";
    if (strstr(soapClient->path, expectedPath.c_str()) != &soapClient->path[0])
    {
        sLog->outDebug(LOG_FILTER_BATTLENET, "REST [%s:%d] Handling POST 404", ip_address.c_str(), soapClient->port);
        return 404;
    }
    ...
    if (!JSON::Deserialize(buf, &loginForm))
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_BATTLENET, "REST [%s:%d] Handling POST 500", ip_address.c_str(), soapClient->port);
            return 500;
        }
...
void LoginRESTService::CleanupLoginTickets(boost::system::error_code const& error)
{
    if (error)
    {
        sLog->outTrace(LOG_FILTER_BATTLENET, "REST LoginRESTService::CleanupLoginTickets error %s", error.message().c_str());
        return;
    }
