...
int main(int argc, char** argv)
{
    ...
    sLog->outInfo(LOG_FILTER_BATTLENET, "%s (bnetserver)", GitRevision::GetFullVersion());
    ...
    {
        sLog->outError(LOG_FILTER_BATTLENET, "Failed to initialize SSL context");
        return 1;
    }
    ...
    if (!sLoginService.Start(_ioService))
    {
        ...
        // delete _ioService;
        ShutdownThreadPool(threadPool);
        sLog->outError(LOG_FILTER_BATTLENET, "Failed to initialize login service");
        return 1;
    }
    ...
    sLog->outInfo(LOG_FILTER_BATTLENET, "Battlenet::Initialized");
