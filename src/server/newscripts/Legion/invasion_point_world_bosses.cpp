...
struct boss_mistress_alluradel : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.ScheduleEvent(EVENT_HEART_BREAKER, 4000);
        events.ScheduleEvent(EVENT_FEL_LASH, 15000);
        events.ScheduleEvent(EVENT_BEGUILING_CHARM, 29000);
        events.ScheduleEvent(EVENT_PHANTASM, 33000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_HEART_BREAKER:
                ...
                events.ScheduleEvent(EVENT_HEART_BREAKER, 22000);
                break;
            case EVENT_FEL_LASH:
                DoCast(SPELL_FEL_LASH);
                events.ScheduleEvent(EVENT_FEL_LASH, 32000);
                break;
            case EVENT_BEGUILING_CHARM:
                ZoneTalk(1);
                DoCast(SPELL_BEGUILING_CHARM);
                events.ScheduleEvent(EVENT_BEGUILING_CHARM, 40000);
                break;
            default:
                break;
            }
...
struct boss_pit_lor_vilemus : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.ScheduleEvent(EVENT_FEL_BREATH, 20000);
        events.ScheduleEvent(EVENT_STOMP, 13000);
        events.ScheduleEvent(EVENT_DRAIN, 22000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_FEL_BREATH:
                ...
                events.ScheduleEvent(EVENT_FEL_BREATH, 19000);
                break;
            case EVENT_STOMP:
                DoCast(SPELL_STOMP);
                events.ScheduleEvent(EVENT_STOMP, 19000);
                break;
            case EVENT_DRAIN:
                DoCast(SPELL_DRAIN);
                events.ScheduleEvent(EVENT_DRAIN, 15400);
                break;
            default:
                break;
            }
...
struct boss_occularus : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.ScheduleEvent(EVENT_SEARING_GAZE, 7000);
        events.ScheduleEvent(EVENT_GUSHING_WOUND, 10000);
        events.ScheduleEvent(EVENT_LASH, 45500);
        events.ScheduleEvent(EVENT_EYE_SORE, 24000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SEARING_GAZE:
                ...
                events.ScheduleEvent(EVENT_SEARING_GAZE, 10000);
                break;
            case EVENT_GUSHING_WOUND:
                if (auto target = me->getVictim())
                    DoCast(target, SPELL_GUSHING_WOUND, false);
                events.ScheduleEvent(EVENT_GUSHING_WOUND, 11000);
            case EVENT_LASH:
                if (auto target = me->getVictim())
                    DoCast(target, SPELL_LASH, false);
                events.ScheduleEvent(EVENT_LASH, 18000);
                break;
            case EVENT_EYE_SORE:
...
struct boss_inquisitor_meto : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.ScheduleEvent(EVENT_REAP, 8000);
        events.ScheduleEvent(EVENT_SOW, 5000);
        events.ScheduleEvent(EVENT_DEATH_FIELD, 12000);
        events.ScheduleEvent(EVENT_SEEDS_OF_CHAOS, 26000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_REAP:
                DoCast(SPELL_REAP);
                events.ScheduleEvent(EVENT_REAP, 20000);
                break;
            case EVENT_SOW:
                DoCast(SPELL_SOW);
                DoCast(SPELL_SOW_DMG);
                events.ScheduleEvent(EVENT_SOW, 15000);
                break;
            case EVENT_DEATH_FIELD:
            {
                ...
                events.ScheduleEvent(EVENT_DEATH_FIELD, 15000);
                break;
            }
            case EVENT_SEEDS_OF_CHAOS:
            {
                ...
                events.ScheduleEvent(EVENT_SEEDS_OF_CHAOS, 30000);
                break;
            }
            default:
                break;
...
struct boss_sotanathor : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.ScheduleEvent(EVENT_SOUL_CLEAVE, 17000);
        events.ScheduleEvent(EVENT_SILENCE, 23000);
        events.ScheduleEvent(EVENT_CAVITATION, 21000);
        events.ScheduleEvent(EVENT_SEED_OF_DESTRUCTION, 16000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SOUL_CLEAVE:
                ...
                events.ScheduleEvent(SPELL_SOUL_CLEAVE, 28000);
                break;
            case EVENT_SILENCE:
                DoCast(SPELL_SILENCE);
                events.ScheduleEvent(EVENT_SILENCE, 28000);
                break;
            case EVENT_CAVITATION:
                ZoneTalk(2);
                DoCast(SPELL_CAVITATION);
                events.ScheduleEvent(EVENT_CAVITATION, 27000);
                break;
            case EVENT_SEED_OF_DESTRUCTION:
                ZoneTalk(1);
                DoCast(SPELL_SEED_OF_DESTRUCTION);
                events.ScheduleEvent(EVENT_SEED_OF_DESTRUCTION, 36000);
                break;
            default:
                break;
            }
...
struct boss_matron_folnuna : public ScriptedAI
{
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            slimbering = false;
            me->SetReactState(REACT_AGGRESSIVE);
            events.ScheduleEvent(EVENT_FEL_BLAST, 2000);
            events.ScheduleEvent(EVENT_GROTESQUE_SPAWN, 8000);
            break;
        }
    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_FEL_BLAST:
                if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, 200.0f, true))
                    DoCast(target, SPELL_FEL_BLAST);
                events.ScheduleEvent(EVENT_FEL_BLAST, 10000);
                break;
            case EVENT_GROTESQUE_SPAWN:
                DoCast(SPELL_GROTESQUE_SPAWN);
                events.ScheduleEvent(EVENT_GROTESQUE_SPAWN, 35000);
                break;
            default:
                break;
            }
...
class spell_slimbering_gasp : public AuraScript
{
    ...
    void Register() override
    {
        ...
        OnEffectApply += AuraEffectApplyFn(spell_slimbering_gasp::OnApply, EFFECT_1, SPELL_AURA_CREATE_AREATRIGGER, AURA_EFFECT_HANDLE_REAL);
        OnEffectUpdate += AuraEffectUpdateFn(spell_slimbering_gasp::OnUpdate, EFFECT_1, SPELL_AURA_CREATE_AREATRIGGER);
    }
};
