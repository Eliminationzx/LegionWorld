...
struct boss_tos_kiljaeden : BossAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        instance->instance->ApplyOnEveryPlayer([&](Player* player) { player->CastSpell(player, SPELL_KILJAEDEN_PLAYERS_TELEPOT, true); });

        if (auto chest = instance->instance->GetGameObject(instance->GetGuidData(GO_KILJAEDEN_CHEST)))
            chest->SetRespawnTime(86400);
    }

    void DefaultEvents(uint8 stage)
    {
        events.Reset();
        ...
        {
            case S1_P1:
                events.ScheduleEvent(EVENT_FELCLAWS, 25000);
                events.ScheduleEvent(EVENT_ARMAGEDDON, armageddonTime[phase].front());
                if (!IsLfrRaid())
                    events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, singularityTime[phase].front());
                if (IsMythicRaid())
                {
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_ERUPTING, 18000);
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_WAILING, 56000);
                }
                break;
            case S3_P2:
                if (!IsLfrRaid())
                {
                    events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, singularityTime[phase].front());
                    events.ScheduleEvent(EVENT_FOCUSED_DREADFLAME, focusedDreadFlameTime[phase].front());
                }
                events.ScheduleEvent(EVENT_FELCLAWS, 10000);
                events.ScheduleEvent(EVENT_ARMAGEDDON, armageddonTime[phase].front());
                events.ScheduleEvent(EVENT_BURSTING_DREADFLAME, burstingDreadflameTime[phase].front());
                events.ScheduleEvent(EVENT_SHADOW_REFLECTION_ERUPTING, IsMythicRaid() ? 163000 : 12000);
                if (IsMythicRaid())
                {
                    wailingTimers = 168000;
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_WAILING, 50000);
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_HOPELESS, 26000);
                }
                else if (IsHeroicRaid())
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_WAILING, 48000);
                break;
            case S5_P3:
                events.ScheduleEvent(EVENT_FELCLAWS, 10000);
                events.ScheduleEvent(EVENT_DARKNESS_THOUSAND_SOULS, 1000);
                events.ScheduleEvent(EVENT_DEMONIC_OBELISK, 25000);
                events.ScheduleEvent(EVENT_TEAR_RIFT, 14000);
                events.ScheduleEvent(EVENT_BURSTING_DREADFLAME, burstingDreadflameTime[phase].front());
                if (IsHeroicPlusRaid())
                    events.ScheduleEvent(EVENT_FLAMING_ORB, 30000);
                if (!IsLfrRaid())
                    events.ScheduleEvent(EVENT_FOCUSED_DREADFLAME, focusedDreadFlameTime[phase].front());
                break;
        }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        else if (id == 2)
        {
            ...
            events.ScheduleEvent(EVENT_ARMAGEDDON, 1000);
            events.ScheduleEvent(EVENT_BURSTING_DREADFLAME, IsMythicRaid() ? 4000 : 2000);
            if (!IsLfrRaid())
                events.ScheduleEvent(EVENT_FOCUSED_DREADFLAME, IsMythicRaid() ? 25000 : 18000);
            if (IsHeroicRaid())
                events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, 7000);
            else if (IsMythicRaid())
            {
        ...
        else if (id == 3)
        {
            ...
            me->SetFacingTo(flyPos[4].GetAngle(me) - M_PI);
            events.ScheduleEvent(EVENT_ARMAGEDDON, 1000);
            events.ScheduleEvent(EVENT_BURSTING_DREADFLAME, IsMythicRaid() ? 24000 : 20000);
            if (!IsLfrRaid())
                events.ScheduleEvent(EVENT_FOCUSED_DREADFLAME, IsMythicRaid() ? 4000 : 2000);
            if (IsHeroicRaid())
                events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, 8000);
            else if (IsMythicRaid())
            {
                // Rupturing Singularity
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
    {
        if (me->HealthBelowPct(healthPct))
        {
            ...
            else if (healthPct == 41)
            {
                ...
                DefaultEvents(++phase);
                me->StopAttack(true);
                events.ScheduleEvent(EVENT_DECEIVER_VEIL, 500);
                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, 20000);
            }
        }
    }
    ...
    void RupturingSingularity()
    {
        ...
        while (sungularRandOld == sungularRand)
        {
            ...
            //if (phase == S2_CHANGE || phase == S4_CHANGE)
            //    sungularRand = urand(0, 3);
            //else
            //    sungularRand = urand(0, 4);
        }
    ...
    void Armageddon()
    {
        ...
        bool badPos = false;
        uint16 traiCount = 0;
        while (randPosList.size() < armageddonCount)
        {
            badPos = false;
            singularPos[4].SimplePosXYRelocationByAngle(pos, frand(15.0f, 38.0f), frand(0.0f, 6.28f));
            ++traiCount;

            for (auto _pos : randPosList)
            {
                if (pos.GetExactDist(&_pos) <= 8.0f)
                {
                    badPos = true;
                    break;
                }
            }
            if (!badPos || traiCount > 30)
        ...
        for (auto _pos : randPosList)
        {
            ...
            {
                if (traiCount == 0 || (traiCount == 1 && IsMythicRaid()))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (me->HasUnitState(UNIT_STATE_CASTING) || flymove)
            return;

        if (phase != S2_CHANGE && CheckHomeDistToEvade(diff, 62.0f, 4500.27f, -1510.24f, 5385.64f))
            return;
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_RUPTURING_SINGULARITY:
                    ...
                    if (singularityTime[phase].size() > 1)
                        singularityTime[phase].pop_front();
                    if (!singularityTime[phase].empty())
                        events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, singularityTime[phase].front());
                    if (phase == S4_CHANGE)
                        events.ScheduleEvent(EVENT_RUPTURING_SINGULARITY, 10000);
                    break;
                case EVENT_ARMAGEDDON:
                    DoCast(SPELL_ARMAGEDDON_DUMMY);
                    if (armageddonTime[phase].size() > 1)
                        armageddonTime[phase].pop_front();
                    if (!armageddonTime[phase].empty())
                    {
                        Talk(SAY_ARMAGEDDON);
                        events.ScheduleEvent(EVENT_ARMAGEDDON, armageddonTime[phase].front());
                    }
                ...
                case EVENT_BURSTING_DREADFLAME:
                    ...
                    if (burstingDreadflameTime[phase].size() > 1)
                    {
                        if (phase == S5_P3 && burstingDreadflameTime[phase].size() == 2)
                            burstingDreadflameTime[phase].reverse();
                        else
                            burstingDreadflameTime[phase].pop_front();
                    }
                    if (!burstingDreadflameTime[phase].empty())
                        events.ScheduleEvent(EVENT_BURSTING_DREADFLAME, burstingDreadflameTime[phase].front());
                    break;
                case EVENT_FOCUSED_DREADFLAME:
                    Talk(SAY_FOCUSED_DREADFLAME);
                    DoCast(SPELL_FOCUSED_DREADFLAME_FILTER);
                    if (focusedDreadFlameTime[phase].size() > 1)
                    {
                        if (phase == S5_P3 && focusedDreadFlameTime[phase].size() == 2)
                            focusedDreadFlameTime[phase].reverse();
                        else
                            focusedDreadFlameTime[phase].pop_front();
                    }
                    if (!focusedDreadFlameTime[phase].empty())
                        events.ScheduleEvent(EVENT_FOCUSED_DREADFLAME, focusedDreadFlameTime[phase].front());
                    break;
                case EVENT_SHADOW_REFLECTION_ERUPTING:
                    Talk(SAY_REFLECTION_ERUPTING);
                    DoCast(SPELL_SHADOW_REFLECTION_ERUPTING);
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_ERUPTING, IsMythicRaid() ? 163000 : 111000);
                    break;
                case EVENT_DECEIVER_VEIL:
                {
                    ...
                    for (uint8 i = 0; i < 5; ++i)
                    {
                        singularPos[4].SimplePosXYRelocationByAngle(pos, frand(15.0f, 35.0f), frand(0.0f, 6.28f));
                ...
                case EVENT_DARKNESS_THOUSAND_SOULS:
                    ...
                    DoCast(SPELL_DARKNESS_THOUSAND_SOULS);
                    events.ScheduleEvent(EVENT_DARKNESS_THOUSAND_SOULS, 90000);
                    break;
                case EVENT_DEMONIC_OBELISK:
                {
                    ...
                    {
                        singularPos[4].SimplePosXYRelocationByAngle(pos, frand(25.0f, 35.0f), frand(0.0f, 6.28f));
                        me->SummonCreature(NPC_DEMONIC_OBELISK, pos);
                    }
                    events.ScheduleEvent(EVENT_DEMONIC_OBELISK, timer);
                    break;
                }
                case EVENT_TEAR_RIFT:
                {
                    ...
                    events.ScheduleEvent(EVENT_TEAR_RIFT, 94000);
                    break;
                }
                case EVENT_FLAMING_ORB:
                {
                    ...
                    events.ScheduleEvent(EVENT_FLAMING_ORB, timer);
                    break;
                }
                case EVENT_SHADOW_REFLECTION_WAILING:
                    ...
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_WAILING, phase == S1_P1 ? 114000 : wailingTimers);
                    break;
                case EVENT_SHADOW_REFLECTION_HOPELESS:
                    Talk(SAY_HOPELESS);
                    DoCast(SPELL_SHADOW_REFLECTION_HOPELESS);
                    events.ScheduleEvent(EVENT_SHADOW_REFLECTION_HOPELESS, 196000);
                    break;
            }
        }
