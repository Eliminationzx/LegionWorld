...
MySpecialList<uint32> unboundChaosHeroicTimers{ 47000, 35000, 42000, 56000, 35000, 50000 };
MySpecialList<uint32> shadowBladesTimers{ 34000, 48000, 43000, 36000, 49000, 43000 };
MySpecialList<uint32> shadowBladesHeroicTimers{ 49000, 59000, 55000, 35000, 38000 };
...
struct boss_fallen_avatar : BossAI
{
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_CHECK_ENERGY, urand(4000, 5000));
        events.ScheduleEvent(EVENT_RUPTURE_REALISTIC, 35000);
        events.ScheduleEvent(EVENT_UNBOUND_CHAOS, 7000);
        events.ScheduleEvent(EVENT_SHADOWY_BLADES, 30000);
        events.ScheduleEvent(EVENT_DESOLATE, 13000);
        events.ScheduleEvent(EVENT_SELECT_PILONE, 1000);
        if (IsHeroicPlusRaid())
            events.ScheduleEvent(EVENT_TOUCH_OF_SARGERAS, 15000);
        if (IsMythicRaid())
            events.ScheduleEvent(EVENT_SELECT_PILONE_FEL, 1100);
        ...
        switch (spell->Id)
        {
            ...
            case SPELL_ANNIHILATION:
                ...
                me->AddDelayedEvent(500, [this]() -> void
                {
                    ...
                    // second phase
                    events.ScheduleEvent(EVENT_DESOLATE, 19000);
                    events.ScheduleEvent(EVENT_RUPTURE_REALISTIC_AND_ENERGY, 1000);
                    events.ScheduleEvent(EVENT_DARK_MARK, (IsMythicRaid() ? 31000 : 21000));
                    if (IsHeroicPlusRaid())
                        me->CastSpell(me, SPELL_BLACK_WIND_TICKS, true);

                    if (IsMythicRaid())
                        events.ScheduleEvent(EVENT_RAIN, 16000);
            ...
            case SPELL_RUPTURE_REALISTIC_2:
                for (auto pair : localFloorBlocks)
                    if (me->GetDistance(pair.second) <= 35.0f)
                    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_SHADOWY_BLADES:
                {
                    ...
                    events.ScheduleEvent(EVENT_SHADOWY_BLADES, m_shadowBladesTimers.popAndSafeLast());
                    break;
                }
                case EVENT_RUPTURE_REALISTIC:
                    ...
                    events.ScheduleEvent(EVENT_RUPTURE_REALISTIC, 60000);
                    break;
                case EVENT_UNBOUND_CHAOS:
                    DoCast(SPELL_UNBOUND_CHAOS);
                    Talk(SAY_CHAOS);
                    events.ScheduleEvent(EVENT_UNBOUND_CHAOS, m_unboundChaosTimers.popAndSafeLast());
                    break;
                case EVENT_DESOLATE:
                ...
                case EVENT_SELECT_PILONE:
                    ...
                    events.ScheduleEvent(EVENT_SELECT_PILONE, (IsMythicRaid() ? 12000 : 40000) + 10000);
                    break;
                case EVENT_SELECT_PILONE_FEL:
                {
                    ...
                    events.ScheduleEvent(EVENT_SELECT_PILONE_FEL, 49000 + 10000);
                    break;
                }
                ...
                case EVENT_DARK_MARK:
                    ...
                    events.ScheduleEvent(EVENT_DARK_MARK, IsMythicRaid() ? 25000 : 34000);
                    break;
                case EVENT_TOUCH_OF_SARGERAS:
                {
                    ...
                    events.ScheduleEvent(EVENT_TOUCH_OF_SARGERAS, IsMythicRaid() ? 63000 : 45000);
                    break;
                }
                case EVENT_RAIN:
                {
                    ...
                    events.ScheduleEvent(EVENT_RAIN, 35000);
                    break;
                }
            }
        }
...
struct npc_tos_touch_of_sargeras : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* owner) override
    {
        ...
        events.ScheduleEvent(EVENT_1, 8000+3000);
    }
...
class spell_tos_rupture_realistic : public SpellScript
{
    ...
    void HandleDamage(SpellEffIndex /*effectIndex*/)
    {
        ...
        if (dist < 90.0f)
            SetHitDamage(GetHitDamage() * (0.1f + 0.9f*((90.0f - dist) / 90.0f)));
        else
...
class spell_tos_avatara_energy : public AuraScript
{
    ...
    void OnTick(AuraEffect const* /*aurEff*/)
    {
        ...
        target->SetPower(target->getPowerType(), target->GetPower(target->getPowerType()) + urand(2, 4));
    }
...
class spell_tos_black_winds : public AuraScript
{
    ...
    void OnTick(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Creature* add = caster->SummonCreature(NPC_BLACK_WINDS, start, TEMPSUMMON_TIMED_DESPAWN, 6000))
        {
