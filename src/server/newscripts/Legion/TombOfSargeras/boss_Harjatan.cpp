...
public:
    ...
    struct boss_harjatanAI : BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_CHECK_ENERGY, 500);

            events.ScheduleEvent(EVENT_ENRAGE, IsMythicRaid() ? 360000 : 480000);
            events.ScheduleEvent(EVENT_DRAW_IN, 58000);
            events.ScheduleEvent(EVENT_CALL_ADDS, 17000);
            if (IsMythicRaid())
                events.ScheduleEvent(EVENT_HATCHING, 30000);
        }
        ...
        void SpellFinishCast(SpellInfo const* spell) override
        {
            switch (spell->Id)
            {
                ...
                case SPELL_FROSTY_DISCHARGE:
                    events.ScheduleEvent(EVENT_DRAW_IN, 59000);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CALL_ADDS:
                        ...
                        events.ScheduleEvent(EVENT_CALL_ADDS, 36000);
                        break;
                    case EVENT_DRAW_IN:
                        Talk(SAY_DRAW_IN);
                    ...
                    case EVENT_HATCHING:
                    {
                        ...
                        events.ScheduleEvent(EVENT_HATCHING_CLEAN_UP, 28000);
                        break;
                    }
...
public:
    ...
    struct npc_elder_murk_eyeAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_ELDER1, 6000); //wave
            events.ScheduleEvent(EVENT_ELDER2, 8000); //splash
            events.ScheduleEvent(EVENT_ELDER3, 25000); //rage
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_ELDER1:
                    ...
                    events.ScheduleEvent(EVENT_ELDER1, 6000);
                    break;
                case EVENT_ELDER2:
                    DoCast(SPELL_SPLASH);
                    events.ScheduleEvent(EVENT_ELDER2, 20000);
                    break;
                case EVENT_ELDER3:
                    DoCast(SPELL_ELDERS_RAGE);
                    events.ScheduleEvent(EVENT_ELDER3, 45000);
                    break;
                }
            }
            DoMeleeAttackIfReady();
