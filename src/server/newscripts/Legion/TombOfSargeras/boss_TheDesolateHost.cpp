...
enum Spells
{
    ...
    SPELL_SPEAR_OF_ANGUISH                  = 235923, //Heroic+
...
struct boss_the_desolate_host_generic : BossAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        if (summonTimer)
        {
            if (summonTimer <= diff)
            {
                summonTimer = 60000; //1 minute

                Position pos;
                for (auto entry : {NPC_REANIMATED_TEMPLAR, NPC_GHASTLY_BONEWARDEN, NPC_FALLEN_PRIESTESS, NPC_SOUL_RESIDUE})
                {
...
struct npc_tos_engine_of_souls : ScriptedAI
{
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_COLLAPSING_FISSURE, 7000);
        events.ScheduleEvent(EVENT_TORMENTED_CRIES, 2 * MINUTE * IN_MILLISECONDS);

        if (IsHeroicPlusRaid())
            events.ScheduleEvent(EVENT_SPEAR_OF_ANGUISH, 20000);
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_COLLAPSING_FISSURE:
                    ...
                    events.ScheduleEvent(EVENT_COLLAPSING_FISSURE, 32000);
                    break;
                ...
                case EVENT_SPEAR_OF_ANGUISH:
                    if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 1, [this](Unit const* t) { return !t->HasAura(SPELL_SPIRIT_REALM) && !t->HasAura(SPELL_SOULBIND_DUMMY); }))
                    {
                        ...
                        DoCast(target, SPELL_SPEAR_OF_ANGUISH);
                    }
                    events.ScheduleEvent(EVENT_SPEAR_OF_ANGUISH, 20000);
                    break;
            }
...
struct npc_tos_soul_queen_dejahna : ScriptedAI
{
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        DoCast(me, SPELL_QUIETUS, true);
        events.ScheduleEvent(EVENT_WAILING_SOULS, 1 * MINUTE * IN_MILLISECONDS);
        events.ScheduleEvent(EVENT_SOULBIND, 12000);

        if (IsHeroicPlusRaid())
            events.ScheduleEvent(EVENT_WITHER, 20000);
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_WAILING_SOULS:
                    ...
                    events.ScheduleEvent(EVENT_WAILING_SOULS, 2 * MINUTE * IN_MILLISECONDS);
                    break;
                case EVENT_SOULBIND:
                {
                    soulsList.clear();
                    ...
                    events.ScheduleEvent(EVENT_SOULBIND, 24000);
                    break;
                }
                case EVENT_WITHER:
                    if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 1, [this](Unit const* t) { return t->HasAura(SPELL_SPIRIT_REALM) && !t->HasAura(SPELL_SOULBIND_DUMMY); }))
                        DoCast(target, SPELL_WITHER);
                    events.ScheduleEvent(EVENT_WITHER, 38000);
                    events.ScheduleEvent(EVENT_WITHER_2, 10000);
                    break;
                case EVENT_WITHER_2:
                    if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 1, [this](Unit const* t) { return t->HasAura(SPELL_SPIRIT_REALM) && !t->HasAura(SPELL_SOULBIND_DUMMY); }))
...
struct npc_tos_desolate_host : ScriptedAI
{
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_SUNDERING_DOOM, 14000);
        events.ScheduleEvent(EVENT_DOOMED_SUNDERING, 24000);
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_SUNDERING_DOOM:
                    DoCast(SPELL_SUNDERING_DOOM);
                    events.ScheduleEvent(EVENT_SUNDERING_DOOM, 26000);
                    break;
                case EVENT_DOOMED_SUNDERING:
                    DoCast(SPELL_DOOMED_SUNDERING);
                    events.ScheduleEvent(EVENT_DOOMED_SUNDERING, 26000);
                    break;
            }
        }
...
class spell_tos_spear_of_anguish : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effectIndex*/)
    {
        if (GetHitUnit())
        {
            GetHitUnit()->CastSpell(GetHitUnit(), SPELL_SOUL_TRANSFER, true);
            GetHitUnit()->CastSpell(GetHitUnit(), SPELL_SPIRIT_REALM, true);
            GetHitUnit()->RemoveAurasDueToSpell(SPELL_SPIRITUAL_BARRIER_REAL_REALM);
        }
    }
...
class spell_tos_soulbind : public AuraScript
{
    ...
    void OnTick(AuraEffect const* /*aurEff*/)
    {
        ...
        if (GetUnitOwner())
        {
            ...
            {
                skipDamage = skipDamage ? false : true;
                counter = 0;
                if (!skipDamage)
    ...
    void OnUpdate(uint32 diff, AuraEffect* auraEffect)
    {
        if (tickTimer)
        {
            if (tickTimer <= diff)
            {
                ...
                if (GetUnitOwner() && GetUnitOwner()->GetMap()->IsHeroicPlusRaid() && GetUnitOwner()->isAlive() && GetUnitOwner()->isInCombat())
                {
