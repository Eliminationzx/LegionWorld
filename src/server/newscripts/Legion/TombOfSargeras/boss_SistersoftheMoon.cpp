...
struct npc_sister_kasparian : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        {
            case 1:
                events.ScheduleEvent(EVENT_MOON_GLAIVE, 14000);
                events.ScheduleEvent(EVENT_TWILIGHT_GLAIVE, 20000);
                break;
            case 2:
                me->StopAttack(true);
                ...
                events.ScheduleEvent(EVENT_JUMP_KASPARIAN, 20000);
                events.ScheduleEvent(EVENT_TWILIGHT_GLAIVE, 8000);
                events.ScheduleEvent(EVENT_SPECTRAL_GLAIVE, 5000);
                break;
        }
    }
        ...
        switch (actionID)
        {
            case ACTION_4:
                if ((phase == 2 && IsHeroicPlusRaid()) || phase == 3)
                    events.ScheduleEvent(EVENT_GLAIVE_STORM, 500);
                break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (CheckHomeDistToEvade(diff, 30.0f, 6190.06f, -1020.03f, 2899.5f))
            return;
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_MOON_GLAIVE:
                    ...
                    events.ScheduleEvent(EVENT_MOON_GLAIVE, 18000);
                    break;
                case EVENT_TWILIGHT_GLAIVE:
                    DoCast(SPELL_TWILIGHT_GLAIVE_FILTER);
                    if (phase > 1)
                        events.DelayEvents(3000);
                    events.ScheduleEvent(EVENT_TWILIGHT_GLAIVE, 18000);
                    break;
                case EVENT_JUMP_KASPARIAN:
                {
                    ...
                    events.ScheduleEvent(EVENT_JUMP_KASPARIAN, urand(20, 35) * IN_MILLISECONDS);
                    break;
                }
...
struct npc_sister_lunaspyre : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        {
            case 1:
                events.ScheduleEvent(EVENT_MOON_BURN, 10000);
                events.ScheduleEvent(EVENT_LUNAR_STRIKE, 500);
                events.ScheduleEvent(EVENT_TP_LUNASPERE, 16000);
                break;
            case 3:
                ...
                events.ScheduleEvent(EVENT_LUNAR_BEACON, 18000);
                events.ScheduleEvent(EVENT_MOON_BURN, 10000);
                events.ScheduleEvent(EVENT_LUNAR_FIRE, 7000);
                break;
        }
    }
        ...
        switch (actionID)
        {
            ...
            case ACTION_4:
                if ((phase == 1 && IsHeroicPlusRaid()) || phase == 2)
                    events.ScheduleEvent(EVENT_EMBRACE_ECLIPSE, 500);
                break;
        }
    }
        ...
        if (me->isInCombat() && target && mode == AURA_REMOVE_BY_EXPIRE)
        {
            switch (spellId)
            {
                case SPELL_LUNAR_BEACON:
                    ...
                    target->CastSpell(target, SPELL_LUNAR_BARRAGE_AT, true, nullptr, nullptr, me->GetGUID());
                    break;
            }
        }
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_MOON_BURN:
                    ...
                    events.ScheduleEvent(EVENT_MOON_BURN, 24000);
                    break;
                case EVENT_LUNAR_STRIKE:
                    ...
                    events.ScheduleEvent(EVENT_LUNAR_STRIKE, 5500);
                    break;
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 1:
                ...
                events.ScheduleEvent(EVENT_TWILIGHT_VOLLEY, 16000);
                events.ScheduleEvent(EVENT_SHADOW_SHOT, 5000);
                events.ScheduleEvent(EVENT_TP_YATHAE, 5000);
                break;
            case 2:
                ...
                me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
                me->SetReactState(REACT_AGGRESSIVE, 2000);
                events.ScheduleEvent(EVENT_CALL_MOONTALON, 6000);
                events.ScheduleEvent(EVENT_TWILIGHT_VOLLEY, 12000);
                events.ScheduleEvent(EVENT_RAPID_SHOT, 18000);
                break;
            case 3:
                me->SetUInt32Value(UNIT_FIELD_EMOTE_STATE, EMOTE_STATE_READY_BOW);
                me->SetSheath(SHEATH_STATE_RANGED);
                ...
                events.ScheduleEvent(EVENT_TWILIGHT_VOLLEY, 28000);
                events.ScheduleEvent(EVENT_SHADOW_SHOT, 5000);
                break;
        }
    }
        ...
        switch (actionID)
        {
            ...
            case ACTION_3:
                if (phase == 1 || (phase == 3 && IsHeroicPlusRaid()))
                    events.ScheduleEvent(EVENT_INCORPOREAL_SHOT, 500);
                break;
        }
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_TWILIGHT_VOLLEY:
                    ...
                    if (phase == 1 || phase == 3)
                        events.ScheduleEvent(EVENT_TWILIGHT_VOLLEY, 20000);
                    else if (phase == 2)
                        events.ScheduleEvent(EVENT_TWILIGHT_VOLLEY, 15000);
                    break;
                case EVENT_SHADOW_SHOT:
                    ...
                    events.ScheduleEvent(EVENT_SHADOW_SHOT, 5500);
                    break;
                case EVENT_TP_YATHAE:
                {
                    ...
                    me->CastSpell(pos, SPELL_INCORPOREAL_TELEPORT, false);
                    events.DelayEvents(1500);
                    events.ScheduleEvent(EVENT_TP_YATHAE, 18000);
                    break;
                }
