        ...
        void EnterCombat(Unit* who) override
        {
            ...
            switch (me->GetEntry())
            {
                case NPC_ATRIGAN:
                    ...
                    events.ScheduleEvent(EVENT_SCYTHE_SWEEP, 5200);
                    if (IsHeroicRaid() || IsMythicRaid())
                        events.ScheduleEvent(EVENT_CALCIFIED_QUILS, 8500);
                    Talk(0);
                    events.ScheduleEvent(EVENT_SUMMON_ADD, 1000);
                    break;
                case NPC_BELAK:
                    DoCast(me, SPELL_GET_ENERGY);
                    events.ScheduleEvent(EVENT_PANGS_OF_GUILT, 100);
                    events.ScheduleEvent(EVENT_ECHOING_ANGUISH, 5000);
                    events.ScheduleEvent(EVENT_TORMENTING_BURST, 9000);
                    if (IsHeroicRaid() || IsMythicRaid())
                        events.ScheduleEvent(EVENT_SUFFOCATING_DARK, 10000);
            ...
            events.ScheduleEvent(EVENT_CHECK_ENERGY, 100); // both
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SCYTHE_SWEEP:
                        ...
                        events.ScheduleEvent(EVENT_SCYTHE_SWEEP, 24500);
                        break;
                    ...
                    case EVENT_CALCIFIED_QUILS:
                        ...
                        events.ScheduleEvent(EVENT_CALCIFIED_QUILS, 20000);
                        break;
                    case EVENT_PANGS_OF_GUILT:
                        DoCast(SPELL_PANGS_OF_GUILT);
                        events.ScheduleEvent(EVENT_PANGS_OF_GUILT, 10000);
                        break;
                    case EVENT_ECHOING_ANGUISH:
                        DoCast(SPELL_ECHOING_ANGUISH);
                        events.ScheduleEvent(EVENT_ECHOING_ANGUISH, 22000);
                        break;
                    case EVENT_TORMENTING_BURST:
                        DoCast(SPELL_TORMENTING_BURST);
                        events.ScheduleEvent(EVENT_TORMENTING_BURST, 17000);
                        break;
                    case EVENT_SUFFOCATING_DARK:
                    {
                        ...
                        events.ScheduleEvent(EVENT_SUFFOCATING_DARK, 24000);
                        break;
                    }
                    case EVENT_ENRAGE:
                        DoCast(SPELL_ENRAGE);
        ...
        if (Unit* caster = GetCaster())
            if (InstanceScript* instance = caster->GetInstanceScript())
                if (instance->instance->IsMythicRaid())
                    caster->CastSpellDelay(caster, 241505, true, 100);
    }
