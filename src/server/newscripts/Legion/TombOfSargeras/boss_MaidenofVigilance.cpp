...
struct boss_maiden_of_vigilance : BossAI
{
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        if (!IsLfrRaid())
        {
            events.ScheduleEvent(EVENT_INFUSION, 2000);
            events.ScheduleEvent(EVENT_LIGHT_HAMMER, 12000);
            events.ScheduleEvent(EVENT_MASS_INSTABILITY, 21000);
            events.ScheduleEvent(EVENT_SECOND_PHASE, 40000);
            if (IsMythicRaid())
                events.ScheduleEvent(EVENT_FRAGMENTS, 10000);
        }
        else
        {
            events.ScheduleEvent(EVENT_MASS_INSTABILITY, 6000);
            events.ScheduleEvent(EVENT_INFUSION, 40000);
            events.ScheduleEvent(EVENT_SECOND_PHASE, 47000);
        }
    }
    ...
    void SpellFinishCast(SpellInfo const* spell) override
    {
        switch (spell->Id)
        {
            case SPELL_MASS_INSTABILITY:
            {
                ...
                for (uint8 i =0; i< threatlist.size();)
                {
                    auto itr = threatlist.begin();
                    std::advance(itr, i);
                    if (!(*itr)->getTarget() || !(*itr)->getTarget()->IsPlayer())
                    {
                        threatlist.remove((*itr));
                        continue;
                    }

                    if (Player* pl = (*itr)->getTarget()->ToPlayer())
                ...
                bool useTank = false;
                if (!tanks.empty())
                    for (uint8 i = 0; i < tanks.size() && !useTank; ++i)
                    {
                        Player* target = tanks[i];
                        if (!target)
                            continue;
            ...
            case SPELL_HAMMER_OF_OBLITERATION:
                if (IsHeroicPlusRaid())
                    me->CastSpell(me, spell->Id == SPELL_HAMMER_OF_OBLITERATION ? SPELL_FEL_REMANENCE : SPELL_LIGHT_REMANENCE, true);
                break;
        }
    }
    ...
    void OnInterruptCast(Unit* /*caster*/, uint32 spellId, uint32 curSpellID, uint32 /*schoolMask*/) override
    {
        if (curSpellID == SPELL_WRATH_OF_THE_CREATORS)
        {
            ...
            if (!IsLfrRaid())
            {
                events.ScheduleEvent(EVENT_INFUSION, 3000);
                events.ScheduleEvent(EVENT_LIGHT_HAMMER, 12000);
                events.ScheduleEvent(EVENT_MASS_INSTABILITY, 21000);
                events.ScheduleEvent(EVENT_SECOND_PHASE, 82000);
                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_FRAGMENTS, 8000);
            }
            else
            {
                events.ScheduleEvent(EVENT_MASS_INSTABILITY, 8000);
                events.ScheduleEvent(EVENT_INFUSION, 21000);
                events.ScheduleEvent(EVENT_SECOND_PHASE, 67000);
            }
        }
    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_INFUSION:
                    ...
                    if (empoweredPhase)
                        events.ScheduleEvent(EVENT_INFUSION, 38000);
                    break;
                case EVENT_LIGHT_HAMMER:
                    DoCast(SPELL_HAMMER_OF_CREATION);
                    events.ScheduleEvent(EVENT_FEL_HAMMER, 19000);
                    break;
                case EVENT_FEL_HAMMER:
                    DoCast(SPELL_HAMMER_OF_OBLITERATION);
                    if (empoweredPhase)
                        events.ScheduleEvent(EVENT_LIGHT_HAMMER, 19000);
                    break;
                case EVENT_MASS_INSTABILITY:
                    DoCast(SPELL_MASS_INSTABILITY);
                    if (empoweredPhase)
                        events.ScheduleEvent(EVENT_MASS_INSTABILITY, 36000);
                    break;
    ...
    void UpdateAI(uint32 diff) override 
    {
        ...
        if (updateTimer <= diff)
        {
            ...
            for (const auto& itr : players)
            {
                ...
                if (me->GetDistance(who) < 1.1f && !justDoIt)
                {
                    justDoIt = true;
...
class spell_tos_infusion : public SpellScript
{
    ...
    void CorrectTargets(std::list<WorldObject*>& targets)
    {
        ...
        for (auto target : targets)
        {
            ...
            if (i < targets.size() / 2)
                caster->CastSpell(target->ToUnit(), urand(1, 4) == 1 ? SPELL_LIGHT_INFUSION : SPELL_FEL_INFUSION, true);
            else
                caster->CastSpell(target->ToUnit(), urand(1, 4) != 1 ? SPELL_LIGHT_INFUSION : SPELL_FEL_INFUSION, true);
        }
    }
