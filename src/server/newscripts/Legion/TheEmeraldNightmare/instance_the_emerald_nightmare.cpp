...
public:
    ...
    struct instance_the_emerald_nightmare_InstanceMapScript : public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            {
                case NPC_NYTHENDRA:
                    NythendraGUID = creature->GetGUID();
                    break;
                case NPC_YSONDRE:
                    YsondreGUID = creature->GetGUID();
                    //no break
                case NPC_TAERAR:
                case NPC_LETHON:
                case NPC_EMERISS:
        ...
        void CreatureDies(Creature* creature, Unit* /*killer*/) override
        {
            ...
            if (creature->GetMap()->GetDifficultyID() == DIFFICULTY_HEROIC_RAID || creature->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC_RAID)
            {
                Creature* ysondre = instance->GetCreature(YsondreGUID);
                if (!ysondre)
                    return;
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            switch (type)
            {
                case DATA_RENFERAL:
                {
                    ...
                    {
                        case IN_PROGRESS:
                            for (std::map<uint32, ObjectGuid>::iterator itr = eggGUIDconteiner.begin(); itr != eggGUIDconteiner.end(); ++itr)
                                if (Creature* egg = instance->GetCreature(itr->second))
                                    if (!egg->isAlive())
                                        egg->Respawn();
                            break;
                    }
