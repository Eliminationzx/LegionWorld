...
struct boss_kingaroth : BossAI
{
    ...
    void DefaultEvents(bool enterCombat = false)
    {
        if (enterCombat)
        {
            events.ScheduleEvent(EVENT_FORGING_STRIKE, 6000);
            events.ScheduleEvent(EVENT_REVERBERATING_STRIKE, 14000);
            events.ScheduleEvent(EVENT_DIABOLIC_BOMB, 14000);
            events.ScheduleEvent(EVENT_RUINER, 26000);
            events.ScheduleEvent(EVENT_CHECK_NEAREST_PLAYER, 3000);
        }
        else
        {
            events.ScheduleEvent(EVENT_FORGING_STRIKE, 1000);
            events.ScheduleEvent(EVENT_REVERBERATING_STRIKE, 2000);
            events.ScheduleEvent(EVENT_DIABOLIC_BOMB, 20000);
            events.ScheduleEvent(EVENT_RUINER, 22000);
            events.ScheduleEvent(EVENT_CHECK_NEAREST_PLAYER, 3000);
        }
    }
    ...
    void DoAction(int32 const actionID) override
    {
        if (actionID == ACTION_1 && !phaseConstruction)
        {
            ...
            events.Reset();
            events.ScheduleEvent(EVENT_PHASE_2, 500);
        }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_FORGING_STRIKE:
                    ...
                    events.ScheduleEvent(EVENT_FORGING_STRIKE, 14000);
                    break;
                case EVENT_REVERBERATING_STRIKE:
                    ...
                    events.ScheduleEvent(EVENT_REVERBERATING_STRIKE, 30000);
                    break;
                ...
                case EVENT_RUINER:
                {
                    ...
                    Talk(SAY_RUINER);
                    DoCast(SPELL_RUINER_FILTER);
                    events.ScheduleEvent(EVENT_RUINER, 30000);
                    break;
                }
                case EVENT_CHECK_NEAREST_PLAYER:
                {
                    events.ScheduleEvent(EVENT_CHECK_NEAREST_PLAYER, 1500);
...
struct npc_kingaroth_ruiner : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        summoner->CastSpell(me, SPELL_RUINER_CHANNEL);
    }
...
struct npc_kingaroth_garothi : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        switch (me->GetEntry())
        {
            ...
            case NPC_GAROTHI_DECIMATOR:
                ...
                break;
            case NPC_GAROTHI_DEMOLISHER:
                events.ScheduleEvent(EVENT_3, 46000);
                break;
        }
    }
    ...
    void OnRemoveAuraTarget(Unit* target, uint32 spellId, AuraRemoveMode mode) override
    {
        ...
        switch (spellId)
        {
            ...
            case SPELL_DECIMATION_MARK:
                target->CastSpell(target, SPELL_DECIMATION_MISSILE, true);
                break;
        }
    }
    ...
    void SpellFinishCast(const SpellInfo* spell) override
    {
        if (spell->Id == SPELL_ANNIHILATION)
        {
            ...
            me->GenerateNonDuplicatePoints(randPos, me->GetPosition(), maxPoint, 5.0f, 30.0f, 5.0f);
...
class spell_kingaroth_diabolic_bomb : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        WorldLocation loc;
        GetCaster()->GetNearPosition(loc, frand(25.0f, 45.0f), frand(0.0f, 6.28f));
...
class spell_kingaroth_diabolic_bomb_dmg : public SpellScript
{
    ...
    void HandleDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        float distance = caster->GetExactDist2d(target) * 2.0f;
        uint8 pct = 10;
        if (distance < 100.0f)
            pct = 100 - distance;
