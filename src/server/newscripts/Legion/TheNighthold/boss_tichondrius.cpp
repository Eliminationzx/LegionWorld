...
struct boss_tichonrius : BossAI
{
    ...
    void DefaultEvents()
    {
        ...
        events.ScheduleEvent(EVENT_CARRION_PLAGUE, (5 - delay) * IN_MILLISECONDS);
        events.ScheduleEvent(EVENT_SEEKER_SWARM, (25 - delay) * IN_MILLISECONDS);
        events.ScheduleEvent(EVENT_FEAST_OF_BLOOD, (20 - delay) * IN_MILLISECONDS);
        events.ScheduleEvent(EVENT_ECHOES_OF_THE_VOID, (55 - delay) * IN_MILLISECONDS);

        if (IsHeroicPlusRaid())
            events.ScheduleEvent(EVENT_BRAND_OF_ARGUS, (15 - delay) * IN_MILLISECONDS);

        if (phase < 2)
            IllusionaryStartTimer = 125 * IN_MILLISECONDS;

        if (phase == 1 || phase == 2)
            events.ScheduleEvent(EVENT_SUM_GUARDS, 24 * IN_MILLISECONDS);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (IllusionaryStartTimer)
        {
            if (IllusionaryStartTimer <= diff)
            {
                IllusionaryStartTimer = 0;
                events.Reset();
                events.ScheduleEvent(EVENT_ILLUSIONARY_NIGHT_START, 1000);
            }
            else
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CARRION_PLAGUE:
                    ...
                    events.ScheduleEvent(EVENT_CARRION_PLAGUE, 25000);
                    break;
                case EVENT_SEEKER_SWARM:
                    DoCast(SPELL_SEEKER_SWARM_FILTER);
                    events.ScheduleEvent(EVENT_SEEKER_SWARM, 25000);
                    break;
                case EVENT_FEAST_OF_BLOOD:
                    Talk(SAY_FEAST_OF_BLOOD);
                    DoCastTopAggro(SPELL_FEAST_OF_BLOOD);
                    events.ScheduleEvent(EVENT_FEAST_OF_BLOOD, 25000);
                    break;
                case EVENT_ECHOES_OF_THE_VOID:
                {
                    ...
                    events.ScheduleEvent(EVENT_ECHOES_OF_THE_VOID, 65000);
                    break;
                }
                ...
                case EVENT_SUM_GUARDS:
                    ...
                    events.ScheduleEvent(EVENT_SUM_GUARDS, 52000);
                    break;
                case EVENT_BRAND_OF_ARGUS:
                    DoCast(SPELL_BRAND_OF_ARGUS);
                    events.ScheduleEvent(EVENT_BRAND_OF_ARGUS, 25000 + argusDelay);
...
struct npc_tichonrius_felsworn_spellguard : ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        events.ScheduleEvent(EVENT_2, 4500);
    }
