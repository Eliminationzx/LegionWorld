...
struct boss_elisande : BossAI
{
    ...
    void DefaultEvents(bool enterCombat)
    {
        ...
        events.ScheduleEvent(EVENT_SPANNING_SINGULARITY, singularityTimeList.front());

        if (!IsLfrRaid())
            events.ScheduleEvent(EVENT_ARCANETIC_RING, arcaneticRingTimeList.front());

        if (phase > 1)
        {
            events.ScheduleEvent(EVENT_DELPHURIC_BEAM, delphuricBeamTimeList.front());
            if (!IsLfrRaid())
                events.ScheduleEvent(EVENT_EPOCHERIC_ORB, epochericTimeList.front());
        }

        if (IsMythicRaid())
        {
            events.ScheduleEvent(EVENT_SUM_SLOW_ELEMENTAL, SummonElementalTimeList[0][phase].front() + modTime);
            events.ScheduleEvent(EVENT_SUM_FAST_ELEMENTAL, SummonElementalTimeList[1][phase].front() + modTime);
        }
        else
        {
            events.ScheduleEvent(EVENT_SUM_SLOW_ELEMENTAL, SummonElementalTimeList[0][0].front() + modTime);
            events.ScheduleEvent(EVENT_SUM_FAST_ELEMENTAL, SummonElementalTimeList[1][0].front() + modTime);
        }

        switch (phase)
        {
            case 1:
                events.ScheduleEvent(EVENT_CASCADENT_STAR, 12000);
                events.ScheduleEvent(EVENT_ABLATION, 13000);
                break;
            case 2:
                events.ScheduleEvent(EVENT_ABLATING_EXPLOSION, 24000);
                break;
            case 3:
                events.ScheduleEvent(EVENT_ABLATIVE_PULSE, 24000);
                events.ScheduleEvent(EVENT_PERMELIATIVE_TORMENT, permeliativeTormentTimeList.front());
                if (!IsLfrRaid())
                    events.ScheduleEvent(EVENT_CONFLEXIVE_BURST, conflexiveBurstTimeList.front());
                break;
        }
    }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
    {
        if (phase < 3)
        {
            ...
            if (!switchPhase && me->HealthBelowPct(11))
            {
                ...
                me->StopAttack(true);
                events.ScheduleEvent(EVENT_CHANGE_PHASE_1, 100);
    ...
    void SpellFinishCast(const SpellInfo* spell) override
    {
        switch (spell->Id)
        {
            case SPELL_TIME_STOP:
                events.ScheduleEvent(EVENT_CHANGE_PHASE_2, 3000);
                break;
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_CHANGE_PHASE_2:
                    ...
                    events.ScheduleEvent(EVENT_CHANGE_PHASE_3, 2000);
                    break;
                case EVENT_CHANGE_PHASE_3:
                    ...
                    me->NearTeleportTo(me->GetHomePosition());
                    DoCast(me, SPELL_ETCHED_IN_TIME, true);
                    events.ScheduleEvent(EVENT_CHANGE_PHASE_4, 5000);
                    break;
                ...
                case EVENT_SUM_SLOW_ELEMENTAL:
                {
                    ...
                    if (!SummonElementalTimeList[0][IsMythicRaid() ? phase : 0].empty())
                        events.ScheduleEvent(EVENT_SUM_SLOW_ELEMENTAL, SummonElementalTimeList[0][IsMythicRaid() ? phase : 0].front());
                    break;
                }
                case EVENT_SUM_FAST_ELEMENTAL:
                {
                    ...
                    if (!SummonElementalTimeList[1][IsMythicRaid() ? phase : 0].empty())
                        events.ScheduleEvent(EVENT_SUM_FAST_ELEMENTAL, SummonElementalTimeList[1][IsMythicRaid() ? phase : 0].front());
                    break;
                }
                ...
                case EVENT_CASCADENT_STAR:
                    ...
                    events.ScheduleEvent(EVENT_CASCADENT_STAR, 5000);
                    break;
                case EVENT_ABLATION:
                    if (me->GetReactState() != REACT_PASSIVE)
                        DoCastTopAggro(SPELL_ABLATION);
                    events.ScheduleEvent(EVENT_ABLATION, 5000);
                    break;
                ...
                case EVENT_ABLATING_EXPLOSION:
                    ...
                    events.ScheduleEvent(EVENT_ABLATING_EXPLOSION, 21000);
                    break;
                case EVENT_ABLATIVE_PULSE:
                    DoCastTopAggro(SPELL_ABLATIVE_PULSE);
                    events.ScheduleEvent(EVENT_ABLATIVE_PULSE, 6000);
                    break;
