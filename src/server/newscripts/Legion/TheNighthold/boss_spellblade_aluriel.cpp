...
public:
    ...
    struct boss_spellblade_alurielAI : BossAI
    {
        ...
        void DefaultEvents(uint32 phase)
        {
            ...
            if (IsHeroicPlusRaid())
                events.ScheduleEvent(EVENT_FROZEN_TEMPEST_FILTER, 16000);

            if (IsMythicRaid())
                events.ScheduleEvent(EVENT_SUM_FEL_SOUL, 18000);

            if (IsMythicRaid() && phase == SPELL_MAGIC_ENCHANTMENT)
                events.ScheduleEvent(EVENT_ANNIHILATE, 5000);
            else
                events.ScheduleEvent(EVENT_ANNIHILATE, 7000);

            switch (phase)
            {
                case SPELL_ICY_ENCHANTMENT:
                    DoCast(me, SPELL_ICY_ENCHANTMENT, true);
                    events.ScheduleEvent(EVENT_MARK_OF_FROST, 18000);
                    if (IsMythicRaid())
                    {
                        events.ScheduleEvent(EVENT_REPLICATE_FROST, 28000);
                        events.ScheduleEvent(EVENT_DETONATE_FROST, 48000);
                        events.ScheduleEvent(EVENT_ANIMATE_FROST, 65000);
                    }
                    else
                    {
                        events.ScheduleEvent(EVENT_REPLICATE_FROST, 38000);
                        events.ScheduleEvent(EVENT_DETONATE_FROST, 68000);
                        events.ScheduleEvent(EVENT_ANIMATE_FROST, 75000);
                    }
                    break;
                case SPELL_FIERY_ENCHANTMENT:
                    DoCast(me, SPELL_FIERY_ENCHANTMENT, true);
                    events.ScheduleEvent(EVENT_SEARING_BRAND, 18000);
                    events.ScheduleEvent(EVENT_REPLICATE_SEARING_BRAND, 26000);
                    if (IsMythicRaid())
                    {
                        events.ScheduleEvent(EVENT_DETONATE_FIRE, 38000);
                        events.ScheduleEvent(EVENT_ANIMATE_FIRE, 54000);
                    }
                    else
                    {
                        events.ScheduleEvent(EVENT_DETONATE_FIRE, 45000);
                        events.ScheduleEvent(EVENT_ANIMATE_FIRE, 62000);
                    }
                    break;
                case SPELL_MAGIC_ENCHANTMENT:
                    DoCast(me, SPELL_MAGIC_ENCHANTMENT, true);
                    events.ScheduleEvent(EVENT_ARCANE_ORB, 1000);
                    events.ScheduleEvent(EVENT_REPLICATE_ARCANE_ORB, 15000);
                    events.ScheduleEvent(EVENT_DETONATE_ARCANE_ORB, 35000);
                    events.ScheduleEvent(EVENT_ANIMATE_ARCANE, 52000);
                    break;
            }
        }
        ...
        void SpellFinishCast(const SpellInfo* spell) override
        {
            switch (spell->Id)
            {
                ...
                case SPELL_ANIMATE_ARCANE_ORB:
                    events.ScheduleEvent(EVENT_ANIMATE_DESPAWN, 1000);
                    break;
                case SPELL_REPLICATE_SEARING_BRAND:
                    events.ScheduleEvent(EVENT_REPLICATE_SEARING_BRAND_2, 100);
                    break;
                default:
                    break;
            }
                    ...
                    case EVENT_ANNIHILATE:
                        ...
                        if (IsMythicRaid())
                        {
                            if (me->HasAura(SPELL_ICY_ENCHANTMENT))
                                ...
                                events.ScheduleEvent(EVENT_ANNIHILATE, 38000);
                            else if (me->HasAura(SPELL_MAGIC_ENCHANTMENT))
                                events.ScheduleEvent(EVENT_ANNIHILATE, 43000);
                        }
                        else
                        {
                            if (me->HasAura(SPELL_MAGIC_ENCHANTMENT))
                                events.ScheduleEvent(EVENT_ANNIHILATE, 35000);
                            else
                                events.ScheduleEvent(EVENT_ANNIHILATE, 45000);
                        }
                        break;
            ...
            switch (me->GetEntry())
            {
                ...
                case NPC_FIERY_ENCHANTMENT:
                {
                    ...
                    if (IsHeroicPlusRaid())
                    {
                        ...
                        events.ScheduleEvent(EVENT_2, 3000); // Bound in Flame
                        events.ScheduleEvent(EVENT_3, urand(5,7) * IN_MILLISECONDS); // Pyroblast
                    }
                    break;
                }
        ...
        void SpellHit(Unit* target, SpellInfo const* spell) override
        {
            switch (spell->Id)
            {
                case SPELL_DETONATE_ARCANE_ORB:
                    ...
                    break;
                case SPELL_ANIMATE_ARCANE_ORB:
                    events.ScheduleEvent(EVENT_3, 1000); //Despawn
                    break;
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE)
            {
                if (id == 2)
                {
                    ...
                    events.ScheduleEvent(EVENT_2, urand(1, 3) * 1000);
                }
                if (id == 3)
                {
                    if (Unit* owner = me->GetAnyOwner())
...
public:
    ...
    struct npc_aluriel_fel_soulAI : public ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            ...
            despawnTimer = 45000;
            events.ScheduleEvent(EVENT_1, 100);   // Cast SPELL_SEVERED_SOUL
            events.ScheduleEvent(EVENT_2, 2000);  // Check distance owner
            events.ScheduleEvent(EVENT_3, 20000); // Cast SPELL_DECIMATE

            if (summoner->HasAura(SPELL_ICY_ENCHANTMENT))
                events.ScheduleEvent(EVENT_4, 10000); // Cast SPELL_WHIRLING_BARRAGE

            if (summoner->HasAura(SPELL_FIERY_ENCHANTMENT))
                events.ScheduleEvent(EVENT_5, 10000); // Cast SPELL_FEL_STOMP_FILTER

            if (summoner->HasAura(SPELL_MAGIC_ENCHANTMENT)) //35:31, 35:42, 35:48, 36:00, 36:06
            {
                lashSwitcher = false;
                events.ScheduleEvent(EVENT_7, 9000); // Cast SPELL_FEL_LASH
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_7: //Arcane Phase
                        ...
                        {
                            lashSwitcher = true;
                            events.ScheduleEvent(EVENT_7, 11000);
                        }
                        else
                        {
                            lashSwitcher = false;
