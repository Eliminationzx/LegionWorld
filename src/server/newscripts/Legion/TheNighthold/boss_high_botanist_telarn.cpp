...
struct boss_high_botanist_telarn : BossAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        {
            case 1:
                events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 35000);
                events.ScheduleEvent(EVENT_PARASITIC_FETTER, 20000);
                events.ScheduleEvent(EVENT_SOLAR_COLLAPSE, 10000);
                break;
            case 2:
                Talk(SAY_PHASE_2);
                events.ScheduleEvent(EVENT_PHASE_2, 100);
                events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 42000);
                events.ScheduleEvent(EVENT_PARASITIC_FETTER, 22000);
                break;
            case 3:
                Talk(SAY_PHASE_3);
                events.ScheduleEvent(EVENT_PHASE_3, 100);
                events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 52000);
                events.ScheduleEvent(EVENT_CALL_OF_NIGHT, 20000);
                break;
        }
    }
            ...
            switch (eventId)
            {
                case EVENT_CONTROLLED_CHAOS:
                    ...
                    if (healthPct == 0)          //Phase 3
                        events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 50000);
                    else if (healthPct == 51)    //Phase 2
                        events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 40000);
                    else                         //Phase 1
                        events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 35000);
                    break;
                case EVENT_PARASITIC_FETTER:
                    ...
                    if (healthPct == 51)         //Phase 2
                        events.ScheduleEvent(EVENT_PARASITIC_FETTER, 40000);
                    else                         //Phase 1
                        events.ScheduleEvent(EVENT_PARASITIC_FETTER, 35000);
                    break;
                case EVENT_SOLAR_COLLAPSE:
                    Talk(SAY_SOLAR_COLLAPSE);
                    DoCast(SPELL_SOLAR_COLLAPSE_FILTER);
                    events.ScheduleEvent(EVENT_SOLAR_COLLAPSE, 35000);
                    break;
                case EVENT_PHASE_2:
                ...
                case EVENT_CALL_OF_NIGHT:
                    ...
                    events.ScheduleEvent(EVENT_CALL_OF_NIGHT, 50000);
                    break;
            }
        }
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 0: // LFR/NORMAL/HEROIC
                ...
                events.ScheduleEvent(EVENT_FLARE, 7000);
                events.ScheduleEvent(EVENT_SOLAR_COLLAPSE, 32000);
                events.ScheduleEvent(EVENT_SUM_PLASMA_SPHERES, 10000);
                break;
            case 1:
                globalTimer = 64 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_SOLAR_COLLAPSE, 5000);
                events.ScheduleEvent(EVENT_FLARE, 7000);
                events.ScheduleEvent(EVENT_SUM_PLASMA_SPHERES, 45000);
                break;
            case 2:
                globalTimer = 55 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_FLARE, 2000);
                if (me->HasAura(SPELL_NATURE_INFUSION))
                {
                    events.ScheduleEvent(EVENT_SOLAR_COLLAPSE, 15000);
                    events.ScheduleEvent(EVENT_SUM_PLASMA_SPHERES, 25000);
                }
                else
                {
                    events.ScheduleEvent(EVENT_COLLAPSE_OF_NIGHT, 28000);
                    events.ScheduleEvent(EVENT_SUM_PLASMA_SPHERES, 40000);
                }
                break;
            case 3:
                globalTimer = 35 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_FLARE, 8000);
                events.ScheduleEvent(EVENT_COLLAPSE_OF_NIGHT, 20000);
                events.ScheduleEvent(EVENT_SUM_PLASMA_SPHERES, 35000);
                break;
        }
    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_FLARE:
                    ...
                    events.ScheduleEvent(EVENT_FLARE, 9 * IN_MILLISECONDS);
                    break;
                case EVENT_SOLAR_COLLAPSE:
                    Talk(SAY_S_SOLAR_COLLAPSE);
                    DoCast(SPELL_SOLAR_COLLAPSE_FILTER);
                    events.ScheduleEvent(EVENT_SOLAR_COLLAPSE, globalTimer);
                    break;
                case EVENT_COLLAPSE_OF_NIGHT:
                    DoCast(SPELL_COLLAPSE_OF_NIGHT);
                    events.ScheduleEvent(EVENT_COLLAPSE_OF_NIGHT, globalTimer);
                    break;
                case EVENT_SUM_PLASMA_SPHERES:
                    Talk(SAY_S_PLASMA_SPHERES);
                    DoCast(SPELL_SUM_PLASMA_SPHERES);
                    events.ScheduleEvent(EVENT_SUM_PLASMA_SPHERES, globalTimer);
                    break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_nh_arcanist_telarn : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 1:
                globalTimer = 64 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 30000);
                events.ScheduleEvent(EVENT_CALL_OF_NIGHT, 57000);
                break;
            case 2:
                globalTimer = 55 * IN_MILLISECONDS;
                if (me->HasAura(SPELL_SOLAR_INFUSION))
                    events.ScheduleEvent(EVENT_SUMMON_CHAOS_SPHERES, 25000); // FIXME. Need Timing!
                else
                    events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 55000);
                events.ScheduleEvent(EVENT_CALL_OF_NIGHT, 42000);
                break;
            case 3:
                globalTimer = 35 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, 55000);
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CONTROLLED_CHAOS:
                    ...
                    DoCast(SPELL_CONTROLLED_CHAOS_FILTER);
                    events.ScheduleEvent(EVENT_CONTROLLED_CHAOS, globalTimer);
                    break;
                case EVENT_CALL_OF_NIGHT:
                    Talk(SAY_A_CALL_OF_NIGHT_WARN);
                    Talk(SAY_A_CALL_OF_NIGHT);
                    DoCast(SPELL_CALL_OF_NIGHT_FILTER);
                    events.ScheduleEvent(EVENT_CALL_OF_NIGHT, globalTimer);
                    break;
                case EVENT_SUMMON_CHAOS_SPHERES:
                    DoCast(SPELL_SUMMON_CHAOS_SPHERES);
                    events.ScheduleEvent(EVENT_SUMMON_CHAOS_SPHERES, globalTimer);
                    break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_nh_naturalist_telarn : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 0:
                globalTimer = 64 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_TOXIC_SPORES, 8000);
                events.ScheduleEvent(EVENT_GRACE_OF_NATURE, 9000);
                events.ScheduleEvent(EVENT_PARASITIC_FETTER, 35000);
                break;
            case 1:
                globalTimer = 64 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_TOXIC_SPORES, 6 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_GRACE_OF_NATURE, 64 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_PARASITIC_FETTER, 16 * IN_MILLISECONDS);
                break;
            case 2:
                globalTimer = 55 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_TOXIC_SPORES, 6 * IN_MILLISECONDS);
            ...
            case 3: //FIXME. Need Valid Timing
                globalTimer = 35 * IN_MILLISECONDS;
                events.ScheduleEvent(EVENT_TOXIC_SPORES, 6 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CHAOTIC_SPHERE_NATURE, 35 * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_PARASITIC_FETTER, 16 * IN_MILLISECONDS);
                break;
        }
    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_TOXIC_SPORES:
                    ...
                    events.ScheduleEvent(EVENT_TOXIC_SPORES, 8 * IN_MILLISECONDS);
                    break;
                case EVENT_GRACE_OF_NATURE:
                    Talk(SAY_N_GRACE_OF_NATURE);
                    DoCast(SPELL_GRACE_OF_NATURE_AT);
                    events.ScheduleEvent(EVENT_GRACE_OF_NATURE, globalTimer);
                    break;
                case EVENT_PARASITIC_FETTER:
                    Talk(SAY_N_PARASITIC_FETTER);
                    DoCast(SPELL_PARASITIC_FETTER_FILTER);
                    events.ScheduleEvent(EVENT_PARASITIC_FETTER, globalTimer);
                    break;
                case EVENT_CHAOTIC_SPHERE_NATURE:
                    DoCast(SPELL_SUM_CHAOTIC_SPHERE_NATURE);
                    events.ScheduleEvent(EVENT_CHAOTIC_SPHERE_NATURE, globalTimer);
                    break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_telarn_solar_collapse_stalker : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* owner) override
    {
        events.ScheduleEvent(EVENT_1, 1500); //12c-25y, 12c-20y, 6c-15y, 6c-10y, 6c-5y, 1c-0y
        events.ScheduleEvent(EVENT_2, 15000); //Despawn Event
