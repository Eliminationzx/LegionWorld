...
public:
    ...
    struct boss_chronomatic_anomalyAI : BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            {
                burstTimer = 3000;
                events.ScheduleEvent(EVENT_BURST_OF_TIME, 2000);
                events.ScheduleEvent(EVENT_CHANGE_SPEED, 12000);
            }
            else if (IsHeroicRaid())
            {
                burstTimer = 4000;
                events.ScheduleEvent(EVENT_BURST_OF_TIME, 8000);
            }
            else
            {
                burstTimer = 5000;
                events.ScheduleEvent(EVENT_BURST_OF_TIME, 14000);
            }
        }
        ...
        void SetAnomalySpeed(bool mythicFast = false)
        {
            ...
            {
                anomalySpeed = SPELL_SPEED_FAST;
                events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 7000);
                events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 22000);
            }

            events.ScheduleEvent(EVENT_CHRONOMETRIC_PARTICLES, 10000);
            ...
            switch (anomalySpeed)
            {
                case SPELL_SPEED_SLOW:
                    ...
                    if (IsMythicRaid())
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 14000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 16000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 28000);
                        events.ScheduleEvent(EVENT_TEMPORAL_ORB, 8000);
                        if (speedCounter > 3)
                            events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 18000);
                    }
                    else if (IsHeroicRaid())
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 10000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 15000);
                        events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 43000);
                        events.ScheduleEvent(EVENT_TEMPORAL_ORB, 20000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 53000);
                    }
                    else
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 5000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 20000);
                        events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 38000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 60000);
                        events.ScheduleEvent(EVENT_TEMPORAL_ORB, 30000);
                    }
                    Talk(SAY_TIME_SLOW);
                ...
                case SPELL_SPEED_NORMAL:
                    ...
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 10000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 5000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 20000);
                    }
                    else if (IsHeroicRaid())
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 8000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 28000);
                        events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 23000);
                        events.ScheduleEvent(EVENT_TEMPORAL_ORB, 38000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 53000);
                    }
                    else
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 8000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 35000);
                        events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 28000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 60000);
                        events.ScheduleEvent(EVENT_TEMPORAL_ORB, 48000);
                    }
                ...
                case SPELL_SPEED_FAST:
                    ...
                    if (IsMythicRaid())
                    {
                        ...
                        if (speedCounter == 3 || speedCounter == 6)
                            events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 23000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 30000);
                    }
                    else if (IsHeroicRaid())
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 5000);
                        events.ScheduleEvent(EVENT_TIME_BOMB, 17000);
                        events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 38000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 53000);
                    }
                    else
                    {
                        events.ScheduleEvent(EVENT_TIME_RELEASE, 10000);
                        events.ScheduleEvent(EVENT_SUM_SLOW_ADD, 28000);
                        events.ScheduleEvent(EVENT_TEMPORAL_ORB, 15000);
                        events.ScheduleEvent(EVENT_POWER_OVERWHELMING, 60000);
                    }
                    Talk(SAY_TIME_FAST);
                    Talk(SAY_ADDITIONAL);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_BURST_OF_TIME:
                        ...
                        events.ScheduleEvent(EVENT_BURST_OF_TIME, burstTimer);
                        break;
                    ...
                    case EVENT_TIME_RELEASE:
                        ...
                        if (timerBombCounter == 1)
                        {
                            ...
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TIME_RELEASE, 2000);
                                else if (IsHeroicRaid())
                                    events.ScheduleEvent(EVENT_TIME_RELEASE, 30000);
                                else
                                    events.ScheduleEvent(EVENT_TIME_RELEASE, 16000);
                            }
                            else if (anomalySpeed == SPELL_SPEED_SLOW)
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TIME_RELEASE, 7000);
                                else if (IsHeroicRaid())
                                    events.ScheduleEvent(EVENT_TIME_RELEASE, 5000);
                            }
                        }
                    ...
                    case EVENT_TIME_BOMB:
                        ...
                        if (timerBombCounter == 1)
                        {
                            ...
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 12000);
                                else if (IsHeroicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 5000);
                            }
                            else if (anomalySpeed == SPELL_SPEED_SLOW)
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 2000);
                                else if (IsHeroicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 20000);
                            }
                            ...
                            {
                                if (IsHeroicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 30000);
                            }
                        }
                        else if (timerBombCounter == 2)
                        {
                            ...
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 7000);
                            }
                        }
                        ...
                        else if (timerBombCounter == 4)
                        {
                            ...
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TIME_BOMB, 3000);
                            }
                        }
                        break;
                    ...
                    case EVENT_TEMPORAL_ORB:
                    {
                        ...
                        if (temporalOrbCounter == 1)
                        {
                            ...
                            {
                                if (IsMythicRaid())
                                    events.ScheduleEvent(EVENT_TEMPORAL_ORB, 14000);
                            }
                            else if (anomalySpeed == SPELL_SPEED_SLOW)
                            {
                                if (IsHeroicPlusRaid())
...
public:
    ...
    struct npc_anomaly_waning_time_particleAI : public ScriptedAI
    {
        ...
        void NonInterruptCast(Unit* caster, uint32 spellId, uint32 /*schoolMask*/) override
        {
            if (spellId == SPELL_WARP_NIGHTWELL_BIG)
            {
