...
public:
    ...
    struct boss_guldanAI : BossAI
    {
            ...
            switch(spell->Id)
            {
                ...
                case SPELL_FEL_OF_SARGERAS:
                {
                    ...
                    if (IsNormalRaid() || IsLfrRaid())
                        Trinity::Containers::RandomResizeList(threatList, (felOfSargerasCasts % 2) + 2); // 2-3
                    else
                        Trinity::Containers::RandomResizeList(threatList, (felOfSargerasCasts % 3) + 2); // 2-4
        ...
        void DefaultEvents(uint8 _phase)
        {
            ...
            switch(_phase)
            {
                case PHASE_FIRST: // first phase
                    events.ScheduleEvent(EVENT_LIQUID_HELLFIRE, 2000); 
                    events.ScheduleEvent(EVENT_HAND_OF_GULDAN_1, 6000);
                    events.ScheduleEvent(EVENT_HAND_OF_GULDAN_2, 6000+15000); 
                    events.ScheduleEvent(EVENT_HAND_OF_GULDAN_3, 6000+15000+10000); 
                    events.ScheduleEvent(EVENT_PEREPHASE_1_TO_2, 6000+15000+10000+3000);
                    events.ScheduleEvent(EVENT_FEL_EFFLUX, felEfluxTimer.popAndSafeLast());
                    break;
                case PHASE_PEREPHASE_1_TO_2: // from 1 to 2 
                    events.ScheduleEvent(EVENT_LIQUID_HELLFIRE, 5000); // + 32000
                    events.ScheduleEvent(EVENT_FEL_EFFLUX, 11000);
                    break;
                case PHASE_SECOND: // 2 phase
                {
                    ...
                    eyesOfGuldanCasts++;
                    events.ScheduleEvent(EVENT_EYE_OF_GULDAN, timer);
                    ...
                    events.ScheduleEvent(EVENT_BOUNDS_OF_FEL, timer);
                    events.ScheduleEvent(EVENT_LIQUID_HELLFIRE, liquidHellfireTimer.popAndSafeLast());
                    if (IsHeroicRaid())
                    {
                        events.ScheduleEvent(EVENT_HAND_OF_GULDAN_1, 33000);
                        events.ScheduleEvent(EVENT_HAND_OF_GULDAN_2, 33000+48000);
                        events.ScheduleEvent(EVENT_HAND_OF_GULDAN_3, 33000+48000+138800);
                    }
                    
                    if (IsMythicRaid())
                    {
                        events.ScheduleEvent(EVENT_HAND_OF_GULDAN_1, 17000);
                        events.ScheduleEvent(EVENT_HAND_OF_GULDAN_2, 17000+165000);
                    }
                    break;
                }
                case PHASE_THIRD:
                {
                    ...
                    events.ScheduleEvent(EVENT_BLACK_HARVEST, timer);
                    ...
                    if (IsMythicRaid())
                        ...
                        timer = 29000;
                    events.ScheduleEvent(EVENT_FEL_OF_SARGERAS, timer);
                    ...
                    if (IsMythicRaid())
                        ...
                        timer = 94000;
                    events.ScheduleEvent(EVENT_STORM_OF_DESTROYER, timer);

                    if (!IsLfrRaid())
                        events.ScheduleEvent(EVENT_SOUL_SIPHON, urand(25000, 30000));
                    break;
                }
            }
       }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                {                        
                    case EVENT_LIQUID_HELLFIRE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_LIQUID_HELLFIRE, timer);
                        break;
                    }
                    ...
                    case EVENT_BOUNDS_OF_FEL:
                    {
                        ...
                        events.ScheduleEvent(EVENT_BOUNDS_OF_FEL, boundsOfFelTimer);
                        break;
                    }
                    ...
                    case EVENT_BLACK_HARVEST:
                        ...
                        if (IsMythicRaid())
                        {
                            if (blackHarvestTimer.size() == 3)
                                events.ScheduleEvent(EVENT_WIND, 67000);
                            else if (blackHarvestTimer.size() == 2)
                                events.ScheduleEvent(EVENT_WIND, 75000);
                        }
                        events.ScheduleEvent(EVENT_SOUL_SIPHON, urand(25000, 30000));
                        events.ScheduleEvent(EVENT_BLACK_HARVEST, blackHarvestTimer.popAndSafeLast());
                        break;
                    case EVENT_FEL_OF_SARGERAS:
                    {
                        ...
                        if (felOfSargerasCasts == 2)
                            events.ScheduleEvent(EVENT_WIND, 31000);
                        break;
                    }
                    case EVENT_STORM_OF_DESTROYER:
                        ...
                        events.ScheduleEvent(EVENT_STORM_OF_DESTROYER, stormTimer.popAndSafeLast());
                        break;
        ...
        void DoVisualCircle(float min_radius, float max_radius)
        {
            ...
            uint32 timer = max_radius == 60 ? 4000 : 2000;
            ...
            for (uint8 j = min_radius; j <= max_radius; j+= (max_radius-min_radius) / 2)
                for (uint8 i = 0; i < 7; angle += step, ++i)
                {
                    for (uint8 x = urand(0, 1); x < 2; ++x)
                    {
                        ...
                        Creature* _me = me;
                        me->AddDelayedEvent(urand(i, i + 2)*timer / 12 + urand(j, timer), [_me, point]() -> void
                        {
                            if (_me)
            ...
            switch(uint32(max_radius))
            {
                case 10:
                    ...
                    me->AddDelayedEvent(timer, [this] () -> void { DoVisualCircle(10.0f, 20.0f); });
                    break;
                case 20:
                    visual_id = STORM_OF_THE_DESTROYER_ORPHAN_2;
                    me->AddDelayedEvent(timer, [this] () -> void { DoVisualCircle(20.0f, 30.0f); });
                    break;
                case 30:
                    visual_id = STORM_OF_THE_DESTROYER_ORPHAN_3;
                    me->AddDelayedEvent(timer, [this] () -> void { DoVisualCircle(30.0f, 60.0f); });
                    break;
                case 60:
            ...
            me->AddDelayedEvent(timer, [this, visual_id] () -> void
            {
                if (!UpdateVictim())
                    return;
        ...
        void SpellFinishCast(const SpellInfo* spell) override
        {
            switch(spell->Id)
            {
                ...
                case SPELL_DEMON_INTRO_END:
                    ...
                    me->SetPower(POWER_ENERGY, 0);
                    DoZoneInCombat(me);
                    events.ScheduleEvent(EVENT_PARASITIC,  8000);
                    events.ScheduleEvent(EVENT_AZZINOTHES, 26000);
                    events.ScheduleEvent(EVENT_NIGHTHOLD_SPHERE, nightOrbTimer.popAndSafeLast());
                    events.ScheduleEvent(EVENT_DARK_TITAN, darkTitanTimer.popAndSafeLast());
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_ATTACKABLE_1);
                    break;
            }
        }
        ...
        {      
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_PARASITIC:
                        ...
                        events.ScheduleEvent(EVENT_PARASITIC, 36000);
                        break;
                    ...
                    case EVENT_AZZINOTHES:
                        ...
                        events.ScheduleEvent(EVENT_AZZINOTHES, 47000);
                        break;
            ...
            if (DoMeleeAttackIfReady())
            {
                ...
                if (me->GetPower(POWER_ENERGY) >= 100)
                {
                    ...
                    events.ScheduleEvent(EVENT_FULL_ENERGY, 1);
                }
            }
        }
    };
        ...
        void EnterCombat(Unit* ) override
        {
            switch(me->GetEntry())
            {
                case NPC_KURAZMAL:
                    ...
                    events.ScheduleEvent(EVENT_1, IsMythicRaid() ? 21000: 19000); 
                    if (IsHeroicRaid() || IsMythicRaid())
                        events.ScheduleEvent(EVENT_5, IsMythicRaid() ? 16000 : 23000);
                    break;
                case NPC_VETRIZ:
                    me->SetPower(POWER_ENERGY, 0);
                    can_regenerate_energy = true;
                    ...
                    break;
                case NPC_DZORIKS:
                    events.ScheduleEvent(EVENT_4, IsMythicRaid() ? 3000 :  35000);
                    if (IsHeroicRaid() || IsMythicRaid())
                        events.ScheduleEvent(EVENT_6, 19000 );
                    break;
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        events.ScheduleEvent(EVENT_1, IsMythicRaid() ? 21000: 54000);
                        break;
                    case EVENT_2:
                        if (me->GetPower(POWER_ENERGY) >= 100)
                    ...
                    case EVENT_6:
                    {
                        ...
                        events.ScheduleEvent(EVENT_6, 26000 );
                        break;
                    }
                }
            }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                    {
                        ...
                        else if (me->GetAnyOwner()->ToCreature())
                        {
                            Unit* target = ObjectAccessor::GetUnit(*me, targetGUID);
                            if (!target)
                                target = me->GetAnyOwner()->ToCreature()->AI()->SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true);
                            if (target)
                            {
                                targetGUID = target->GetGUID();
    ...
    public:
        ...
        class spell_shadow_shroud_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* /*aurEff*/, DamageInfo & dmgInfo, float & absorbAmount)
            {
                ...
                if (caster->GetDistance2d(attacker) < 15.0f)
