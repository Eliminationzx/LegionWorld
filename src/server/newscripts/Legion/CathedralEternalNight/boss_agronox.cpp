...
public:
    ...
    struct boss_agronoxAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_TIMBER_SMASH, 5000); 
            events.ScheduleEvent(EVENT_FLUMINATING, 10000);
            events.ScheduleEvent(EVENT_FLUMINATING_2, 40000);
            events.ScheduleEvent(EVENT_FLUMINATING_DEBUFF, 10000);
            events.ScheduleEvent(EVENT_CHOKING_VINES, 24000);
            
            if (GetDifficultyID() != DIFFICULTY_HEROIC) // mythic and other. normal mode isn't exist
                events.ScheduleEvent(EVENT_SUCCULENT_LAHSERS, 18000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_TIMBER_SMASH:
                        ...
                        events.ScheduleEvent(EVENT_TIMBER_SMASH, 31000);
                        break;
                    ...
                    case EVENT_FLUMINATING_DEBUFF:
                        ...
                        events.ScheduleEvent(EVENT_FLUMINATING_DEBUFF, 20000);
                        break;
                    case EVENT_CHOKING_VINES:
                        ...
                        Talk(SAY_VINES);
                        events.ScheduleEvent(EVENT_CHOKING_VINES, 35000); 
                        break;
                    case EVENT_SUCCULENT_LAHSERS:
                        DoCast(SPELL_SUCCULENT_LASHERS);
                        events.ScheduleEvent(EVENT_SUCCULENT_LAHSERS, urand(30000, 42000)); // real random ?
                        break;
                }
            }
