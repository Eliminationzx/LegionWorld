...
public:
    ...
    struct boss_thrashbite_the_scornfulAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_PULVERIZING_CUDGEL, 5000);
            events.ScheduleEvent(EVENT_HEAVE_CUDGEL, 14000);
            events.ScheduleEvent(EVENT_SCORNFUL_GAZE, 29000);
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
                events.ScheduleEvent(EVENT_SCORNFUL_DMG, 10);
            else
                events.ScheduleEvent(EVENT_SCORNFUL_STUN, 10);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_PULVERIZING_CUDGEL:
                        ...
                        events.ScheduleEvent(EVENT_PULVERIZING_CUDGEL, 34000);
                        break;
                    case EVENT_HEAVE_CUDGEL:
                        DoCast(SPELL_HEAVE_CUDGEL);
                        events.ScheduleEvent(EVENT_HEAVE_CUDGEL, 36000);
                        break;
                    case EVENT_SCORNFUL_GAZE:
                        Talk(SAY_SCORNFUL);
                        // if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                        DoCast(SPELL_SCORNFUL_GAZE);
                        events.ScheduleEvent(EVENT_SCORNFUL_GAZE, 36000);
                        break;
