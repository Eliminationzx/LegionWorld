    ...
    struct npc_q38689AI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            if (me->GetEntry() == 92782)
                events.ScheduleEvent(EVENT_1, 15000); // 200992
            if (me->GetEntry() == 92776)
            {
                if (urand(0, 1) == 1)
                    Talk(urand(0, 10));
                events.ScheduleEvent(EVENT_2, 14000); // 200930
                events.ScheduleEvent(EVENT_3, 20000); // 200963
            }
        }
...
public:
    ...
    struct npc_q38723AI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            {
                events.ScheduleEvent(EVENT_1, 20000); // 199602
                events.ScheduleEvent(EVENT_4, 5000);
            }
            if (me->GetEntry() == 97632)
                events.ScheduleEvent(EVENT_2, 16000); // 199645
            events.ScheduleEvent(EVENT_3, 42000); // 199556
        }
    ...
    struct npc_q39683AI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* victim) override
        {
         ...
         //   me->SetHealth(float(me->GetMaxHealth() / 100.0f)*health_pct);
            me->SetHealth(me->CountPctFromMaxHealth(21));
            events.ScheduleEvent(EVENT_1, 20000); // 199758
            events.ScheduleEvent(EVENT_2, 15000); // 199836
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(199758);
                        events.ScheduleEvent(EVENT_1, 40000); // 199758
                        break;
                    case EVENT_2:
                        DoCast(199836);
                        events.ScheduleEvent(EVENT_2, 31000); // 199836
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_q39694AI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* victim) override
        {
            ...
            events.ScheduleEvent(EVENT_4, 23000);
        }
