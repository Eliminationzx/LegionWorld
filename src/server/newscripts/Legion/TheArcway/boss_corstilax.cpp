...
public:
    ...
    struct boss_corstilaxAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_PROTOCOL, 5000);
            events.ScheduleEvent(EVENT_QUARANTINE, 20000);
            events.ScheduleEvent(EVENT_CLEANSING_FORCE, 28000); 
            events.ScheduleEvent(EVENT_ENERGY_BURST, 18000);
            events.ScheduleEvent(EVENT_DESTABILIZED_ORB, 13000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_DESTABILIZED_ORB:
                        ...
                        events.ScheduleEvent(EVENT_DESTABILIZED_ORB, 32000);
                        break;
                }
            }
