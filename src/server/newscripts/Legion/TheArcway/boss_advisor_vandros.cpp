...
public:
    ...
    struct boss_advisor_vandrosAI : public BossAI
    {
        ...
        void DefaultEvents()
        {
            events.ScheduleEvent(EVENT_A_BLAST, 3000);
        ...
        void OnAreaTriggerCast(Unit* caster, Unit* target, uint32 spellId, uint32 createATSpellId) override
        {
            if (spellId == SPELL_BANISH_IN_TIME_REMOVE && !specialPhaseComplete)
            {
                ...
                Talk(SAY_TIME_REM);
                DefaultEvents();
                events.ScheduleEvent(EVENT_UNSTABLE_MANA, 3000);
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_UNSTABLE_MANA:
                        ...
                        events.ScheduleEvent(EVENT_UNSTABLE_MANA, 30000);
                        break;
                }
            }
