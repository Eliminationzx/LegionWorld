...
public:
    ...
    struct boss_naltiraAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_BLINK_STRIKES_1, 15000);
            events.ScheduleEvent(EVENT_NETHER_VENOM, 26000);
            events.ScheduleEvent(EVENT_TANGLED_WEB, 32000);

            if (GetDifficultyID() != DIFFICULTY_NORMAL)
                events.ScheduleEvent(EVENT_SUM_MANAFANG, 25000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SUM_MANAFANG:
                        ...
                        events.ScheduleEvent(EVENT_SUM_MANAFANG, 22000);
                        break;
                }
            }
