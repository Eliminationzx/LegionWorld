...
struct boss_tov_odyn : BossAI
{
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (instance->GetBossState(Data::BossIDs::OdynID) != DONE)
        {
            if ((instance->GetData(Data::DataMisc::OdynIntroEvent) != DONE) && !introDone && me->IsWithinDistInMap(who, 200.0f))
            {
    ...
    void SetPower(Powers power, int32 value) override
    {
        ...
        if (value == 100)
        {
            if (!unneringBlast && !me->HasAura(SpellRunicBrandMythic))
            {
                unneringBlast = true;
    ...
    void JustSummoned(Creature* sum) override
    {
        ...
        case Data::Creatures::ValarjarRunebearer:
            if (Unit* target = ObjectAccessor::GetUnit(*sum, sum->GetTargetGUID()))
            {
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
    {
        ...
        if (me->HealthBelowPct(10) && !encounterComplete)
        {
            ...
            DespawnAllSummons();
            RemoveDebuffsOnPlayer();
            instance->DoUpdateAchievementCriteria(CRITERIA_TYPE_COMPLETE_DUNGEON_ENCOUNTER, 1958);
        }
    ...
    void sGossipSelect(Player* player, uint32 /*sender*/, uint32 action) override
    {
        if (action = 1)
        {
            player->CastSpell(player, SpellSkyJump);
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_1:
            if ((instance->GetData(Data::DataMisc::OdynEventComplete1) == DONE) && (instance->GetData(Data::DataMisc::OdynEventComplete2) == DONE))
            {
                ...
                events.ScheduleEvent(EVENT_START_ODYN, 7000);
                events.CancelEvent(EVENT_FELMAGNARON_RUNE);
            }
            break;
        case ACTION_2:
            events.ScheduleEvent(EVENT_INTRO, 55000);
            break;
        case ACTION_3:
            BossAI::EnterEvadeMode();
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            ...
            events.ScheduleEvent(EVENT_FELMAGNARON_RUNE, 40000);
            break;
        case PHASE_2:
            if (auto player = me->FindNearestPlayer(300.0f))
            {
            ...
            disableEvade = false;
            DoCast(me, SpellSpearTransitionHoly);
            events.ScheduleEvent(EVENT_SPEAR_OF_LIGHT, 5000);
            events.ScheduleEvent(EVENT_ODYNS_TEST, 30000);
            events.ScheduleEvent(EVENT_FELMAGNARON_RUNE, 40000);

            if (IsHeroicPlusRaid())
            {
                instance->SetData(Data::DataMisc::OdynTestOfAges, DONE);
                events.ScheduleEvent(EVENT_PHASE_TWO_HYRJA, 10000);
                events.ScheduleEvent(EVENT_PHASE_TWO_HYMDALL, 76000);
            }
            if (IsMythicRaid())
                if (instance->GetBossState(Data::BossIDs::OdynID) != DONE)
        ...
        case PHASE_3:
            ...
            me->AttackStop();
            DespawnAllSummons();
            events.ScheduleEvent(EVENT_START_THIRD_PHASE, 4000);
            break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_START_ODYN:
                ...
                events.ScheduleEvent(EVENT_START_SECOND_PHASE, 2000);
                break;
            ...
            case EVENT_START_THIRD_PHASE:
                me->SetReactState(REACT_AGGRESSIVE);
                DoCast(me, SpellSpearTransitionThunder);
                DoCast(me, SpellArcingStormProc);
                events.ScheduleEvent(EVENT_STORM_OF_JUSTICE, 4000);
                events.ScheduleEvent(EVENT_STORM_FORGED_SPEAR, 14000);
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_1, 1000);
                events.ScheduleEvent(EVENT_ARCING_STORM_BUFF, 6000);
                break;
            case EVENT_FELMAGNARON_RUNE:
                ...
                events.ScheduleEvent(EVENT_FELMAGNARON_RUNE, 70000);
                break;
            case EVENT_ODYNS_TEST:
                DoCast(me, SpellOdynsTest);
                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_ODYNS_TEST, 80000);
                else
                    events.ScheduleEvent(EVENT_ODYNS_TEST, 70000);
                break;
            case EVENT_STORM_FORGED_SPEAR:
                DoCast(SpellStormForgedSpear);
                events.ScheduleEvent(EVENT_STORM_FORGED_SPEAR, 11000);
                break;
            case EVENT_STORM_OF_JUSTICE:
            {
                DoCast(SpellStormOfJustice);
                events.ScheduleEvent(EVENT_STORM_OF_JUSTICE, 12000);
                break;
            }
            ...
            case EVENT_CLEANSING_FLAME_AT_1:
                ...
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_1, 105000);
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_2, 35000);
                break;
            case EVENT_CLEANSING_FLAME_AT_2:
                DoCast(SpellCleansingFlameAT_2);
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_2, 105000);
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_3, 35000);
                break;
            case EVENT_CLEANSING_FLAME_AT_3:
                DoCast(SpellCleansingFlameAT_3);
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_3, 105000);
                events.ScheduleEvent(EVENT_CLEANSING_FLAME_AT_1, 35000);
                break;
            case EVENT_PHASE_TWO_HYMDALL:
                ...
                events.ScheduleEvent(EVENT_PHASE_TWO_HYMDALL, 135000);
                break;
            case EVENT_PHASE_TWO_HYRJA:
                ...
                events.ScheduleEvent(EVENT_PHASE_TWO_HYRJA, 135000);
                break;
            case EVENT_ARCING_STORM_BUFF:
                DoCast(me, SpellArcingStormBuff);
                events.ScheduleEvent(EVENT_ARCING_STORM_BUFF, 9000);
                break;
            }
        }
        ...
        if (RemoveInCorrectAuras)
        {
            ...
            else RemoveInCorrectAuras -= diff;
        }
...
struct npc_tov_hymdall : ScriptedAI
{
    ...
    {
        DoZoneInCombat();
        instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, me, 3);

        if (IsMythicRaid())
        {
            events.ScheduleEvent(EVENT_HORN_OF_VALOUR, 8000);
            events.ScheduleEvent(EVENT_DANCING_BLADE, 15000);
        }
        else if (IsHeroicRaid())
        {
            events.ScheduleEvent(EVENT_HORN_OF_VALOUR, 8000);
            events.ScheduleEvent(EVENT_DANCING_BLADE, 16000);
        }
        else if (IsNormalRaid() || IsLfrRaid())
        {
            events.ScheduleEvent(EVENT_HORN_OF_VALOUR, 10000);
            events.ScheduleEvent(EVENT_DANCING_BLADE, 20000);
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_HORN_OF_VALOUR:
                ...
                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_HORN_OF_VALOUR, 22000);
                else if (IsHeroicRaid())
                    events.ScheduleEvent(EVENT_HORN_OF_VALOUR, 27000);
                else if (IsLfrRaid() || IsNormalRaid())
                    events.ScheduleEvent(EVENT_HORN_OF_VALOUR, 69000);

                break;
            case EVENT_DANCING_BLADE:
                if (auto target = MS::ScriptUtils::SelectRandomPlayerExcludedTank(me, 300.0f))
                    DoCast(target, SpellDancingBlade);

                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_DANCING_BLADE, 20000);
                else
                    events.ScheduleEvent(EVENT_DANCING_BLADE, 30000);
                break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_tov_hyrja : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        if (IsMythicRaid())
        {
            ...
            events.ScheduleEvent(EVENT_EXPEL_LIGHT, 25000);
        }
        else if (IsHeroicRaid())
        {
            events.ScheduleEvent(EVENT_SHIELD_OF_LIGHT, 23000);
            events.ScheduleEvent(EVENT_EXPEL_LIGHT, 32000);
        }
        else if (IsNormalRaid() || IsLfrRaid())
        {
            events.ScheduleEvent(EVENT_SHIELD_OF_LIGHT, 30000);
            events.ScheduleEvent(EVENT_EXPEL_LIGHT, 40000);
        }
        ...
        if (instance->GetData(Data::DataMisc::OdynTestOfAges) != DONE)
        {
            ...
            if (auto Hyrja = instance->instance->GetCreature(instance->GetGuidData(Data::Creatures::Hymdall)))
                if (auto player = me->FindNearestPlayer(300.0f))
                    Hyrja->AI()->AttackStart(player);
        }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_EXPEL_LIGHT:
                if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, 300.0f, true))
                    DoCast(target, SpellExpelLight);

                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_EXPEL_LIGHT, 20000);
                else if (IsHeroicRaid())
                    events.ScheduleEvent(EVENT_EXPEL_LIGHT, 32000);
                else if (IsLfrRaid() || IsNormalRaid())
                    events.ScheduleEvent(EVENT_EXPEL_LIGHT, 19500);
                break;
            case EVENT_SHIELD_OF_LIGHT:
                if (auto target = MS::ScriptUtils::SelectRandomPlayerExcludedTank(me, 300.0f))
                {
                    me->SetReactState(REACT_PASSIVE);
                    events.ScheduleEvent(EVENT_SHIELD_OF_LIGHT_REACT, 5500);
                    DoCast(target, SpellShieldOfLight2);
                    DoCast(target, SpellShieldOfLight);
                    DoCast(target, SpellShieldOfLight3);
                ...
                if (IsMythicRaid())
                    ...
                    events.ScheduleEvent(EVENT_SHIELD_OF_LIGHT, 32000);
                break;
...
struct npc_valarjar_runebearer : ScriptedAI
{
    ...
    void IsSummonedBy(Unit* /*owner*/) override
    {
        DoCast(me, SpellRunebearerSpawn);
        me->SetReactState(REACT_PASSIVE);
    }
    ...
    void Periodic(AuraEffect const* aurEff)
    {
        ...
        if (auto odyn = GetCaster()->ToCreature())
        {
            ...
            for (std::list<Player*>::iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                if (!playerList.empty())
                    odyn->CastSpell((*itr), SpellArcingStorm, true);
        }
    }
...
class spell_odyn_test : public AuraScript
{
    ...
    void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        odyn->CastSpell(odyn, SpellOdynsTestProc, false);
    }
        ...
        if (GetId() == SpellBrandedValarjarBlue)
        {
            ...
            else
            {
                ...
                for (std::list<Player*>::iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                {
                    if (!playerList.empty())
                    {
                        if (Player* players = (*itr)->ToPlayer())
                        {
