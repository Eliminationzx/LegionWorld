    ...
    class spell_vantus_rune_trial_of_valor_AuraScript : public AuraScript
    {
        ...
        void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Player* player = caster->ToPlayer())
                ...
                {
                    Quest const* quest = sQuestDataStore->GetQuestTemplate(39695);
        ...
        void OnUpdate(uint32 diff, AuraEffect* /*aurEff*/)
        {
            ...
            if (checkOnProc <= diff)
            {
                if (instance->GetBossState(Data::BossIDs::GarmID) == IN_PROGRESS)
                {
                    if (!player->HasAura(229187) && player->HasAura(229175))
                    {
                ...
                {
                    if (!player->HasAura(229186) && player->HasAura(229174))
                    {
                        player->CastSpell(player, 229186, false);
                    }
                }
                if (instance->GetBossState(Data::BossIDs::HelyaID) == IN_PROGRESS)
                {
                    if (!player->HasAura(229188) && player->HasAura(229176))
                    {
