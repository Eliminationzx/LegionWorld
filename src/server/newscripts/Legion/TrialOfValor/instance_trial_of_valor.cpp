...
public:
    ...
    struct instance_trial_of_valor_InstanceMapScript : InstanceScript
    {
            ...
            switch (creature->GetEntry())
            {
                case Data::Creatures::Odyn:
                    ...
                    if (GetBossState(Data::BossIDs::OdynID) == DONE)
                    {
                        ...
                        creature->CastSpell(creature, 227882);
                    }
                    break;
        ...
        std::string GetSaveData() override
        {
            ...
            sLog->outError(LOG_FILTER_UWOW_CORE, "instance_trial_of_valor GetSaveData %s", saveStream.str().c_str());
        ...
        void Load(const char* data) override
        {
            if (!data)
            {
                ...
                sLog->outError(LOG_FILTER_UWOW_CORE, "instance_trial_of_valor Load OUT_LOAD_INST_DATA_FAIL");
                return;
            }
            ...
            if (dataHead1 == 'T' && dataHead2 == 'O' && dataHead3 == 'V')
            {
                ...
                OUT_LOAD_INST_DATA_FAIL;
            sLog->outError(LOG_FILTER_UWOW_CORE, "instance_trial_of_valor Load %s", loadStream.str().c_str());
