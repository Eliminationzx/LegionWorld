...
namespace
{
    enum Conversations
    {
        CONV_ODYN_INTRO_1   = 3916,
        CONV_ODYN_INTRO_2   = 3917
    };
...
struct boss_garm : BossAI
{
        ...
        me->AddDelayedEvent(2000, [=]() -> void
        {
            for (auto x = 0; x < std::extent<decltype(passengerData)>::value; x++)
                if (auto passenger = me->SummonCreature(passengerData[x][0], me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()))
                {
                    ...
                    passenger->SetUnitMovementFlags(MOVEMENTFLAG_DISABLE_GRAVITY | MOVEMENTFLAG_ROOT | MOVEMENTFLAG_DISABLE_COLLISION);
                    passenger->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNK_15 | UNIT_FLAG_NOT_SELECTABLE);
    ...
    {
        if (instance->GetData(Data::DataMisc::GarmIntroEvent) != DONE)
        {
            if (!IntroDone && me->IsWithinDistInMap(who, 200.0f))
            {
                IntroDone = true;
                instance->SetData(Data::DataMisc::GarmIntroEvent, DONE);
                me->CreateConversation(CONV_ODYN_INTRO_1);
            }
        }
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        RemoveDebaffsOnPlayer();
        me->CreateConversation(CONV_ODYN_INTRO_2);
    }
    ...
    void SpellHit(Unit* /*owner*/, SpellInfo const* spell) override
    {
        ...
        case SpellHeadlongCharge:
            ...
            events.ScheduleEvent(EVENT_HEADLONG_CHARGE_WP, 2000);
            break;
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            DoCast(SpellHelyatosis);
            events.ScheduleEvent(EVENT_OFF_THE_LEASH, 30000);
            events.ScheduleEvent(EVENT_FLAME_LICK, 12000);
            events.ScheduleEvent(EVENT_FROST_LICK, 24000);
            events.ScheduleEvent(EVENT_SHADOW_LICK, 27000);
            if (IsMythicRaid())
            {
                events.ScheduleEvent(EVENT_FLAMING_VOLATILE_FOAM, 10000);
                events.ScheduleEvent(EVENT_BRINEY_VOLATILE_FOAM, 10000);
                events.ScheduleEvent(EVENT_SHADOWY_VOLATILE_FOAM, 10000);
            }
            break;
        case PHASE_2:
            me->RemoveAura(SpellHelyatosis);
            ...
            events.ScheduleEvent(EVENT_ROARING_LEAP, 2000);
            events.ScheduleEvent(EVENT_HEADLONG_CHARGE, 7000);
            break;
        }
    }
    ...
    void MovementInform(uint32 type, uint32 data)
    {
        if (type == POINT_MOTION_TYPE)
        {
            ...
            {
            case 1:
                events.ScheduleEvent(EVENT_POINT_2, 100);
                break;
            case 2:
                events.ScheduleEvent(EVENT_POINT_3, 100);
                break;
            case 3:
                events.ScheduleEvent(EVENT_POINT_COMPLETE, 100);
                break;
            }
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_HEADLONG_CHARGE:
                ...
                DoCast(me, SpellHeadlongCharge);
                break;
            case EVENT_HEADLONG_CHARGE_WP:
                if (!berserkcharge)
                    DoCast(me, SpellHeadlongChargeAT);
                else
                    DoCast(me, SpellBerserkCharge);
            ...
            case EVENT_ROARING_LEAP:
                ...
                events.ScheduleEvent(EVENT_ROARING_LEAP_JUMP, 2000);
                break;
            ...
            case EVENT_FLAME_LICK:
                if (eventPhase != PHASE_2)
                {
                    ...
                    events.ScheduleEvent(EVENT_FLAME_LICK, 13000);
                }
                break;
            ...
            case EVENT_FLAMING_VOLATILE_FOAM:
                if (eventPhase != PHASE_2)
                {
                    ...
                    events.ScheduleEvent(EVENT_FLAMING_VOLATILE_FOAM, 30000);
                }
                break;
            case EVENT_BRINEY_VOLATILE_FOAM:
                if (eventPhase != PHASE_2)
                {
                    ...
                    events.ScheduleEvent(EVENT_BRINEY_VOLATILE_FOAM, 30000);
                }
                 break;
            case EVENT_SHADOWY_VOLATILE_FOAM:
                if (eventPhase != PHASE_2)
                {
                    ...
                    events.ScheduleEvent(EVENT_SHADOWY_VOLATILE_FOAM, 30000);
                }
                break;
            }
        }
