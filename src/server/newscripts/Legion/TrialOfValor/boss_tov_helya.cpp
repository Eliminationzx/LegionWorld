...
struct boss_helya_tov : BossAI
{
    ...
    void Reset() override
    {
        ...
        events.ScheduleEvent(EVENT_REANIMATION, 1000);
    }
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        {
            if (!HelyaIntro && me->IsWithinDistInMap(who, 200.0f))
            {
                HelyaIntro = true;
                Talk(SAY_INTRO_1);
    ...
    void SetPower(Powers power, int32 value) override
    {
        if (power == POWER_ENERGY)
        {
            if (value == 100 && eventPhase == PHASE_1)
            {
                ...
                {
                    DoCast(me, SpellBilewaterBreath, false);
            ...
            if (value == 100 && eventPhase == PHASE_3)
            {
                if (!me->HasUnitState(UNIT_STATE_CASTING))
                {
                    DoCast(me, SpellCorruptedBreath, false);
                ...
                if (!me->HasUnitState(UNIT_STATE_CASTING))
                {
                    ...
                    else if (IsLfrRaid() || IsNormalRaid())
                    {
                        if (eventPhase == PHASE_2)
                        {
                            ...
                            events.ScheduleEvent(EVENT_FURY_OF_THE_MAW_CLEAN_AT, 4000);
                            me->AddDelayedCombat(500, [=]() -> void { me->SetPower(POWER_ALTERNATE, 0); });
                        }
                    }
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            ...
            if (IsMythicRaid())
            {
                events.ScheduleEvent(EVENT_TAINT_OF_THE_SEA, 15000);
                events.ScheduleEvent(EVENT_TENTECLE_STRIKE, 35000);
                events.ScheduleEvent(EVENT_ORB_OF_CURRUPTION, 14000);
            }
            else if (IsHeroicRaid())
            {
                events.ScheduleEvent(EVENT_TAINT_OF_THE_SEA, 19000);
                events.ScheduleEvent(EVENT_TENTECLE_STRIKE, 36000);
                events.ScheduleEvent(EVENT_ORB_OF_CURRUPTION, 29000);
            }
            else if (IsNormalRaid() || IsLfrRaid())
            {
                events.ScheduleEvent(EVENT_TAINT_OF_THE_SEA, 12000);
                events.ScheduleEvent(EVENT_TENTECLE_STRIKE, 53000);
                events.ScheduleEvent(EVENT_ORB_OF_CURRUPTION, 18000);
            }
            ...
            break;
        case PHASE_2:
            events.ScheduleEvent(EVENT_TELEPORT_OUTSIDE, 1000);
            checkrephase = true;
            ...
            me->AddDelayedCombat(5000, [=]() -> void
            {
                DoCast(SpellHelyaReuse);
                Talk(SAY_PHASE_2);
                events.ScheduleEvent(EVENT_TORRENT, 9000);
                events.ScheduleEvent(EVENT_REANIMATION, 30000);
                if (IsLfrRaid() || IsNormalRaid())
                {
                    DoCast(SpellHelyaAltPowerRegen);
                    events.ScheduleEvent(EVENT_FURY_OF_THE_MAW, 2000);
                }
            ...
            break;
        case PHASE_3:
            events.ScheduleEvent(EVENT_TELEPORT_HOME, 2000);
            DoCast(SpellHelyaReuse);
            me->CastStop();
            ...
            me->AddDelayedCombat(5000, [=]() -> void
            {
                ...
                events.ScheduleEvent(EVENT_REANIMATION, 52000);
                if (IsMythicRaid())
                    events.ScheduleEvent(EVENT_ORB_OF_CORROSION, 6000);
                else if (IsHeroicRaid())
                    events.ScheduleEvent(EVENT_ORB_OF_CORROSION, 14000);
                else if (IsNormalRaid() || IsLfrRaid())
                    events.ScheduleEvent(EVENT_ORB_OF_CORROSION, 18000);
            });
            break;
        }
    }
    ...
    void SpellHit(Unit* owner, SpellInfo const* spell) override
    {
        ...
        case SpellOrbOfCorruption:
        {
            ...
            for (Player* player : playerList)
            {
                ...
                {
                    me->SummonCreature(Data::Creatures::OrbOfCorruption, pos, playerGuid, TEMPSUMMON_TIMED_DESPAWN, 9000);
                });
            }
            break;
        }
        case SpellOrbOfCorrosion:
        {
            ...
            for (Player* player : playerList)
            {
                ...
                me->AddDelayedCombat(2000, [=]() -> void
                {
                    me->SummonCreature(Data::Creatures::OrbOfCorrosion, pos, playerGuid, TEMPSUMMON_TIMED_DESPAWN, 9000);
                });
            }
            break;
        }
    ...
    void SpellFinishCast(SpellInfo const* spell) override
    {
        ...
        case SpellFuryOfTheMawAT:
            if (IsHeroicPlusRaid())
            {
                ...
                {
                    events.ScheduleEvent(EVENT_BOAT, 500);
                    events.ScheduleEvent(EVENT_SUMMON_SKELETION, 31000);
                    events.ScheduleEvent(EVENT_SUMMON_SKELETION, 41000);
                    FuryOfTheMawTrash = true;
                }
                events.ScheduleEvent(EVENT_FURY_OF_THE_MAW_CLEAN_AT, 3000);
            }
            else if (IsLfrRaid() || IsNormalRaid())
            {
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_1:
            DoCast(SpellHelyaAltPowerRegen);
            events.ScheduleEvent(EVENT_TORRENT, 2000);
            break;
        default:
            break;
        }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_TAINT_OF_THE_SEA:
            {
                if (eventPhase == PHASE_1 || eventPhase == PHASE_3)
                {
                    ...
                    if (IsMythicRaid())
                        ...
                        events.ScheduleEvent(EVENT_TAINT_OF_THE_SEA, 14000);
                    else if (IsNormalRaid() || IsLfrRaid())
                        events.ScheduleEvent(EVENT_TAINT_OF_THE_SEA, 12000);
                }
                break;
            ...
            case EVENT_TENTECLE_STRIKE:
                if (eventPhase == PHASE_1)
                {
                    ...
                    if (IsMythicRaid())
                        ...
                        events.ScheduleEvent(EVENT_TENTECLE_STRIKE, 30000);
                }
                break;
            case EVENT_ORB_OF_CURRUPTION:
                if (eventPhase == PHASE_1)
                {
                    ...
                    if (IsMythicRaid())
                        events.ScheduleEvent(EVENT_ORB_OF_CURRUPTION, 23000);
                    else
                        events.ScheduleEvent(EVENT_ORB_OF_CURRUPTION, 26000);
                }
                break;
            case EVENT_ORB_OF_CORROSION:
                if (eventPhase == PHASE_3)
                {
                    ...
                    if (IsMythicRaid())
                        events.ScheduleEvent(EVENT_ORB_OF_CORROSION, 13000);
                    else
                        events.ScheduleEvent(EVENT_ORB_OF_CORROSION, 17000);
                }
                break;
            case EVENT_TORRENT:
                if (!me->HasUnitState(UNIT_STATE_CASTING))
                {
                    if (eventPhase == PHASE_2)
                    {
                        if (!me->HasAura(SpellFureOfTheMawLong))
                        {
                            ...
                            {
                                DoCast(SpellTorrent);
                                events.ScheduleEvent(EVENT_TORRENT, 10000);
                            }
                        }
                    }
                }
            ...
            case EVENT_REANIMATION:
                if (eventPhase == 0)
                {
                    ...
                    events.ScheduleEvent(EVENT_REANIMATION, 15000);
                }
                break;
...
struct npc_orb_of_corruption : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
            case EVENT_1:
                if (Unit* target = ObjectAccessor::GetUnit(*me, me->GetTargetGUID()))
...
struct npc_bilewater_slime : ScriptedAI
{
    ...
    void SpellHit(Unit* /*owner*/, SpellInfo const* spell) override
    {
        switch (spell->Id)
        {
        case SpellBilewaterLiquefaction:
            if (!spellhit)
            {
                spellhit = true;
...
struct npc_grimelord : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        events.ScheduleEvent(EVENT_1, 14000);
        events.ScheduleEvent(EVENT_2, 16000);
        events.ScheduleEvent(EVENT_3, 9000);
    }
    ...
    void SetPower(Powers power, int32 value) override
    {
        if (power == POWER_ENERGY)
        {
            if (value == 100)
            {
                ...
                me->AddDelayedCombat(1000, [=]() -> void { me->SetPower(POWER_ENERGY, 0); });
            }
        }
    }
...
struct npc_decaying_minion : ScriptedAI
{
    ...
    void IsSummonedBy(Unit* /*summoner*/) override
    {
        events.ScheduleEvent(EVENT_3, urand(500, 2000));
    }
...
struct npc_striking_tentacle : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        me->AddDelayedCombat(1000, [=]() -> void
        {
            if (auto target = me->FindNearestPlayer(40.0f))
    ...
    void MovementInform(uint32 type, uint32 data)
    {
        if (type == POINT_MOTION_TYPE)
        {
            ...
            case 4:
                ...
                break;
            case 5:
                events.ScheduleEvent(EVENT_5, 100);
                break;
            }
        }
    }
...
class spell_tentacle_strike : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (targets.empty())
            GetCaster()->CastSpell(GetCaster(), SpellTentacleSlam, true);
        else targetsCount = targets.size();
    }
...
class spell_corrupted_axion : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (targets.empty())
            GetCaster()->CastSpell(GetCaster(), SpellCorruptedAxionAOE, true);
    }
...
class spell_corrupted_axion_aura : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetCaster()->GetMap() && (GetCaster()->GetMap()->GetSpawnMode() == DIFFICULTY_MYTHIC_RAID))
            if (GetTargetApplication()->GetRemoveMode() != AURA_REMOVE_BY_ENEMY_SPELL)
                GetCaster()->CastSpell(target, SpellCorrupted, false);
    }
