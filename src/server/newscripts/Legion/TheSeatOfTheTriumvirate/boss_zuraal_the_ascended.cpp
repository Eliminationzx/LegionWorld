...
public:
    ...
    struct boss_zuraal_the_ascendedAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override 
        {
            _EnterCombat();
            Talk(SAY_AGGRO);
            events.ScheduleEvent(EVENT_NULL_PALM, 11000);
            events.ScheduleEvent(EVENT_DECIMATE, 18000);
            events.ScheduleEvent(EVENT_COALESCED_VOID, 20000);
            events.ScheduleEvent(EVENT_UMBRA_SHIFT, 41000);
        }
        ...
        void UpdateAI(uint32 diff) override 
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_NULL_PALM:
                    ...
                    events.ScheduleEvent(EVENT_NULL_PALM, 55000);
                    break;
                case EVENT_DECIMATE:
                    if (!phase)
                        DoCast(SPELL_DECIMATE);

                    events.ScheduleEvent(EVENT_DECIMATE, 12000);
                    break;
                case EVENT_COALESCED_VOID:
                    ...
                    events.ScheduleEvent(EVENT_COALESCED_VOID, 55000);
                    break;
                ...
                case EVENT_FIXATE:
                    if (phase)
                    {
                        if (auto pla = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true, -SPELL_VOID_REALM_1))
                        {
                            me->getThreatManager().resetAllAggro();
                            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, true);
                            me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, true);
                            DoCast(SPELL_MADDENED_FRENZY);
                            me->AddThreat(pla, 50000000.0f);
                            me->GetMotionMaster()->MoveChase(pla);
                            DoCast(pla, SPELL_FIXATE);
                        }
                    }
                    break;
                }
...
public:
    ...
    struct npc_sott_shadowguard_subjugatorAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_NB, urand(10000,15000));
            events.ScheduleEvent(EVENT_FIELD, 20000);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FIELD:
                    ...
                    events.ScheduleEvent(EVENT_FIELD, urand(20000, 25000));
                    break;
                }
            }
            DoMeleeAttackIfReady();
    ...
    struct npc_zuraal_alleriaAI : public ScriptedAI
    {
        ...
        void DoAction(int32 const action) override
        {
            ...
            case 1:
                ...
                break;
            case 2:
                instance->DoCastSpellOnPlayers(249945);
                break;
            }
        }
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            ...
            if (summoner->GetEntry() == NPC_LURA_INTROPORTAL)
            {
                ...
                me->AddDelayedEvent(2000, [this] {
                    ...
                    instance->DoCastSpellOnPlayers(SPELL_CONVERSATION_LURA);
                });
...
public:
    ...
    bool IsTriggerMeets(Player* player, EventObject* eo) override
    {
        if (InstanceScript* instance = eo->GetInstanceScript())
            if ((instance->GetBossState(DATA_ZURAAL) == DONE) && !meeted)
