...
public:
    ...
    struct boss_viceroy_nezharAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override 
        {
            ...
            events.ScheduleEvent(EVENT_TENTACLES, 12000);
            events.ScheduleEvent(EVENT_FEAR, 16000);
            events.ScheduleEvent(EVENT_ENTROPIC_FORCE, 32000);
        }
        ...
        void UpdateAI(uint32 diff) override 
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_TENTACLES:
                    Tentacles();
                    events.ScheduleEvent(EVENT_TENTACLES, 30000);
                    break;
                case EVENT_FEAR:
                    if (!cast)
                        me->InterruptSpell(CURRENT_GENERIC_SPELL);
                    DoCast(SPELL_FEAR);
                    events.ScheduleEvent(EVENT_FEAR, 33000);
                    break;
                case EVENT_ENTROPIC_FORCE:
                    if (!cast)
                        me->InterruptSpell(CURRENT_GENERIC_SPELL);
                    DoCast(SPELL_ENTROPIC_FORCE);
                    events.ScheduleEvent(EVENT_ENTROPIC_FORCE, 61000);
                    break;
                }
            }
            if (!pause && !cast)
...
public:
    ...
    struct npc_nezhar_tentacleAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_1, urand(2500, 5000));
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_1:
                    ...
                    events.ScheduleEvent(EVENT_1, 5500);
                    break;
                }
            }
        }
