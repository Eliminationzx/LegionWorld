...
enum Spells
{
    ...
    SPELL_TRAP_AT               = 245903, //AT doesnt disapear instantly
...
public:
    ...
    struct boss_saprishAI : public BossAI
    {
        ...
        {
            me->SetVisible(false);
            if (instance->GetBossState(DATA_WAVEPORTAL) == DONE)
                me->SetVisible(true);
        }

        uint32 CheckPets = 3000;
        ...
        void Pets()
        {
            if (instance->GetBossState(DATA_WAVEPORTAL) == DONE)
            {
                summons.DespawnEntry(NPC_SAPRISH_DARKFANG);
                summons.DespawnEntry(NPC_SAPRISH_SHADEWING);
                me->SummonCreature(NPC_SAPRISH_DARKFANG, petPos[0]);
                if (me->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC_DUNGEON)
                    me->SummonCreature(NPC_SAPRISH_SHADEWING, petPos[1]);
            }
        }
        ...
        void EnterCombat(Unit* /*who*/) override 
        {
            ...
            Talk(SAY_AGGRO);
            events.ScheduleEvent(EVENT_TRAPS, 8000);
            events.ScheduleEvent(EVENT_DETONATE_TRAPS, 15000);
            events.ScheduleEvent(EVENT_GHOSTSTRIKE_PRECAST, 20000);
        }
        ...
        void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType dmgType) override
        {
            ...
            if (Creature* shadewing = me->FindNearestCreature(NPC_SAPRISH_SHADEWING, 100.0f, true))
            {
            ...
            if (Creature* darkfang = me->FindNearestCreature(NPC_SAPRISH_DARKFANG, 100.0f, true))
            {
                if (damage >= me->GetHealth())
        ...
        void SpellFinishCast(SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_GHOSTSTRIKE_MARKED)
                events.ScheduleEvent(EVENT_GHOSTSTRIKE_CHARGE, 6000);
        }
        ...
        void UpdateAI(uint32 diff) override 
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_TRAPS:
                    Traps();
                    events.ScheduleEvent(EVENT_TRAPS, 16000);
                    break;
                case EVENT_DETONATE_TRAPS:
                    DoCast(me, SPELL_DETONATE_TRAPS, true);
                    events.ScheduleEvent(EVENT_DETONATE_TRAPS, 20000);
                    break;
                case EVENT_GHOSTSTRIKE_PRECAST:
                    ...
                    events.ScheduleEvent(EVENT_GHOSTSTRIKE_PRECAST, 35000);
                    break;
...
public:
    ...
    struct npc_saprish_conv_controlAI : public ScriptedAI
    {
        npc_saprish_conv_controlAI(Creature* creature) : ScriptedAI(creature) 
        {
            ...
            if (instance->GetBossState(DATA_SAPRISH) == DONE || instance->GetBossState(DATA_WAVEPORTAL) == DONE)
        ...
        void StartEvent()
        {
            instance->SetBossState(DATA_WAVEPORTAL, IN_PROGRESS);
            instance->DoCastSpellOnPlayers(246876);
            events.ScheduleEvent(EVENT_PORT1, 1000);
        }
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            ...
            case NPC_SAPRISH_EVENTPORTAL:
                eventportalclosed++;
                events.ScheduleEvent(urand(EVENT_PORT2, EVENT_PORT4), 1000);
                break;
            }
            if (eventportalclosed == 3)
            {
                instance->SetBossState(DATA_WAVEPORTAL, DONE);
                if (Creature* saprish = instance->instance->GetCreature(instance->GetGuidData(NPC_SAPRISH)))
                {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!eventact)
            {
                if (CheckTimer <= diff)
                {
                    if (Player* player = me->FindNearestPlayer(30.0f,true))
                        if (!player->ToPlayer()->isGameMaster())
                            if (instance->GetBossState(DATA_ZURAAL) == DONE)
                                if (instance->GetBossState(DATA_WAVEPORTAL) != IN_PROGRESS || instance->GetBossState(DATA_WAVEPORTAL) != DONE)
                                {
                                    eventact = true;
                                    StartEvent();
                                }
...
public:
    ...
    struct npc_saprish_rift_wardenAI : public ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, urand(8000, 12000));
            events.ScheduleEvent(EVENT_2, urand(13000, 16000));
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_2:
                    ...
                    events.ScheduleEvent(EVENT_2, urand(20000, 25000));
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_saprish_darkfangAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType dmgType) override
        {
            ...
            if (Creature* saprish = me->FindNearestCreature(NPC_SAPRISH, 100.0f, true))
            {
                if (damage >= me->GetHealth())
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_POUNCE, 1500);
            events.ScheduleEvent(EVENT_JUMPTO, 3000);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_POUNCE:
                    ...
                    events.ScheduleEvent(EVENT_POUNCE, 11000);
                    break;
                case EVENT_JUMPTO:
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 50.0f, true))
                    {
                    ...
                    events.ScheduleEvent(EVENT_JUMPTO, 11000);
                    break;
                }
            }
        }
...
public:
    ...
    struct npc_saprish_shadewingAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_SWOOP, 3000);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SWOOP:
                    ...
                    events.ScheduleEvent(EVENT_SWOOP, urand(13000,15000));
                    break;
                }
            }
            DoMeleeAttackIfReady();
    ...
    struct npc_saprish_locus_walker_after_bossAI : public ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            ...
            me->AddDelayedEvent(4000, [this] {
                instance->DoCastSpellOnPlayers(249963);
            });
