...
public:
    ...
    struct boss_cordanaAI : public BossAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            ScheduleEvents();
            events.ScheduleEvent(EVENT_SHADOWSTEP, 20000);  // only one time
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            {
                //! summon->CastSpell(summon, 204461);    we need swith on this after rewrite invisible's casts =C
        ...
        void SpellHitTarget(Unit* target, const SpellInfo* spell) override
        {
            ...
            {    
                case SPELL_KNOCKDOWN_KICK:
                    events.ScheduleEvent(EVENT_KNOCKDOWN_KICK_TRIGGER, 500);
                    Talk(SAY_TURN_KICK);
                    break;
            }
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
        {
            ...
            {
                creeping_phase = true;
                events.ScheduleEvent(EVENT_CREEPING_FIRST, 1);
            }
        ...
        void ScheduleEvents()
        {
            events.ScheduleEvent(EVENT_KNOCKDOWN_KICK, 8300);
            events.ScheduleEvent(EVENT_DEEPENING_SHADOWS, 11000);
            events.ScheduleEvent(EVENT_DETONATING_MOONGLAIVE, 11000);
            if (me->HealthBelowPct(vengeance_health) && vengeancePhase)
                events.ScheduleEvent(EVENT_VENGEANCE, 35000);
            if (me->HealthBelowPct(creeping_health) && creeping_phase)
                events.ScheduleEvent(EVENT_CREEPING_SECOND, 35000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_KNOCKDOWN_KICK:
                        ...
                        events.ScheduleEvent(EVENT_KNOCKDOWN_KICK, 16000);
                        break;
                    ...
                    case EVENT_DEEPENING_SHADOWS:
                        ...
                        events.ScheduleEvent(EVENT_DEEPENING_SHADOWS, 30500);
                        break;
                    case EVENT_DETONATING_MOONGLAIVE:
                        Talk(SAY_GLAIVE);
                        DoCast(SPELL_DETONATING_MOONGLAIVE);
                        events.ScheduleEvent(EVENT_DETONATING_MOONGLAIVE, 9000);
                        break;
                    ...
                    case EVENT_CREEPING_SECOND:
                        ...
                        // SummonCreepings(2);
                        events.ScheduleEvent(EVENT_CREEPING_SECOND, 70000);  // second time
                        break;
    ...
    struct npc_cordana_glowing_sentryAI : public ScriptedAI
    {
        ...
        void OnSpellClick(Unit* clicker) override
        {
            if (me->HasAura(SPELL_ELUNES_LIGHT_VIS_1) && me->HasAura(SPELL_ELUNES_LIGHT_VIS_2))  //active?
            {
               ...
               DoCast(clicker, SPELL_ELUNES_LIGHT_OVERRIDE);
