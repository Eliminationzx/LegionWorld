...
public:
    ...
    struct boss_advisor_melandrusAI : public BossAI
    {
        ...
        //48:11
        {
            ...
            events.ScheduleEvent(EVENT_PIERCING_GALE, 6000); //48:17, 48:37, 48:56
            events.ScheduleEvent(EVENT_SLICING_MAELSTROM, 11000); //48:22, 48:43, 49:01
            events.ScheduleEvent(EVENT_BLADE_SURGE, 19000); //48:31, 48:50, 49:09
...
public:
    ...
    struct npc_suspicious_nobleAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_1, 3000); // 214688 25
            events.ScheduleEvent(EVENT_2, 9000); // 214690
            events.ScheduleEvent(EVENT_3, 16000); // 214692
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(214688);
                        events.ScheduleEvent(EVENT_1, 25000); // 214688 25
                        break;
                    case EVENT_2:
                        DoCast(214690);
                        events.ScheduleEvent(EVENT_2, 25000); // 214690
                        break;
                    case EVENT_3:
                        DoCast(214692);
                        events.ScheduleEvent(EVENT_3, 25000); // 214692
                        break;
                }
            }
            DoMeleeAttackIfReady();
