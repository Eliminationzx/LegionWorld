...
public:
    ...
    struct boss_flotsamAI : public ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, urand(8000, 9000)); //триггер на рандома SPELL_STALKER линк SPELL_JETSAM
            events.ScheduleEvent(EVENT_2, urand (16000, 19000)); // SPELL_BREAKSAM
            events.ScheduleEvent(EVENT_3, 40000); //SPELL_YAKSAM
            events.ScheduleEvent(EVENT_4, 1000); //проверка маны. Если 100 - каст SPELL_GETSAM
            me->SetPower(POWER_MANA, 0);
            DoCast(SPELL_REGEN);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_3:
                        ...
                        Talk(0);
                        events.ScheduleEvent(EVENT_3, 40000); //SPELL_YAKSAM
                        events.ScheduleEvent(EVENT_5, 6000); // adds
                        break;
                    case EVENT_4:
    ...
    struct boss_levantusAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
           // DoCast(SPELL_GUST_OF_WIND);
            events.ScheduleEvent(EVENT_1, urand(7000, 8000)); //SPELL_RENDING_WHIRL + 40
            events.ScheduleEvent(EVENT_2, urand(25000, 27000)); //SPELL_MASSIVE_SPOUL + 43
            events.ScheduleEvent(EVENT_3, urand(50000, 51000)); //SPELL_ELECTRIFY + 43  (доделать)
            events.ScheduleEvent(EVENT_4, 11000); // SPELL_TURBULENT_VORTEX
            events.ScheduleEvent(EVENT_5, 1000); //check
            events.ScheduleEvent(EVENT_6, 3000); // not repeat
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        events.ScheduleEvent(EVENT_1, urand(39000, 41000)); //SPELL_RENDING_WHIRL + 40
                        break;
                    case EVENT_2:
                        DoCast(SPELL_MASSIVE_SPOUL);
                        events.ScheduleEvent(EVENT_2, urand(39000, 41000)); //SPELL_MASSIVE_SPOUL + 43
                        break;
                    case EVENT_3:
...
public:
    ...
    struct boss_nithoggAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, urand(2000, 4000)); // SPELL_TAIL_LASH  скилл проверяет перед, а не зад. Должен зад
            events.ScheduleEvent(EVENT_2, urand(20000, 21000)); // SPELL_STORM_BREATH
            events.ScheduleEvent(EVENT_3, 30000); // SPELL_ELECTRICAL_STORM
            events.ScheduleEvent(EVENT_4, 41000); // SPELL_STATIC_CHARGE
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
        {
            ...
            {
                healthPct -= 20;
                events.ScheduleEvent(EVENT_5, 10);
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        ...
                        events.ScheduleEvent(EVENT_2, urand(20000, 21000));
                        break;
                    case EVENT_3:
                    ...
                    case EVENT_4:
                        {                               
                            ...
                            events.ScheduleEvent(EVENT_4, 41000);
                        }
                        break;
                    case EVENT_5:
...
public:
    ...
    struct boss_humongrisAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, urand(15000, 16000)); // SPELL_FIRE_BOOM
            events.ScheduleEvent(EVENT_2, urand(25000, 26000)); // SPELL_EARTHSHAKE_STOMP
            events.ScheduleEvent(EVENT_3, urand(28000, 30000)); // SPELL_ICE_FIST
            events.ScheduleEvent(EVENT_4, urand(33000, 35000)); // SPELL_YOU_GO_BANG 25  rand
            events.ScheduleEvent(EVENT_5, urand(37000, 38000)); // SPELL_MAKE_THE_SNOW 34
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_3:
                        ...
                        SayHelper(2);
                        events.ScheduleEvent(EVENT_3, urand(28000, 30000));
                        break;
                    case EVENT_4:
                        if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, 40.0f, true))
                            DoCast(target, SPELL_YOU_GO_BANG, false);
                        events.ScheduleEvent(EVENT_4, urand(24000, 26000));
                        break;
                    case EVENT_5:
                        DoCast(SPELL_MAKE_THE_SNOW);
                        SayHelper(1);
                        events.ScheduleEvent(EVENT_5, urand(34000, 36000));
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct boss_calamirAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, urand(5000, 6000)); // SPELL_1 13
            events.ScheduleEvent(EVENT_2, urand(15000, 16000)); // SPELL_2 25
        }        
                ...
                switch (eventId)
                {
                    case EVENT_1:
                        if (me->HasAura(SPELL_PHASE_FIRE) || me->HasAura(SPELL_PHASE_FROST) || me->HasAura(SPELL_PHASE_ARCANE))
                        {
                            ...
                            events.ScheduleEvent(EVENT_1, 13000);   
                        } 
                        else 
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        events.ScheduleEvent(EVENT_1, urand(25000, 30000));
                        break;
                    case EVENT_2:
                        DoCast(SPELL_RESONANCE);
                        events.ScheduleEvent(EVENT_1, urand(24000, 30000));
                        break;
                    case EVENT_3:
                        DoCast(SPELL_NIGHTSTABLE_ENERGY);
                        events.ScheduleEvent(EVENT_1, urand(29000, 34000));
                        break;
                    case EVENT_4:
                        ++countclons;
...
public:
    ...
    struct boss_shartosAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            ...
            events.ScheduleEvent(EVENT_5, 24000);
        }  
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        events.ScheduleEvent(EVENT_1, urand(7000, 10000));
                        break;
                    case EVENT_2:
...
public:
    ...
    struct boss_drugon_the_frostbloodAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            ...
            events.ScheduleEvent(EVENT_3, 20000);
            events.ScheduleEvent(EVENT_4, 33000);
        }  
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_4:
                        ...
                        inphase = true;
                        timer_phase = 3000;
                        events.ScheduleEvent(EVENT_4, 34000);
                        break;
    ...
    struct boss_soultakersAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            if (me->GetEntry() == NPC_CAPITAN || me->GetEntry() == NPC_YDORN)
                events.ScheduleEvent(EVENT_1, urand(13000, 16000));
            if (me->GetEntry() == NPC_MEVRA)
                events.ScheduleEvent(EVENT_3, 1000); // 9000
            
            events.ScheduleEvent(EVENT_10, 1000); // check for energy
                    ...
                    case EVENT_3:
                        {
                            ...
                            events.ScheduleEvent(EVENT_3, 8500);
                        }
                        break;
                    case EVENT_10:
                        {
                            ...
                            // else
                                // me->SetPower(POWER_ENERGY, ++power);
                            events.ScheduleEvent(EVENT_10, urand(1000, 1100));
                        }
                        break;
                }
            }
...
public:
    ...
    struct boss_ana_muzAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, 4000); // SPELL_FEL_GEYZER
            events.ScheduleEvent(EVENT_3, 7000); // SPELL_GASEOUS_BREATH
            events.ScheduleEvent(EVENT_4, 31000); // SPELL_MOTHERS_EMBRACE
        }  
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        events.ScheduleEvent(EVENT_1, 22000);
                    ...
                    case EVENT_3:
                        ...
                        events.ScheduleEvent(EVENT_3, 31000);
                        break;
                    case EVENT_4:
                        DoCast(SPELL_MOTHERS_EMBRACE);
                        events.ScheduleEvent(EVENT_4, 65000);
                        break;
                }
            }
...
public:
    ...
    struct boss_nazak_the_fiendAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, 23000);
            events.ScheduleEvent(EVENT_2, 42000);
            events.ScheduleEvent(EVENT_3, 57000);
        }  
...
public:
    ...
    struct boss_sivashAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, 25000); // summons 233968
            events.ScheduleEvent(EVENT_2, 13000); // 241433
            events.ScheduleEvent(EVENT_3, 23000); // 233996 sea with help 233977
        }  
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_3:
                        ...
                        events.ScheduleEvent(EVENT_3, 23000); 
                        break;
                }
            }
...
public:
    ...
    struct boss_apocronAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, 23000); // 241458
            events.ScheduleEvent(EVENT_2, 35000); // 241518 (random)
            events.ScheduleEvent(EVENT_3, 11000); // 241498 script (241507)
        }  
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        DoCast(241518);
                        events.ScheduleEvent(EVENT_2, 24000); // 241518 (random)
                        break;
                    case EVENT_3:
                        DoCast(241498);
                        ...
                        events.ScheduleEvent(EVENT_3, 16000); // 241498 script (241507)
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct boss_bs_brutallusAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, 19000); // 233484
            events.ScheduleEvent(EVENT_2, 30000); // 233566
            events.ScheduleEvent(EVENT_3, 20000); // 233515
        }  
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        DoCast(233566);
                        events.ScheduleEvent(EVENT_2, 18000); // 233566
                        break;
                    case EVENT_3:   
                        DoCast(233515);
                        events.ScheduleEvent(EVENT_3, 20+18000); // 233515
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct boss_malificusAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* unit) override
        {
            events.ScheduleEvent(EVENT_1, 17000); // 234452
            events.ScheduleEvent(EVENT_2, 15000); // 233614
            events.ScheduleEvent(EVENT_3, 16000); // 233569
        }  
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:     
                        DoCast(234452);
                        events.ScheduleEvent(EVENT_1, 19000); // 234452
                        break;
                    case EVENT_2:
                        DoCast(233614);
                        events.ScheduleEvent(EVENT_2, 16000); // 233614
                        break;
                    case EVENT_3:
                        DoCast(233569);
                        events.ScheduleEvent(EVENT_3, 18000); // 233569
                        break;
                }
            }
