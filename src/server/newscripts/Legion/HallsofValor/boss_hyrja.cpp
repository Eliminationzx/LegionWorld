...
public:
    ...
    struct boss_hyrjaAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_SHIELD_OF_LIGHT, 10000);
            events.ScheduleEvent(EVENT_SPECIAL_ABILITY, 20000);
            events.ScheduleEvent(EVENT_ARCING_BOLT, 12000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_EXPEL_LIGHT:
                        ...
                        events.ScheduleEvent(EVENT_EXPEL_LIGHT, 30000);
                        break;
                    case EVENT_SHIELD_OF_LIGHT:
                        DoCastVictim(SPELL_SHIELD_OF_LIGHT);
