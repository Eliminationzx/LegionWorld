...
public:
    ...
    struct boss_rtk_midnightAI : public BossAI
    {
        ...
        void DefaultEvents(uint8 phaseEvent)
        {
            ...
            {
                case PHASE_1:
                    events.ScheduleEvent(EVENT_INTANGIBLE_PRESENCE, 6000);
                    events.ScheduleEvent(EVENT_MIGHTY_STOMP, 14000);
                    //events.ScheduleEvent(EVENT_SPECTRAL_CHARGE, 47000); //47s
                    break;
                case PHASE_2:
                    events.ScheduleEvent(EVENT_CHECK_HEALTH, 1000);
                    events.ScheduleEvent(EVENT_MEZAIR, 3000);
                    break;
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE && id == 1003) // charge - SPELL_MEZAIR
                events.ScheduleEvent(EVENT_MOVING_TO_HUNTSMAN, 500);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_INTANGIBLE_PRESENCE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_INTANGIBLE_PRESENCE, 58000);
                        break;
                    }
                    ...
                    case EVENT_SPECTRAL_CHARGE:
                        ...
                        events.ScheduleEvent(EVENT_SPECTRAL_CHARGE, 47000);
                        break;
                    ...
                    case EVENT_MEZAIR:
                        ...
                        events.ScheduleEvent(EVENT_MEZAIR, 6000);
                        break;
...
public:
    ...
    struct npc_attumen_the_huntsmanAI : public ScriptedAI
    {
        ...
        {
            Talk(SAY_AGGRO);
            events.ScheduleEvent(EVENT_CHECK_VICTIM, 1000);
        }
        ...
        void DoAction(int32 const action) override
        {
            switch (action)
            {
                case ACTION_1:
                    ...
                    events.ScheduleEvent(EVENT_SHARED_SUFFERING, 18000);
                    break;
                case ACTION_2:
                    ride = true;
                    events.Reset();
