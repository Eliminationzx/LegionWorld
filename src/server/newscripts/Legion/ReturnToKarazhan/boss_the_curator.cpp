...
public:
    ...
    struct boss_the_curatorAI : public BossAI
    {
        ...
        //47:34
        {
            ...
            events.ScheduleEvent(EVENT_MANA_CHECK, 500);
        }

        void DefaultEvent()
        {
            events.ScheduleEvent(EVENT_SUM_VOLATILE_ENERGY, 6000); //47:40 47:50 47:59
            events.ScheduleEvent(EVENT_POWER_DISCHARGE, 13000); //47:47 48:00 48:15 48:30
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SUM_VOLATILE_ENERGY:
                        ...
                        events.ScheduleEvent(EVENT_SUM_VOLATILE_ENERGY, 10000);
                        break;
                    case EVENT_POWER_DISCHARGE:
                        DoCast(me, SPELL_POWER_DISCHARGE_FILTER, true);
                        events.ScheduleEvent(EVENT_POWER_DISCHARGE, 14000);
                        break;
...
public:
    ...
    struct npc_curator_volatile_energyAI : public ScriptedAI
    {
        ...
        //47:50
        {
            ...
            events.ScheduleEvent(EVENT_1, 7000); //47:57
        }
