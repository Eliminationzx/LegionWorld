...
public:
    ...
    struct boss_vizaduum_the_watcherAI : public BossAI
    {
        ...
        void DefaultEvent(uint8 phase)
        {
            ...
            {
                case PHASE_1:
                    events.ScheduleEvent(EVENT_BURNING_BLAST, 2000);
                    events.ScheduleEvent(EVENT_COMMAND_FEL_BEAM, 6000);
                    events.ScheduleEvent(EVENT_DISINTEGRATE, 11000);
                    events.ScheduleEvent(EVENT_CHAOTIC_SHADOWS, 16000);
                    events.ScheduleEvent(EVENT_COMMAND_BOMBARDMENT, 26000);
                    break;
                case PHASE_2:
                    events.ScheduleEvent(EVENT_BURNING_BLAST, 2000);
                    events.ScheduleEvent(EVENT_DISINTEGRATE, 11000);
                ...
                case PHASE_3:
                    ...
                    events.ScheduleEvent(EVENT_DISINTEGRATE, 11000);
                    events.ScheduleEvent(EVENT_CHAOTIC_SHADOWS, 16000);
                    events.ScheduleEvent(EVENT_SUMMON_GUARD, 2000);
                    break;
                case PHASE_CHANGE:
                    me->StopAttack();
                    if (healthPct == 33)
                        Talk(SAY_DEMONIC_PORTAL);
                    events.ScheduleEvent(EVENT_DEMONIC_PORTAL_1, 1000);
                    break;
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == WAYPOINT_MOTION_TYPE)
            {
                if (id == 6 && healthPct || id == 7 && !healthPct)
                {
                    ...
                    {
                        me->SetFacingTo(1.1f);
                        events.ScheduleEvent(EVENT_STABILIZE_RIFT, 2000);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CHAOTIC_SHADOWS:
                        ...
                        events.ScheduleEvent(EVENT_CHAOTIC_SHADOWS, 30000);
                        break;
                    case EVENT_COMMAND_BOMBARDMENT: 
                        DoCast(SPELL_COMMAND_BOMBARDMENT);
                        events.ScheduleEvent(EVENT_COMMAND_BOMBARDMENT, 25000);
                        break;
                    case EVENT_DEMONIC_PORTAL_1:
                    {
                        ...
                        ///<<<
                        events.ScheduleEvent(EVENT_DEMONIC_PORTAL_2, 500);
                        break;
                    }
                    case EVENT_DEMONIC_PORTAL_2:
                        DoCast(me, SPELL_DEMONIC_PORTAL_AT, true);
                        events.ScheduleEvent(EVENT_DEMONIC_PORTAL_3, 3000);
                        break;
                    case EVENT_DEMONIC_PORTAL_3:
                        evadePhase = 0;
                        SetFlyMode(true);
                        DoCast(me, SPELL_DEMONIC_PORTAL_SCALE, true);
                        if (healthPct == 33)
                    ...
                    case EVENT_CHECK_PLAYER:
                        ...
                        else
                        {
                            ...
                            events.ScheduleEvent(EVENT_CHECK_PLAYER, 3000);
                        }
                        break;
                    ...
                    case EVENT_SUMMON_GUARD:
                        ...
                        events.ScheduleEvent(EVENT_SUMMON_GUARD, sumGuardTimer);
                        break;
                }
            }
