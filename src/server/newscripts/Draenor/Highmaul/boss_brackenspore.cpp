...
struct boss_brackenspore : public BossAI
{
    ...
    void DoAction(int32 const action) override
    {
        switch (action)
        {
            ...
            {
                m_Events.ScheduleEvent(EventInfestingSpores, 1);
                break;
            }
            default:
                break;
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        ...
        m_Events.ScheduleEvent(EventNecroticBreath, 30 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventBerserker, 600 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventMindFungus, 10 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventLivingMushroom, 17 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventSporeShooter, 20 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventFungalFleshEater, 32 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventRejuvenatingMushroom, 80 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventRot, 10 * IN_MILLISECONDS);

        /// Mythic Specials. Shared cd, which special he uses is random.
        if (IsMythicRaid())
            m_Events.ScheduleEvent(EventSpecialAbility, 20 * IN_MILLISECONDS);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (m_CosmeticEvent.ExecuteEvent())
        {
            ...
            if (canSchedule)
                ...
                m_CosmeticEvent.ScheduleEvent(EventCheckForIntro, 1000);
        }
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventNecroticBreath:
            {
                ...
                m_Events.ScheduleEvent(EventNecroticBreath, 32 * IN_MILLISECONDS);
                break;
            }
            case EventBerserker:
            {
            ...
            case EventInfestingSpores:
            {
                ...
                m_Events.ScheduleEvent(EventScheduleEnergy, 12 * IN_MILLISECONDS);
                break;
            }
            case EventMindFungus:
            {
                me->CastSpell(me, SummonMindFungus, true);
                m_Events.ScheduleEvent(EventMindFungus, 30 * IN_MILLISECONDS);
                break;
            }
            case EventLivingMushroom:
            {
                me->CastSpell(me, SummonLivingMushroom, true);
                m_Events.ScheduleEvent(EventLivingMushroom, 55 * IN_MILLISECONDS);
                break;
            }
            case EventSporeShooter:
            {
                me->CastSpell(me, SporeShooterDummy, true);
                m_Events.ScheduleEvent(EventSporeShooter, 57 * IN_MILLISECONDS);
                break;
            }
            case EventFungalFleshEater:
            {
                me->CastSpell(me, SummonFungalFleshEater, true);
                m_Events.ScheduleEvent(EventFungalFleshEater, 120 * IN_MILLISECONDS);
                break;
            }
            case EventRejuvenatingMushroom:
            {
                me->CastSpell(me, RejuvenatingMushDummy, true);
                m_Events.ScheduleEvent(EventRejuvenatingMushroom, 130 * IN_MILLISECONDS);
                break;
            }
...
struct npc_highmaul_spore_shooter : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        m_Events.ScheduleEvent(EventSporeShot, urand(100, 1500));
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (m_Events.ExecuteEvent() == EventSporeShot)
        {
            ...
            m_Events.ScheduleEvent(EventSporeShot, 4500);
        }
    }
};
...
struct npc_highmaul_fungal_flesh_eater : public MS::AI::CosmeticAI
{
    ...
    void DamageDealt(Unit* /*victim*/, uint32& /*damage*/, DamageEffectType /*damageType*/) override
    {
        ...
        {
            m_Scheduled = true;
            m_Events.ScheduleEvent(EventDecay, 5 * IN_MILLISECONDS);
            m_Events.ScheduleEvent(EventFleshEater, 1 * IN_MILLISECONDS);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventDecay:
                ...
                m_Events.ScheduleEvent(EventDecay, 10 * IN_MILLISECONDS);
                break;
            case EventFleshEater:
                me->CastSpell(me, FleshEater, true);
                m_Events.ScheduleEvent(EventFleshEater, 10 * IN_MILLISECONDS);
                break;
            default:
                break;
        }
