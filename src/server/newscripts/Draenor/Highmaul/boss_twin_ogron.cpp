...
struct boss_twin_ogron_pol : public BossAI
{
    ...
    void EnterCombat(Unit* attacker) override
    {
        ...
        m_Events.ScheduleEvent(EventBerserker, IsMythicRaid() ? 420 * IN_MILLISECONDS : 480 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventShieldBash, 22 * IN_MILLISECONDS);
...
struct boss_twin_ogron_phemos : public BossAI
{
    ...
    {
        if (!m_HomeChanged)
            m_CosmeticEvents.ScheduleEvent(EventCheckPlayer, 1 * IN_MILLISECONDS);
    ...
    void DoAction(int32 const action) override
    {
        switch (action)
        {
            ...
            case ScheduleQuake:
            {
                ...
                m_CosmeticEvents.ScheduleEvent(EventSpawnFirstBlaze, 5 * IN_MILLISECONDS);
                ...
                m_CosmeticEvents.ScheduleEvent(EventSpawnSecondBlaze, 1500);
    ...
    void EnterCombat(Unit* attacker) override
    {
        ...
        m_Events.ScheduleEvent(EventBerserker, IsMythicRaid() ? 420 * IN_MILLISECONDS : 480 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventDoubleSlash, 26 * IN_MILLISECONDS);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_CosmeticEvents.ExecuteEvent())
        {
            ...
            case EventSpawnSecondBlaze:
            {
                ...
                if (m_SecondWaveCounter <= BlazeTotalWaveCount)
                    m_CosmeticEvents.ScheduleEvent(EventSpawnSecondBlaze, 5 * IN_MILLISECONDS);
                break;
            }
            default:
                break;
        ...
        switch (m_Events.ExecuteEvent())
        {
            ...
            case EventDoubleSlash:
            {
                ...
                m_Events.ScheduleEvent(EventDoubleSlash, 27 * IN_MILLISECONDS);
                break;
            }
            default:
                break;
