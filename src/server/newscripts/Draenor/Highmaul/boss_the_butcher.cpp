...
struct boss_the_butcher : public BossAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        ...
        m_Events.ScheduleEvent(EventTenderizer, 6 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventCleave, 10 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventCleaver, 12 * IN_MILLISECONDS);

        if (!m_Instance->instance->IsLfr())
            m_Events.ScheduleEvent(EventBerserk, IsMythicRaid() ? 240 * IN_MILLISECONDS : 300 * IN_MILLISECONDS);
        ...
        m_Events.ScheduleEvent(EventMeatHook, 5 * IN_MILLISECONDS);

        /// Mythic mode only
        if (IsMythicRaid())
        {
            m_AddCount = 0;
            m_Events.ScheduleEvent(EventCadaver, 18 * IN_MILLISECONDS);
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventTenderizer:
            {
                ...
                m_Events.ScheduleEvent(EventTenderizer, 16 * IN_MILLISECONDS);
                break;
            }
            case EventCleave:
            {
                ...
                m_Events.ScheduleEvent(EventCleave, 5 * IN_MILLISECONDS);
                break;
            }
            case EventCleaver:
            {
                ...
                m_Events.ScheduleEvent(EventCleaver, 8 * IN_MILLISECONDS);
                break;
            }
            case EventBerserk:
            {
            ...
            case EventCadaver:
            {
                ...
                m_Events.ScheduleEvent(EventCadaver, 14 * IN_MILLISECONDS);
                break;
            }
            default:
                break;
    ...
    void ScheduleEnergy(uint32 const /*diff*/)
    {
        if (me->GetPower(POWER_ENERGY) >= 100)
            m_Events.ScheduleEvent(EventBoundingCleave, 50);
    }
};
