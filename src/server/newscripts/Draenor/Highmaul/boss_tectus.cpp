...
struct boss_tectus : public BossAI
{
    ...
    void DoAction(int32 const action) override
    {
        switch (action)
        {
            ...
            case ScheduleEarthenPillar:
            {
                ...
                m_Events.ScheduleEvent(EventEarthenPillar, 100);
                break;
            }
            case ScheduleTectonicUpheaval:
            {
                ...
                m_Events.ScheduleEvent(EventTectonicUpheaval, 100);
                break;
            }
            case MoteKilled:
            {
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        ...
        m_Events.ScheduleEvent(EventCrystallineBarrage, 5 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventFracture, 8 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventAccretion, 5 * IN_MILLISECONDS);
        ...
        switch (me->GetEntry())
        {
            ...
            case Tectus:
                ...
                Talk(Aggro);
                m_Events.ScheduleEvent(EventEnrage, IsMythicRaid() ? 480 * IN_MILLISECONDS : 600 * IN_MILLISECONDS);
                m_Events.ScheduleEvent(EventSpawnBersererk, 18 * IN_MILLISECONDS);
                m_Events.ScheduleEvent(EventSpawnEarthwarper, 8 * IN_MILLISECONDS);
                break;
            default:
                break;
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            ...
            case EventSpawnBersererk:
                ...
                m_Events.ScheduleEvent(EventSpawnBersererk, 40 * IN_MILLISECONDS);
                break;
            case EventSpawnEarthwarper:
                SpawnAdd(NightTwistedEarthwarper);
                m_Events.ScheduleEvent(EventSpawnEarthwarper, 40 * IN_MILLISECONDS);
                break;
            case EventCrystallineBarrage:
            {
                ...
                if (target != nullptr)
                {
                    ...
                    AddDelayedEvent(1 * IN_MILLISECONDS, [this]() -> void
                    {
                        ...
                        m_Events.ScheduleEvent(EventSpawnCrystalline, 500);
                    });
                }

                m_Events.ScheduleEvent(EventCrystallineBarrage, 30 * IN_MILLISECONDS);
                break;
            }
            ...
            case EventFracture:
                ...
                m_Events.ScheduleEvent(EventFracture, 6 * IN_MILLISECONDS);
                break;
...
struct npc_highmaul_rokka_and_lokk : public MS::AI::CosmeticAI
{
    ...
    void EnterCombat(Unit* attacker) override
    {
        ...
        m_Events.ScheduleEvent(EventEarthenThrust, 6 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventMeteoricEarthspire, 10 * IN_MILLISECONDS);
    }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
    {
        ...
        if (me->HealthBelowPctDamaged(21, damage))
            m_Events.ScheduleEvent(EventReconstitution, 100);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventEarthenThrust:
                ...
                m_Events.ScheduleEvent(EventEarthenThrust, 12 * IN_MILLISECONDS);
                break;
            case EventMeteoricEarthspire:
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                    me->CastSpell(target, MeteoricEarthspire, false);
                m_Events.ScheduleEvent(EventMeteoricEarthspire, 12 * IN_MILLISECONDS);
                break;
...
struct npc_highmaul_oro : public MS::AI::CosmeticAI
{
    ...
    void EnterCombat(Unit* attacker) override
    {
        ...
        m_Events.ScheduleEvent(EventStoneboltVolley, 6 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventRadiatingPoison, 10 * IN_MILLISECONDS);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventStoneboltVolley:
                ...
                m_Events.ScheduleEvent(EventStoneboltVolley, 12 * IN_MILLISECONDS);
                break;
            case EventRadiatingPoison:
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                    me->CastSpell(target, RadiatingPoison, false);
                m_Events.ScheduleEvent(EventRadiatingPoison, 12 * IN_MILLISECONDS);
                break;
            case EventReconstitution:
                me->CastSpell(me, ReconstitutionScale, true);
                me->CastSpell(me, Reconstitution, false);
...
struct npc_highmaul_night_twisted_berserker : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        ...
        m_Events.ScheduleEvent(EventRavingAssault, 8 * IN_MILLISECONDS);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventRavingAssault:
            {
                ...
                m_Events.ScheduleEvent(EventRavingAssault, 20 * IN_MILLISECONDS);
                break;
            }
            default:
                break;
...
struct npc_highmaul_night_twisted_earthwarper : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        ...
        m_Events.ScheduleEvent(EventEarthenFlechettes, 5 * IN_MILLISECONDS);
        m_Events.ScheduleEvent(EventGiftOfEarth, 12 * IN_MILLISECONDS);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (m_Events.ExecuteEvent())
        {
            case EventEarthenFlechettes:
                ...
                m_Events.ScheduleEvent(EventEarthenFlechettes, 15 * IN_MILLISECONDS);
                break;
            case EventGiftOfEarth:
                me->CastSpell(me, GiftOfEarth, false);
                m_Events.ScheduleEvent(EventGiftOfEarth, 25 * IN_MILLISECONDS);
                break;
            default:
                break;
        }
