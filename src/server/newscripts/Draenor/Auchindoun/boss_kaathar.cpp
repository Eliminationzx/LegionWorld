    ...
    struct boss_kaatharAI : public BossAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            if (me->GetMap() && me->GetMap()->IsHeroic())
                events.ScheduleEvent(EventFate, 45 * IN_MILLISECONDS);

            events.ScheduleEvent(EventHallowedGround, urand(12 * IN_MILLISECONDS, 17 * IN_MILLISECONDS));
            events.ScheduleEvent(EventSanctifiedStrike, 8 * IN_MILLISECONDS);
            events.ScheduleEvent(EventHolyShield, 30 * IN_MILLISECONDS);
        ...
        void UpdateAI(uint32 p_Diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EventFate:
                {
                    ...
                    events.ScheduleEvent(EventFate, 30 * IN_MILLISECONDS);
                    break;
                }
                case EventHallowedGround:
                {
                    ...
                    events.ScheduleEvent(EventHallowedGround, 8 * IN_MILLISECONDS);
                    break;
                }
                case EventHolyShield:
                {
                    if (Unit* l_Target = SelectTarget(SELECT_TARGET_RANDOM, 0, 50.0f, true))
                    {
                        ...
                        events.ScheduleEvent(EventConsecratedLight, 4 * IN_MILLISECONDS);
                        events.ScheduleEvent(EventHolyShieldReturn, 14 * IN_MILLISECONDS);
                    }
                    events.ScheduleEvent(EventHolyShield, 30 * IN_MILLISECONDS);
                    break;
                }
...
public:
    ...
    struct auchindoun_kaathar_mob_hallowed_groundAI : public Scripted_NoMovementAI
    {
        ...
        void Reset() override
        {
            ...
            events.ScheduleEvent(EventCheckPlayer, 4 * IN_MILLISECONDS); // Takes 4 seconds to charge
        ...
        void UpdateAI(uint32 p_Diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EventCheckPlayer:
                    ...
                    events.ScheduleEvent(EventCheckPlayer, 1 * IN_MILLISECONDS);
                    break;
                default:
                    break;
            }
