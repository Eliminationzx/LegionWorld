            ...
            switch (actionID)
            {
                case ActionCountPre3StBossKill:
                {
                    ...
                    if (m_Counting > 6 && !m_Summoned)
                    {
                        ...
                        events.ScheduleEvent(EventSummonAzzakel01, 2 * IN_MILLISECONDS);
                    }
                    break;
                }
                default:
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EventSummonAzzakel01:
                {
                    ...
                    events.ScheduleEvent(EventSummonAzzakel02, 1 * IN_MILLISECONDS);
                    break;
                }
        ...
        void DoAction(int32 const actionID) override
        {
            switch (actionID)
            {
                ...
                case ActionBoolActivate:
                    ...
                    break;
                case ActionFelSpark:
                    events.ScheduleEvent(EventFelSpark, 1 * IN_MILLISECONDS);
                    break;
                case ActionRenewEvents:
                {
                    ...
                    events.ScheduleEvent(EventFelLash, 8 * IN_MILLISECONDS);
                    events.ScheduleEvent(EventClawsOfArgus, 45 * IN_MILLISECONDS);
                    events.ScheduleEvent(EventCurtainOfFlame, 14 * IN_MILLISECONDS);
                    events.ScheduleEvent(EventMalevolentCrush01, 20 * IN_MILLISECONDS);
                    if (Unit* l_Target = SelectTarget(SELECT_TARGET_TOPAGGRO, 0, 100.0f, true))
                        AttackStart(l_Target);
                    break;
                }
                case ActionMalevolentCrash:
                    events.ScheduleEvent(EventMalevolentCrush01, 1 * IN_MILLISECONDS);
                    break;
                default:
                    break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EventFelLash:
                    ...
                    events.ScheduleEvent(EventFelLash, urand(8 * IN_MILLISECONDS, 12 * IN_MILLISECONDS));
                    break;
                ...
                case EventCurtainOfFlame:
                {
                    ...
                    events.ScheduleEvent(EventCurtainOfFlame, urand(8 * IN_MILLISECONDS, 15 * IN_MILLISECONDS));
                    break;
                }
