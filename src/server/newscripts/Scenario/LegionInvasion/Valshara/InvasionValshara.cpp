...
public:
    ...
    struct npc_crushfistAI : ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, 4000); // 234473 + 17
            events.ScheduleEvent(EVENT_2, 8000); // 234446 + 15
        }
        ...
        {            
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(234473);
                        events.ScheduleEvent(EVENT_1, 17000); // 234473 + 17
                        break;
                    case EVENT_2:
                        DoCast(234446);
                        events.ScheduleEvent(EVENT_2, 16000); // 234446 + 15
                        break;
                }
            }
...
public:
    ...
    struct npc_benefit_akrazarAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 12000); // 234694 + 23
            events.ScheduleEvent(EVENT_2, 16000); // 235048 + 16 (todo)
        }
        ...
        {            
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(234694);
                        events.ScheduleEvent(EVENT_1, 23000); // 234694 + 23
                        break;
                    case EVENT_2:
                        DoCast(235048);
                        events.ScheduleEvent(EVENT_2, 16000); // 235048 + 16 (todo)
