        ...
        void OnUnitDeath(Unit* unit) override
        {
            switch (unit->GetEntry())
            {
                ...
                case 110618:    // horde
                {
                    ...
                    unit->GetCreatureListWithEntryInGrid(creatureList, 109494, 30);
                    for (auto data : creatureList)
                    {
                        if (Vehicle* veh = data->GetVehicleKit())
                        {
                            for (uint32 i = 0; i < 3; ++i)
                            {
                                Unit* pas = veh->GetPassenger(i);
                                if (!pas)
                                {
                                    ++c;
                                    continue;
                    ...
                    if (c >= 3)
                    {
                        ...
                        if (Creature* cr = unit->FindNearestCreature(109494, 30.0f))
