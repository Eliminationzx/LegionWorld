...
public:
    ...
    struct npc_sr_orik_cato_scriptAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            {
                events.ScheduleEvent(EVENT_7, 3000); //Cato
                events.ScheduleEvent(EVENT_8, 4000); //Cato
                events.ScheduleEvent(EVENT_9, 7000); //Cato
                events.ScheduleEvent(EVENT_10, 9000); //Cato
            }
            else
            {
                events.ScheduleEvent(EVENT_11, 2000); //Orik
                events.ScheduleEvent(EVENT_12, 6000); //Orik
                events.ScheduleEvent(EVENT_13, 15000); //Orik
                events.ScheduleEvent(EVENT_14, 8000); //Orik
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_8: //Cato
                        ...
                        events.ScheduleEvent(EVENT_8, 4000);
                        break;
                    case EVENT_9: //Cato
                    ...
                    case EVENT_10: //Cato
                        ...
                        events.ScheduleEvent(EVENT_10, 10000);
                        break;
                    case EVENT_11: //Orik
                        DoCastVictim(SPELL_ORIK_CRUSADER_STRIKE);
                        events.ScheduleEvent(EVENT_11, 5000);
                        break;
                    case EVENT_12: //Orik
                        DoCast(SPELL_ORIK_DIVINE_STORM);
                        events.ScheduleEvent(EVENT_12, 9000);
                        break;
                    case EVENT_13: //Orik
                        DoCastVictim(SPELL_ORIK_HAMMER_JUSTICE);
                        events.ScheduleEvent(EVENT_13, 30000);
                        break;
                    case EVENT_14: //Orik
                        DoCastVictim(SPELL_ORIK_JUDGMENT);
                        events.ScheduleEvent(EVENT_14, 8000);
                        break;
                }
            }
    ...
    struct npc_sr_cryptstalker_eventAI : ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            if (me->GetEntry() == NPC_DRAKE_SHAE)
            {
                ...
                SetFlyMode(true);
                events.ScheduleEvent(EVENT_1, 2000); //Drake
            }
        }
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            if (me->GetEntry() == NPC_DRAKE_SHAE)
            {
                events.ScheduleEvent(EVENT_4, 1000); //Drake
            }
            else
            {
                events.ScheduleEvent(EVENT_5, 2000); //Inna
                events.ScheduleEvent(EVENT_6, 8000); //Inna
            }
        }
        ...
        void DoAction(int32 const action) override
        {
            events.ScheduleEvent(EVENT_2, 1000); //Inna
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_6: //Inna
                        ...
                        events.ScheduleEvent(EVENT_6, 40000);
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_sr_spectral_windshaperAI : ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_1, urand(1, 6) * IN_MILLISECONDS);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(windSpell);
                        events.ScheduleEvent(EVENT_1, urand(6, 12) * IN_MILLISECONDS);
                        break;
                    case EVENT_2:
...
public:
    ...
    struct npc_sr_yrgrim_the_truthseekerAI : ScriptedAI
    {
        ...
        //16:03
        {
            ...
            events.ScheduleEvent(EVENT_7, 17000);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_5:
                        ...
                        events.ScheduleEvent(EVENT_5, 17000);
                        break;
                    case EVENT_6:
                        DoCast(SPELL_BLADESTORM);
                        events.ScheduleEvent(EVENT_6, 21000);
                        break;
                    case EVENT_7:
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_4:
                        ...
                        events.ScheduleEvent(EVENT_4, 29000);
                        break;
                }
            }
            DoMeleeAttackIfReady();
