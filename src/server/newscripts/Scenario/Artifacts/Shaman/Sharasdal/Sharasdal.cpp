...
public:
    ...
    struct npc_erunak_stonespeakerAI : ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, 4000); // 209912
            events.ScheduleEvent(EVENT_2, 34000); // 209913
            events.ScheduleEvent(EVENT_3, 29000); // 79890
            events.ScheduleEvent(EVENT_4, 10000); // 162074
        }
...
public:
    ...
    struct npc_grashAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 5000); // 209904
            events.ScheduleEvent(EVENT_2, 13000); // 209895
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        DoCast(209895);
                        events.ScheduleEvent(EVENT_2, 13000); // 20989
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_adel_waterspeakerAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 10000); // 51876
            events.ScheduleEvent(EVENT_2, 11000); // 209914
        }
...
public:
    ...
    struct npc_kralissAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 18000); // 209748 с m_Functions
            events.ScheduleEvent(EVENT_2, 20000); // 209764 на рандома. Через спелл хит 209755, который делает рывок и 209757
        }
...
public:
    ...
    struct npc_lady_zitrinAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 2500); // 209919
            events.ScheduleEvent(EVENT_2, 16000); // 209707
            events.ScheduleEvent(EVENT_3, 28000); // 209934 // add
            events.ScheduleEvent(EVENT_4, 25000); // 209983 + script
        }
