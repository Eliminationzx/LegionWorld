...
public:
    ...
    struct npc_tuure_small_eventsAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            {
                events.ScheduleEvent(EVENT_1, 4000); // 210477
                events.ScheduleEvent(EVENT_2, 11000); // 210478
                events.ScheduleEvent(EVENT_3, 15000); // 210479
            }
        }
...
public:
    ...
    struct npc_capitan_naranotAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_1, 8000, 0, PHASE_COMBAT); // 211480
         //   events.ScheduleEvent(EVENT_2, 25000, 0, PHASE_COMBAT); // rephase 211186 + 211190    
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                    {
                        ...
                        events.ScheduleEvent(EVENT_3, 2000, 0, PHASE_REPHASE);
                        break;
                    }
                    case EVENT_3:
                        ...
                        events.ScheduleEvent(EVENT_4, 4000, 0, PHASE_REPHASE);
                        break;
...
public:
    ...
    struct npc_lady_kalindriaAI : ScriptedAI
    {
        ...
        {
            Talk(0);
            events.ScheduleEvent(EVENT_1, 2000); //211185
            events.ScheduleEvent(EVENT_2, 13000); //211380 + script for summon (17)
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(211185);
                        events.ScheduleEvent(EVENT_1, 2500); //211185
                        break;
                    case EVENT_2:
                        if (Player* pl = me->FindNearestPlayer(100.0f, true))
                        {
                        ...
                        events.ScheduleEvent(EVENT_2, 34000); //211380 + script for summon (17)
                        break;
                    case EVENT_3:
                        DoCast(210703);
                        if (Player* pl = me->FindNearestPlayer(100.0f, true))
