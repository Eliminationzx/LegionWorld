...
public:
    ...
    struct npc_tazhan_chuAI : ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, 7000); // 203859
            events.ScheduleEvent(EVENT_2, 17000); // 203854
            events.ScheduleEvent(EVENT_3, 19000); // 203858
            events.ScheduleEvent(EVENT_4, 23000); // 202517
        }
...
public:
    ...
    struct npc_sheylun_small_eventsAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            if (me->GetEntry() == 101882)
            {
                events.ScheduleEvent(EVENT_1, 5000); // 203832
                events.ScheduleEvent(EVENT_2, 11000); // 203831
            }
            if (me->GetEntry() == 101885)
            {
                events.ScheduleEvent(EVENT_3, 10000); // 203869
                events.ScheduleEvent(EVENT_4, 22000); // 203867
                events.ScheduleEvent(EVENT_5, 2600); // 203865
            }
            if (me->GetEntry() == 101883)
            {
                events.ScheduleEvent(EVENT_6, 2500); // 131995
                events.ScheduleEvent(EVENT_7, 3500); // 80016 31
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_7:
                        DoCast(80016);
                        events.ScheduleEvent(EVENT_7, 31000); // 80016 31
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_aspersiyAI : public Scripted_NoMovementAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 21000); // 204779
            events.ScheduleEvent(EVENT_2, 35000); // 204773  21
            events.ScheduleEvent(EVENT_3, 36000); // 43 summons
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        DoCast(204773);
                        events.ScheduleEvent(EVENT_2, 21000); // 204773  21
                        break;
                    case EVENT_3:
                        ...
                        events.ScheduleEvent(EVENT_3, 43000); // 43 summons
                        break;
                }
            }
            DoMeleeAttackIfReady();
