...
public:
    ...
    struct npc_gorgonnashAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_1, 6000); // 215925
            events.ScheduleEvent(EVENT_2, 10000); // add  not repeat
            events.ScheduleEvent(EVENT_3, 7000); // not repeat
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        DoCast(215925); 
                        events.ScheduleEvent(EVENT_1, urand(6000, 11000));
                        break;
                    case EVENT_2:
                        for(int i = 0; i < 2; ++i)
                        {
