...
public:
    ...
    struct npc_vigfus_bladewindAI : ScriptedAI
    {
        ...
        {
            Talk(0);
            events.ScheduleEvent(EVENT_1, 7000); // 195574
            events.ScheduleEvent(EVENT_2, 10000); // 195575 23 + after 195996
            events.ScheduleEvent(EVENT_3, 13000); // 195996
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(195574);
                        events.ScheduleEvent(EVENT_1, 11000); // 195574
                        break;
                    case EVENT_2:
                        DoCast(195575);
                        events.ScheduleEvent(EVENT_2, 23000); // 195575 23 + after 195996
                        break;
                    case EVENT_3:
                        DoCast(195996);
                        me->StopAttack();
                        ...
                        events.ScheduleEvent(EVENT_3, 51000);
                        break;
                }
            }
...
public:
    ...
    struct npc_vigfus_bladewind_lastAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_2, 10000); // 195575 23 + after 195996
            events.ScheduleEvent(EVENT_3, 13000); // 195996
            events.ScheduleEvent(EVENT_4, 54000); // 196135
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_4:
                        DoCast(196135);
                        events.ScheduleEvent(EVENT_4, 53000);
                        break;
                }
            }
            DoMeleeAttackIfReady();
