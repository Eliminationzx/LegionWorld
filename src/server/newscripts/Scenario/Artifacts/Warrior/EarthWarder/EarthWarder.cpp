...
public:
    ...
    struct npc_hrutnirAI : ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, 6000); // 57846
            events.ScheduleEvent(EVENT_2, 18000); // 32736
            events.ScheduleEvent(EVENT_3, 20000); // 13730
            events.ScheduleEvent(EVENT_4, 24000); // 9080
            if (who && who->GetEntry() == 96034)
            {
                events.ScheduleEvent(EVENT_5, 25000); // 79871
                events.ScheduleEvent(EVENT_6, 29000); // 191882
            }
        }
...
public:
    ...
    struct npc_magnar_icebreakerAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            events.ScheduleEvent(EVENT_1, 7000); // 191883
            events.ScheduleEvent(EVENT_2, 14000); // 191885
            events.ScheduleEvent(EVENT_3, 15000); // 128749
            events.ScheduleEvent(EVENT_4, 33000); // event
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        DoCast(191883);
                        events.ScheduleEvent(EVENT_1, 15000); // 191883
                        break;
                    case EVENT_2:
                        DoCast(191885);
                        events.ScheduleEvent(EVENT_2, 15000); // 191885
                        break;
                    case EVENT_3:
                        DoCast(128749);
                        events.ScheduleEvent(EVENT_3, 15000); // 128749
