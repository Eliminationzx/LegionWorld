            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                    {
                        ...
                        events.ScheduleEvent(EVENT_1, 25500);
                        break;
                    }
                    case EVENT_2:
                    {
...
public:
    ...
    struct boss_brawguild_grandpaAI : public BrawlersBossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_1, 4000); // throw
            events.ScheduleEvent(EVENT_2, 18000); // song
            events.ScheduleEvent(EVENT_3, 8000); // kantata urand(40, 48)
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
        {
            if (me->HealthBelowPct(healthPct))
            {
                if (healthPct == 50)
                {
                    ...
                    events.ScheduleEvent(EVENT_4, urand(5000, 8000));
                }
                else
                {
                    healthPct = 0;
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        ...
                        events.ScheduleEvent(EVENT_2, urand(18000, 25000));
                        break;
                    case EVENT_3:
                        DoCast(SPELL_KANTATA_OF_FLEIT);
                        events.ScheduleEvent(EVENT_3, urand(35000, 48000));
                        break;
                    case EVENT_4:
                    {
                        if (Unit* owner = me->GetAnyOwner())
...
public:
    ...
    struct boss_brawguild_osoAI : public BrawlersBossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_3, urand(21000, 22000)); 
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        ...
                        events.ScheduleEvent(EVENT_2, urand(20000, 22000));
                        break;
                    case EVENT_3:
                    {
                        Position pos;
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_4:
                        ...
                        events.ScheduleEvent(EVENT_4, 7500);
                        break;
                }
            }
            DoMeleeAttackIfReady();
