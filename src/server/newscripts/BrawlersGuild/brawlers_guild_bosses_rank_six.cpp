            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_2:
                        DoCast(232261);
                        if (!need_stop)
                            events.ScheduleEvent(EVENT_2, 400);
                        break;
                }
            }
...
public:
    ...
    struct boss_brawguild_millieAI : public BrawlersBossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_3, 33000);
        }     
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        events.ScheduleEvent(EVENT_2, 5200);
                        break;
                    case EVENT_2:
                        DoCast(SPELL_ELECTRIC_DYNAMITE);
                        events.ScheduleEvent(EVENT_1, 1100);
                        break;
                    case EVENT_3:
...
public:
    ...
    struct boss_brawguild_carlAI : public BrawlersBossAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_1, 2000);
            events.ScheduleEvent(EVENT_2, 33000);
        ...
        void EnterCombat(Unit* who) override
        {
            switch(me->GetEntry())
            {
                ...
                case 114955:
					...
					events.ScheduleEvent(EVENT_7, 80000);
