...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(std::string const& basePath, uint32 mapId)
    {
        ...
        if (!file)
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMapData: Error: Could not open mmap file '%s'", fileName.c_str());
            return false;
        }
        ...
        if (count != 1)
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMapData: Error: Could not read params from file '%s'", fileName.c_str());
            return false;
        }
        ...
        {
            dtFreeNavMesh(mesh);
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMapData: Failed to initialize dtNavMesh for mmap %04u from file %s", mapId, fileName.c_str());
            return false;
        }

        sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMapData: Loaded %04i.mmap", mapId);
    ...
    bool MMapManager::loadMapImpl(std::string const& basePath, uint32 mapId, int32 x, int32 y)
    {
        ...
        if (!file)
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: Could not open mmtile file '%s'", fileName.c_str());
            return false;
        }
        ...
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: Bad header in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: %04u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: %04u%02i%02i.mmtile has corrupted data size", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        if(!data)
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: %04u%02i%02i.mmtile has corrupted fileHeader.size %u", mapId, x, y, fileHeader.size);
            fclose(file);
            return false;
        }
        ...
        if (!result)
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: Bad header or data in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        if (dtStatusSucceed(mmap->navMesh->addTile(data, fileHeader.size, DT_TILE_FREE_DATA, 0, &tileRef)))
        {
            ...
            ++loadedTiles;
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: Loaded mmtile %04i[%02i, %02i] into %04i[%02i, %02i]", mapId, x, y, mapId, header->x, header->y);
            return true;
        }
        else
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadMap: Could not load %04u%02i%02i.mmtile into navmesh", mapId, x, y);
            dtFree(data);
            return false;
        }
    }
    ...
    bool MMapManager::loadMapInstanceImpl(std::string const& basePath, uint32 mapId, uint32 instanceId)
    {
        ...
        if (dtStatusFailed(query->init(mmap->navMesh, 1024)))
        {
            ...
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %04u instanceId %u", mapId, instanceId);
            return false;
        }

        sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId %04u instanceId %u", mapId, instanceId);
    ...
    bool MMapManager::unloadMapImpl(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (itr == loadedMMaps.end())
        {
            ...
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh map. %04u%02i%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        {
            // file may not exist, therefore not loaded
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %04u%02i%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        if (dtStatusFailed(mmap->navMesh->removeTile(tileRefItr->second, nullptr, nullptr)))
        {
            ...
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Could not unload %04u%02i%02i.mmtile from navmesh", mapId, x, y);
            ABORT();
        }
        else
        {
            ...
            --loadedTiles;
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Unloaded mmtile %04i[%02i, %02i] from %03i", mapId, x, y, mapId);
            return true;
        }
    ...
    bool MMapManager::unloadMapImpl(uint32 mapId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh map %04u", mapId);
            return false;
        }
        ...
        for (MMapTileSet::iterator i = mmap->loadedTileRefs.begin(); i != mmap->loadedTileRefs.end(); ++i)
        {
            ...
            {
                --loadedTiles;
                sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Unloaded mmtile %04i[%02i, %02i] from %04i", mapId, x, y, mapId);
            }
        }

        delete mmap;
        itr->second = nullptr;
        sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMap: Unloaded %04i.mmap", mapId);
    ...
    bool MMapManager::unloadMapInstance(uint32 mapId, uint32 instanceId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMapInstance: Asked to unload not loaded navmesh map %04u", mapId);
            return false;
        }
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId %04u instanceId %u", mapId, instanceId);
            return false;
        }
        ...
        sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:unloadMapInstance: Unloaded mapId %04u instanceId %u", mapId, instanceId);
    ...
    bool MMapManager::loadGameObject(uint32 displayId)
    {
        ...
        if (!file)
        {
            // sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadGameObject: Error: Could not open mmap file %s", fileName);
            delete [] fileName;
            return false;
        }
        ...
        {
            sLog->outError(LOG_FILTER_MMAPS, "MMAP:loadGameObject: Bad header in mmap %s", fileName);
            fclose(file);
            delete [] fileName;
            return false;
        }
        ...
        if (fileHeader.mmapVersion != MMAP_VERSION)
        {
            sLog->outError(LOG_FILTER_MMAPS, "MMAP:loadGameObject: %s was built with generator v%i, expected v%i", fileName, fileHeader.mmapVersion, MMAP_VERSION);
            fclose(file);
            delete [] fileName;
            return false;
        }
        ...
        if (!result)
        {
            sLog->outError(LOG_FILTER_MMAPS, "MMAP:loadGameObject: Bad header or data in mmap %s", fileName);
            fclose(file);
            delete [] fileName;
            return false;
        }
        ...
        {
            dtFreeNavMesh(mesh);
            sLog->outError(LOG_FILTER_MMAPS, "MMAP:loadGameObject: Failed to initialize dtNavMesh from file %s. Result 0x%x.", fileName, r);
            delete [] fileName;
            return false;
        }
        sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:loadGameObject: Loaded file %s [size=%u]", fileName, fileHeader.size);
        delete [] fileName;
    ...
    dtNavMeshQuery const* MMapManager::GetModelNavMeshQuery(uint32 displayId, uint32 instanceId)
    {
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            ...
            {
                dtFreeNavMeshQuery(query);
                sLog->outError(LOG_FILTER_MMAPS, "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for displayid %03u instanceId %u", displayId, instanceId);
                return NULL;
            }

            sLog->outDebug(LOG_FILTER_MMAPS, "MMAP:GetNavMeshQuery: created dtNavMeshQuery for displayid %03u instanceId %u", displayId, instanceId);
