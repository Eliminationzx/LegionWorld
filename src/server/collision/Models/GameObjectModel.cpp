...
void LoadGameObjectModelList(std::string const& dataPath)
{
    ...
    if (!model_list_file)
    {
        sLog->outDebug(LOG_FILTER_VMAPS, "Unable to open '%s' file.", VMAP::GAMEOBJECT_MODELS);
        return;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_VMAPS, "File '%s' has wrong header, expected %s.", VMAP::GAMEOBJECT_MODELS, VMAP::VMAP_MAGIC);
    ...
    while (true)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_VMAPS, "File '%s' seems to be corrupted!", VMAP::GAMEOBJECT_MODELS);
            break;
        }

        if (v1.isNaN() || v2.isNaN())
        {
            sLog->outDebug(LOG_FILTER_VMAPS, "File '%s' Model '%s' has invalid v1%s v2%s values!", VMAP::GAMEOBJECT_MODELS, std::string(buff, name_length).c_str(), v1.toString().c_str(), v2.toString().c_str());
            continue;
        }
    ...
    fclose(model_list_file);
    sLog->outDebug(LOG_FILTER_VMAPS, ">> Loaded %u GameObject models in %u ms", uint32(model_list.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
bool GameObjectModel::initialize(std::unique_ptr<GameObjectModelOwnerBase> modelOwner, std::string const& dataPath)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_VMAPS, "GameObject model %s has zero bounds, loading skipped", it->second.name.c_str());
        return false;
    }
...
bool GameObjectModel::intersectLine(G3D::Ray const& ray, float& maxDist, bool stopAtFirstHit, std::set<uint32> const& phases) const
{
    ...
    // sLog->outError(LOG_FILTER_DYNLOS, "GameObjectModel::intersectLine distance %f hit %u iInvScale %f iScale %f maxDist %f", distance, hit, iInvScale, iScale, maxDist);
...
bool GameObjectModel::getObjectHitPos(std::set<uint32> const& phases, G3D::Vector3 const& startPos, G3D::Vector3 const& endPos, G3D::Vector3& resultHitPos, float modifyDist) const
{
    ...
    // sLog->outError(LOG_FILTER_DYNLOS, "GameObjectModel::getObjectHitPos dist %f maxDist %f hit %u resultHitPos %f %f %f", dist, maxDist, result, resultHitPos.x, resultHitPos.y, resultHitPos.x);
    return result;
}
...
bool GameObjectModel::isInLineOfSight(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos, std::set<uint32> const& phases) const
{
    ...
    // sLog->outError(LOG_FILTER_DYNLOS, "GameObjectModel::isInLineOfSight maxSearchDist %f hit %u", maxDist, hit);
...
float GameObjectModel::getHeight(float x, float y, float z, float maxSearchDist, std::set<uint32> const& phases) const
{
    ...
    // sLog->outError(LOG_FILTER_DYNLOS, "GameObjectModel::getHeight maxSearchDist %f v.z %f hit %u height %f", maxSearchDist, v.z, hit, v.z - maxSearchDist);
