...
namespace VMAP
{
    ...
    struct WModelLineCallBack
    {
        ...
        bool operator()(const G3D::Ray& ray, uint32 entry, float& distance, bool pStopAtFirstHit)
        {
            ...
            if (result)
            {
                // sLog->outDebug(LOG_FILTER_UWOW_CORE, "WModelLineCallBack entry %i distance %f pStopAtFirstHit %i iGroupWMOID %u", entry, distance, pStopAtFirstHit, models[entry].GetWmoID());
    ...
    bool WorldModel::intersectLine(const G3D::Ray &ray, float &distance, bool stopAtFirstHit) const
    {
        ...
        if (groupModels.size() == 1)
        {
            ...
            if (hit)
            {
                // sLog->outDebug(LOG_FILTER_UWOW_CORE, "WorldModel::intersectLine RootWMOID %i iGroupWMOID %u _filename %s", RootWMOID, groupModels[0].GetWmoID(), _filename.c_str());
        ...
        groupTree.intersectRay(ray, isc, distance, stopAtFirstHit);
        // if (isc.hit)
            // sLog->outDebug(LOG_FILTER_UWOW_CORE, "WorldModel::intersectLine RootWMOID %i _filename %s", RootWMOID, _filename.c_str());
        return isc.hit;
    }
