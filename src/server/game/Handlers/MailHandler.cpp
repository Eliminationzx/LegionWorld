...
void WorldSession::HandleSendMail(WorldPackets::Mail::SendMail& packet)
{
    ...
    sLog->outInfo(LOG_FILTER_NETWORKIO, "Player %u includes %u items, " UI64FMTD " copper and " UI64FMTD " COD copper with stationery = %u", player->GetGUIDLow(), packet.Info.Attachments.size(), packet.Info.SendMoney, packet.Info.Cod, packet.Info.StationeryID);
    ...
    if (packet.Info.SendMoney && player->getLevel() < sWorld->getIntConfig(CONFIG_MAIL_GOLD_LEVEL_REQ))
    {
    ...
    {
        sLog->outInfo(LOG_FILTER_NETWORKIO, "Player %u is sending mail to %s (GUID: not existed!) with subject %s and body %s includes %u items, " UI64FMTD " copper and " UI64FMTD " COD copper with stationery = %u",
    ...
    sLog->outInfo(LOG_FILTER_NETWORKIO, "Player %u is sending mail to %s (GUID: %u) with subject %s and body %s includes %u items, " UI64FMTD " copper and " UI64FMTD " COD copper with stationery = %u",
    ...
    if (!packet.Info.Attachments.empty() || packet.Info.SendMoney > 0)
    {
        if (!packet.Info.Attachments.empty())
        {
            for (auto const& item : items)
            {
                if (!AccountMgr::IsPlayerAccount(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                    sLog->outCommand(GetAccountId(), "GM %s (Account: %u) mail item: %s (Entry: %u Count: %u) to player: %s (Account: %u)", GetPlayerName().c_str(), GetAccountId(), item->GetTemplate()->Name->Str[receive ? receive->GetSession()->GetSessionDbLocaleIndex() : player->GetSession()->GetSessionDbLocaleIndex()], item->GetEntry(), item->GetCount(), packet.Info.Target.c_str(), rc_account);
        ...
        if (packet.Info.SendMoney >= sWorld->getIntConfig(CONFIG_LOG_GOLD_FROM))
            sLog->outDebug(LOG_FILTER_GOLD, "SendMail Player %s GUID %u (Account: %u) mail money: " UI64FMTD " to player: %s GUID %u (Account: %u)", GetPlayerName().c_str(), _player->GetGUIDLow(), GetAccountId(), packet.Info.SendMoney, packet.Info.Target.c_str(), receiverGuid.GetGUIDLow(), rc_account);
    }

    draft.AddMoney(packet.Info.SendMoney).AddCOD(packet.Info.Cod).SendMailTo(trans, MailReceiver(receive, receiverGuid.GetCounter()), MailSender(player), packet.Info.Body.empty() ? MAIL_CHECK_MASK_COPIED : MAIL_CHECK_MASK_HAS_BODY, needItemDelay ? sWorld->getIntConfig(CONFIG_MAIL_DELIVERY_DELAY) : 0);
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        if (m->COD > 0)                                     //if there is COD, take COD money from player and send them to sender by mail
        {
            ...
            if (!AccountMgr::IsPlayerAccount(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
            {
                ...
                sLog->outCommand(GetAccountId(), "GM %s (Account: %u) receive mail item: %s (Entry: %u Count: %u) and send COD money: " UI64FMTD " to player: %s (Account: %u)", GetPlayerName().c_str(), GetAccountId(), item->GetTemplate()->Name->Str[_player->GetSession()->GetSessionDbLocaleIndex()], item->GetEntry(), item->GetCount(), m->COD, sender_name.c_str(), sender_accId);
            }
            else if (!receive)
                sender_accId = ObjectMgr::GetPlayerAccountIdByGUID(sender_guid);
            ...
            {
                if (item)
                    sLog->outDebug(LOG_FILTER_GOLD, "MailTakeItem: %s GUID %u (Account: %u) receive mail item: %s (Entry: %u Count: %u) and send COD money: " UI64FMTD " to player: %u (Account: %u)", GetPlayerName().c_str(), _player->GetGUIDLow(), GetAccountId(), item->GetTemplate()->Name->Str[_player->GetSession()->GetSessionDbLocaleIndex()], item->GetEntry(), item->GetCount(), m->COD, sender_guid.GetGUIDLow(), sender_accId);
                else
                    sLog->outDebug(LOG_FILTER_GOLD, "MailTakeItem: %s GUID %u (Account: %u) receive send COD money: " UI64FMTD " to player: %u (Account: %u)", GetPlayerName().c_str(), _player->GetGUIDLow(), GetAccountId(), m->COD, sender_guid.GetGUIDLow(), sender_accId);
            }
