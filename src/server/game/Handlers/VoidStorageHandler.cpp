...
void WorldSession::HandleVoidStorageTransfer(WorldPackets::VoidStorage::VoidStorageTransfer& packet)
{
    ...
    for (size_t i = 0; i < packet.Deposits.size(); ++i)
    {
        ...
        { // donate status before destroy item
            ...
            sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 5 item guid = %u, entry = %u, %s", itemVS.ItemId, item->GetEntry(), player->GetInfoForDonate().c_str());
            uint8 index = 0;
    ...
    for (size_t i = 0; i < packet.Withdrawals.size(); ++i)
    {
        ...
        { // donate status before destroy VS
            SQLTransaction transs = LoginDatabase.BeginTransaction();
            sLog->outDebug(LOG_FILTER_DONATE, "[Status] (Return from void storage) Status = 0 item guid = %u, entry = %u, %s", item->GetGUID().GetGUIDLow(), itemVS->ItemId, player->GetInfoForDonate().c_str());
            uint8 index = 0;
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_HISTORY_STATUS_AND_GUID_BY_STATUS);
            stmt->setUInt32(  index, 0);
            stmt->setUInt32(  ++index, item->GetGUID().GetGUIDLow());
