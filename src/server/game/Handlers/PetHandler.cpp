...
void WorldSession::HandlePetAction(WorldPackets::PetPackets::PetAction& packet)
{
    ...
    else
    {
        ...
        for (auto const& itr : GetPlayer()->m_Controlled)
            if ((itr->GetEntry() == pet->GetEntry() || itr->ToCreature() && itr->ToCreature()->m_isHati) && itr->isAlive())
            {
                if (itr->ToCreature())
                {
...
void WorldSession::HandlePetRename(WorldPackets::PetPackets::PetRename& packet)
{
    ...
    if (!pet || !pet->isPet() || static_cast<Pet*>(pet)->getPetType() != HUNTER_PET ||!pet->HasByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PET_FLAGS, UNIT_CAN_BE_RENAMED) || pet->GetOwnerGUID() != _player->GetGUID() || !pet->GetCharmInfo())
        return;
...
void WorldSession::HandlePetAbandon(WorldPackets::PetPackets::PetAbandon& packet)
{
    ...
    if (Creature* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, packet.Pet))
    {
        if (pet->isPet())
        {
            _player->RemovePet(dynamic_cast<Pet*>(pet), true);
...
void WorldSession::HandlePetSpellAutocast(WorldPackets::PetPackets::PetSpellAutocast& packet)
{
    ...
    if (pet->isPet())
        dynamic_cast<Pet*>(pet)->ToggleAutocast(spellInfo, packet.AutocastEnabled);
    else
...
void WorldSession::HanleSetPetSlot(WorldPackets::PetPackets::SetPetSlot& packet)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Stablemaster (GUID:%u) not found or you can't interact with him.", packet.NpcGUID.GetGUIDLow());
...
void WorldSession::HandlePetActionHelper(Unit* pet, ObjectGuid petGuid, uint32 spellid, uint16 flag, ObjectGuid targetGuid, const Position& pos)
{
    CharmInfo* charmInfo = pet->GetCharmInfo();
    if (!charmInfo)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSession::HandlePetAction(petGuid: " UI64FMTD ", tagGuid: " UI64FMTD ", spellId: %u, flag: %u): object (entry: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", petGuid.GetCounter(), targetGuid.GetCounter(), spellid, flag, pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
            ...
            switch (spellid)
            {
                ...
                case COMMAND_ATTACK:                        //spellid=1792  //ATTACK
                {
                    ...
                    if (pet->getVictim() != TargetUnit || (pet->getVictim() == TargetUnit && !pet->GetCharmInfo()->IsCommandAttack()))
                    {
                        ...
                        if (!pet->IsPlayer() && pet->ToCreature()->IsAIEnabled)
                        {
                            ...
                            //10% chance to play special pet attack talk, else growl
                            if (pet->ToCreature()->isPet() && dynamic_cast<Pet*>(pet)->getPetType() == SUMMON_PET && pet != TargetUnit && urand(0, 100) < 10)
                ...
                case COMMAND_ABANDON:                       // abandon (hunter pet) or dismiss (summoned pet)
                    ...
                    else if (pet->GetOwnerGUID() == GetPlayer()->GetGUID())
                    {
                        ...
                        if (pet->isPet())
                            GetPlayer()->RemovePet(dynamic_cast<Pet*>(pet));
