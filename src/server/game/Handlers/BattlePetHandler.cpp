...
void WorldSession::HandleCageBattlePet(WorldPackets::BattlePet::BattlePetGuidRead& packet)
{
    ...
    battlePet->Remove(_player);
}

void WorldSession::HandleBattlePetSetSlot(WorldPackets::BattlePet::SetBattleSlot& packet)
{
    // sLog->outDebug(LOG_FILTER_BATTLEPET, "HandleBattlePetSetSlot m_IsPetBattleJournalLocked %u", m_IsPetBattleJournalLocked);
...
void WorldSession::HandlePetBattleRequestWild(WorldPackets::BattlePet::RequestWild& packet)
{
    ...
    UnitList nearCreature;
    Trinity::AnyFriendlyUnitInObjectRangeCheck uCheck(wildBattlePet, wildBattlePet, 50.0f);
    Trinity::UnitListSearcher<Trinity::AnyFriendlyUnitInObjectRangeCheck> searcher(wildBattlePet, nearCreature, uCheck);
    Trinity::VisitNearbyObject(wildBattlePet, 40.0f, searcher);

    uint32 wildsPetCount = 1;
    for (auto current : nearCreature)
    {
        if (wildsPetCount >= MAX_PETBATTLE_SLOTS)
            break;

        if (!current->ToCreature() || !current->isAlive() || current->GetGUID() == wildBattlePet->GetGUID() || !sWildBattlePetMgr->IsWildPet(current->ToCreature()))
            continue;

        if (sWildBattlePetMgr->GetWildBattlePet(current->ToCreature()) != nullptr && roll_chance_i(80))
        {
            wildBattlePets[wildsPetCount] = sWildBattlePetMgr->GetWildBattlePet(current->ToCreature());
...
{
    // sLog->outDebug(LOG_FILTER_BATTLEPET, "SendPetBattleSlotUpdates");
...
{
    // sLog->outDebug(LOG_FILTER_BATTLEPET, "SendPetBattleFirstRound");
...
{
    // sLog->outDebug(LOG_FILTER_BATTLEPET, "SendPetBattleRoundResult");
...
{
    // sLog->outDebug(LOG_FILTER_BATTLEPET, "SendPetBattleFinalRound");
