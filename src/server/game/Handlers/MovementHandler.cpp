...
void WorldSession::HandleWorldPortAck()
{
    ...
    if (newMap && GetMap() != newMap)
    {
        ...
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSession::HandleMoveWorldportAckOpcode Map %d could not be enter player %d with bed session code", loc.GetMapId(), player->GetGUIDLow());
...
void WorldSession::HandleMoveTeleportAck(WorldPackets::Movement::MoveTeleportAck& packet)
{
    ...
    if (Unit* mover = _player->m_mover)
    {
        ...
        mover->m_movementInfo.clientTickCount = packet.MoveTime;
...
void WorldSession::HandleMovementOpcode(OpcodeClient opcode, MovementInfo& movementInfo)
{
    ...
    if (mover->GetVehicleKit())
        if (mover->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
            if (Unit* charmer = mover->GetCharmer())
                if (charmer->IsPlayer())
                    plrMover = dynamic_cast<Player*>(charmer);
    ...
    if (plrMover)
    {
        // save previous data for anticheat checks
        plrMover->GetAnticheatMgr()->SetLastClientMoveTime(plrMover->m_movementInfo.clientTickCount);
        plrMover->GetAnticheatMgr()->SetLastServerMoveTime(plrMover->m_movementInfo.MoveTime);

        // base movement anticheat checks
        if (!plrMover->GetAnticheatMgr()->MovementChecks(opcode, movementInfo))
        {
            KickPlayer();
            return;
        }
    ...
    // update moveStartTime on server
    // TODO: remove it
    /*if (m_clientTimeDelay == 0)
    m_clientTimeDelay = getMSTime() - movementInfo.MoveTime;

    movementInfo.MoveTime += m_clientTimeDelay + MOVEMENT_PACKET_TIME_DELAY;*/
    ...
    if (opcode == CMSG_MOVE_FALL_LAND && plrMover && !plrMover->isInFlight())
    {
        ...
        plrMover->GetAnticheatMgr()->ResetFallData();
    }
    ...
    if (plrMover)
    {
        ...
        if (no_fly_auras && fly_flags && plrMover->GetSession()->GetSecurity() < SEC_MODERATOR)
...
void WorldSession::HandleForceSpeedChangeAck(WorldPackets::Movement::MovementSpeedAck& packet)
{
    ...
    // update time
    packet.Ack.movementInfo.MoveTime = getMSTime();
    // relocate
    ...
    // anticheat processing here
    //player->GetAnticheatMgr()->SetClientSpeed(moveType, packet.Speed);
}
...
void WorldSession::HandleTimeSyncResponse(WorldPackets::Movement::TimeSyncResponse& packet)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Received CMSG_TIME_SYNC_RESPONSE from player %s without requesting it (hacker?)", player->GetName());
        return;
    }

    if (packet.SequenceIndex != player->m_timeSyncQueue.front())
        sLog->outError(LOG_FILTER_NETWORKIO, "Wrong time sync counter from player %s (cheater?)", player->GetName());
...
void WorldSession::HandleDiscardedTimeSyncAcks(WorldPackets::Movement::DiscardedTimeSyncAcks& packet)
{
    ...
    if (player->m_sequenceIndex != packet.MaxSequenceIndex)
        sLog->outError(LOG_FILTER_NETWORKIO, "Received CMSG_DISCARDED_TIME_SYNC_ACKS from player %s, but maxSequenceIndex %u isn't equal real server SequenceIndex %u", player->GetName(), packet.MaxSequenceIndex, player->m_sequenceIndex);
...
void WorldSession::HandleTimeSyncResponseFailed(WorldPackets::Movement::TimeSyncResponseFailed& packet)
{
    ...
    sLog->outError(LOG_FILTER_NETWORKIO, "WorldSession::HandleTimeSyncResponseFailed:: what we should do with it?: Player: %s, packet.SequenceIndex: %u, PlayerSequenceIndex: %u ",
