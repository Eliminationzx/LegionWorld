            ...
            if (myItems[i])
            {
                ...
                if (!AccountMgr::IsPlayerAccount(_player->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
                    sLog->outCommand(_player->GetSession()->GetAccountId(), "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: %s (Account: %u)",
                        ...
                        myItems[i]->GetTemplate()->Name->Str[_player->GetSession()->GetSessionDbLocaleIndex()], myItems[i]->GetEntry(), myItems[i]->GetCount(),
            ...
            if (hisItems[i])
            {
                ...
                if (!AccountMgr::IsPlayerAccount(trader->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
                    sLog->outCommand(trader->GetSession()->GetAccountId(), "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: %s (Account: %u)",
                        trader->GetName(), trader->GetSession()->GetAccountId(),
                        hisItems[i]->GetTemplate()->Name->Str[trader->GetSession()->GetSessionDbLocaleIndex()], hisItems[i]->GetEntry(), hisItems[i]->GetCount(),
                        _player->GetName(), _player->GetSession()->GetAccountId());
                }
...
void WorldSession::HandleAcceptTrade(WorldPackets::Trade::AcceptTrade& acceptTrade)
{
    ...
    if (hisTrade->IsAccepted())
    {
        ...
        if(myTrade->GetMoney() >= sWorld->getIntConfig(CONFIG_LOG_GOLD_FROM))
            sLog->outDebug(LOG_FILTER_GOLD, "Trade: From player %s GUID %u Account: %u give money (Amount: " UI64FMTD ") to player: %s GUID %u Account: %u",
