...
void WorldSession::HandleGossipSelectOption(WorldPackets::NPC::GossipSelectOption& packet)
{
    ...
    if (_ms > 100)
        sLog->outDiff("HandleGossipSelectOption Entry %u GossipIndex %u GossipID %u wait - %ums", unit->GetEntry(), packet.GossipIndex, packet.GossipID, _ms);
}
    ...
    for (uint32 slot = 0; slot < rawItemCount; ++slot)
    {
        ...
        if (vendorItem->Type == ITEM_VENDOR_TYPE_ITEM)
        {
            ...
            if (!player->isGameMaster())
            {
                ...
                if ((itemTemplate->Flags2 & ITEM_FLAG2_FACTION_HORDE && player->GetTeam() == ALLIANCE) ||
                    (itemTemplate->Flags2 & ITEM_FLAG2_FACTION_ALLIANCE && player->GetTeam() == HORDE))
                    continue;
            ...
            if (vendorItem->Money != 0)
                ...
                price = uint64(itemTemplate->BuyPrice * discountMod);
...
void WorldSession::HandleRequestStabledPets(WorldPackets::NPC::RequestStabledPets& packet)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Stablemaster (GUID:%u) not found or you can't interact with him.", packet.StableMaster.GetGUIDLow());
