...
void WorldSession::HandleUseItemOpcode(WorldPackets::Spells::ItemUse& cast)
{
    ...
    if (proto->Class == ITEM_CLASS_CONSUMABLE && !(proto->Flags2 & ITEM_FLAG2_IGNORE_DEFAULT_RATED_BG_RESTRICTIONS) && pUser->InRBG())
    {
        pUser->SendEquipError(EQUIP_ERR_NOT_DURING_ARENA_MATCH, pItem);
        return;
    }
    ...
    if (proto->Flags2 & ITEM_FLAG2_NOT_USABLE_IN_RATED_BG && pUser->InRBG())
    {
        pUser->SendEquipError(EQUIP_ERR_NOT_DURING_ARENA_MATCH, pItem);
        return;
    }
    ...
    else
    {
        switch (type)
        {
            ...
            case ACTION_BUTTON_MOUNT:
                sLog->outInfo(LOG_FILTER_NETWORKIO, "MISC: Added mount or favorite mount into button %u", action, packet.Index);
                break;
            default:
