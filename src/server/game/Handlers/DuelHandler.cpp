...
void WorldSession::HandleDuelResponse(WorldPackets::Duel::DuelResponse& packet)
{
    if (GameObject* arbiter = ObjectAccessor::FindGameObject(packet.ArbiterGUID))
    {
        ...
        if (packet.Accepted)
        {
            ...
            sLog->outDebug(LOG_FILTER_NETWORKIO, "Player 1 is: %s (%s)", _player->GetGUID().ToString().c_str(), _player->GetName());
            sLog->outDebug(LOG_FILTER_NETWORKIO, "Player 2 is: %s (%s)", dueler->GetGUID().ToString().c_str(), dueler->GetName());
