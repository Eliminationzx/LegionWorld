...
void WorldSession::HandleDestroyItemOpcode(WorldPackets::Item::DestroyItem& destroyItem)
{
    ...
    if (item->GetTemplate()->Flags & ITEM_FLAG_NO_USER_DESTROY)
    {
    ...
    if (Item* item = player->GetItemByGuid(packet.ItemGUID))
    {
        ...
        if (ItemTemplate const* itemTemplate = item->GetTemplate())
        {
            ...
            if (SellPrice > 0)
            {
                ...
                sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 2 item  guid = %u, entry = %u, %s", item->GetGUIDLow(), item->GetEntry(), player->GetInfoForDonate().c_str());
...
void WorldSession::HandleSocketGems(WorldPackets::Item::SocketGems& packet)
{
    ...
    for (uint32 i = 0; i < MAX_GEM_SOCKETS; ++i)
    {
        ...
        {
            if (auto limitEntry = sItemLimitCategoryStore.LookupEntry(iGemProto->ItemLimitCategory))
            {
...
void WorldSession::HandleUpgradeItem(WorldPackets::Item::UpgradeItem& packet)
{
    ...
    if ((itemTemplate->Flags3 & ITEM_FLAG3_ITEM_CAN_BE_UPGRADED) == 0)
        return;

    ItemUpgradeEntry const* itemUpgradeEntry = sItemUpgradeStore.LookupEntry(packet.UpgradeID);
    if (!itemUpgradeEntry)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleUpgradeItems - ItemUpgradeEntry (%u) not found.", packet.UpgradeID);
        return;
    }

    if (!_player->HasCurrency(itemUpgradeEntry->CurrencyType, itemUpgradeEntry->CurrencyAmount))
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleUpgradeItems - Player has not enougth currency (ID: %u, Cost: %u) not found.", itemUpgradeEntry->CurrencyType, itemUpgradeEntry->CurrencyAmount);
        return;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleUpgradeItems - ItemUpgradeEntry (%u) is not related to this ItemUpgradePath (%u).", itemUpgradeEntry->ID, currentUpgradeId);
        return;
    }
