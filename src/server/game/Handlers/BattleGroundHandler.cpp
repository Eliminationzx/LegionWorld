...
void WorldSession::HandleBattlemasterJoin(WorldPackets::Battleground::Join& packet)
{
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Battleground: invalid bgtype (%u) received. possible cheater? player guid %u", queueID, _player->GetGUIDLow());
        return;
    }
    ...
    if (!packet.JoinAsGroup )
    {
        ...
        GroupQueueInfo* ginfo = bgQueue.AddGroup(player, nullptr, queueID, bracketEntry, 0, isRating, packet.JoinAsGroup, packet.BlacklistMap);
    ...
    else
    {
        ...
        if (!err)
        {
            ginfo = bgQueue.AddGroup(player, grp, queueID, bracketEntry, 0, false, packet.JoinAsGroup, packet.BlacklistMap);
...
void WorldSession::JoinBracket(uint8 bracketType, uint8 rolesMask /*= ROLES_DEFAULT*/, uint16 extraBgTypeId /*= 0*/)
{
    ...
    if (!bg)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Battleground: template bg not found BgTypeID %u", bgTypeId);
        return;
    }
...
void WorldSession::HandleBattlemasterJoinBrawl(WorldPackets::Battleground::BattlemasterJoinBrawl& packet)
{
    ...
    switch (data.Type)
    {
    case MS::Battlegrounds::BrawlTypes::Battleground:
    case MS::Battlegrounds::BrawlTypes::Arena:
        JoinBracket(MS::Battlegrounds::BracketType::Brawl, packet.RolesMask, data.BgTypeId);
        break;
    case MS::Battlegrounds::BrawlTypes::Lfg:
        // joint lfg by data.BgTypeId
        break;
    default:
        break;
    }
...
void WorldSession::HandleJoinSkirmish(WorldPackets::Battleground::JoinSkirmish& packet)
{
    ...
    switch (packet.Bracket)
    {
        case MS::Battlegrounds::JoinType::Skirmish2v2:
            jointype = MS::Battlegrounds::JoinType::Arena2v2;
            break;
        case MS::Battlegrounds::JoinType::Skirmish3v3:
            jointype = MS::Battlegrounds::JoinType::Arena3v3;
            break;
        default:
            break;
    }
    ...
    else
    {
        ...
        if (!err)
        {
            ginfo = bgQueue.AddGroup(player, grp, bgTypeId, bracketEntry, jointype);
            avgTime = bgQueue.GetAverageQueueWaitTime(ginfo, bracketEntry->RangeIndex);
        }
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            WorldPackets::Battleground::BattlefieldStatusQueued battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusQueued(&battlefieldStatus, bg, member, member->AddBattlegroundQueueId(bgQueueTypeId), ginfo->JoinTime, avgTime, jointype, true);
            member->SendDirectMessage(battlefieldStatus.Write());
        }
    }
...
{
    auto expirationTimer = MS::Utilities::Globals::InSeconds::Week;
    auto isHolidayActive = true; // sGameEventMgr->IsHolidayActive(268, expirationTimer);

    WorldPackets::Battleground::RequestPvpBrawlInfoResponse response;
    response.UnkBit = isHolidayActive;
    ...
    if (isHolidayActive)
    {
        ...
        response.Description2 = data.Description2;
        response.BrawlType = MS::Battlegrounds::BrawlTypes::None; //@TODO check that shit = data.Type;
    }
