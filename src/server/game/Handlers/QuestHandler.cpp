...
void WorldSession::HandleQuestGiverStatusQuery(WorldPackets::Quest::QuestGiverStatusQuery& packet)
{
    ...
    switch (questgiver->GetTypeId())
    {
        ...
        {
            GameObject* go_questgiver = dynamic_cast<GameObject*>(questgiver);
                ...
                case TYPEID_CONTAINER:
                {
                    ...
                    for (QuestObjective const& obj : quest->GetObjectives())
                    {
                        if (obj.Type == QUEST_OBJECTIVE_ITEM)
                            if ((obj.ObjectID == item->GetEntry()) && (item->GetTemplate()->MaxCount > 0))
                        {
                            destroyItem = false;
                            break;
                        }
...
void WorldSession::HandleQueryQuestRewards(WorldPackets::Quest::QueryQuestRewards& packet)
{
    ...
    if (quest->QuestInfoID)
    {
        ...
        if (ItemID)
        {
            ...
            BonusListIDs = sObjectMgr->GetItemBonusForLevel(ItemID, modTreeID, GetPlayer()->getLevel(), needLevel);

            itemRew.Item.ItemBonus = boost::in_place();
            itemRew.Item.ItemBonus->Context = modTreeID;
            itemRew.Item.ItemBonus->BonusListIDs = BonusListIDs;
    ...
    else
    {
        ...
        WorldPackets::Quest::QueryQuestRewardResponse::ItemReward itemRew;
        itemRew.ItemCount = 1;
        itemRew.Item.ItemID = 147421; // Only for 7.2.5 for next version 7.3.5 need change to 150369
        itemRew.Item.ItemBonus = boost::in_place();
        itemRew.Item.ItemBonus->Context = modTreeID;
        itemRew.Item.ItemBonus->BonusListIDs = BonusListIDs;
        response.Items.emplace_back(itemRew);
...
{
    sLog->outDebug(LOG_FILTER_WORLD_QUEST, "HandleRequestWorldQuestUpdate");
            ...
            for (WorldQuestMap::const_iterator itr = worldQuestInfo->begin(); itr != worldQuestInfo->end(); ++itr)
            {
                for (std::map<uint32 /*QuestID*/, WorldQuest>::const_iterator iter = itr->second.begin(); iter != itr->second.end(); ++iter)
                {
                    ...
                    if (QuestV2CliTaskEntry const* questTask = sQuestV2CliTaskStore.LookupEntry(worldQuest->QuestID))
                    {
                        ...
                        if (needQuest)
                            for (uint8 slot = 0; slot < QUEST_REQUIRED; ++slot)
                                if (questTask->FiltCompletedQuest[slot] && _player->GetQuestStatus(questTask->FiltCompletedQuest[slot]) != QUEST_STATUS_NONE)
                                    canStart = true;
