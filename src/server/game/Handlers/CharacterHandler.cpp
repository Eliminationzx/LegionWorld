...
void WorldSession::HandleCharCreateOpcode(WorldPackets::Character::CreateChar& charCreate)
{
        ...
        .WithChainingPreparedCallback([this, createInfo, SendCharCreate](QueryCallback& queryCallback, PreparedQueryResult result)
    {
        ...
        std::function<void(PreparedQueryResult)> finalizeCharacterCreation = [this, createInfo, SendCharCreate](PreparedQueryResult result)
        {
            ...
            sLog->outInfo(LOG_FILTER_CHARACTER, "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), GetRemoteAddress().c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPackets::Character::DeleteChar& charDelete)
{
    ...
    sLog->outInfo(LOG_FILTER_CHARACTER, "Account: %d (IP: %s) Delete Character:[%s] (GUID: %u)", GetAccountId(), GetRemoteAddress().c_str(), name.c_str(), charDelete.Guid.GetGUIDLow());
...
void WorldSession::HandleLoadScreenOpcode(WorldPackets::Character::LoadingScreenNotify& loadingScreenNotify)
{
    ...
    {
        if (_player)
            _player->SendInitialPacketsAfterAddToMap(m_playerLoading);
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    season.PreviousSeason = sWorldStateMgr.GetWorldStateValue(WS_ARENA_SEASON_ID) - sWorld->getBoolConfig(CONFIG_ARENA_SEASON_IN_PROGRESS);
    if (sWorld->getBoolConfig(CONFIG_ARENA_SEASON_IN_PROGRESS))
        season.CurrentSeason = sWorldStateMgr.GetWorldStateValue(WS_ARENA_SEASON_ID);
    ...
    AddDelayedEvent(100, [=]() -> void
    {
        ...
        if (!player->getCinematic())
        {
            ...
            if (!sWorld->getBoolConfig(CONFIG_FUN_OPTION_ENABLED))
            {
                ...
                {
                    if (player->getClass() == CLASS_DEMON_HUNTER) /// @todo: find a more generic solution
        ...
        //sLog->outDebug(LOG_FILTER_GENERAL, "Player %s added to Map.", player->GetName());
        ...
        if (!player->HasPlayerExtraFlag(PLAYER_EXTRA_INVISIBLE_STATUS))
        {
            if (player->GetGuildId() != 0)
            {
                ...
                else
                {
                    ...
                    sLog->outError(LOG_FILTER_GENERAL, "Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", player->GetName(), player->GetGUIDLow(), player->GetGuildId());
                    player->SetInGuild(0);
                }
            }
        }
        ...
        sLog->outInfo(LOG_FILTER_CHARACTER, "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d", this->GetAccountId(), GetRemoteAddress().c_str(), player->GetName(), player->GetGUIDLow(), player->getLevel());
...
void WorldSession::HandleAlterAppearance(WorldPackets::Character::AlterApperance& packet)
{
    ...
    uint32 cost = player->GetBarberShopCost(bs_hair, packet.NewHairColor, bs_facialHair, bs_skinColor, bs_face, customDisplayEntries);
...
void WorldSession::HandleCharCustomizeCallback(std::shared_ptr<WorldPackets::Character::CharCustomizeInfo> customizeInfo, PreparedQueryResult result)
{
    ...
    if (result = CharacterDatabase.Query(stmt))
        sLog->outInfo(LOG_FILTER_CHARACTER, "Account: %d (IP: %s), Character[%s] (guid:%u) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), result->Fetch()[0].GetString().c_str(), lowGuid, customizeInfo->CharName.c_str());
...
void WorldSession::HandleCharRaceOrFactionChange(WorldPackets::Character::CharRaceOrFactionChange& packet)
{
    ...
    sLog->outDebug(LOG_FILTER_UNITS, "Account: %d Started faction change", GetAccountId());
    ...
    if (used_loginFlag & AT_LOGIN_CHANGE_FACTION)
    {
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change. Home bind start", GetAccountId());
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change, HOME BIND success", GetAccountId());
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change Achievements success", GetAccountId());
        // Item conversion
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change Items success", GetAccountId());
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change spells success", GetAccountId());
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change reputation success", GetAccountId());
        // Title conversion
        if (!knownTitlesStr.empty())
        {
            ...
            sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change titles process... tokens size %u ktcount %u input %s", GetAccountId(), tokens.size(), ktcount, knownTitlesStr.c_str());
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change titles success", GetAccountId());
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change actions 110 success", GetAccountId());
    }

    CharacterDatabase.CommitTransaction(trans);
    sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change commit transaction success", GetAccountId());

    sLog->outDebug(LOG_FILTER_UNITS, "Account: %d (IP: %s), Character guid: %u Change Race/Faction to: %s", GetAccountId(), GetRemoteAddress().c_str(), lowGuid, info->Name.c_str());

    SendCharFactionChange(RESPONSE_SUCCESS, info);
    sLog->outDebug(LOG_FILTER_UNITS, "Account: %d faction change success end", GetAccountId());
}
...
void WorldSession::HandleContinuePlayerLogin()
{
    ...
    sLog->outInfo(LOG_FILTER_OPCODES, "WorldSession::HandleContinuePlayerLogin");
...
void WorldSession::HandleEngineSurvey(WorldPackets::Character::EngineSurvey& packet)
{
    ...
    uint64 str_hash = hash_gen(baseData);
    ...
    if (_hwid != 0)
    {
        if (QueryResult result = LoginDatabase.PQuery("SELECT penalties, last_reason from hwid_penalties where hwid = " UI64FMTD, _hwid))
        {
            Field* fields = result->Fetch();
            ...
            if ((sWorld->getIntConfig(CONFIG_ANTI_FLOOD_HWID_BANS_COUNT) && _countPenaltiesHwid >= sWorld->getIntConfig(CONFIG_ANTI_FLOOD_HWID_BANS_COUNT)) || _countPenaltiesHwid < 0)
            {
                ...
                if (sWorld->getBoolConfig(CONFIG_ANTI_FLOOD_HWID_BANS_ALLOW))
                    sWorld->BanAccount(BAN_ACCOUNT, GetAccountName(), "-1", ss.str().c_str(), "Server");
                else if (sWorld->getBoolConfig(CONFIG_ANTI_FLOOD_HWID_MUTE_ALLOW))
                    sWorld->MuteAccount(GetAccountId(), -1, ss.str().c_str(), "Server", this);
