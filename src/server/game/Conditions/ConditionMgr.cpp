...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_GARRRISON_BUILDING:
        {
            condMeets = false;
            if (Player* player = object->ToPlayer())
                if (GarrisonPtr g = player->GetGarrisonPtr())
        ...
        {
            if (Player* player = object->ToPlayer())
                if (GarrisonPtr garr = player->GetGarrisonPtr())
    ...
    switch (cond->SourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_LOOT_ITEM:
        {
            ...
            if (cond->SourceGroup == 2)
            {
                ...
                if (!currencyEntry)
                {
                    sLog->outError(LOG_FILTER_SQL, "SourceEntry %u in `condition` table, does not exist in `CurrencyTypesEntry`, ignoring.", cond->SourceEntry);
                    return false;
                }
            }
        }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_QUEST_OBJECTIVE_DONE:
        {
            ...
            if (!quest)
            {
                sLog->outError(LOG_FILTER_SQL, "Quest condition specifies non-existing quest (%u), skipped (objective_done)", cond->ConditionValue1 );
                return false;
            }
            ...
            if (cond->ConditionValue2 > 0)
            {
                ...
                if (!found)
                    sLog->outError(LOG_FILTER_SQL, "Quest condition specifies non-existing quest (%d) non existen objective %d, skipped", cond->ConditionValue1, cond->ConditionValue2);
            }
            break;
        }
        case CONDITION_AREA_EXPLORED:
        ...
        case CONDITION_ACTIVE_EVENT:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "ActiveEvent condition has non existing event id (%u), skipped max arr. size %u", cond->ConditionValue1, events.size());
                return false;
            }
        ...
        case CONDITION_REALM_ACHIEVEMENT:
        {
            ...
            if (!achievement)
            {
                sLog->outError(LOG_FILTER_SQL, "AchievementEntry has non existing realm first achivement id (%u), skipped.", cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        case CONDITION_STAND_STATE:
        {
            ...
            if (!valid)
            {
                sLog->outError(LOG_FILTER_SQL, "StandState has non-existing stand state (%u,%u), skipped.", cond->ConditionValue1, cond->ConditionValue2);
                return false;
            }
            break;
        }
        case CONDITION_CLASS_HALL_ADVANCEMENT:
        {
            ...
            if (!entry)
            {
                sLog->outError(LOG_FILTER_SQL, "CONDITION_CLASS_HALL_ADVANCEMENT has non-existing class hall tallentID  - %u, skipped.", cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        case CONDITION_CURRENCY_ON_WEEK:
        {
            ...
            if (!currencyEntry)
            {
                sLog->outError(LOG_FILTER_SQL, "Currency condition has non existing currency (%u), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
