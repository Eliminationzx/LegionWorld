    ...
    do
    {
        ...
        switch (type)
        {
            ...
            case DISABLE_TYPE_CRITERIA:
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Criteria entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->outError(LOG_FILTER_SQL, "Disable flags specified for Criteria %u, useless data.", entry);
                break;
            case DISABLE_TYPE_CRITERIA_TREE:
                if (!sCriteriaTreeStore.LookupEntry(entry))
                {
                    sLog->outError(LOG_FILTER_SQL, "Criteria Tree entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->outError(LOG_FILTER_SQL, "Disable flags specified for Criteria Tree %u, useless data.", entry);
                break;
            case DISABLE_TYPE_ACHIEVEMENT:
                if (!sAchievementStore.LookupEntry(entry))
                {
                    sLog->outError(LOG_FILTER_SQL, "Achievement entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->outError(LOG_FILTER_SQL, "Disable flags specified for Achievement %u, useless data.", entry);
                break;
            case DISABLE_TYPE_LFG:
                if (!sLfgDungeonsStore.LookupEntry(entry))
                {
                    sLog->outError(LOG_FILTER_SQL, "LFGDungeons entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->outError(LOG_FILTER_SQL, "Disable flags specified for LFGDungeons %u, useless data.", entry);
                break;
