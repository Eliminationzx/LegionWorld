...
void GameEventMgr::StopEvent(uint16 event_id, bool overwrite)
{
    ...
    else if (serverwide_evt)
    {
        ...
        if (overwrite || data.state != GAMEEVENT_WORLD_FINISHED)
        {
            ...
            for (GameEventConditionMap::iterator itr = data.conditions.begin(); itr != data.conditions.end(); ++itr)
                itr->second.done = 0;
...
void GameEventMgr::LoadFromDB()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Game Event WorldState Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 npcflags in game events. DB table `game_event_worldstate` is empty.");
        else
        {
            uint32 count = 0;
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    sLog->outError(LOG_FILTER_SQL, "`game_event_worldstate` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u WorldState in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
}
...
{
    for (WorldStateList::iterator itr = mGameEventWorldState[event_id].begin(); itr != mGameEventWorldState[event_id].end(); ++itr)
    {
        switch (itr->Type)
        {
            case WorldStatesData::Types::Map:
                if (Map* map = sMapMgr->CreateBaseMap(itr->TypeID))
                    sWorldStateMgr.SetWorldStateValueFor(map, itr->WorldStateID, activate ? itr->Value : 0);
                break;
            case WorldStatesData::Types::Area:
            case WorldStatesData::Types::Zone:
                sWorldStateMgr.SetWorldStateValueFor(itr->TypeID, itr->WorldStateID, activate ? itr->Value : 0);
                break;
            default:
                break;
        }
    ...
    for (GuidList::iterator itr = mGameEventCreatureGuids[internal_event_id].begin(); itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
            {
                ...
                // sLog->outDebug(LOG_FILTER_GAMEEVENTS, "Spawning creature %u", *itr);
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (event_id > 0)
    {
        for (ObjectGuidList::iterator itr = mGameEventCreatureSpawns[event_id].begin(); itr != mGameEventCreatureSpawns[event_id].end(); ++itr)
        {
    ...
    if (event_id > 0)
    {
        for (ObjectGuidList::iterator itr = mGameEventGameobjectSpawns[event_id].begin(); itr != mGameEventGameobjectSpawns[event_id].end(); ++itr)
        {
