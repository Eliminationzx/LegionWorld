...
{
    SendLFGListStatusUpdate(GetEntrybyGuidLow(group->GetGUIDLow()), player->GetSession(), true);
}

void LFGListMgr::PlayerRemoveFromGroup(Player* player, Group* group)
{
    SendLFGListStatusUpdate(GetEntrybyGuidLow(group->GetGUIDLow()), player->GetSession(), false);
}
...
void LFGListMgr::SendSocialQueueUpdateNotify(LFGListEntry const* lfgEntry, Player* player)
{
    ...
    switch (updateData.Type)
    {
        ...
        case 1:
        {
            ...
            for (auto const& member : group->GetMemberSlots())
                searchResult.Members.emplace_back(member.Class, member.Roles);
            break;
        }
        default:
            //updateData.UnkData2;
