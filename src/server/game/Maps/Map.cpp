...
uint32 Map::ObjectUpdater::updateCollected(uint32 diff)
{
    uint32 count = i_objectsToUpdate.size();
    if (!i_objectsToUpdate.empty())
    {
        for (auto &object : i_objectsToUpdate)
            if (object && object->IsInWorld())
            {
                ...
                if (_ms > 100)
                    sLog->outDiff("ObjectUpdater::updateCollected: type - %u, e:%u(g:%u). time - %ums diff %u visible %f mapId %u ZoneID %u InstanceId %u pos (%f %f %f)",
                        object->GetTypeId(), object->GetEntry(), object->GetGUIDLow(), _ms, diff, object->GetVisibilityRange(), object->GetMapId(), object->GetCurrentZoneID(), object->GetInstanceId(), object->GetPositionX(), object->GetPositionY(), object->GetPositionZ());
            }
...
void Map::LoadMMap(int gx, int gy)
{
    ...
    if (mmapLoadResult)
        sLog->outDebug(LOG_FILTER_MMAPS, "MMAP loaded name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
    else
        sLog->outDebug(LOG_FILTER_MMAPS, "Could not load MMAP name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
}
...
void Map::LoadVMap(int gx, int gy)
{
    ...
    switch (vmapLoadResult)
    {
        case VMAP::VMAP_LOAD_RESULT_OK:
            sLog->outDebug(LOG_FILTER_VMAPS, "VMAP loaded name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case VMAP::VMAP_LOAD_RESULT_ERROR:
            sLog->outDebug(LOG_FILTER_VMAPS, "Could not load VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case VMAP::VMAP_LOAD_RESULT_IGNORED:
            sLog->outDebug(LOG_FILTER_VMAPS, "Ignored VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        default:
            break;
    }
...
void Map::LoadMapImpl(Map* map, int gx, int gy, bool reload)
{
    ...
    {
        #ifdef WIN32
        sLog->outInfo(LOG_FILTER_MAPS, "Unloading previously loaded map %u before reloading.", map->GetId());
        #endif
    ...
    #ifdef WIN32
    sLog->outInfo(LOG_FILTER_MAPS, "Loading map %s gx: %i, gy: %i", fileName.c_str(), gx, gy);
    #endif
    // loading data
...
m_activeNonPlayersIter(m_activeNonPlayers.end()), i_grids(), GridMaps()
{
    ...
    if (CanCreatedZone() || CanCreatedThread())
    {
        ...
        if (_parent && CanCreatedZone())
        {
            if (_parent->OutdoorPvPList && !_parent->OutdoorPvPList->empty())
            {
                OutdoorPvPList = _parent->OutdoorPvPList;
                for (auto itr : *OutdoorPvPList)
                    if (itr && itr->ThisZone(InstanceId))
                        itr->SetMap(this);
            }
        }
    }
...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    if (!p.IsCoordValid())
    {
        sLog->outError(LOG_FILTER_MAPS, "Map::SwitchGridContainers: Object " UI64FMTD " has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID().GetCounter(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
        return;
    }
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "Switch object " UI64FMTD " from grid[%u, %u] %u", obj->GetGUID().GetCounter(), cell.data.Part.grid_x, cell.data.Part.grid_y, on);
...
void Map::SwitchGridContainers(GameObject* obj, bool on)
{
    ...
    if (!p.IsCoordValid())
    {
        sLog->outDebug(LOG_FILTER_MAPS, "Map::SwitchGridContainers: Object %s has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID().ToString().c_str(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
        return;
    }
    ...
    if (sLog->ShouldLog(LOG_FILTER_MAPS, LOG_LEVEL_DEBUG))
    {
        ...
        sLog->outDebug(LOG_FILTER_MAPS, "Switch object %s from grid[%u, %u] %u", obj->GetGUID().ToString().c_str(), grid_x, grid_y, on);
    }
...
void Map::EnsureGridLoadedForActiveObject(const Cell &cell, WorldObject* object)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_MAPS, "Active object " UI64FMTD " triggers loading of grid [%u, %u] on map %u", object->GetGUID().GetCounter(), cell.GridX(), cell.GridY(), GetId());
...
bool Map::AddToMap(Transport* obj)
{
    ...
    if (!cellCoord.IsCoordValid())
    {
        sLog->outDebug(LOG_FILTER_MAPS, "Map::Add: Object %s has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID().ToString().c_str(), obj->GetPositionX(), obj->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
        return false; //Should delete object
    }
...
void Map::Update(const uint32 t_diff)
{
    ...
    for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
    {
        ...
        {
            if (player->IsDelete()) // If object in delete list or ported don`t update it
                continue;
            ...
            if (player->IsInWorld())
            {
                ...
                player->SendUpdatedWorldStates(false);
                uint32 _mssw = GetMSTimeDiffToNow(_ss);
                VisitNearbyCellsOf(this, player, gridObjectUpdate, worldObjectUpdate);
                uint32 _mss = GetMSTimeDiffToNow(_ss);
                if (_mss > 250)
                    sLog->outDiff("player->Update: type - player, g:%u All %ums player %ums WS %ums mapId %u diff %u i_InstanceId %u activeEntry %u", player->GetGUIDLow(), _mss, _mssu, _mssw, GetId(), t_diff, i_InstanceId, m_activeEntry);
            }
    ...
    uint32 collectedCount = i_objectUpdater.updateCollected(t_diff);
    ...
    {
        RecursiveGuard _objectsAddToMap_lock(m_objectsAddToMap_lock);
...
void Map::AreaTriggerRelocation(AreaTrigger* at, float x, float y, float z, float orientation)
{
    ...
    {
        #ifdef TRINITY_DEBUG
        sLog->outDebug(LOG_FILTER_MAPS, "AreaTrigger (%s) added to moving list from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", at->GetGUID().ToString().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
        #endif
...
void Map::MoveAllCreaturesInMoveList()
{
    ...
    for (auto itr : _creaturesToMove)
    {
        if (!itr || itr->FindMap() != this) //pet is teleported to another map
            continue;

        volatile uint32 creatureEntry = itr->GetEntry();

        if (itr->_moveState != MAP_OBJECT_CELL_MOVE_ACTIVE)
        {
            itr->_moveState = MAP_OBJECT_CELL_MOVE_NONE;
            continue;
        }
        ...
        if (CreatureCellRelocation(itr, Cell(itr->_newPosition.m_positionX, itr->_newPosition.m_positionY)))
        {
            // update position and visibility for server and client
            itr->Relocate(itr->_newPosition);
            itr->UpdateObjectVisibility(false);
        }
        else
        {
            // if creature can't be move in new cell/grid (not loaded) move it to repawn cell/grid
            // creature coordinates will be updated and notifiers send
            if (!CreatureRespawnRelocation(itr, false))
            {
                // ... or unload (if respawn grid also not loaded)
                #ifdef TRINITY_DEBUG
                    sLog->outDebug(LOG_FILTER_MAPS, "Creature (GUID: %u Entry: %u) cannot be move to unloaded respawn grid.", itr->GetGUIDLow(), itr->GetEntry());
                #endif
...
void Map::MoveAllAreaTriggersInMoveList()
{
    ...
    for (auto at : _areaTriggersToMove)
    {
        ...
        {
#ifdef TRINITY_DEBUG
            sLog->outDebug(LOG_FILTER_MAPS, "AreaTrigger (%s) cannot be moved to unloaded grid.", at->GetGUID().ToString().c_str());
#endif
        }
    }
...
bool Map::AreaTriggerCellRelocation(AreaTrigger* at, Cell new_cell)
{
    ...
    if (!old_cell.DiffGrid(new_cell))                       // in same grid
    {
        ...
        {
#ifdef TRINITY_DEBUG
            sLog->outDebug(LOG_FILTER_MAPS, "AreaTrigger (%s) moved in grid[%u, %u] from cell[%u, %u] to cell[%u, %u].", at->GetGUID().ToString().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.CellX(), new_cell.CellY());
#endif
        ...
        {
#ifdef TRINITY_DEBUG
            sLog->outDebug(LOG_FILTER_MAPS, "AreaTrigger (%s) moved in same grid[%u, %u]cell[%u, %u].", at->GetGUID().ToString().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY());
#endif
        }
...
#ifdef TRINITY_DEBUG
        sLog->outDebug(LOG_FILTER_MAPS, "Active AreaTrigger (%s) moved from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", at->GetGUID().ToString().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
    ...
    {
#ifdef TRINITY_DEBUG
        sLog->outDebug(LOG_FILTER_MAPS, "AreaTrigger (%s) moved from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", at->GetGUID().ToString().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
...
#ifdef TRINITY_DEBUG
    sLog->outDebug(LOG_FILTER_MAPS, "AreaTrigger (%s) attempted to move from grid[%u, %u]cell[%u, %u] to unloaded grid[%u, %u]cell[%u, %u].", at->GetGUID().ToString().c_str(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
#endif
    return false;
}
...
void Map::RemoveAllObjectsInRemoveList()
{
    ...
    {
        WorldObjectSet::iterator itr = i_objectsToRemove.begin();
        WorldObject* obj = *itr;
        if (!obj || obj->IsDelete())
        {
            i_objectsToRemove.erase_at(itr);
            continue;
        }
...
BattlegroundMap::BattlegroundMap(uint32 id, time_t expiry, uint32 InstanceId, Map* _parent, Difficulty difficulty) : Map(id, expiry, InstanceId, difficulty, _parent)
{
    m_bg = nullptr;
...
{
    if (summoner && entry != 114975 && entry != 44548)  // hack for brawguild
    {
                ...
                switch (properties->Title)
                {
                    ...
                    default:
                    {
                        if (properties->Flags & 512 ||
                            ...
                            mask = UNIT_MASK_GUARDIAN;
                        if (properties->ID == 3934 || properties->ID == 3234 || properties->ID == 3712 || properties->ID == 3456) // Totem Mastery, Transcendence, Call Dreadstalkers, Efflorescence
                            mask = UNIT_MASK_TOTEM;
                        break;
                    }
                }
...
{
    auto itr = m_StaticTransports.find(guid);
    if (itr != m_StaticTransports.end())
        return itr->second;
    return nullptr;
}
...
void Map::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry, Unit* source, Unit* unit)
{
    ...
    if (!encounters)
    {
        sLog->outDebug(LOG_FILTER_LFG, "UpdateEncounterState DungeonEncounterList not find");
        return;
    }
    ...
    // sLog->outDebug(LOG_FILTER_LFG, "UpdateEncounterState dungeonId %u fullEncounterIndex %u completedEncounters %u", dungeonId, fullEncounterIndex, completedEncounters);
...
void Map::UpdateLoop(volatile uint32 _mapID)
{
    ...
    // sLog->outError(LOG_FILTER_WORLDSERVER, "Map::UpdateLoop Run _mapID %u thread %u", _mapID, std::this_thread::get_id());
    ...
    while (!b_isMapStop)
    {
        if (m_worldCrashChecker) // Crashing detected, need stop map
        {
            ...
            UnloadAll();
            b_isMapStop = true;
            sLog->outError(LOG_FILTER_WORLDSERVER, "Map::UpdateLoop Crash _mapID %u thread %u", _mapID, std::this_thread::get_id());
            break;
        }
        ...
        {
            sLog->outDiff("Exception caught in Map::UpdateLoop %s _mapID %u", e.what(), _mapID);
            // throw; // If need call stack
        }
    }

    sLog->outError(LOG_FILTER_WORLDSERVER, "Map::UpdateLoop Stop _mapID %u thread %u", _mapID, std::this_thread::get_id());
