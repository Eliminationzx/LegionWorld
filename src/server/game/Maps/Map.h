...
class Map
{
    ...
    public:
        ...
        uint32 updateCollected(uint32 diff);
    ...
    public:
        ...
        {
            if (Instanceable())  return reinterpret_cast<MapInstanced*>(this);
            return nullptr;
        }
        const MapInstanced* ToMapInstanced() const
        {
            if (Instanceable())  return (const MapInstanced*)((MapInstanced*)this);
            return nullptr;
        }

        InstanceMap* ToInstanceMap()
        {
            if (IsDungeon())  return reinterpret_cast<InstanceMap*>(this);
            return nullptr;
        }
        const InstanceMap* ToInstanceMap() const
        {
            if (IsDungeon())  return (const InstanceMap*)((InstanceMap*)this);
            return nullptr;
        }
...
{
public:
    BattlegroundMap(uint32 id, time_t, uint32 InstanceId, Map* _parent, Difficulty difficulty);
    ~BattlegroundMap();
