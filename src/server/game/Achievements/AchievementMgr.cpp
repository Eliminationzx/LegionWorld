...
AchievementCache::AchievementCache(Player* _player, Unit* _target, CriteriaTypes _type, uint32 _miscValue1, uint32 _miscValue2, uint32 _miscValue3)
{
    ...
    if (_target)
    {
        ...
        _target->GetAuraSet(target.auraList);
...
bool AchievementCriteriaData::Meets(uint32 criteria_id, AchievementCachePtr cachePtr) const
{
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA:
            return cachePtr->HasAura(aura.spell_id);
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AREA:
            return area.id == cachePtr->ZoneID || area.id == cachePtr->AreaID;
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA:
            return cachePtr->HasTargetAura(aura.spell_id);
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE:
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT:
            return sScriptMgr->OnCriteriaCheck(this, const_cast<Player*>(cachePtr->player), const_cast<Unit*>(cachePtr->target.unit));
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT:
...
{
    // sLog->outDebug(LOG_FILTER_ACHIEVEMENTSYS, "UpdateAchievementCriteria type %u (%u, %u, %u) CriteriaSort %u", cachePtr->type, cachePtr->miscValue1, cachePtr->miscValue2, cachePtr->miscValue3, GetCriteriaSort());
    ...
    // sLog->outDebug(LOG_FILTER_ACHIEVEMENTSYS, "UpdateAchievementCriteria type %u criteriaList %u", cachePtr->type, criteriaList.size());
...
bool AchievementMgr<T>::SetCriteriaProgress(CriteriaTree const* tree, uint32 changeValue, Player* player, ProgressType ptype)
{
    ...
    if (criteria->Entry->StartTimer)
    {
        ...
        if (progress->completed)
        {
            ...
            RecursiveGuard guard(i_timeCriteriaTreesLock);
    ...
    if (GetCriteriaSort() == PLAYER_CRITERIA && player && timedCompleted && treeParent)
    {
        ...
        {
            uint32 criteriaID = progress->parent->ID;
...
void AchievementMgr<T>::CompletedAchievement(AchievementEntry const* achievement, Player* player)
{
    ...
    {
        RecursiveGuard guard(i_completedAchievementsLock);
...
{
    // sLog->outDebug(LOG_FILTER_ACHIEVEMENTSYS, "AchievementMgr::AdditionalRequirementsSatisfied start miscValue1 %u%u, miscValue2 %u%u miscValue3 %u%u ModifierTree %u", cachePtr->miscValue1, cachePtr->miscValue2, cachePtr->miscValue3, tree->Entry->ID);
    ...
    for (ModifierTreeNode const* node : tree->Children)
    {
        ...
        switch (CriteriaAdditionalCondition(reqType))
        {
            ...
            case CRITERIA_ADDITIONAL_CONDITION_SOURCE_HAS_AURA: // 8
                if (!cachePtr->HasAura(reqValue))
                    check = false;
                break;
            case CRITERIA_ADDITIONAL_CONDITION_TARGET_HAS_AURA: // 10
                if (!cachePtr->HasTarget() || !cachePtr->HasTargetAura(reqValue))
                    check = false;
                break;
            ...
            case CRITERIA_ADDITIONAL_CONDITION_TARGET_ZONE: // 41
                if (!cachePtr->HasTarget() || cachePtr->target.ZoneID != reqValue)
                    check = false;
                break;
            ...
            case CRITERIA_ADDITIONAL_CONDITION_BONUS_EVENT_WEEKEND: // 123
                check = false; // For use this criteria need realese bonus event
                break;
            ...
            case CRITERIA_ADDITIONAL_CONDITION_GARRISON_FOLLOWER_ID: // 157
            {
                ...
                GarrisonPtr garrison = referencePlayer->GetGarrisonPtr();
                if (!garrison)
                {
                    check = false;
                    break;
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu completed achievements in %u ms", static_cast<uint64>(m_allCompletedAchievements.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    do
    {
        ...
        else if (reward.castSpell)
        {
            ...
            if (!spellEntry)
            {
                sLog->outError(LOG_FILTER_SQL, "Table `achievement_reward` (Entry: %u) have not existent cast spell %i.", entry, reward.castSpell);
                continue;
            }
        }
        else
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu achievement reward locale strings in %u ms", static_cast<uint64>(m_achievementRewardLocales.size()), GetMSTimeDiffToNow(oldMSTime));
}
