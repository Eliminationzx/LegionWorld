...
public:
    ...
    bool HasAura(uint32 spellID) { return auraList.find(spellID) != auraList.end(); }
    bool HasTargetAura(uint32 spellID) { return target.auraList.find(spellID) != target.auraList.end(); }

    Player* player = nullptr;
    CriteriaTypes type = CRITERIA_TYPE_KILL_CREATURE;
    std::set<uint32> auraList;
