...
void AggressorAI::DoActionAI(uint32 diff, CreatureActionType type)
{
    if (type == CREATURE_ACTION_TYPE_SUMMON)
    {
        ...
        {
            for (std::vector<CreatureActionData>::const_iterator itr = me->m_actionData[CREATURE_ACTION_TYPE_SUMMON]->begin(); itr != me->m_actionData[CREATURE_ACTION_TYPE_SUMMON]->end(); ++itr)
                if (itr->spellId)
    ...
    if (m_checkTimer <= diff)
    {
        ...
        {
            for (std::vector<CreatureActionData>::const_iterator itr = me->m_actionData[CREATURE_ACTION_TYPE_ATTACK]->begin(); itr != me->m_actionData[CREATURE_ACTION_TYPE_ATTACK]->end(); ++itr)
            {
        ...
        {
            for (std::vector<CreatureActionData>::const_iterator itr = me->m_actionData[CREATURE_ACTION_TYPE_CAST]->begin(); itr != me->m_actionData[CREATURE_ACTION_TYPE_CAST]->end(); ++itr)
            {
                if (!itr->type)
                {
                    if (Creature* target = me->FindNearestCreature(itr->target, 11.0f))
...
void CombatAI::InitializeAI()
{
    for (uint32 i = 0; i < CREATURE_MAX_SPELLS; ++i)
        if (me->m_templateSpells[i] && sSpellMgr->GetSpellInfo(me->m_templateSpells[i]))
            spells.push_back(me->m_templateSpells[i]);
...
void AnyPetAI::UpdateAI(uint32 diff)
{
    ...
    if(!me->HasReactState(REACT_PASSIVE))
    {
        ...
        if(targetOwner != nullptr && targetOwner != target)
        {
            ...
            // sLog->outDebug(LOG_FILTER_PETS, "AnyPetAI::UpdateAI AttackStart X %f Y %f", me->m_positionX, me->m_positionY);
        }
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        // sLog->outDebug(LOG_FILTER_PETS, "AnyPetAI::UpdateAI GetPetCastSpellSize %i owner %u victim %u target %u", me->GetPetCastSpellSize(), bool(owner), bool(owner ? owner->getAttackerForHelper() : 0), bool(target));
