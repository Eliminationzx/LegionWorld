...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_PLAY_ANIMKIT:
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry " SI64FMTD " SourceType %u Event %u Action %u uses invalid AnimKit type %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.animKit.type);
                return false;
            }
            break;
        ...
        case SMART_ACTION_REMOVE_POWER:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry " SI64FMTD " SourceType %u Event %u Action %u uses non-existent Power %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.power.powerType);
                return false;
            }
            break;
        }
        ...
        case SMART_ACTION_SET_SPEED:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: SMART_ACTION_SET_SPEED have wrong speed type %u for creature " SI64FMTD ", skipped", e.action.setSpeed.type, e.entryOrGuid);
                return false;
            }
...
bool SmartAIMgr::IsAnimKitValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry " SI64FMTD " SourceType %u Event %u Action %u uses non-existent AnimKit entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
