...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit* /*invoker*/)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_GENERAL, "SmartAI::StartPath: Creature entry %u wanted to start waypoint movement while in combat, ignoring.", me->GetEntry());
        return;
    }
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_GENERAL, "SmartAI::StartPath: Creature entry %u wanted to pause waypoint movement while already paused, ignoring.", me->GetEntry());
        return;
    }
...
public:
    ...
    bool OnTrigger(Player* player, EventObject* event) override
    {
        if (!player->isAlive())
            return false;

        sLog->outDebug(LOG_FILTER_DATABASE_AI, "EventObject %u is using SmartEventObject script", event->GetEntry());
        SmartScript script;
