    ...
    switch (e.GetActionType())
    {
                ...
                if (IsUnit(*itr))
                    if (Vehicle* vehicle = (*itr)->ToUnit()->GetVehicleKit())
                        for (auto& Seat : vehicle->Seats)
                            if (Player* player = ObjectAccessor::FindPlayer(Seat.second.Passenger))
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    if (!(e.action.castCustom.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(spellId))
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_DATABASE_AI, "Spell %u not casted because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", spellId, (*itr)->GetGUID(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_CAST_CUSTOM:: Creature %u casts spell %u on target %u with castflags %u",
            ...
            else if (GetBaseObject())
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    if (IsUnit(*itr))
                        if (Vehicle* vehicle = (*itr)->ToUnit()->GetVehicleKit())
                            for (auto& Seat : vehicle->Seats)
                                if (Player* player2 = ObjectAccessor::FindPlayer(Seat.second.Passenger))
        ...
        case SMART_ACTION_SUMMON_CREATURE_IN_PERS_VISIBILITY:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                float o = e.target.o;
                if (Player* player = (*itr)->ToPlayer())
                    if (Creature* summon = player->SummonCreature(e.action.sumCreaturePV.creature, x, y, z, o, static_cast<TempSummonType>(e.action.sumCreaturePV.type), e.action.sumCreaturePV.duration))
                    {
        ...
        case SMART_ACTION_SUMMON_ARIATRIGGER:
        {
            ...
            sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_ARIATRIGGER: AreaTrigger %d", e.action.areatrigger.id, e.action.areatrigger.targetX, e.action.areatrigger.targetY, e.action.areatrigger.targetZ);
        ...
        case SMART_ACTION_SUMMON_SCENE:
        {
            ...
            sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_SCENE: scene %d duration %u", e.action.scene.id, e.action.scene.duration);
        ...
        case SMART_ACTION_PLAY_ANIMKIT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "SmartScript: Invalid type for SMART_ACTION_PLAY_ANIMKIT, skipping");
                        break;
                    }

                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_PLAY_ANIMKIT: target: %s (%s), AnimKit: %u, Type: %u",
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        {
            if (!me || (!me->isInCombat() && !(e.event.event_flags & SMART_EVENT_FLAG_ALLOW_EVENT_NON_COMBAT)) || !me->GetMaxHealth())
                return;
        ...
        {
            if (!me || (!me->isInCombat() && !(e.event.event_flags & SMART_EVENT_FLAG_ALLOW_EVENT_NON_COMBAT)) || !me->getVictim() || !me->getVictim()->GetMaxHealth())
                return;
        ...
        {
            if (!me || (!me->isInCombat() && !(e.event.event_flags & SMART_EVENT_FLAG_ALLOW_EVENT_NON_COMBAT)) || !me->GetMaxPower(me->getPowerType()))
                return;
        ...
        {
            if (!me || (!me->isInCombat() && !(e.event.event_flags & SMART_EVENT_FLAG_ALLOW_EVENT_NON_COMBAT)) || !me->getVictim() || !me->getVictim()->GetMaxPower(me->getPowerType()))
                return;
    ...
    if (obj)//handle object based scripts
    {
        switch (obj->GetTypeId())
        {
            ...
            case TYPEID_EVENTOBJECT:
                ...
                sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::OnInitialize: source is EventObject %u", event->GetEntry());
                break;
            default:
