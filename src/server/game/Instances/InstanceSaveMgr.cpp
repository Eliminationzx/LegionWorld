...
InstanceSaveManager::InstResetEvent::InstResetEvent(): type(0), difficulty(DIFFICULTY_NORMAL), mapid(0), instanceId(0)
{
}

InstanceSaveManager::InstResetEvent::InstResetEvent(uint8 t, uint32 _mapid, Difficulty d, uint16 _instanceid): type(t), difficulty(d), mapid(_mapid), instanceId(_instanceid)
{
}
...
{
    // sLog->outInfo(LOG_FILTER_SERVER_LOADING, "InstanceSaveManager::_ResetSave GetMapId %u GetDifficultyID %u GetResetTime %u", itr->second->GetMapId(), itr->second->GetDifficultyID(), itr->second->GetResetTime());
...
{
    // sLog->outDebug(LOG_FILTER_SERVER_LOADING, "InstanceSaveMgr::_ResetInstance mapid %u, instanceId %u", mapid, instanceId);
...
void InstanceSaveManager::ResetOrWarnAll(uint32 mapid, Difficulty difficulty, SQLTransaction& trans)
{
    ...
    for (InstanceSaveHashMap::iterator itr = m_instanceSaveById.begin(); itr != m_instanceSaveById.end();)
    {
        if (itr->second && itr->second->GetMapId() == mapid && itr->second->GetDifficultyID() == difficulty)
        {
            ...
            // sLog->outDebug(LOG_FILTER_SERVER_LOADING, "InstanceSaveMgr::ResetOrWarnAll mapid %u, difficulty %u time %u GetResetTime %u", mapid, difficulty, time(NULL), (itr->second->GetResetTime() + MONTH));
        }
        else
            ++itr;
    }
