...
public:
    CreatureTextBuilder(WorldObject const* obj, uint8 gender, ChatMsg msgtype, std::string baseText, uint32 BroadcastTextID, uint32 language, WorldObject const* target) :
        _source(obj), _target(target), _msgType(msgtype), _BroadcastTextID(BroadcastTextID), _language(language), _baseText(baseText), _gender(gender) { }

    WorldPackets::Chat::Chat* operator()(LocaleConstant locale) const
    {
        auto chat = new WorldPackets::Chat::Chat();

        std::string baseText = _baseText;
        if (BroadcastTextEntry const* bct = sBroadcastTextStore.LookupEntry(_BroadcastTextID))
...
{
public:
    PlayerTextBuilder(WorldObject const* obj, WorldObject const* speaker, uint8 gender, ChatMsg msgtype, std::string baseText, uint32 BroadcastTextID, uint32 language, WorldObject const* target) :
        _source(obj), _talker(speaker), _target(target), _msgType(msgtype), _BroadcastTextID(BroadcastTextID), _language(language), _gender(gender), _baseText(baseText) { }
...
void CreatureTextMgr::LoadCreatureTexts()
{
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "CreatureTextMgr: Entry %u, Group %u, Id %u in table `creature_text` has non-existing or incompatible SpellID %u.", temp.entry, temp.group, temp.id, temp.SpellID);
            temp.SpellID = 0;
        }
...
uint32 CreatureTextMgr::SendChat(Creature* source, uint8 textGroup, ObjectGuid whisperGuid /*= 0*/, ChatMsg msgType /*= CHAT_MSG_ADDON*/, Language language /*= LANG_ADDON*/, TextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/, Team team /*= TEAM_OTHER*/, bool gmOnly /*= false*/, Player* srcPlr /*= NULL*/, bool ingoreProbality /*= false*/)
{
    ...
    if (!textHolder)
    {
        sLog->outDebug(LOG_FILTER_SQL, "CreatureTextMgr: Could not find Text for Creature(%s) Entry %u in 'creature_text' table. Ignoring.", source->GetName(), source->GetEntry());
        return 0;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_SQL, "CreatureTextMgr: Could not find TextGroup %u for Creature(%s) GuidLow %u Entry %u. Ignoring.", uint32(textGroup), source->GetName(), source->GetGUIDLow(), source->GetEntry());
        return 0;
    }
...
void CreatureTextMgr::SendText(Creature* source, CreatureTextEntry const* text, ObjectGuid whisperGuid /*= 0*/, ChatMsg msgType /*= CHAT_MSG_ADDON*/, Language language /*= LANG_ADDON*/, TextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/, Team team /*= TEAM_OTHER*/, bool gmOnly /*= false*/, Player* srcPlr /*= NULL*/)
{
    ...
    if (srcPlr)
    {
        PlayerTextBuilder builder(source, finalSource, finalSource->getGender(), finalType, text->text, text->BroadcastTextID, finalLang, whisperTarget);
...
bool CreatureTextMgr::IsDuplicateText(uint32 entry, std::string text)
{
    if (!entry || mTextList.size() <= entry)
        return false;
