...
bool OPvPCapturePoint::HandlePlayerEnter(Player* player)
{
    if (m_capturePoint)
    {
        sLog->outTrace(LOG_FILTER_GENERAL, "OPvPCapturePoint::HandlePlayerEnter!");
        ...
        sLog->outTrace(LOG_FILTER_GENERAL, "OPvPCapturePoint::HandlePlayerEnter !!!!!!m_capturePoint!");
...
{
    sLog->outTrace(LOG_FILTER_GENERAL, "Creating capture point %u", entry);
    GameObjectTemplate const* goinfo = sObjectMgr->GetGameObjectTemplate(entry);
    if (!goinfo || goinfo->type != GAMEOBJECT_TYPE_CONTROL_ZONE)
    {
        sLog->outTrace(LOG_FILTER_OUTDOORPVP, "OutdoorPvP: GO %u is not capture point!", entry);
        return false;
    }
    ...
    {
        sLog->outTrace(LOG_FILTER_GENERAL, "OPvPCapturePoint::SetCapturePointData  - NOT CREATEDCreating capture point %u", entry);
        return false;
    }

    sLog->outTrace(LOG_FILTER_GENERAL, "OPvPCapturePoint::SetCapturePointData::Creating capture point entry %u, map%u, x %f, y %f, z %f m_capturePointGUID %s", entry, map, x, y, z, m_capturePointGUID.ToString().c_str());
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    if (!m_Creatures[type])
    {
        sLog->outTrace(LOG_FILTER_GENERAL, "opvp creature type %u was already deleted", type);
        return false;
    }
    ...
    sLog->outTrace(LOG_FILTER_GENERAL, "deleting opvp creature type %u", type);
...
void OutdoorPvP::HandlePlayerLeaveZone(ObjectGuid guid, uint32 /*zone*/)
{
    ...
    sLog->outTrace(LOG_FILTER_GENERAL, "Player %s left an outdoorpvp zone", player->GetName());
}
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    {
        //sLog->outTrace(LOG_FILTER_OUTDOORPVP, "%u->%u", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        ChangeState();
        return true;
...
void OutdoorPvP::AddCapturePoint(OPvPCapturePoint* cp)
{
    ...
    if (auto go = sObjectAccessor->FindGameObject(cp->m_capturePointGUID))
        ...
        sLog->outTrace(LOG_FILTER_GENERAL, "OutdoorPvP::AddCapturePoint !!!!!!m_capturePoint! %s", cp->m_capturePointGUID.ToString().c_str());
}

OPvPCapturePoint* OutdoorPvP::GetCapturePoint(ObjectGuid guid) const
{
    auto itr = m_capturePoints.find(guid);
...
bool OutdoorPvP::DelCreature(uint32 type)
{
    if (!m_Creatures[type])
    {
        sLog->outTrace(LOG_FILTER_GENERAL, "OutdoorPvP::DelCreature, creature type %u was already deleted", type);
        return false;
    }
    ...
    sLog->outTrace(LOG_FILTER_GENERAL, "OutdoorPvP::DelCreature, deleting creature type %u", type);
...
OutdoorGraveyard* OutdoorPvP::GetGraveyardById(uint32 id)
{
    if (id < m_GraveyardList.size())
    {
        ...
        sLog->outTrace(LOG_FILTER_BATTLEFIELD, "OutdoorPvP::GetGraveyardById Id:%u not existed", id);
    }
    else
        sLog->outTrace(LOG_FILTER_BATTLEFIELD, "OutdoorPvP::GetGraveyardById Id:%u cant be found", id);
...
void OutdoorGraveyard::SetSpirit(Creature* p_Spirit, TeamId team)
{
    if (p_Spirit == nullptr)
    {
        sLog->outTrace(LOG_FILTER_BATTLEFIELD, "OutdoorGraveyard::SetSpirit -> Invalid Spirit.");
        return;
    }
