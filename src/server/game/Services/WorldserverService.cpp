...
uint32 Battlenet::GameUtilitiesService::HandleProcessClientRequest(game_utilities::v1::ClientRequest const* request, game_utilities::v1::ClientResponse* response)
{
    ...
    if (!command)
    {
        sLog->outError(LOG_FILTER_BATTLENET, "%s sent ClientRequest with no command.", GetCallerInfo().c_str());
    ...
    sLog->outError(LOG_FILTER_BATTLENET, "%s sent ClientRequest with ERROR_RPC_NOT_IMPLEMENTED", GetCallerInfo().c_str());
