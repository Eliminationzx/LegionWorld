...
{
    if (LootItem const* lootItemInSlot = const_cast<Roll*>(this)->getLoot()->GetItemInSlot(itemSlot))
    {
...
Group::~Group()
{
    if (m_bgGroup)
    {
        ...
        if (m_bgGroup->GetBgRaid(ALLIANCE) == this)
            ...
            sLog->outError(LOG_FILTER_GENERAL, "Group::~Group: battleground group is not linked to the correct battleground.");
    }
...
bool Group::Create(Player* leader, uint8 subType /*= 0*/)
{
...
void Group::ChangeLeader(ObjectGuid const& guid, int8 partyIndex /*= 0*/)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        RecursiveGuard _lock(m_bound_lock);
    ...
    for (auto itr = m_boundInstances[boundType].begin(); itr != m_boundInstances[boundType].end();)
    {
        ...
        if (map && map->IsDungeon() && !(method == INSTANCE_RESET_GROUP_DISBAND && !instanceSave->CanReset()))
        {
            if (instanceSave->CanReset())
                isEmpty = static_cast<InstanceMap*>(map)->Reset(method);
            else
...
{
    RecursiveGuard _lock(const_cast<Group*>(this)->m_lock);
