...
uint32 MapDifficultyEntry::GetRaidDuration() const
{
    ...
    if (MapID == 1676) // Hack for custom duration time instance CD
        return 3;
...
#define UNPACK_INT32(x) { x = *((int32*)((char const*)*unpackedExpression)); *unpackedExpression += sizeof(int32);}
    ...
    *_worldStateID = 0;
    ...
    if (opType == WorldStateExpressionCustomOpType::PushWorldStateValue)
    {
        ...
        // sLog->outDebug(LOG_FILTER_WORLD_QUEST, "INSERT INTO `world_state_expression` (`WorldStateExpressionID`, `WorldStateID`) VALUES (%u, %u);", ID, l_WorldStateID); // Need for convert WorldStateExpressionID -> WorldStateID
...
bool WorldStateExpression_EvalCompare(Player const* player, char const** unpackedExpression, uint32 ID, int32* _worldStateID, WorldQuest* wq)
{
#define UNPACK_UINT8(x) { x = *(uint8*)(*unpackedExpression); *unpackedExpression += sizeof(uint8);}
    ...
    {
        if (*_worldStateID && wq)
            wq->State.insert(std::make_pair(*_worldStateID, leftValue > 0 ? leftValue : 1));

        // sLog->outDebug(LOG_FILTER_WORLD_QUEST, "INSERT INTO `world_state_expression` (`WorldStateExpressionID`, `WorldStateID`, `Value0`) VALUES (%u, %u, %i);", ID, *_worldStateID, leftValue); // Need for convert WorldStateExpressionID -> WorldStateID
        return !!leftValue;
    }
    ...
    // sLog->outDebug(LOG_FILTER_WORLD_QUEST, "INSERT INTO `world_state_expression` (`WorldStateExpressionID`, `WorldStateID`, `Value0`, `Value1`) VALUES (%u, %u, %i, %i);", ID, *_worldStateID, rValue, opperand); // Need for convert WorldStateExpressionID -> WorldStateID
    ...
    switch (opperand)
    {
        ...
        {
            if (*_worldStateID && wq)
                wq->State.insert(std::make_pair(*_worldStateID, rValue > 0 ? rValue : 1));
...
bool WorldStateExpressionEntry::Eval(Player const* player, WorldQuest* wq) const
{
#define UNPACK_UINT8(x) { x = *unpackedExpression; unpackedExpression += sizeof(uint8);}
    ...
    if (Expression && strlen(Expression))
    {
        ...
        if (value == 1)
        {
            ...
            auto result = WorldStateExpression_EvalCompare(player, &unpackedExpression, ID, &_worldStateID, wq);
            char resultLogic;
            UNPACK_UINT8(resultLogic);

            if (resultLogic)
            {
                auto routine2 = WorldStateExpression_EvalCompare(player, &unpackedExpression, ID, &_worldStateID, wq);
                result = WorldStateExpression_EvalResult(resultLogic, 2, routine2, 1, result);
                UNPACK_UINT8(resultLogic);

                if (resultLogic)
                {
                    auto routine3 = WorldStateExpression_EvalCompare(player, &unpackedExpression, ID, &_worldStateID, wq);
