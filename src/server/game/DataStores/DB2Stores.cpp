...
void LoadDB2(uint32& availableDb2Locales, DB2StoreProblemList& errlist, StorageMap& stores, DB2StorageBase* storage, std::string const& db2Path, uint32 defaultLocale, DB2<T> const& /*hint*/)
{
    auto loadInfo = storage->GetLoadInfo();
    {
        ...
        if (loadInfo->Meta->GetRecordSize() != sizeof(T))
        {
            stream << "Size of " << storage->GetFileName().c_str() << " set by format string (" << loadInfo->Meta->GetRecordSize() << ") not equal size of C++ structure (" << sizeof(T) << ").";
            errlist.push_back(stream.str());
            return;
        }
    }
    ...
    else
    {
        if (auto f = fopen((db2Path + localeNames[defaultLocale] + '/' + storage->GetFileName()).c_str(), "rb"))
        {
            ...
            errlist.push_back(stream.str());
            fclose(f);
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "%s", stream.str().c_str());
        }
        else
...
#undef LOAD_DB2
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Initialized " SZFMTD " DB2 data stores in %u ms", _stores.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void DB2Manager::InitDB2CustomStores()
{
    ...
    for (MapChallengeModeEntry const* entry : sMapChallengeModeStore)
    {
        ...
        if ((entry->Flags == 4 || entry->Flags == 0) && entry->ID != 239) // Enable this after open argus
    ...
    for (ResearchSiteEntry const* rs : sResearchSiteStore)
    {
        ...
        if (data.points.empty())
            sLog->outDebug(LOG_FILTER_SERVER_LOADING, "Research siteID %u QuestPoiBlobId %u MapID %u has 0 points points in DB2!", rs->ID, rs->QuestPoiBlobID, rs->MapID);
    }
...
void DB2Manager::LoadingExtraHotfixData()
{
    ...
    DB2HotfixGenerator<ParagonReputationEntry> paragonRepHotfixes(sParagonReputationStore);

    for (auto const& itr : sParagonReputationStore) // loop for changing data in all rows
    {
        paragonRepHotfixes.ApplyHotfix(itr->ID, [](ParagonReputationEntry* entry)
        {
            entry->LevelThreshold *= sWorld->getRate(RATE_REPUTATION_GAIN);
        }, true);
    }
    ...
    {
        if (itr->ItemNameDescriptionID != 13297 && itr->ItemNameDescriptionID != 13298/* && itr->ItemNameDescriptionID != 13299 && itr->ItemNameDescriptionID != 13300*/) // Legion Season 4 and 5
            continue;
...
void DB2Manager::LoadHotfixData()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 hotfix info entries.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outInfo(LOG_FILTER_SQL, "Table `hotfix_data` references unknown DB2 store by hash 0x%X in hotfix id %d", tableHash, id);
            continue;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u hotfix records in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
