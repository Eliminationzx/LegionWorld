...
uint32 LoadGameTable(StringVector& errors, GameTable<T>& storage, boost::filesystem::path const& path)
{
    ...
    if (columnDefs.size() != sizeof(T) / sizeof(float))
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "GameTable '%s' has different count of columns " SZFMTD " than expected by size of C++ structure (" SZFMTD ").", path.string().c_str(), columnDefs.size(), sizeof(T) / sizeof(float));
...
#undef LOAD_GT
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Initialized %d GameTables in %u ms", gameTableCount, GetMSTimeDiffToNow(oldMSTime));
}
