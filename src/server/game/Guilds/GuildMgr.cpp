...
void GuildMgr::LoadGuilds()
{
    ...
    sLog->outInfo(LOG_FILTER_GENERAL, "Loading guild challenges...");
    {
        ...
        if (auto result = CharacterDatabase.Query(CharacterDatabase.GetPreparedStatement(CHAR_LOAD_GUILD_CHALLENGES)))
        {
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u guild challenges in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
}
...
void GuildMgr::LoadGuildChallengeRewardInfo()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "%s >> Loaded 0 guild challenge reward data.", __FUNCTION__);
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "%s >> guild_challenge_reward has unknown challenge type %u, skip.", __FUNCTION__, type);
            continue;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "%s >> Loaded %u guild challenge reward data in %u ms.", __FUNCTION__, count, GetMSTimeDiffToNow(oldMSTime));
}
