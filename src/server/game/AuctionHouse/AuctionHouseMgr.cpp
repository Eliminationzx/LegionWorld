...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, SQLTransaction& trans)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
    {
        ...
        if (!AccountMgr::IsPlayerAccount(bidder_security))
        {
            ...
            sLog->outCommand(bidder_accId, "GM %s (Account: %u) won item in auction: %s (Entry: %u Count: %u) and pay money: %u. Original owner %s (Account: %u)",
                bidder_name.c_str(), bidder_accId, pItem->GetTemplate()->Name->Str[bidder ? bidder->GetSession()->GetSessionDbLocaleIndex() : DEFAULT_LOCALE], pItem->GetEntry(), pItem->GetCount(), auction->bid, owner_name.c_str(), owner_accid);
        }
    }
    ...
    {
        ObjectGuid ownerGuid = ObjectGuid::Create<HighGuid::Player>(auction->owner);
        sLog->outDebug(LOG_FILTER_GOLD, "AuctionHouse: GUID %u won item in auction: %s (Entry: %u Count: %u) and pay money: " UI64FMTD ". Original ownerGuid %u",
            bidderGuid.GetGUIDLow(), pItem->GetTemplate()->Name->Str[bidder ? bidder->GetSession()->GetSessionDbLocaleIndex() : DEFAULT_LOCALE], pItem->GetEntry(), pItem->GetCount(), auction->bid, ownerGuid.GetGUIDLow());
    }
...
void AuctionHouseMgr::SendAuctionExpiredMail(AuctionEntry* auction, SQLTransaction& trans)
{
    ...
    if (!pItem)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "SendAuctionExpiredMail %u has not a existing item : %u", auction->Id, auction->itemGUIDLow);
        return;
    }
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    do
    {
        ...
        if (!proto)
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: " UI64FMTD " id: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
        ...
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "AuctionHouseMgr::LoadAuctionItems: Error item load (GUID: " UI64FMTD " id: #%u) in auction, skipped.", item_guid, itemEntry);
            delete item;
            continue;
        }
        AddAItem(item);
...
void AuctionHouseObject::BuildListAuctionItems(WorldPackets::AuctionHouse::AuctionListItemsResult& packet, Player* player, std::wstring const& searchedname, uint32 listfrom, uint8 levelmin, uint8 levelmax, bool usable, Optional<AuctionSearchFilters> const& filters, uint32 quality)
{
    ...
    for (AuctionEntryMap::const_iterator itr = AuctionsMap.begin(); itr != AuctionsMap.end(); ++itr)
    {
        ...
        {
            std::string name = proto->Name->Str[localeConstant];
            if (name.empty())
                continue;
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerInfo)
        sLog->outError(LOG_FILTER_SERVER_LOADING, "Auction %u has not a existing auctioneer Entry: %u", Id, auctioneer);
    else
    ...
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "Auction %u has auctioneer Entry: %u with wrong faction %u", Id, auctioneer, factionTemplateId);
        return false;
    }
    ...
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
...
bool AuctionEntry::LoadFromFieldList(Field* fields)
{
    ...
    CreatureTemplate const* auctioneerInfo = sObjectMgr->GetCreatureTemplate(auctioneer);
    if (!auctioneerInfo)
        sLog->outError(LOG_FILTER_SERVER_LOADING, "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer Entry: %u", Id, auctioneer);
    else
        factionTemplateId = auctioneerInfo->faction;
    ...
    if (!auctionHouseEntry)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "AuctionEntry::LoadFromFieldList() - Auction %u has auctioneer Entry: %u with wrong faction %u", Id, auctioneer, factionTemplateId);
        return false;
    }
