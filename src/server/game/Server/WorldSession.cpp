...
m_muteTime(mute_time), m_timeOutTime(0), _player(nullptr), m_map(nullptr), _security(sec), _accountId(id), _battlenetAccountId(battlenetAccountId), m_expansion(expansion), m_accountExpansion(expansion), _logoutTime(0), m_inQueue(false), m_playerLogout(false), m_playerRecentlyLogout(false), m_playerSave(false),
m_sessionDbLocaleIndex(locale), m_latency(0), _tutorialsChanged(false), recruiterId(recruiter), isRecruiter(isARecruiter), timeCharEnumOpcode(0), playerLoginCounter(0), forceExit(false), m_sUpdate(false), wardenModuleFailed(false), atAuthFlag(flag), canLogout(false), _countPenaltiesHwid(0)
{
    _os = std::move(os);
...
void WorldSession::SendPacket(WorldPacket const* packet, bool forced /*= false*/)
{
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "Prevented sending of wrong opcode to %s", GetPlayerName(false).c_str());
        return;
    }
    ...
    if (!handler)
    {
        sLog->outError(LOG_FILTER_GENERAL, "Prevented sending of opcode %u with non existing handler to %s", opcode, GetPlayerName().c_str());
        return;
    }
    ...
    if (packet->GetConnection() != CONNECTION_TYPE_DEFAULT)
    {
        ...
        {
            sLog->outError(LOG_FILTER_GENERAL, "Prevented sending of instance only opcode %u with connection type %u to %s", opcode, packet->GetConnection(), GetPlayerName().c_str());
            return;
        }
    ...
    if (!m_Socket[conIdx])
    {
        sLog->outDebug(LOG_FILTER_GENERAL, "Prevented sending of %s to non existent socket %u to %s", GetOpcodeNameForLogging(static_cast<OpcodeServer>(opcode)).c_str(), conIdx, GetPlayerName().c_str());
        return;
    }

    if (!forced && handler->Status == STATUS_UNHANDLED)
    {
        sLog->outError(LOG_FILTER_GENERAL, "Prevented sending disabled opcode %s to %s", GetOpcodeNameForLogging(static_cast<OpcodeServer>(opcode)).c_str(), GetPlayerName().c_str());
        return;
    }
    ...
    while (m_Socket[CONNECTION_TYPE_REALM] && map == m_map && _recvQueue.next(packet))
    {
        ...
        try
        {
            switch (opHandle->Status)
            {
                case STATUS_LOGGEDIN:
                    if (!_player)
                    {
                        ...
                        if (!m_playerRecentlyLogout)
                        {
                            ...
                            deletePacket = false;
                            #ifdef WIN32
                            sLog->outError(LOG_FILTER_NETWORKIO, "Re-enqueueing packet with opcode %s with with status STATUS_LOGGEDIN. Player is currently not in world yet.", GetOpcodeNameForLogging(opcode).c_str());
                            #endif
                        }
                    }
                ...
                case STATUS_NEVER:
                    #ifdef WIN32
                    sLog->outError(LOG_FILTER_OPCODES, "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(opcode).c_str(), GetPlayerName(false).c_str());
                    #endif
                    break;
                case STATUS_UNHANDLED:
                    #ifdef WIN32
                    sLog->outError(LOG_FILTER_OPCODES, "Received not handled opcode %s from %s", GetOpcodeNameForLogging(opcode).c_str(), GetPlayerName(false).c_str());
                    #endif
                    break;
            }
        }
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        else if (!_player->getAttackers()->empty() && _player->isInCombat())
        {
            ...
            for (auto itr : *_player->getAttackers())
            {
                ...
                if (owner && owner->IsPlayer())
                    ...
                    aset.insert(dynamic_cast<Player*>(itr));
        ...
        if (Battleground* bg = _player->GetBattleground())
            if (bg->IsArena() && (bg->GetStatus() == STATUS_IN_PROGRESS || bg->GetStatus() == STATUS_WAIT_JOIN) && bg->GetJoinType() != MS::Battlegrounds::JoinType::Arena1v1)
        ...
        for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
        {
            if (uint8 bgQueueTypeId = _player->GetBattlegroundQueueTypeId(i))
            {
                if (MS::Battlegrounds::CheckIsBattlegroundTypeByBgTypeID(bgQueueTypeId) || MS::Battlegrounds::CheckIsArenaTypeByBgTypeID(bgQueueTypeId))
                    if (_player->IsInvitedForBattlegroundQueueType(bgQueueTypeId) && !_player->HasAura(158263))
                        player->AddDelayedEvent(100, [player]() -> void { player->HandleArenaDeserter(); });
...
{
    sLog->outInfo(LOG_FILTER_OPCODES, "WorldSession::SendConnectToInstance");
