...
void WorldSocket::Start()
{
    ...
    if (result)
    {
        sLog->outInfo(LOG_FILTER_NETWORKIO, "WorldSocket::CheckIpCallback: Sent Auth Response (IP %s banned).", GetRemoteIpAddress().to_string().c_str());
        DelayedCloseSocket();
        return;
    }
...
{
    sLog->outTrace(LOG_FILTER_NETWORKIO, "WorldSocket::InitializeHandler: called");

    if (error)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::InitializeHandler: closed with error %s (address: %s)", error.message().c_str(), GetRemoteIpAddress().to_string().c_str());
        CloseSocket();
        return;
    }
    ...
    if (packet.GetActiveSize() > 0)
    {
        if (_packetBuffer.GetRemainingSpace() > 0)
        {
            ...
            if (_packetBuffer.GetRemainingSpace() > 0)
            {
                sLog->outTrace(LOG_FILTER_NETWORKIO, "WorldSocket::InitializeHandler: Couldn't receive the whole header this time (address: %s)", GetRemoteIpAddress().to_string().c_str());
            ...
            {
                sLog->outTrace(LOG_FILTER_NETWORKIO, "WorldSocket::InitializeHandler: initializer (address: %s)", GetRemoteIpAddress().to_string().c_str());
                CloseSocket();
                return;
            }
            ...
            {
                sLog->outTrace(LOG_FILTER_NETWORKIO, "WorldSocket::InitializeHandler: terminator (address: %s)", GetRemoteIpAddress().to_string().c_str());
                CloseSocket();
                return;
            }
...
bool WorldSocket::Update()
{
    ...
    {
        auto* queued = _aBufferQueue.front();
        _aBufferQueue.pop();
        ...
        if (buffer.GetRemainingSpace() < packetSize + SizeOfHeader)
        {
            ...
            buffer.Resize(4096); //@TODO ﻿'buffer' used after it was moved[bugprone - use - after - move]
        }
...
bool WorldSocket::ReadHeaderHandler()
{
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::ReadHeaderHandler(): client %s sent malformed packet (size: %u, cmd: %u)", GetRemoteIpAddress().to_string().c_str(), header->Size - 2, header->Command);
        return false;
    }
...
WorldSocket::ReadDataHandlerResult WorldSocket::ReadDataHandler()
{
    ...
    switch (opcode)
    {
        case CMSG_PING:
        {
            ...
            if (!ping.ReadNoThrow())
            {
                sLog->outError(LOG_FILTER_OPCODES, "WorldSocket::ReadDataHandler(): client %s sent malformed CMSG_PING", GetRemoteIpAddress().to_string().c_str());
        ...
        case CMSG_AUTH_SESSION:
        {
            ...
            {
                sLog->outError(LOG_FILTER_OPCODES, "WorldSocket::ReadDataHandler(): client %s sent malformed CMSG_AUTH_SESSION", GetRemoteIpAddress().to_string().c_str());
                return ReadDataHandlerResult::Error;
            }
        ...
        case CMSG_AUTH_CONTINUED_SESSION:
        {
            ...
            {
                if (sessionGuard.try_lock())
                    sLog->outError(LOG_FILTER_OPCODES, "WorldSocket::ProcessIncoming: received duplicate CMSG_AUTH_CONTINUED_SESSION from %s", _worldSession->GetPlayerName().c_str());
                return ReadDataHandlerResult::Error;
            }
            ...
            if (!authSession->ReadNoThrow())
            {
                sLog->outError(LOG_FILTER_OPCODES, "WorldSocket::ReadDataHandler(): client %s sent malformed CMSG_AUTH_CONTINUED_SESSION", GetRemoteIpAddress().to_string().c_str());
                return ReadDataHandlerResult::Error;
            }
        ...
        case CMSG_LOG_DISCONNECT:
        {
            ...
            if (_worldSession)
            {
                sLog->outInfo(LOG_FILTER_NETWORKIO, "WorldPackets::Auth::LogDisconnect: %s was disconnected due to reason %u", _worldSession->GetPlayerName().c_str(), log->Reason);
        ...
        case CMSG_CONNECT_TO_FAILED:
        {
            ...
            {
                sLog->outError(LOG_FILTER_OPCODES, "WorldSocket::ReadDataHandler(): client %s sent malformed CMSG_CONNECT_TO_FAILED", GetRemoteIpAddress().to_string().c_str());
                return ReadDataHandlerResult::Error;
            }
        ...
        case CMSG_WARDEN_DATA:
        {
            ...
            {
                sLog->outError(LOG_FILTER_OPCODES, "WorldSocket::ReadDataHandler(): client %s sent malformed CMSG_WARDEN_DATA", GetRemoteIpAddress().to_string().c_str());
                return ReadDataHandlerResult::Error;
            }
        ...
        default:
        {
            ...
            {
                #ifdef WIN32
                sLog->outError(LOG_FILTER_OPCODES, "No defined handler for opcode %s sent by %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet.GetOpcode())).c_str(), _worldSession->GetPlayerName().c_str());
                #endif
                break;
            }
...
void WorldSocket::LogOpcodeText(OpcodeClient opcode, std::unique_lock<std::mutex> const& guard) const
{
    #ifdef WIN32
    if (!guard)
        sLog->outInfo(LOG_FILTER_OPCODES, "C->S: %s %s conn %i", GetOpcodeNameForLogging(opcode).c_str(), GetRemoteIpAddress().to_string().c_str(), GetConnectionType());
    else
        sLog->outInfo(LOG_FILTER_OPCODES, "C->S: %s %s conn %i", GetOpcodeNameForLogging(opcode).c_str(), (_worldSession ? _worldSession->GetPlayerName() : GetRemoteIpAddress().to_string()).c_str(), GetConnectionType());
    #endif
}
...
void WorldSocket::SendPacket(WorldPacket const& packet)
{
    ...
    if (SMSG_ON_MONSTER_MOVE != static_cast<OpcodeServer>(packet.GetOpcode()))
        sLog->outInfo(LOG_FILTER_OPCODES, "S->C: %s Size %u %s connection %i, connectionType %i", GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet.GetOpcode())).c_str(), packetSize, GetRemoteIpAddress().to_string().c_str(), packet.GetConnection(), GetConnectionType());
    #endif

    auto* queued = new EncryptablePacket(packet, _authCrypt.IsInitialized());
    _bufferQueueLock.lock();
    _bufferQueue.push(queued);
    _bufferQueueLock.unlock();
}
...
uint32 WorldSocket::CompressPacket(uint8* buffer, WorldPacket const& packet)
{
    ...
    if (z_res != Z_OK)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Can't compress packet opcode (zlib: deflate) Error code: %i (%s, msg: %s)", z_res, zError(z_res), _compressionStream->msg);
        return 0;
    }
    ...
    if (z_res != Z_OK)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Can't compress packet data (zlib: deflate) Error code: %i (%s, msg: %s)", z_res, zError(z_res), _compressionStream->msg);
        return 0;
    }
...
{
    // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession");
...
{
    // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSessionCallback");
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Authentication failed for account: %u ('%s') address: %s", account.Game.Id, authSession->RealmJoinTicket.c_str(), address.c_str());
        DelayedCloseSocket();
        return;
    }
    ...
    // {
        ...
        // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s requested connecting with realm id %u but this realm has id %u set in config.",
    ...
    if (wardenActive && account.Game.OS != "Win" && account.Game.OS != "Wn64" && account.Game.OS != "Mc64")
    {
        ...
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s attempted to log in using invalid client OS (%s).", address.c_str(), account.Game.OS.c_str());
        DelayedCloseSocket();
        return;
    }
    ...
    if (account.BattleNet.IsLockedToIP)
    {
        if (account.BattleNet.LastIP != address)
        {
            ...
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s attempted to log in using IsLockedToIP.", address.c_str());
            DelayedCloseSocket();
            return;
        }
    }
    else if (!account.BattleNet.LockCountry.empty() && account.BattleNet.LockCountry != "00" && !_ipCountry.empty())
    {
        ...
        {
            SendAuthResponseError(ERROR_RISK_ACCOUNT_LOCKED);
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s LockCountry != _ipCountry", address.c_str());
            DelayedCloseSocket();
            return;
        }
    }
    ...
    if (account.IsBanned())
    {
        ...
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s IsBanned()", address.c_str());
        DelayedCloseSocket();
        return;
    }
    ...
    if (allowedAccountType > SEC_PLAYER && account.Game.Security < allowedAccountType)
    {
        ...
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s allowedAccountType %u Security %u", address.c_str(), allowedAccountType, account.Game.Security);
        DelayedCloseSocket();
        return;
    }
    ...
    {
        if (QueryResult result = LoginDatabase.PQuery("SELECT penalties, last_reason from hwid_penalties where hwid = " UI64FMTD, _worldSession->_hwid))
        {
            Field* fields = result->Fetch();
                ...
                || _worldSession->_countPenaltiesHwid < 0)
            {
                ...
                {
                    sWorld->BanAccount(BAN_ACCOUNT, _worldSession->GetAccountName(), "-1", ss.str().c_str(), "Server");
                    SendAuthResponseError(ERROR_GAME_ACCOUNT_BANNED);
                    DelayedCloseSocket();
                    return;
                }
                ...
                {
                    sWorld->MuteAccount(_worldSession->GetAccountId(), -1, ss.str().c_str(), "Server", _worldSession.get());
                }
    ...
    if (wardenActive)
    {
        ...
        {
            SendAuthResponseError(ERROR_DENIED);
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Warden initializtion failed for account: %u ('%s') address: %s", account.Game.Id, authSession->RealmJoinTicket.c_str(), address.c_str());
            DelayedCloseSocket();
            return;
        }
    }
...
{
    // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthContinuedSession");
    ...
    {
        SendAuthResponseError(ERROR_DENIED);
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthContinuedSession: Client %s !_type != CONNECTION_TYPE_INSTANCE", GetRemoteIpAddress().to_string().c_str());
        DelayedCloseSocket();
        return;
    }
...
{
    // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthContinuedSessionCallback _worldSession %u", bool(_worldSession));

    if (!result)
    {
        SendAuthResponseError(ERROR_DENIED);
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthContinuedSessionCallback: Client %s !result", GetRemoteIpAddress().to_string().c_str());
        DelayedCloseSocket();
        return;
    }
    ...
    if (memcmp(hmac.GetDigest(), authSession->Digest.data(), authSession->Digest.size()) != 0)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthContinuedSession: Authentication failed for account: %u ('%s') address: %s",
...
{
    // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleConnectToFailed");
    ...
    if (_worldSession)
    {
        if (_worldSession->PlayerLoading())
        {
            switch (connectToFailed.Serial)
            {
                ...
                {
                    sLog->outTrace(LOG_FILTER_NETWORKIO, "%s failed to connect 5 times to world socket, aborting login", _worldSession->GetPlayerName().c_str());
...
{
    // sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleEnableEncryptionAck _worldSession %u _type %u", bool(_worldSession), _type);
...
bool WorldSocket::HandlePing(WorldPackets::Auth::Ping& ping)
{
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandlePing: peer sent CMSG_PING, but is not authenticated or got recently kicked, address = %s", GetRemoteIpAddress().to_string().c_str());
        return false;
    }
...
bool WorldSocket::HandleWardenData(WorldPackets::Auth::WardenData& packet)
{
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleWardenData: peer sent CMSG_WARDEN_DATA, but packet length is not valid, address = %s", GetRemoteIpAddress().to_string().c_str());
        return false;
    }
    ...
    if (!_worldSession)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleWardenData: peer sent CMSG_WARDEN_DATA, but is not authenticated or got recently kicked, address = %s", GetRemoteIpAddress().to_string().c_str());
        return false;
    }
    ...
    if (!_warden)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleWardenData: peer sent CMSG_WARDEN_DATA, but server is not prepared for handle it, address = %s", GetRemoteIpAddress().to_string().c_str());
        return false;
    }
    ...
    uint8 opcode;
    packet.Data >> opcode;
    sLog->outDebug(LOG_FILTER_WARDEN, "WARDEN: CMSG opcode %02X, size %u", opcode, uint32(packet.Data.size()));
