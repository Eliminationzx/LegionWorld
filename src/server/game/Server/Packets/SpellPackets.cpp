...
bool WorldPackets::Spells::SandboxScalingData::GenerateDataFromUnits(Unit* attacker, Unit* target)
{
    ...
    //if (attacker->ToCreature() && target->IsPlayer())
    //    GenerateDataForNpcToPlayer(attacker->ToCreature(), target->ToPlayer());
    //else if (attacker->IsPlayer() && target->ToCreature())
    //    GenerateDataForPlayerToNpc(attacker->ToPlayer(), target->ToCreature());
    ////else if (attacker->IsPlayer() && target->IsPlayer())
    ////    GenerateDataForPlayerToPlayer(attacker->ToPlayer(), target->ToPlayer());
    //else if (attacker->ToCreature() && target->ToCreature())
...
void WorldPackets::Spells::SandboxScalingData::GenerateDataForNpcToPlayer(Creature* attacker, Player* target)
{
    ...
    Type = CREATURE_TO_PLAYER_DAMAGE;
    PlayerItemLevel = target->GetAverageItemLevelEquipped();
    TargetLevel = target->GetEffectiveLevel();
...
void WorldPackets::Spells::SandboxScalingData::GenerateDataForPlayerToNpc(Player* attacker, Creature* target)
{
    ...
    Type = PLAYER_TO_CREATURE_DAMAGE;
...
void WorldPackets::Spells::SandboxScalingData::GenerateDataForNpcToNpc(Creature* attacker, Creature* target)
{
    ...
    Type = CREATURE_TO_CREATURE_DAMAGE;
    TargetLevel = target->GetEffectiveLevel();
    Expansion = creatureTemplate->RequiredExpansion;
    Class = creatureTemplate->unit_class;
    TargetMinScalingLevel = creatureTemplate->ScaleLevelMin;
...
void WorldPackets::Spells::SandboxScalingData::GenerateDataForPlayerToPlayer(Player* /*attacker*/, Player* target)
{
    if (!target)
        return;
...
WorldPackets::Spells::SpellLogPowerData::
...
WorldPackets::Spells::SpellCooldownStruct::SpellCooldownStruct(uint32 spellId, uint32 forcedCooldown) : SrecID(spellId), ForcedCooldown(forcedCooldown) { }
WorldPackets::Spells::SpellCooldownStruct::SpellCooldownStruct(uint32 spellId, uint32 forcedCooldown, float modRate): SrecID(spellId), ForcedCooldown(forcedCooldown), ModRate(modRate) { }
...
ByteBuffer& operator<<(ByteBuffer& data, AreaTriggerCircle const& triggerCircleData)
{
    ...
    data << triggerCircleData.UnkUInt2;
    ...
    if (triggerCircleData.TargetGuid)
        data << *triggerCircleData.TargetGuid;
