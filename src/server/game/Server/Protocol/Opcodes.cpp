...
void OpcodeTable::ValidateAndSetClientOpcode(OpcodeClient opcode, char const* name, SessionStatus status, PacketProcessing processing)
{
    ...
    {
        sLog->outTrace(LOG_FILTER_NETWORKIO, "Opcode %s does not have a value", name);
        return;
    }
...
void OpcodeTable::ValidateAndSetServerOpcode(OpcodeServer opcode, char const* name, SessionStatus status, ConnectionType conIdx)
{
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Tried to set invalid connection type %u for opcode %s", conIdx, name);
        return;
    }

    if (IsInstanceOnlyOpcode(opcode) && conIdx != CONNECTION_TYPE_INSTANCE)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Tried to set invalid connection type %u for instance only opcode %s", conIdx, name);
        return;
    }
...
#define DEFINE_HANDLER(opcode, status, processing, handler) \
    ...
    DEFINE_HANDLER(CMSG_CHAT_MESSAGE_DND,                                   STATUS_LOGGEDIN,  PROCESS_THREADUNSAFE, &WorldSession::HandleChatMessageOpcode);
