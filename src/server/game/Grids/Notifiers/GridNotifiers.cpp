...
void VisibleChangesNotifier::Visit(DynamicObjectMapType &m)
{
    for (auto &obj : m)
    {
        ...
        auto const caster = static_cast<Player*>(obj->GetCaster());
...
MessageDistDeliverer::MessageDistDeliverer(WorldObject* src, WorldPacket const* msg, float dist, bool own_team_only, Player const* skipped, GuidUnorderedSet ignoredSet) :
    i_source(src), i_message(msg), i_phaseMask(src->GetPhaseMask()), i_distSq(dist * dist), team((own_team_only && src->IsPlayer()) ? static_cast<Player*>(src)->GetTeam() : 0), skipped_receiver(skipped), m_IgnoredGUIDs(ignoredSet)
{
}
...
void MessageDistDeliverer::Visit(DynamicObjectMapType &m)
{
    for (auto &target : m)
    {
        ...
        if (target->GetCasterGUID().IsPlayer())
        {
            ...
            auto const caster = static_cast<Player*>(target->GetCaster());
...
AnyUnitInObjectRangeCheck::AnyUnitInObjectRangeCheck(WorldObject const* obj, float range) : i_obj(obj), i_range(range)
{
}
...
UnitSortDistance::UnitSortDistance(bool sortlow, Unit* caster) : _sortlow(sortlow), _caster(caster)
{
}
