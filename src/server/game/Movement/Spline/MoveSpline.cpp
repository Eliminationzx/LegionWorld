...
void MoveSpline::init_spline(const MoveSplineInitArgs& args)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_GENERAL, "MoveSpline::init_spline: zero length spline, wrong input data?");
...
MoveSpline::MoveSpline() : durationModifier(0), nextDurationModifier(0), onTransport(false), splineIsFacingOnly(false)
{
...
#define CHECK(exp) \
    if (!(exp))\
    {\
        /* sLog->outDebug(LOG_FILTER_GENERAL, "MoveSplineInitArgs::Validate: expression '%s' failed", #exp); */\
        return false;\
    }
#define CHECK_V(exp) \
    if (!(exp))\
