...
////////////////// PathGenerator //////////////////
PathGenerator::PathGenerator(const Unit* owner) : _polyLength(0), _type(PATHFIND_BLANK), _useStraightPath(false), _forceDestination(false), _stopOnlyOnEndPos(false), _pointPathLimit(MAX_POINT_PATH_LENGTH), 
_straightLine(false), _endPosition(G3D::Vector3::zero()), _sourceUnit(owner), _navMesh(nullptr), _navMeshQuery(nullptr), _transport(nullptr), _go(nullptr), _enableShort(true), _charges(false)
{
    memset(_pathPolyRefs, 0, sizeof(_pathPolyRefs));

    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ PathGenerator::PathGenerator for %s", _sourceUnit->GetGUID().ToString().c_str());
...
void PathGenerator::InitPath()
{
    ...
    if (DisableMgr::IsPathfindingEnabled(mapId))
    {
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        {
            ...
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "PathGenerator 0 displayId %i _transport %i _navMeshQuery %i _navMesh %i", _go ? _go->GetDisplayId() : 0, _transport ? _transport->GetDisplayId() : 0, bool(_navMeshQuery), bool(_navMesh));
        }
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "PathGenerator 1 displayId %i _transport %i _navMeshQuery %i _navMesh %i", _go ? _go->GetDisplayId() : 0, _transport ? _transport->GetDisplayId() : 0, bool(_navMeshQuery), bool(_navMesh));
    }
}
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine, bool stopOnlyOnEndPos)
{
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "CalculatePath ::0 startPos %f %f %f endPos %f %f %f", x, y, z, destX, destY, destZ);
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "CalculatePath :: 1 startPos %f %f %f endPos %f %f %f", x, y, z, destX, destY, destZ);
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ PathGenerator::CalculatePath() for %s _navMesh %u _navMeshQuery %u start %u dest %u",
...
bool PathGenerator::CalculateShortcutPath(float destX, float destY, float destZ)
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "CalculatePath :: startPos %f %f %f endPos %f %f %f", x, y, z, destX, destY, destZ);
    ...
    // if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        // sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ PathGenerator::CalculateShortcutPath() for %s _navMesh %u _navMeshQuery %u start %u dest %u",
            // _sourceUnit->GetGUID().ToString().c_str(), bool(_navMesh), bool(_navMeshQuery), HaveTile(start), HaveTile(dest));
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: startPos %f %f %f endPos %f %f %f", startPos.y, startPos.z, startPos.x, endPos.y, endPos.z, endPos.x);
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: (startPoly == %u || endPoly == %u)", startPoly, endPoly);
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: farFromPoly distToStartPoly=%.3f distToEndPoly=%.3f\n", distToStartPoly, distToEndPoly);
        ...
        if (_sourceUnit->GetTypeId() == TYPEID_UNIT)
        {
            ...
            {
                if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
                    sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: underWater case\n");
                if (owner->CanSwim())
                    buildShotrcut = true;
            }
            else
            {
                if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
                    sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: flying case\n");
                if (owner->CanFly())
                    buildShotrcut = true;
            }
        }
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: (startPoly == endPoly)\n");
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: path type %d\n", _type);
        return;
    }
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                sLog->outError(LOG_FILTER_PATH_GENERATOR, "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u," " startPos: %s, endPos: %s, mapid: %u", _polyLength, pathStartIndex, startPos.toString().c_str(), endPos.toString().c_str(), _sourceUnit->GetMapId());
                break;
            }
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildPolyPath :: (startPolyFound && endPolyFound)\n");
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            sLog->outError(LOG_FILTER_PATH_GENERATOR, "%s's Path Build failed: 0 length path", _sourceUnit->GetGUID().ToString().c_str());
        }

        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++  m_polyLength=%u prefixPolyLength=%u suffixPolyLength=%u \n", _polyLength, prefixPolyLength, suffixPolyLength);
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ PathGenerator::BuildPointPath FAILED! path sized %d returned\n", pointCount);
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
    }
    ...
    {
        if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ PathGenerator::BuildPointPath FAILED! path sized %d returned, lower than limit set to %d\n", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ PathGenerator::BuildPointPath path type %d size %d poly-size %d _forceDestination %u", _type, pointCount, _polyLength, _forceDestination);
}
...
void PathGenerator::NormalizePath()
{
    ...
    for (uint32 i = 0, next = 1; next < _pathPoints.size(); ++i, ++next)
    {
        ...
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "PathGenerator::NormalizePath next %u _pathPoints %u vector %f %f %f dist %f 2ddist %f", next, i, _pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z, dtVdist(lastPoint, nextPoint), dtVdist2D(lastPoint, nextPoint));
    }

    sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "PathGenerator::NormalizePath _pathPoints %u", _pathPoints.size());
    ...
    for (uint32 i = 0; next < _pathPoints.size(); ++i, ++next)
    {
        ...
        while (!result)
        {
            ...
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "NormalizePath 0 %f %f %f moveCur %f %f %f", lastPoint[0], lastPoint[1], lastPoint[2], moveCur[0], moveCur[1], moveCur[2]);
            ...
            if (dtVdist(lastPoint, moveTgt) <= 2.0f) // min distance 2m for divide patch
            {
                ...
                dtVcopy(moveCur, nextPoint);
                sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "NormalizePath 0 push_back %u vector %f %f %f dist %f 2ddist %f", _newPathPoints.size(), moveTgt[0], moveTgt[1], moveTgt[2], dtVdist(lastPoint, moveCur), dtVdist2D(lastPoint, moveCur));
                break;
            }
            ...
            if (result)
            {
                ...
                sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "NormalizePath 1 push_back %u vector %f %f %f dist %f 2ddist %f", _newPathPoints.size(), moveTgt[0], moveTgt[1], moveTgt[2], dtVdist(lastPoint, moveCur), dtVdist2D(lastPoint, moveCur));
            ...
            sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "NormalizePath 1 %f %f %f moveCur %f %f %f", lastPoint[0], lastPoint[1], lastPoint[2], moveCur[0], moveCur[1], moveCur[2]);
        }
    }
    ...
    for (uint32 i = 0; i < _pathPoints.size(); ++i)
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "NormalizePath _pathPoints %u vector %f %f %f", i, _pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z);

    sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "PathGenerator::NormalizePath end _pathPoints %u counter %u", _pathPoints.size(), counter);
}
...
void PathGenerator::BuildShortcut()
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        sLog->outDebug(LOG_FILTER_PATH_GENERATOR, "++ BuildShortcut :: making shortcut\n");
...
bool PathGenerator::HaveTile(const G3D::Vector3& p) const
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->isSummon())
        sLog->outError(LOG_FILTER_PATH_GENERATOR, "++ HaveTile tx %i ty %i p %f %f %f",tx, ty, p.y, p.z, p.x);
...
void PathGenerator::ReducePathLenghtByDist(float dist)
{
    ...
    {
        sLog->outError(LOG_FILTER_PATH_GENERATOR, "PathGenerator::ReducePathLenghtByDist called before path was built");
        return;
    }
