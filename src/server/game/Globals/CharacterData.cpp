...
void CharacterDataStoreMgr::LoadCharacterTemplates()
{
    ...
    if (!templates)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadCharacterTemplates() >> Loaded 0 character templates. DB table `character_template` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadCharacterTemplates() >> Loaded %u character templates with character_template for %u classes in %u ms.", count[0], count[1], GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDataStoreMgr::LoadFactionChangeAchievements()
{
    ...
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeAchievements() >> Loaded 0 faction change achievement pairs. DB table `player_factionchange_achievement` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeAchievements() >> Loaded %u faction change achievement pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDataStoreMgr::LoadFactionChangeItems()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeItems() >> Loaded 0 faction change item pairs. DB table `player_factionchange_items` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeItems() >> Loaded %u faction change item pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDataStoreMgr::LoadFactionChangeSpells()
{
    ...
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeSpells() >> Loaded 0 faction change spell pairs. DB table `player_factionchange_spells` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeSpells() >> Loaded %u faction change spell pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDataStoreMgr::LoadFactionChangeReputations()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeReputations() >> Loaded 0 faction change reputation pairs. DB table `player_factionchange_reputations` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeReputations() >> Loaded %u faction change reputation pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDataStoreMgr::LoadFactionChangeTitles()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeTitles() >> Loaded 0 faction change title pairs. DB table `player_factionchange_title` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadFactionChangeTitles() >> Loaded %u faction change title pairs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDataStoreMgr::LoadReservedPlayersNames()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadReservedPlayersNames() >> Loaded 0 reserved player names. DB table `reserved_name` is empty!");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "CharacterDataStoreMgr::LoadReservedPlayersNames() >> Loaded %u reserved player names in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
