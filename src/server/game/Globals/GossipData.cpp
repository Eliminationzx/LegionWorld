...
void GossipDataStoreMgr::LoadGossipMenu()
{
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Table gossip_menu Entry %u are using non-existing TextID %u", gMenu.Entry, gMenu.TextID);
            continue;
        }

        if (gMenu.FriendshipFactionID && !sFriendshipReputationStore.LookupEntry(gMenu.FriendshipFactionID))
        {
            sLog->outError(LOG_FILTER_SQL, "Table gossip_menu Entry %u are using non-existing FriendshipFactionID %u", gMenu.Entry, gMenu.FriendshipFactionID);
...
void GossipDataStoreMgr::LoadGossipMenuItems()
{
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Table gossip_menu_option for menu %u, id %u has unknown icon id %u. Replacing with GOSSIP_ICON_CHAT", gMenuItem.MenuID, gMenuItem.OptionIndex, gMenuItem.OptionNPC);
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Table gossip_menu_option for menu %u, id %u has GOSSIP_ICON_SHIPMENT, but handler %i != GOSSIP_OPTION_GARRISON_SHIPMENT  ", gMenuItem.MenuID, gMenuItem.OptionIndex, gMenuItem.OptionNPC);
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Table gossip_menu_option for menu %u, id %u has GOSSIP_ICON_CLASS_HALL_UPGRADE, but handler %i != GOSSIP_OPTION_CLASS_HALL_UPGRADE  ", gMenuItem.MenuID, gMenuItem.OptionIndex, gMenuItem.OptionNPC);
