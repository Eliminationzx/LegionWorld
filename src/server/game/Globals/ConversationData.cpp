...
void ConversationDataStoreMgr::LoadConversations()
{
    ...
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversations() >> Loaded 0 conversation. DB table `conversation` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "ConversationDataStoreMgr::LoadConversations() >> Table `conversation` has conversation (GUID: " UI64FMTD ") with non existing conversation data %u, skipped.", guid, entry);
            continue;
        }
        ...
        if (lastCreature != lastEntryCreature.end())
        {
            if (data.mapid == lastCreature->second->mapid)
            {
                ...
                if (distsq1 < 0.5f)
                {
                    ...
                    sLog->outError(LOG_FILTER_SQL, "ConversationDataStoreMgr::LoadConversations() >> Table `conversation` have clone npc %u witch stay too close (dist: %f). original npc guid %u. npc with guid %u will be deleted.", entry, distsq1, lastCreature->second->guid, guid);
                    continue;
                }
            }
            else
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "LoadConversations >> Table `conversation` have conversation (GUID: " UI64FMTD ") that spawned at not existed map (Id: %u), skipped.", guid, data.mapid);
            continue;
        }

        if (data.spawnMask & ~spawnMasks[data.mapid])
        {
            sLog->outError(LOG_FILTER_SQL, "LoadConversations >> Table `conversation` have conversation (GUID: " UI64FMTD ") that have wrong spawn mask " UI64FMTD " including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %u.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
        ...
        if (data.phaseMask == 0)
        {
            sLog->outError(LOG_FILTER_SQL, "LoadConversations >> Table `conversation` have conversation (GUID: " UI64FMTD " Entry: %u) with `phaseMask`=0 (not visible for anyone), set to 1.", guid, data.id);
            data.phaseMask = 1;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversations() >> Loaded %u conversation in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ConversationDataStoreMgr::LoadConversationData()
{
    ...
    if (result)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversationData() >> Loaded %u conversation data.", counter);
    }
    else
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversationData() >> Loaded 0 conversation data. DB table `conversation_data` is empty.");
    ...
    if (result)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversationData() >> Loaded %u conversation creature data.", counter);
    }
    else
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversationData() >> Loaded 0 conversation creature data. DB table `conversation_creature` is empty.");
    ...
    if (result)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversationData() >> Loaded %u conversation actor data.", counter);
    }
    else
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ConversationDataStoreMgr::LoadConversationData() >> Loaded 0 conversation actor data. DB table `conversation_actor` is empty.");
}
