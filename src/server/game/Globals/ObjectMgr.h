...
typedef std::unordered_map<uint8, float> PriceForLevelUp;
typedef std::unordered_map<uint8, float> PriceForArtLevelUp;
...
class ObjectMgr
{
    ...
    static ObjectMgr* instance();
        ...
        static void ChooseCreatureFlags(const CreatureTemplate* cinfo, uint32& npcflag, uint32& npcflag2, uint32& unit_flags, uint32& dynamicflags, const CreatureData* data = nullptr);
    ...
    void LoadLinkedRespawn();
        ...
        void FillDisenchantFields(uint8* disenchantID, uint16* requiredDisenchantSkill, Item* item);
    ...
    ObjectGuid GetLinkedRespawnGuid(ObjectGuid const& guid) const;
        ...
        const SeamlessTeleportData* GetSeamlessTeleportData(uint16 zoneId) const;
        ...
        void DeleteDuplicateBonus(std::vector<uint32>& bonusListIDs);
        ...
        DonVenAdd const* GetDonateVendorAdditionalInfo(uint32 type, uint32 entry) const
        {
            ...
            auto pPtr = (*ptr).second.find(entry);
            if (pPtr == (*ptr).second.end())
                return nullptr;
        ...
        {
            return Trinity::Containers::MapGetValuePtr(_priceforlevelupStore, level);
        };
        
        float const* GetPriceForArtLevelUp(uint8 level) 
        {
            return Trinity::Containers::MapGetValuePtr(_priceforArtlevelupStore, level);
        };
        ...
        {
            return Trinity::Containers::MapGetValuePtr(_scenarioDataSpellStep, ScenarioId);
        }
    ...
    private:
        ...
        SeamlessTeleportMap _seamlessTeleportData;
