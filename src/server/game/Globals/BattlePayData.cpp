...
void BattlePayDataStoreMgr::LoadDisplayInfos()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay display info in %u ms.", uint64(_displayInfos.size()), GetMSTimeDiffToNow(oldMsTime));
}

void BattlePayDataStoreMgr::LoadDisplayInfoVisuals()
{
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Battlepay display info visuals ...");
    _visuals.clear();
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay display info visuals in %u ms.", uint64(_visuals.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadProductGroups()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay product groups in %u ms", uint64(_productGroups.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadProduct()
{
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "BattlePayDataStoreMgr: battlepay_product websiteType >= max types - skip loading: type %u; productId: %u ", product.WebsiteType, product.ProductID);
            continue;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay products in %u ms", uint64(_products.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadShopEntires()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay shop entries in %u ms", uint64(_shopEntries.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadProductGroupLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay product group locales strings in %u ms", uint64(_productGroupLocales.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadDisplayInfoLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Battlepay display info locales strings in %u ms", uint64(_displayInfoLocales.size()), GetMSTimeDiffToNow(oldMsTime));
}
