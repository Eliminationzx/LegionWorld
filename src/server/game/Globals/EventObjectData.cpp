...
void EventObjectDataStoreMgr::LoadEventObjectTemplates()
{
    ...
    //                                               0      1      2       3            4             5
    QueryResult result = WorldDatabase.Query("SELECT entry, name, radius, SpellID, WorldSafeLocID, ScriptName FROM eventobject_template;");
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "EventObjectDataStoreMgr::LoadEventObjectTemplates() >> Loaded 0 eventobject. DB table `eventobject_template` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "EventObjectDataStoreMgr::LoadEventObjectTemplates() >> Loaded %u eventobject template in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void EventObjectDataStoreMgr::LoadEventObjects()
{
    ...
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "EventObjectDataStoreMgr::LoadEventObjects() >> Loaded 0 eventobject. DB table `eventobject` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (lastObject != lastEntry.end())
        {
            if (data.mapid == lastObject->second->mapid)
            {
                ...
                if (distsq1 < 0.5f)
                {
                    ...
                    sLog->outError(LOG_FILTER_SQL, "LoadEventObjects >> Table `eventobject` have clone npc %u witch stay too close (dist: %f). original npc guid %u. npc with guid %u will be deleted.", entry, distsq1, lastObject->second->guid, guid);
                    continue;
                }
            }
            else
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "LoadEventObjects >> Table `eventobject` have eventobject (GUID: " UI64FMTD ") that spawned at not existed map (Id: %u), skipped.", guid, data.mapid);
            continue;
        }

        if (data.spawnMask & ~spawnMasks[data.mapid])
        {
            sLog->outError(LOG_FILTER_SQL, "LoadEventObjects >> Table `eventobject` have eventobject (GUID: " UI64FMTD ") that have wrong spawn mask %u including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %u.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
        ...
        if (data.phaseMask == 0)
        {
            sLog->outError(LOG_FILTER_SQL, "LoadEventObjects >> Table `eventobject` have eventobject (GUID: " UI64FMTD " Entry: %u) with `phaseMask`=0 (not visible for anyone), set to 1.", guid, data.id);
            data.phaseMask = 1;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "EventObjectDataStoreMgr::LoadEventObjects() >> Loaded %u eventobject in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
