...
void BattlePetDataStoreMgr::LoadBattlePetTemplate()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 battlepet template. DB table `battlepet_info` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u battlepet template in %u ms.", _battlePetTemplateStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void BattlePetDataStoreMgr::LoadBattlePetNpcTeamMember()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u battlepet npc team member in %u ms.", static_cast<uint32>(_battlePetNpcTeamMembers.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void BattlePetDataStoreMgr::ComputeBattlePetSpawns()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ComputeBattlePetSpawns %u ms.", GetMSTimeDiffToNow(oldMSTime));
}
