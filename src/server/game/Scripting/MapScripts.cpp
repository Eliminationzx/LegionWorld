...
void Map::ScriptsStart(ScriptMapMap const& scripts, uint32 id, Object* source, Object* target)
{
    ...
    for (ScriptMap::const_iterator iter = s2->begin(); iter != s2->end(); ++iter)
    {
        ...
        {
            WriteGuardType guard(m_scriptScheduleLock);
        ...
        if (step.sourceGUID)
        {
            switch (step.sourceGUID.GetHigh())
            {
                ...
                case HighGuid::Corpse:
                    ...
                    break;
                default:
                    sLog->outError(LOG_FILTER_TSCR, "%s source with unsupported high guid (GUID: " UI64FMTD ", high guid: %u).", step.script->GetDebugInfo().c_str(), step.sourceGUID.GetCounter(), step.sourceGUID.GetHigh());
                    break;
            }
        }
        ...
        if (step.targetGUID)
        {
            switch (step.targetGUID.GetHigh())
            {
                ...
                case HighGuid::Corpse:
                    ...
                    break;
                default:
                    sLog->outError(LOG_FILTER_TSCR, "%s target with unsupported high guid (GUID: " UI64FMTD ", high guid: %u).", step.script->GetDebugInfo().c_str(), step.targetGUID.GetCounter(), step.targetGUID.GetHigh());
                    break;
            }
        }
