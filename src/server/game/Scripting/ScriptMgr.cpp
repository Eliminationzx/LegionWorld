...
void ScriptMgr::OnCreateMap(Map* map)
{
    ...
    SCR_MAP_BGN(InstanceMapScript, map, itr, end, entry, IsDungeon);
        itr->second->OnCreate(static_cast<InstanceMap*>(map));
    SCR_MAP_END;
...
void ScriptMgr::OnDestroyMap(Map* map)
{
    ...
    SCR_MAP_BGN(InstanceMapScript, map, itr, end, entry, IsDungeon);
        itr->second->OnDestroy(static_cast<InstanceMap*>(map));
    SCR_MAP_END;
...
void ScriptMgr::OnLoadGridMap(Map* map, GridMap* gmap, uint32 gx, uint32 gy)
{
    ...
    SCR_MAP_BGN(InstanceMapScript, map, itr, end, entry, IsDungeon);
        itr->second->OnLoadGridMap(static_cast<InstanceMap*>(map), gmap, gx, gy);
    SCR_MAP_END;
...
void ScriptMgr::OnUnloadGridMap(Map* map, GridMap* gmap, uint32 gx, uint32 gy)
{
    ...
    SCR_MAP_BGN(InstanceMapScript, map, itr, end, entry, IsDungeon);
        itr->second->OnUnloadGridMap(static_cast<InstanceMap*>(map), gmap, gx, gy);
    SCR_MAP_END;
...
void ScriptMgr::OnPlayerEnterMap(Map* map, Player* player)
{
    ...
    SCR_MAP_BGN(InstanceMapScript, map, itr, end, entry, IsDungeon);
        itr->second->OnPlayerEnter(static_cast<InstanceMap*>(map), player);
    SCR_MAP_END;
...
void ScriptMgr::OnPlayerLeaveMap(Map* map, Player* player)
{
    ...
    SCR_MAP_BGN(InstanceMapScript, map, itr, end, entry, IsDungeon);
        itr->second->OnPlayerLeave(static_cast<InstanceMap*>(map), player);
    SCR_MAP_END;
...
bool ScriptMgr::OnEventObject(Player* player, EventObject* event)
{
    ASSERT(player);
    ASSERT(event);

    GET_SCRIPT_RET(EventObjectScript, event->GetScriptId(), tmpscript, false);
    return tmpscript->OnTrigger(player, event);
}
