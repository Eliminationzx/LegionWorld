...
float ThreatCalcHelper::calcThreat(Unit* hatedUnit, Unit* /*hatingUnit*/, float threat, SpellSchoolMask schoolMask, SpellInfo const* threatSpell)
{
    if (threatSpell)
    {
        ...
        {
            if (threatSpell->EffectMask < uint32(1 << i))
                break;
...
Trinity::ThreatOrderPred::ThreatOrderPred(bool ascending) : m_ascending(ascending)
{
}

bool Trinity::ThreatOrderPred::operator()(HostileReference const* a, HostileReference const* b) const
{
