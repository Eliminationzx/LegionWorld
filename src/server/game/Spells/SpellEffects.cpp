...
void Spell::EffectSchoolDMG(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        ...
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "EffectSchoolDMG end %i, m_diffMode %i, effIndex %i, spellId %u, damage %i GetComboPoints %i", m_damage, m_diffMode, effIndex, m_spellInfo->Id, damage, GetComboPoints());
...
bool Spell::SpellDummyTriggered(SpellEffIndex effIndex)
{
    if (std::vector<SpellDummyTrigger> const* spellTrigger = sSpellMgr->GetSpellDummyTrigger(m_spellInfo->Id))
    {
        ...
        {
            #ifdef WIN32
            sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::EffectDummy %u, 1<<effIndex %u, itr->effectmask %u, option %u, spell_trigger %i, target %u (%u ==> %u) explTarget %u effectHandleMode %u",
            ...
            #ifdef WIN32
            sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::EffectDummy2: %u, 1<<effIndex %u, itr->effectmask %u, option %u, spell_trigger %i, target %u (%u ==> %u)", m_spellInfo->Id, 1 << effIndex,
            ...
            switch (m_spellInfo->Id)
            {
                case 196529: // Renew the Faith
                    if (unitTarget)
                    {
                        ...
                        {
                            float basepoints0 = _aura->GetStackAmount();
                            m_caster->CastCustomSpell(unitTarget, 155793, &basepoints0, &basepoints0, &basepoints0, true, nullptr, nullptr, m_originalCasterGUID);
                            _aura->Remove();
                        }
                    }
                    break;
                case 155793: // Prayer of Mending
                    if (unitTarget)
                    {
                        if (AuraEffect* aurEff = (m_originalCaster ? m_originalCaster : m_caster)->GetAuraEffect(196358, EFFECT_0)) // Say Your Prayers
                            if (roll_chance_i(aurEff->GetAmount()) && !(m_originalCaster ? m_originalCaster : m_caster)->HasAura(210994))
                                damage += 1;

                        if (AuraEffect* const aurEff2 = m_caster->GetAuraEffect(193157, EFFECT_0)) // Benediction
                            if (roll_chance_i(aurEff2->GetAmount()))
                                m_caster->CastSpell(unitTarget, 139, true);

                        m_caster->CastSpellDuration(unitTarget, 41635, true, 0, damage);
                        m_caster->CastSpell(unitTarget, 225275, true);
                    }
                    break;
            }
            break;
    ...
    switch (m_spellInfo->Id)
    {
        case 228596: // Flurry
        {
            if (effectHandleMode == SPELL_EFFECT_HANDLE_HIT_TARGET)
            {
                ...
                if (Aura* aura = m_caster->GetAura(231584))
                {
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
    ...
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "EffectJumpDest spell %u triggered_spell_id %u x %f y %f z %f o %f", m_spellInfo->Id, triggered_spell_id, x, y, z, o);
...
void Spell::CalculateJumpSpeeds(uint8 i, float dist, float & speedXY, float & speedZ)
{
    ...
    if (m_spellInfo->Id == 49575 || m_spellInfo->Id == 146599)
        speedXY = 38;
    ...
    if (m_spellInfo->GetEffect(effIndex, m_diffMode)->TargetA.GetTarget() == TARGET_UNIT_CASTER && m_spellInfo->GetEffect(effIndex, m_diffMode)->TargetB.GetTarget() == TARGET_NONE)
        if (SpellTargetPosition const* st = sSpellMgr->GetSpellTargetPosition(m_spellInfo->Id))
            if (m_spellInfo->HasEffect(SPELL_EFFECT_TELEPORT_L) || m_spellInfo->HasEffect(SPELL_EFFECT_TELEPORT))
                destTarget->Relocate(st->target_X, st->target_Y, st->target_Z, st->target_Orientation, (int32)st->target_mapId);
    ...
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        switch (m_spellInfo->Id)
        {
            case 115464: // Healing Sphere
            {
                ...
                {
                    if (aurApp->GetBase()->GetSpellInfo()->GetSchoolMask() & SPELL_SCHOOL_MASK_MAGIC)
                        return true;
                    return false;
                });
                break;
        ...
        if (m_caster->getClass() == CLASS_DRUID)
        ...
        if (m_caster->getClass() == CLASS_PALADIN && m_spellInfo->Id != 53652)
        {
            if (AuraEffect* aurEff = m_caster->GetAuraEffect(183997, EFFECT_0)) // Mastery: Lightbringer
            {
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "HealthLeech damage %i bonus %u variance %f", damage, bonus, variance);
    ...
    if (m_caster->isAlive())
    {
        ...
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "HealthLeech damage %i healthGain %u", damage, healthGain);
...
void Spell::DoCreateItem(uint32 /*effIndex*/, uint32 itemtype, std::vector<uint32> const& bonusListIDs)
{
    ...
    if (num_to_add)
    {
        ...
        if (Guild* guild = sGuildMgr->GetGuildById(player->GetGuildId()))
        {
            ...
            guild->UpdateAchievementCriteria(CRITERIA_TYPE_CRAFT_ITEMS_GUILD, pProto->ItemId, num_to_add, 0, nullptr, player);
        }
        ...
        if ((pItem->GetTemplate()->Class != ITEM_CLASS_CONSUMABLE && pItem->GetTemplate()->Class != ITEM_CLASS_QUEST && newitemid != 6265 && newitemid != 6948) || newitemid == 5512)
        ...
        else if (goInfo->type == GAMEOBJECT_TYPE_FLAGSTAND || goInfo->type == GAMEOBJECT_TYPE_NEW_FLAG)
        {
            if (Battleground* bg = player->GetBattleground())
            {
                switch (bg->GetTypeID(true))
                {
                    ...
                    case MS::Battlegrounds::BattlegroundTypeId::BrawlBattlegroundKotmoguTemplate:
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    switch (entry)
    {
        case 55659:
        case 98035:
            if (AuraEffect* aurEff = m_caster->GetAuraEffect(214345, EFFECT_0)) // Wilfred's Sigil of Superior Summoning
            {
                if (m_caster->IsPlayer())
                {
        ...
        case SUMMON_CATEGORY_UNK:
        {
            switch (properties->Title)
            {
                ...
                case SUMMON_TYPE_BANNER:
                {
                    ...
                    switch (m_spellInfo->Id)
                    {
                        case 204336: // Grounding Totem (Honor Talent)
                        {
                            summon->CastSpell(summon, 242900, true);
                            break;
                        }
                        case 198838: // Earthen Shield
                        {
    ...
    else
    {
        ...
        if (item_owner != caster && !AccountMgr::IsPlayerAccount(caster->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
        {
            sLog->outCommand(caster->GetSession()->GetAccountId(), "GM %s (Account: %u) enchanting(perm): %s (Entry: %d) for player: %s (Account: %u)",
                             ...
                             itemTarget->GetTemplate()->Name->Str[caster->GetSession()->GetSessionDbLocaleIndex()], itemTarget->GetEntry(),
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    #ifdef WIN32
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell EffectWeaponDmg spellid %u in Effect(%u) fixed_bonus %i, weaponDamagePercentMod %f weaponDamage %i", m_spellInfo->Id, effIndex, fixed_bonus, weaponDamagePercentMod, weaponDamage);
    #endif

    if (auto const& shapeshift = sSpellShapeshiftFormStore.LookupEntry(_caster->GetShapeshiftForm()))
        if (shapeshift->CombatRoundTime)
            weaponDamage = weaponDamage * shapeshift->CombatRoundTime / 1000.0f / _caster->GetAPMultiplier(m_attackType, normalized);
    ...
    #ifdef WIN32
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell EffectWeaponDmg spellid %u in Effect(%u) weaponDamage %i, _damage %i m_damage %i effectHandleMode %i ComboPoints %i", m_spellInfo->Id, effIndex, weaponDamage, _damage, m_damage, effectHandleMode, GetComboPoints());
    #endif
    ...
    switch (m_spellInfo->Id)
    {
        ...
        {
            int32 pctDamage = m_spellInfo->GetEffect(EFFECT_2, m_diffMode)->CalcValue(_caster);
            ...
            float bp = _caster->CountPctFromMaxHealth(pctHeal) + CalculatePct(_caster->GetDamageTakenInPastSecs(lastTime, true, false), pctDamage);
...
void Spell::EffectInterruptCast(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "EffectInterruptCast Id %u", m_spellInfo->Id);
    ...
    for (uint32 i = CURRENT_GENERIC_SPELL; i <= CURRENT_CHANNELED_SPELL; ++i)
    {
        if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
                ...
                && (curSpellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT || curSpellInfo->HasChannelInterruptFlag(CHANNEL_INTERRUPT_FLAG_INTERRUPT)))
            {
                ...
                m_caster->ProcDamageAndSpell(unitTarget, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, (PROC_EX_INTERRUPT | PROC_EX_NORMAL_HIT), &dmgInfoProc, BASE_ATTACK, m_spellInfo);
            }
            else if ((spell->getState() == SPELL_STATE_CASTING || (spell->getState() == SPELL_STATE_PREPARING && spell->GetCastTime() > 0.0f)) &&
                    (curSpellInfo->Categories.PreventionType & SPELL_PREVENTION_TYPE_SILENCE || curSpellInfo->Categories.PreventionType & SPELL_PREVENTION_TYPE_PACIFY))
            {
                if (Creature* creature = unitTarget->ToCreature())
                    if (creature->IsAIEnabled)
                        creature->AI()->NonInterruptCast(m_caster, curSpellInfo->Id, curSpellInfo->GetSchoolMask());
            }
        }
    }
}
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    if (pGameObj->GetGoType() == GAMEOBJECT_TYPE_NEW_FLAG_DROP && m_caster->IsPlayer())
    {
        ...
        {
            case 489:                                       //WS
                if (bg && bg->GetTypeID(true) == MS::Battlegrounds::BattlegroundTypeId::BattlegroundWarsongGulch && bg->GetStatus() == STATUS_IN_PROGRESS)
                {
                    ...
                    static_cast<BattlegroundWarsongGulch*>(bg)->SetDroppedFlagGUID(pGameObj->GetGUID(), team);
                }
                break;
            case 726:                                       //TP
...
void Spell::EffectDisEnchant(SpellEffIndex /*effIndex*/)
{
    ...
    if (GetCaster()->IsPlayer())
        sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 4 item  guid = %u, entry = %u, %s", itemTarget->GetGUID().GetGUIDLow(), itemTarget->GetEntry(), GetCaster()->ToPlayer()->GetInfoForDonate().c_str());
...
void Spell::EffectSurvey(SpellEffIndex /*effIndex*/)
{
    ...
    if (!go_id)
    {
        sLog->outError(LOG_FILTER_SPELLS_AURAS, "Spell::EffectSurvey: no go id for x: %f y: %f z: %f map: %u", m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), m_caster->GetMapId());
        return;
    }
...
void Spell::EffectKnockBack(SpellEffIndex effIndex)
{
    ...
    if (unitTarget->IsPlayer() && unitTarget->HasUnitState(UNIT_STATE_CHARGING))
    {
...
void Spell::EffectLeapBack(SpellEffIndex effIndex)
{
    ...
    m_caster->JumpTo(speedxy, speedz, angle);
}
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (!goinfo)
    {
        sLog->outError(LOG_FILTER_SQL, "Gameobject (Entry: %u) not exist and not created at spell (ID: %u) cast", objEntry, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectProspecting(SpellEffIndex /*effIndex*/)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_SKILL_PROSPECTING))
        player->UpdateGatherSkill(SKILL_JEWELCRAFTING, player->GetPureSkillValue(SKILL_JEWELCRAFTING), itemTarget->GetTemplate()->RequiredSkillRank);
...
void Spell::EffectMilling(SpellEffIndex /*effIndex*/)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_SKILL_MILLING))
        caster->UpdateGatherSkill(SKILL_INSCRIPTION, caster->GetPureSkillValue(SKILL_INSCRIPTION), itemTarget->GetTemplate()->RequiredSkillRank);
...
void Spell::EffectRedirectThreat(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget)
    {
        if (float threatMod = unitTarget->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_THREAT))
            damage += threatMod;
...
void Spell::EffectRenamePet(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || !unitTarget->IsCreature() || !unitTarget->ToCreature()->isPet() || static_cast<Pet*>(unitTarget)->getPetType() != HUNTER_PET)
        return;
...
void Spell::EffectRemoveAura(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::EffectRemoveAura unitTarget %s", unitTarget->GetGUID().ToString().c_str());
...
void Spell::EffectGiveCurrency(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->ModifyCurrency(currencyID, damage * sDB2Manager.GetCurrencyPrecision(currencyID), true);
}
...
void Spell::EffectBonusLoot(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_LOOT, "Spell::EffectBonusLoot spellCooldownId %i Id %u IsTriggered %u CurrencyID %i", spellCooldownId, m_spellInfo->Id, IsTriggered(), m_spellInfo->Reagents.CurrencyID);
    ...
    sLog->outDebug(LOG_FILTER_LOOT, "Spell::EffectBonusLoot difficulty %i spellCooldownId %u", difficulty, spellCooldownId);
    ...
    switch (m_spellInfo->GetEffect(effIndex, m_diffMode)->MiscValueB)
    {
        ...
        case 436: // Scale loot item by source item level
            ...
            if (lootId >= 242842 && lootId <= 242864 || lootId == 243074 || lootId >= 254773 && lootId <= 254794) // For token 880-910, have other TreeMod
                loot->_itemContext = 43;

            if (lootId >= 240485 && lootId <= 240518) // For legendary token
...
void Spell::EffectGiveArtifactPower(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "EffectGiveArtifactPower Id %u", m_spellInfo->Id);
...
void Spell::EffectGiveArtifactPowerNoBonus(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "EffectGiveArtifactPowerNoBonus Id %u", m_spellInfo->Id);
...
DelayCastEvent* Spell::GetSpellTriggerDelay(SpellEffIndex effIndex, uint32 triggered_spell_id)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "GetSpellTriggerDelay spell %u triggered_spell_id %u TargetGUID %u CasterGUID %u", m_spellInfo->Id, triggered_spell_id, TargetGUID.GetGUIDLow(), CasterGUID.GetGUIDLow());
...
void Spell::EffectObliterateItem(SpellEffIndex /*effIndex*/)
{
    ...
    if (!proto || !(proto->Flags3 & ITEM_FLAG3_OBLITERATABLE))
        return;
    ...
    {
        switch (proto->ItemNameDescriptionID)
        {
        ...
        switch (proto->ItemNameDescriptionID)
        {
            case 13297: // Legion Season 4
            {
                ...
                if (proto->ItemLevel == 810) // Combatant
                    addCount = 1;
                else if (itemLevel < 900) // Gladiator
                    addCount = 1;
                else
                    addCount = 15;
                break;
