...
bool _SpellScript::ValidateSpellInfo(std::vector<uint32> spellIds)
{
    for (auto spellId : spellIds)
        ...
        {
            sLog->outError(LOG_FILTER_TSCR, "%s: Spell %u does not exist.", __FUNCTION__, spellId);
            return false;
        }
...
bool SpellScript::_Validate(SpellInfo const* entry)
{
    ...
    for (auto itr = OnEffectSuccessfulDispel.begin(); itr != OnEffectSuccessfulDispel.end(); ++itr)
        if (!itr->GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match dbc effect data - handler bound to hook `OnEffectSuccessfulDispel` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
