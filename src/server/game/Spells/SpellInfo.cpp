...
bool SpellImplicitTargetInfo::IsArea() const
{
    return GetSelectionCategory() == TARGET_SELECT_CATEGORY_AREA || 
           ...
           GetSelectionCategory() == TARGET_SELECT_CATEGORY_BETWEEN;
}
...
SpellInfo::SpellInfo(SpellInfoLoadHelper const& data, SpellEntry const* spellEntry, SpellVisualMap* visuals)
{
    ...
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        for (uint8 difficulty = DIFFICULTY_NONE; difficulty < MAX_DIFFICULTY; ++difficulty)
        {
            if (SpellEffectEntry const* _effect = spellEntry->GetSpellEffect(i, difficulty))
            {
                ...
                EffectDifficultyMask |= 1 << difficulty;
    ...
    {
        MiscDifficultyMask |= 1 << miscData->DifficultyID;
    ...
    SpellAuraOptionsEntry const* auraOpStore = data.AuraOptions;
    ...
    ProcsPerMinute.ProcBasePPM = _ppm ? _ppm->BaseProcRate : 0.0f;
    if (auraOpStore)
        ProcsPerMinute.ProcPPMMods = sDB2Manager.GetSpellProcsPerMinuteMods(auraOpStore->SpellProcsPerMinuteID);
...
SpellEffectInfo const* SpellInfo::GetEffect(uint8 effect, uint8 difficulty) const
{
    ...
    if (difficulty)
    {
        ...
        if (!(EffectDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_MYTHIC_KEYSTONE)
            difficulty = DIFFICULTY_MYTHIC_DUNGEON;
        if (!(EffectDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_MYTHIC_DUNGEON)
            difficulty = DIFFICULTY_HEROIC;
        if (!(EffectDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_MYTHIC_RAID)
            difficulty = DIFFICULTY_HEROIC_RAID;
        if (!(EffectDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_HEROIC_RAID)
            difficulty = DIFFICULTY_NORMAL_RAID;

        if (EffectDifficultyMask & (1 << difficulty))
        {
            auto itr = EffectsMap.find(MAKE_PAIR16(effect, difficulty));
...
void SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, SpellPowerCost& cost) const
{
    ...
    if (GetSpellPowerByCasterPower(caster, powerData))
    {
        for (SpellPowerEntry const* power : powerData)
        {
            ...
            // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CalcPowerCost Id %u powerCost %i PowerCostPct %f ManaCostAdditional %u", Id, powerCost, power->PowerCostPct, power->ManaCostAdditional);
            ...
            if (power->PowerCostPct)
            {
                switch (PowerType)
                {
                    ...
                    default:
                        ...
                        // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CalcPowerCost: Power type '%i' in spell %d, powerCost %i, GetCreatePowers %i", PowerType, Id, powerCost, caster->GetCreatePowers(PowerType));
                        //return powerCost;
                }
            }
            ...
            // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CalcPowerCost Id %u powerCost %i", Id, powerCost);
            ...
            // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CalcPowerCost Id %u powerCost %i PowerType %i", Id, powerCost, PowerType);
...
{
    float ppm = ProcsPerMinute.ProcBasePPM;
    if (!caster)
        return ppm;

    for (SpellProcsPerMinuteModEntry const* mod : ProcsPerMinute.ProcPPMMods)
    {
        switch (mod->Type)
        {
...
{
    // check alternative diff for get data
    if (!(MiscDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_MYTHIC_KEYSTONE)
        difficulty = DIFFICULTY_MYTHIC_DUNGEON;
    if (!(MiscDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_MYTHIC_DUNGEON)
        difficulty = DIFFICULTY_HEROIC;
    if (!(MiscDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_MYTHIC_RAID)
        difficulty = DIFFICULTY_HEROIC_RAID;
    if (!(MiscDifficultyMask & (1 << difficulty)) && difficulty == DIFFICULTY_HEROIC_RAID)
        difficulty = DIFFICULTY_NORMAL_RAID;

    if (!difficulty || (MiscDifficultyMask & (1 << difficulty)) == 0)
...
bool SpellInfo::IsStack() const
{
    ...
    return !(AuraOptions.ProcTypeMask & PROC_FLAG_DONE_SPELL_MAGIC_DMG_POS_NEG);
}
                ...
                if (!withDamage)
                {
                    ...
                    else
                    {
                        if (AuraEffect* aureff = target->GetAuraEffect(Id, j))
                            if (aureff->GetBase()->GetStackAmount() >= AuraOptions.CumulativeAura)
                                return false;
                    }
                }
