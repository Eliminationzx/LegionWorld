...
{
    for (std::list<uint32>::iterator Itr = mForbiddenSpells.begin(); Itr != mForbiddenSpells.end(); ++Itr)
        if ((*Itr) == spellid)
            return true;
...
SpellInfo const* SpellMgr::GetSpellInfo(uint32 spellId) const
{
    ...
    if (spellId)
        sLog->outError(LOG_FILTER_SERVER_LOADING, "%s: Try to get spellInfo for non-existing spell %u", __FUNCTION__, spellId);
    return nullptr;
}
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            if (entry->Effects[i]->Effect == SPELL_EFFECT_LEARN_SPELL)
            {
                ...
                for (SpellLearnSpellMap::const_iterator itr = db_node_bounds.first; itr != db_node_bounds.second; ++itr)
                {
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "Spell %u auto-learn spell %u in spell.dbc then the record in `spell_learn_spell` is redundant, please fix DB.", spell, dbc_node.spell);
                        found = true;
                        break;
                    }
                }
...
void SpellMgr::LoadSpellGroups()
{
    ...
    for (SpellGroupSpellMap::iterator itr = mSpellGroupSpell.begin(); itr != mSpellGroupSpell.end();)
    {
        if (itr->second < 0)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SpellGroup id %i listed in `spell_group` does not exist", itr->second);
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u spell group definitions in %u ms", uint32(mSpellSpellGroup.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    do
    {
        ...
        if (spell->AuraOptions.ProcTypeMask == 0)
        {
            ...
            const_cast<SpellInfo*>(spell)->IsProcAura = true;
            customProc++;
        }
        ++count;
    } while (result->NextRow());
...
void SpellMgr::LoadSkillLineAbilityMap()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u SkillLineAbility MultiMap Data in %u ms", static_cast<uint32>(mSkillLineAbilityMap.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadTalentSpellLinked()
{
    ...
    do
    {
        ...
        if (!spellInfo && talent)
        {
            sLog->outError(LOG_FILTER_SQL, "Spell %i listed in `spell_talent_linked_spell` does not exist", talent);
        ...
        if (!spellInfo)
        {
            sLog->outError(LOG_FILTER_SQL, "Spell %i listed in `spell_talent_linked_spell` does not exist", triger);
...
void SpellMgr::LoadSpellVisual()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 visual spells. DB table `spell_visual_kit` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Spell %u listed in `spell_visual_kit` does not exist", abs(spellId));
            continue;
        }
...
void SpellMgr::LoadSpellScene()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 visual spells. DB table `spell_scene_event` is empty.");
        return;
    }
...
void SpellMgr::LoadSpellTriggered()
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!GetSpellInfo(abs(spellId)))
            {
                sLog->outError(LOG_FILTER_SQL, "Spell %i listed in `spell_trigger_delay` does not exist", spellId);
...
inline void ApplySpellFix(std::initializer_list<uint32> spellIds, void(*fix)(SpellInfo*))
{
    for (auto spellID : spellIds)
    {
        ...
        if (!spellInfo)
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "Spell info correction specified for non-existing spell %u", spellID);
            continue;
        }
...
void SpellMgr::LoadSpellCustomAttr()
{
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        if (spellInfo->AuraOptions.CumulativeAura > 1)
    ...
    //Affix: Bolster
    ApplySpellFix({209859, 241593 }, [](SpellInfo* spellInfo)
    {
    ...
    // Greater Blessing of Kings , Greater Blessing of Wisdom , Death from Above, Soul Rip
    ApplySpellFix({203538, 203539, 152150, 220893}, [](SpellInfo* spellInfo)
    {
        spellInfo->GetMisc()->MiscData.Attributes[3] |= SPELL_ATTR3_CANT_TRIGGER_PROC;
    });
    ...
    // Fingers of Frost affect Ice Lance
    ...
    //Blade Rush Charge
    ApplySpellFix({124312}, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({136769,  //Horridon charge
    ...
    //Icy Fear Dmg
    ApplySpellFix({145735}, [](SpellInfo* spellInfo)
    {
        spellInfo->AuraRestrictions.TargetAuraSpell = 0;
    });
    ...
    //Residual Corruption
    ApplySpellFix({145073}, [](SpellInfo* spellInfo)
    {
        spellInfo->AuraRestrictions.TargetAuraSpell = 0;
    });
    ...
    // Fingers of Frost
    ApplySpellFix({112965}, [](SpellInfo* spellInfo)
    {
        for (int i = 0; i < 3; ++i)
        {
    ...
    // Sheilun's Gift
    ApplySpellFix({205406}, [](SpellInfo* spellInfo)
    {
    ...
    // Mana Drain
    ApplySpellFix({227545}, [](SpellInfo* spellInfo)
    {
        spellInfo->AttributesCu[0] |= SPELL_ATTR0_CU_NEGATIVE;
    });
    ...
    // Flame Rift, Decomposing Aura, Fragile Echoes
    ApplySpellFix({242983, 199721, 225281, 225292, 225294, 225297, 225298, 225366}, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({ 228174 }, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({197484, //Illysanna, Dark Rush
    ...
    ApplySpellFix({188492, //Rancid Maw
        204513, //Sigil of Flame
        227523 //Energy Void
    }, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->TargetA = TARGET_DEST_DEST;
    ...
    // Demonic Empowerment
    ApplySpellFix({193396}, [](SpellInfo* spellInfo)
    {
    ...
    //Fel Firebomb
    ApplySpellFix({190666}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->TargetA = TARGET_DEST_DEST;
        spellInfo->Effects[EFFECT_0]->TargetB = 0;
    });
    ...
    // Hand of Sacrifice, Dark Side of the Moon
    ApplySpellFix({6940, 213062, 227688, 227689, 227690, 227691, 227692, 239306, 239307}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_1]->BasePoints = 0;
    });
    ...
    // Bloodsail Bombardment
    ApplySpellFix({201871}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_1]->TriggerSpell = 0;
    });
        ...
        [](SpellInfo* spellInfo)
    {
        ...
        spellInfo->GetMisc()->MiscData.Attributes[1] |= SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE;
        spellInfo->AttributesCu[0] |= SPELL_ATTR0_CU_DONT_REMOVE_WITH_MECHANIC;
    });
    ...
    // Shadow Barrage
    ApplySpellFix({196659}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->TriggerSpell = 0;
    });
    ...
    ApplySpellFix({133234}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->BasePoints = spellInfo->Effects[EFFECT_0]->BasePoints / 10;
    });
    ...
    // Akaari's Soul
    ApplySpellFix({209835, 207354}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->TriggerSpell = 0; // From sniff triggered 209837 spell with delay 2s
    });
    ...
    // Way of the Crane (Honor Talent)
    ApplySpellFix({216161}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->RealPointsPerLevel = 0.f;
    });

    // Feet of Wind
    ApplySpellFix({240777}, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({236320}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_1]->TargetA = 87;
        spellInfo->Effects[EFFECT_1]->TargetB = 31;
    });

    // War Banner (Honor Talent)
    ApplySpellFix({236321, 138121, 138123}, [](SpellInfo* spellInfo)
    {
    ...
    {
        spellInfo->Effects[EFFECT_0]->BasePoints = 266;
        spellInfo->Effects[EFFECT_2]->BasePoints = 366;
        spellInfo->Misc.Duration.Duration = 600;
    });
    ...
    // Sudden Doom, Clearcasting (Feral)
    ApplySpellFix({81340, 135700}, [](SpellInfo* spellInfo)
    {
        spellInfo->AttributesCu[1] |= SPELL_ATTR1_CU_IS_USING_STACKS;
    });
    ...
    // Power Word: Fortitude (Honor Talent)
    ApplySpellFix({211681, 220841}, [](SpellInfo* spellInfo)
    {
        spellInfo->GetMisc()->MiscData.Attributes[5] &= ~SPELL_ATTR5_SINGLE_TARGET_SPELL;
    });

    // Ray of Frost
    ApplySpellFix({208141,208166, 221603, 221606}, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({32939}, [](SpellInfo* spellInfo)
    {
    ...
    // Death Grip
    ApplySpellFix({51399, 49575}, [](SpellInfo* spellInfo)
    {
        spellInfo->Categories.Mechanic = 0;
    });
    ...
    ApplySpellFix({ 22842 }, [](SpellInfo* spellInfo)
    {
        for (int i = 0; i < 3; ++i)
        {
            spellInfo->Effects[i]->PvPMultiplier = 0.5f;
        }
    });
    ...
    ApplySpellFix({ 210042, 198533 }, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->PvPMultiplier = 0.8f;
    });

    ApplySpellFix({ 190928 }, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({ 207946, 202743, 206416 }, [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({234867, 236763}, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->TriggerSpell = 0;
    });
    ...
    ApplySpellFix({ 233104 }, [](SpellInfo* spellInfo)
    {
        spellInfo->AuraInterruptFlags[0] = AURA_INTERRUPT_FLAG_CHANGE_MAP;
    });
    ...
    ApplySpellFix({ 233431 }, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0]->TargetA = TARGET_UNIT_TARGET_ENEMY;
        spellInfo->Effects[EFFECT_0]->TargetB = 0;
    });
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        {
            // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "SpellMgr::LoadSpellCustomAttr Id %u j %u", spellInfo->Id, j);
            ...
            switch (spellInfo->Effects[j]->ApplyAuraName)
            {
                ...
                case SPELL_AURA_PROC_MELEE_TRIGGER_SPELL:
                    if (!spellInfo->AuraOptions.ProcTypeMask)
                    {
                        spellInfo->AuraOptions.ProcChance = 100;
    ...
    do
    {
        ...
        if (!spellInfo2)
        {
            sLog->outError(LOG_FILTER_SQL, "spellId %u in `spell_radius` table is not found in dbcs, skipped", spellId);
            continue;
        }
...
void SpellMgr::LoadSpellInfoSpellSpecificAndAuraState()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded SpellInfo SpellSpecific and AuraState in %u ms", GetMSTimeDiffToNow(oldMSTime));
}