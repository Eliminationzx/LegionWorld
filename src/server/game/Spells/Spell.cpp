...
SpellCastTargets::SpellCastTargets() : m_targetMask(0), m_caster(nullptr), m_objectTarget(nullptr), m_itemTarget(nullptr), m_itemTargetEntry(0), m_pitch(0), m_speed(0) { }

SpellCastTargets::SpellCastTargets(Unit* caster, WorldPackets::Spells::SpellCastRequest const& spellCastRequest) : m_targetMask(spellCastRequest.Target.Flags), m_caster(caster),
    m_objectTarget(nullptr), m_itemTarget(nullptr), m_objectTargetGUID(spellCastRequest.Target.Unit), m_itemTargetGUID(spellCastRequest.Target.Item), m_itemTargetEntry(0), m_pitch(0.0f), m_speed(0.0f), m_strTarget(spellCastRequest.Target.Name)
{
...
m_interupted(false), m_replaced(triggerData.replaced), m_caster((info->HasAttribute(SPELL_ATTR6_CAST_BY_CHARMER) && caster->GetCharmerOrOwner()) ? caster->GetCharmerOrOwner() : caster),
m_spellValue(nullptr), m_originalTarget(nullptr), m_selfContainer(nullptr), m_casttime(0), m_castedTime(triggerData.casttime),
m_runesState(0), m_triggerData(triggerData), m_delayAtDamageCount(0), m_delayStart(0), m_referencedFromCurrentSpell(false), m_executedCurrently(false),
hasPredictedDispel(0), m_applyMultiplierMask(0), unitTarget(nullptr), itemTarget(nullptr),
gameObjTarget(nullptr), m_spellAura(nullptr), focusObject(nullptr), m_absorb(0), m_resist(0), m_blocked(0), m_channelTargetEffectMask(0),
m_spellState(SPELL_STATE_NULL), m_timer(0), _triggeredCastFlags(triggerData.triggerFlags), m_dispelResetCD(false), m_triggeredByAuraSpell(nullptr), m_triggeredByAura(nullptr), m_skipCheck(triggerData.skipCheck), m_spellMissMask(0),
m_auraScaleMask(0), m_currentExecutedEffect(SPELL_EFFECT_NONE), m_canLostTarget(true)
{
    if (!m_castedTime)
    ...
    canHitTargetInLOS = m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) || m_spellInfo->HasAttribute(SPELL_ATTR4_TRIGGERED) || (_triggeredCastFlags & TRIGGERED_IGNORE_LOS);
    ...
    m_canReflect = m_spellInfo->Categories.DefenseType == SPELL_DAMAGE_CLASS_MAGIC && !m_spellInfo->HasAttribute(SPELL_ATTR0_ABILITY)
        ...
        && !m_spellInfo->HasAttribute(SPELL_ATTR3_ONLY_TARGET_PLAYERS)
...
void Spell::SelectSpellTargets()
{
    ...
    if (m_targets.HasDst())
    {
        if (m_targets.HasTraj())
        {
            ...
            if (speed > 0.0f)
                m_delayMoment = static_cast<uint64>(floor(m_targets.GetDist2d() / speed * 1000.0f));
        }
        else if (m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed > 0.0f)
        {
            ...
            if (!m_spellInfo->HasAttribute(SPELL_ATTR9_SPECIAL_DELAY_CALCULATION))
                m_delayMoment = uint64(floor(dist / m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed * 1000.0f));
            else
                m_delayMoment = uint64(m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed * 1000.0f);
        }
...
void Spell::SelectImplicitBetweenTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList))
    {
        ...
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitBetweenTargets angle %f, dist %f, x %f, y %f, Id %u, targets.size %u", angle, dist, center->GetPositionX(), center->GetPositionY(), m_spellInfo->Id, targets.size());
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    float coneAngle = m_spellInfo->TargetRestrictions.ConeAngle;
...
void Spell::SelectImplicitTargetDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitTargetDestTargets SpellId %u canHitTargetInLOS %u", m_spellInfo->Id, canHitTargetInLOS);
    m_targets.SetDst(*target);
    m_targets.ModDst(pos);
}
...
void Spell::SelectImplicitDestDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitDestDestTargets SpellId %u canHitTargetInLOS %u position: %f %f %f", m_spellInfo->Id, canHitTargetInLOS, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_UNIT_PET:
            ...
            else
            {
                ...
                if (m_caster->HasAura(197248)) // Master of Beasts
                    if (Unit* hati = m_caster->GetHati())
                        AddUnitTarget(hati, 1 << effIndex, checkIfValid);
            }
            break;
...
void Spell::SelectImplicitChainTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, WorldObject* target, uint32 effMask)
{
    ...
    if (canHavoc && m_caster->getClass() == CLASS_WARLOCK)
    {
        ...
        if (Unit::AuraList* _auraList = m_caster->GetMyAura(80240))
            if (Aura* _aura = _auraList->front())
        ...
        if (Unit::AuraList* _auraList = m_caster->GetMyAura(200548))
            for (auto _aura : *_auraList)
                if (_aura)
                    if (Unit* unitTarget = _aura->GetUnitOwner())
                    {
                        if (m_caster->IsWithinCombatRange(unitTarget, max_range) && m_caster->IsWithinLOSInMap(unitTarget))
    ...
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitChainTargets maxTargets %i additionaltarget %u Id %i", maxTargets, additionaltarget, m_spellInfo->Id);
    ...
    if (additionaltarget || maxTargets > 1)
    {
        ...
        {
            if (m_spellInfo->EffectMask < uint32(1 << k))
                break;
        ...
        for (auto& target : targets)
        {
            if (Unit* unitTarget = target->ToUnit())
            {
                if (m_spellInfo->Id == 148022 && unitTarget == m_caster) // Hack for Icicle
                    continue;
...
void Spell::AddUnitTarget(Unit* target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/)
{
    ...
    if (m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed > 0.0f && m_caster != target)
    {
        float mindist = 5.0f;
        switch (m_spellInfo->Id)
        {
        ...
        if (!m_spellInfo->HasAttribute(SPELL_ATTR9_SPECIAL_DELAY_CALCULATION))
            targetInfo.timeDelay = uint64(floor(dist / m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed * 1000.0f));
        else
            targetInfo.timeDelay = uint64(m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed * 1000.0f);
    ...
    if (targetInfo.missCondition == SPELL_MISS_REFLECT)
    {
        ...
        {
            DamageInfo dmgInfoProc = DamageInfo(m_caster, target, 1, m_spellInfo, m_spellInfo ? SpellSchoolMask(m_spellInfo->GetMisc(m_diffMode)->MiscData.SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE, targetInfo.damageBeforeHit);
...
void Spell::AddGOTarget(GameObject* go, uint32 effectMask)
{
    ...
    // Spell have speed - need calculate incoming time
    if (m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed > 0.0f)
    {
        // calculate spell incoming interval
        ...
        if (dist < 5.0f)
            dist = 5.0f;
       if (!m_spellInfo->HasAttribute(SPELL_ATTR9_SPECIAL_DELAY_CALCULATION))
           target.timeDelay = uint64(floor(dist / m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed * 1000.0f));
       else
           target.timeDelay = uint64(m_spellInfo->GetMisc(m_diffMode)->MiscData.Speed * 1000.0f);
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    {
        bool initCombat = !m_spellInfo->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO) && !m_spellInfo->HasAttribute(SPELL_ATTR1_NO_THREAT);
    ...
    if (m_healing > 0)
    {
        ...
        if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
        {
            ...
            dmgInfoProc.SetMagnet(m_magnetGuid);
            dmgInfoProc.SetIsCrit(target->crit);
    ...
    else if (m_damage > 0)
    {
        ...
        damageInfo.damage = caster->GetDamageFromLevelScale(unitTarget, damageInfo.damage);
        ...
        uint32 _mss = GetMSTimeDiffToNow(_ss);
        if (_mss > 250)
            sLog->outDiff("Spell::DoAllEffectOnTarget m_damage Caster %u entry %u SpellId %u Pos %s wait %ums", m_caster->GetGUIDLow(), m_caster->GetEntry(), m_spellInfo->Id, m_caster->GetPosition().ToString().c_str(), _mss);
...
void Spell::prepare(SpellCastTargets const* targets)
{
    ...
    {
        if (m_caster->ToPlayer()->GetLastPotionId() && (m_CastItem->IsPotion() || m_spellInfo->IsCooldownStartedOnEvent() || IsItemCategoryCombat()))
        {
...
void Spell::cast(bool skipCheck)
{
    ...
    if (!procAttacker)
    {
        ...
        if(canEffectTrigger)
        {
            ...
            sLog->outDebug(LOG_FILTER_PROC, "Cast m_spellInfo->Id %i, m_damage %i, procDamage %i, procEx %i procAttacker %u positive %u", m_spellInfo->Id, m_damage, procDamage, procEx, procAttacker, positive);
            ...
            if(procAttacker)
            {
                if(infoTarget)
                {
                    ...
                    dmgInfoProc.SetCastTime(m_casttime);
                    dmgInfoProc.SetMagnet(m_magnetGuid);
                    caster->ProcDamageAndSpell(procTarget, procAttacker, procVictim, procEx, &dmgInfoProc, m_attackType, m_spellInfo, m_triggeredByAuraSpell, &m_appliedProcMods);
                }
                else
                {
                    DamageInfo dmgInfoProc = DamageInfo(caster, procTarget, procDamage, m_spellInfo, SpellSchoolMask(m_spellInfo->GetMisc(m_diffMode)->MiscData.SchoolMask), SPELL_DIRECT_DAMAGE, procDamage);
...
void Spell::_handle_immediate_phase()
{
    ...
    if (m_UniqueTargetInfo.empty())
    {
        if (m_targets.HasDst())
        {
            ...
            dmgInfoProc.SetCastTime(m_casttime);
            dmgInfoProc.SetMagnet(m_magnetGuid);
            m_originalCaster->ProcDamageAndSpell(nullptr, procAttacker, 0, m_procEx | PROC_EX_NORMAL_HIT, &dmgInfoProc, BASE_ATTACK, m_spellInfo, m_triggeredByAuraSpell);
        }
    }
    else
    {
...
void Spell::update(uint32 difftime)
{
        ...
        !m_caster->HasAuraCastWhileWalking(m_spellInfo))
    {
        ...
        {
            cancel();
            sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell %u is cancelled SPELL_INTERRUPT_FLAG_MOVEMENT", m_spellInfo->Id);
        }
    }
...
void Spell::finish(bool ok)
{
    ...
    #ifdef WIN32
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::finish Spell: %u m_spellMissMask %u", m_spellInfo->Id, m_spellMissMask);
    #endif
...
void Spell::TakeCastItem()
{
    ...
    if (expendable && withoutCharges)
    {
        { // donate
            ...
            sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 3 item  guid = %u, entry = %u, %s", m_CastItem->GetGUID().GetGUIDLow(), m_CastItem->GetEntry(), m_caster->ToPlayer()->GetInfoForDonate().c_str());
            uint8 index = 0;
    ...
    else
    {
        { // donate
            SQLTransaction transs = LoginDatabase.BeginTransaction();
            sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 6 item  guid = %u, entry = %u, %s", m_CastItem->GetGUID().GetGUIDLow(), m_CastItem->GetEntry(), m_caster->ToPlayer()->GetInfoForDonate().c_str());
            uint8 index = 0;
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_HISTORY_STATUS);
...
void Spell::TakePower()
{
    ...
    for (SpellPowerEntry const* power : powerData)
    {
        ...
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::TakePower hit %i, powerType %i, m_powerCost %i Id %u", hit, powerType, GetPowerCost(powerType), m_spellInfo->Id);
...
void Spell::TakeRunePower()
{
    ...
    for (uint8 i = 0; i < player->GetMaxPower(POWER_RUNES); ++i)
    {
        if (_runesCost && !player->GetRuneCooldown(i))
        {
            ...
            sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "TakeRunePower i %u cooldown %u", i, cooldown);
        }
    }
}
...
void Spell::HandleEffects(Unit* pUnitTarget, Item* pItemTarget, GameObject* pGOTarget, uint32 i, SpellEffectHandleMode mode)
{
    ...
    #ifdef WIN32
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell: %u Effect : %u, damage %i, mode %i idx %i preventDefault %u", m_spellInfo->Id, eff, damage, mode, i, preventDefault);
    #endif
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->GetEffect(i, m_diffMode)->Effect)
        {
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                if (m_spellInfo->GetEffect(i, m_diffMode)->TargetA.GetTarget() == TARGET_GAMEOBJECT_ITEM_TARGET && !m_targets.GetGOTarget() && (!pTempItem || !pTempItem->GetTemplate()->LockID || !pTempItem->IsLocked()))
                    return SPELL_FAILED_BAD_TARGETS;
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->GetEffect(i, m_diffMode)->ApplyAuraName)
        {
            ...
            case SPELL_AURA_AOE_CHARM:
            {
                ...
                if (m_spellInfo->GetEffect(i, m_diffMode)->ApplyAuraName == SPELL_AURA_MOD_CHARM || m_spellInfo->GetEffect(i, m_diffMode)->ApplyAuraName == SPELL_AURA_MOD_POSSESS)
                {
                    if (!m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET) && !m_caster->GetPetGUID().IsEmpty())
                        return SPELL_FAILED_ALREADY_HAVE_SUMMON;
...
SpellCastResult Spell::CheckItems()
{
    ...
    else
    {
        ...
        //Check recipe rank
        if (proto->Flags & ITEM_FLAG_HIDE_UNUSABLE_RECIPE)
        {
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        switch (m_spellInfo->GetEffect(i, m_diffMode)->Effect)
        {
            ...
            case SPELL_EFFECT_OBLITERATE_ITEM:
            {
                if (ItemTemplate const* itemProto = m_targets.GetItemTarget()->GetTemplate())
                {
                    if (!(itemProto->Flags3 & ITEM_FLAG3_OBLITERATABLE))
                        return SPELL_FAILED_BAD_TARGETS;
                }
                else
                    return SPELL_FAILED_BAD_TARGETS;
            ...
            case SPELL_EFFECT_DISENCHANT:
            {
                ...
                {
                    // special disenchant
                    if (!(itemProto->Flags2 & ITEM_FLAG2_DISENCHANT_TO_LOOT_TABLE))
                        return SPELL_FAILED_CANT_BE_DISENCHANTED;
                }
                break;
            }
...
SpellEvent::~SpellEvent()
{
    ...
    {
        sLog->outError(LOG_FILTER_SPELLS_AURAS, "~SpellEvent: %s %u tried to delete non-deletable spell %u. Was not deleted, causes memory leak.", (m_Spell->GetCaster()->IsPlayer() ? "Player" : "Creature"), m_Spell->GetCaster()->GetGUIDLow(), m_Spell->m_spellInfo->Id);
        //ASSERT(false);
    }
}
...
void Spell::HandleLaunchPhase()
{
    ...
    for (TargetInfo& info : m_UniqueTargetInfo)
    {
        if (!info.effectMask)
            continue;
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo& targetInfo, float* multiplier)
{
    ...
    if (targetInfo.damage || canCritTo)
        targetInfo.crit = (m_originalCaster ? m_originalCaster : m_caster)->isSpellCrit(unit, m_spellInfo, m_spellSchoolMask, m_attackType, critChance, this);

    if (targetInfo.crit)
        targetInfo.damageBeforeHit = caster->SpellCriticalDamageBonus(m_spellInfo, targetInfo.damageBeforeHit, unit);
}
    ...
    if (std::vector<SpellTargetFilter> const* spellTargetFilter = sSpellMgr->GetSpellTargetFilter(m_spellInfo->Id))
    {
        ...
        for (const auto& itr : *spellTargetFilter)
        {
            ...
            switch (itr.option)
            {
                ...
                {
                    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CustomTargetSelector 0");
                    if (_target)
                    {
                        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CustomTargetSelector 1 guid %u", _target->GetGUIDLow());
                        if(itr.param1 < 0.0f)
                ...
                case SPELL_FILTER_BY_DISTANCE_DEST: //19
                {
                    ...
                    if (m_targets.HasDst())
                    {
                        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CustomTargetSelector HasDst 1");
                        if(itr.param1 < 0.0f)
                    ...
                    {
                        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CustomTargetSelector HasSrc 1");
                        if(itr.param1 < 0.0f)
                    ...
                    for (auto const& target : targets)
                    {
                        if (auto player = target->ToPlayer())
                        {
                            ...
                            else if (player->IsMeleeDamageDealer(uint32(itr.param2))) //param2 == 1 - Also the Tank
                            {
                                if (!player->isInTankSpec())
                                    meleeList.push_back(target);
                                else if (uint32(itr.param2) == 1) // Tank
        ...
        // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CustomTargetSelector Id %u targetId %u effIndex %u targets %u caster %s", m_spellInfo->Id, targetId, effIndex, targets.size(), m_caster->GetGUID().ToString().c_str()) ;
    }
}
...
bool WorldObjectSpellTargetCheck::operator()(WorldObject* target)
{
    ...
    // #ifdef WIN32
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::WorldObjectSpellTargetCheck spell id %u caster %s target %s _condSrcInfo %u res %u ToCorpse %u ToUnit %u _targetSelectionType %u",
