...
float AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    bool CalcStack = bool(m_spellInfo->AuraOptions.CumulativeAura) && m_spellInfo->IsStack();
    ...
    GetBase()->CallScriptEffectBeforeCalcAmountHandlers(const_cast<AuraEffect const*>(this), m_send_baseAmount, m_canBeRecalculated);
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_LEECH:
        {
            ...
            amount = caster->SpellDamageBonusDone(target, m_spellInfo, amount, DOT, ExcludeAuraList, (SpellEffIndex) GetEffIndex());
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
        {
            ...
            if (GetBase()->GetComboPoints() && !m_spellInfo->CanModDuration(m_diffMode))
                m_amount_mod *= GetBase()->GetComboPoints();
            ...
            switch (GetId())
            {
                ...
                {
                    if (AuraEffect* auraEff = caster->GetAuraEffect(5217, EFFECT_0))
                ...
                case 603: // Doom
                {
                    if (AuraEffect const* aurEff = caster->GetAuraEffect(214225, EFFECT_0)) // Kazaaks Final Curse
                    {
                        ...
                        {
                            if (itr->HasUnitTypeMask(UNIT_MASK_CREATED_BY_CLASS_SPELL) || itr->isPet())
                            {
                                count++;
                            }
                        }
                        ...
                        for (uint8 i = 3; i < 5; ++i)
                            if (caster->m_SummonSlot[i])
                                if (Creature* crt = caster->GetMap()->GetCreature(caster->m_SummonSlot[i]))
                                    if (!crt->IsDespawn())
                                        crt->CastCustomSpell(crt, m_spellInfo->Id, &amount, &amount, nullptr, true);
                    break;
                }
                default:
                    break;
    ...
    if (CalcStack)
        m_amount_mod *= GetBase()->GetStackAmount();
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 225776:
        {
            if (AuraEffect* auraInfo = caster->GetAuraEffect(225774, EFFECT_0))
                amount = -1.f * auraInfo->GetAmount();
            break;
        }
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
        {
            ...
            if (m_spellInfo->Id == 210723) // Hack for Ashamane's Frenzy
                ...
                if(GetBase()->GetMaxDuration() != -1 && caster && GetTotalTicks())
    ...
    #ifdef WIN32
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "AuraApplication::CalculateAmount GetId %i GetAuraType %u amount %i effIndex %i m_amount_mod %f m_amount_add %i GetStackAmount %u", GetId(), GetAuraType(), amount, m_effIndex, m_amount_mod, m_amount_add, GetBase()->GetStackAmount());
    #endif
...
void AuraEffect::CalculateFromDummyAmount(Unit* caster, Unit* target, float &amount)
{
    ...
    if (std::vector<SpellAuraDummy> const* spellAuraDummy = sSpellMgr->GetSpellAuraDummy(GetId()))
    {
        ...
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "AuraEffect::CalculateFromDummyAmount end GetId %i, amount %i m_effIndex %u m_amount_mod %u", GetId(), amount, m_effIndex, m_amount_mod);
    }
}
                ...
                if (GetAmount() == -100)
                    if (Player* plr = target->ToPlayer())
                    {
                        for (PlayerSpellMap::const_iterator itr = plr->GetSpellMapConst().begin(); itr != plr->GetSpellMapConst().end(); ++itr)
                        {
                            ...
                            if (spellInfo->IsAffectedBySpellMod(m_spellmod) && spellInfo->GetRecoveryTime() <= 300000)
...
void AuraEffect::RecalculateTickPeriod(Unit* caster)
{
    if (m_period)
    {
        ...
        if (caster)
        {
            if (m_spellInfo->Id == 145108)
            {
                if (auto aurEff = caster->GetAuraEffect(253434, EFFECT_0)) // for Item - Druid T21 Restoration 4P Bonus
                {
                    int32 amplt_mod = 1.f - (aurEff->GetAmount() * 5.f / 100.f);
                    m_period /= amplt_mod;
                }
            }
...
void AuraEffect::HandleAuraTrackCreatures(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetId() == 5225)
        ...
        if (!apply && target->HasAuraTypeWithMiscvalue(SPELL_AURA_TRACK_CREATURES, GetMiscValue()))
...
void AuraEffect::HandleAuraModStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (int32 i = STAT_STRENGTH; i < MAX_STATS; i++)
    {
        ...
        {
            target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), BASE_VALUE, float(GetAmount()), apply);
        }
    }
}
    ...
    else
    {
        ...
        if (target->isAlive())
        {
            if (apply && otherLogick)
                ...
                target->SetHealth(target->CountPctFromMaxHealth(int32(ceil(percent))));
        }
    }
}
...
void AuraEffect::HandleAuraModAttackPower(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    aurApp->GetTarget()->UpdateAttackPowerAndDamage();
}
...
void AuraEffect::HandleAuraBonusArmor(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetAuraType() == SPELL_AURA_MOD_BONUS_ARMOR)
        player->HandleStatModifier(UNIT_MOD_ARMOR, TOTAL_VALUE, (float) GetAmount(), apply);
...
void AuraEffect::HandleShieldBlockValue(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (auto player = aurApp->GetTarget()->ToPlayer())
        player->HandleBaseModValue(SHIELD_BLOCK_VALUE, FLAT_MOD, float(GetAmount()), apply);
}
    ...
    switch (GetSpellInfo()->ClassOptions.SpellClassSet)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (GetSpellInfo()->Id)
            {
                ...
                {
                    float percHeal = m_amount;
                    if(AuraEffect* aurEff = caster->GetAuraEffect(210667, EFFECT_4)) // Ekowraith, Creator of Worlds
                        percHeal += CalculatePct(percHeal, aurEff->GetAmount());
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster, SpellEffIndex effIndex, bool lastTick) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        switch (GetId())
        {
            ...
            default:
            {
                if (!m_spellInfo->HasAttribute(SPELL_ATTR10_STACK_DAMAGE_OR_HEAL))
                {
                    ...
                    if (m_spellInfo->Id == 155722 || m_spellInfo->Id == 1079 || m_spellInfo->Id == 106830 || m_spellInfo->Id == 210705) // Rake, Rip, Thrash, Ashamane's Rip
                    {
        ...
        if (caster->getClass() == CLASS_ROGUE && ((GetSpellInfo()->GetAllEffectsMechanicMask() & (1 << MECHANIC_BLEED)) || (GetSpellInfo()->GetDispelMask() & (1 << DISPEL_POISON))))
    ...
    sLog->outInfo(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: Caster %u attacked %u dmg %u damage %i GetId %u absorb %u", GetCasterGUID().GetGUIDLow(), target->GetGUIDLow(), dmg, damage, GetId(), absorb);
    ...
    pInfo.damage = caster->GetDamageFromLevelScale(target, pInfo.damage);
...
void AuraEffect::HandlePeriodicWeaponPercentDamageAurasTick(Unit* target, Unit* caster, SpellEffIndex effIndex) const
{
    ...
    uint32 damage = uint32(weaponDamage * weaponDamagePercentMod);
    uint32 damageBeforeHit = 0;
    ...
    sLog->outInfo(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: Caster %u attacked %u damage %u GetId %u absorb %u", GetCasterGUID().GetGUIDLow(), target->GetGUIDLow(), damage, GetId(), absorb);
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster, SpellEffIndex effIndex) const
{
    ...
    damage = caster->GetDamageFromLevelScale(target, damage);
    ...
    if (caster->isAlive())
    {
        ...
        uint32 heal = uint32(damage * gainMultiplier);
...
void AuraEffect::HandlePeriodicHealthFunnelAuraTick(Unit* target, Unit* caster, SpellEffIndex /*effIndex*/) const
{
    ...
    caster->ModifyHealth(-(int32)damage, nullptr, GetId());
                            ...
                            switch (creature->GetEntry())
                            {
                                ...
                                case 106551:
                                {
                                    if (creature->isAlive())
                                    {
                                        uint32 _heal = damage;
                                        uint32 _absorb = 0;
                                        caster->CalcHealAbsorb((*itr), GetSpellInfo(), _heal, _absorb);
                                        int32 _gain = caster->DealHeal((*itr), _heal, GetSpellInfo());
                                        SpellPeriodicAuraLogInfo _pInfo(this, _heal, _heal - _gain, _absorb, 0, 0.0f, crit);
                                        (*itr)->SendPeriodicAuraLog(&_pInfo);
                                    }
                                    break;
                                }
                            }
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit* target, Unit* caster, SpellEffIndex effIndex) const
{
    ...
    damageInfo.damage = caster->GetDamageFromLevelScale(damageInfo.target, damageInfo.damage);
...
void AuraEffect::HandleProcTriggerDamageAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo, SpellEffIndex effIndex)
{
    ...
    damageInfo.damage = target->GetDamageFromLevelScale(damageInfo.target, damageInfo.damage);
...
void AuraEffect::HandleAnimReplacementSet(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    if (Unit* target = aurApp->GetTarget())
        target->SetAnimKitId(GetSpellInfo()->GetEffect(GetEffIndex(), m_diffMode)->MiscValue);   
}
    ...
    for (auto& itr : *cd_list)
    {
        ...
        switch (GetId())
        {
            ...
            case 214170: // Spiritual Journey
                if (spellInfo->Id == 51533 || spellInfo->Id == 79914 || spellInfo->Id == 165309 || spellInfo->Id == 176560)
                    amount = GetAmount();
                break;
    ...
    for (auto& itr : *charge_list)
    {
        ...
        switch (GetId())
        {
            //case 209376: // Forbearant Faithful
                ...
                //break;
            case 214170: // Spiritual Journey
                if (data.spellInfo->Id == 51533 || data.spellInfo->Id == 79914 || data.spellInfo->Id == 165309 || data.spellInfo->Id == 176560)
                    amount = GetAmount();
                break;
            case 202776: // Blurred Time
                amount = GetAmount();
...
void AuraEffect::HandleAuraProcOnHpBelow(AuraApplication const* auraApp, uint8 mode, bool /*apply*/) const
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "HandleAuraProcOnHpBelow GetId %u triggered_spell_id %u curCount %u curHealth %u", GetId(), triggered_spell_id, curCount, curHealth);
