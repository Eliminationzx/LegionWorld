...
bool ChatLink::ValidatePlayerGUID(std::istringstream& iss, std::string const& context/*, uint32 realmId, uint32 lowGUID*/)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading %s PlayerGUID string", iss.str().c_str(), context.c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading %s realmID", iss.str().c_str(), context.c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading %s lowGUID", iss.str().c_str(), context.c_str());
        return false;
    }
...
bool ChatLink::ValidateGuildGUID(std::istringstream& iss, std::string const& context/*, uint32 realmId, uint32 lowGUID*/)
{
    ...
    if (strcmp(buffer, "Guild"))
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading %s GuildGUID string", iss.str().c_str(), context.c_str());
        return false;
    }
...
bool LinkExtractor::IsValidMessage()
{
    ...
    while (!_iss.eof())
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence aborted unexpectedly. Char \"%c\"", _iss.str().c_str(), static_cast<char>(_iss.get()));
            return false;
        }
...
bool ItemChatLink::Initialize(std::istringstream& iss)
{
    ...
    if (!goodColor)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): linked item has unknown color %X", iss.str().c_str(), _color);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item enchantId", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item gem id 1", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item gem id 2", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item gem id 3", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading zero", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item random property id", iss.str().c_str());
        return false;
    }
    ...
    if (_randomPropertyId > 0)
    {
        ...
        if (!_property)
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid item property id %u in |item command", iss.str().c_str(), _randomPropertyId);
            return false;
        }
    }
    else if (_randomPropertyId < 0)
    {
        ...
        if (!_suffix)
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid item suffix id %u in |item command", iss.str().c_str(), -_randomPropertyId);
            return false;
        }
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item random property seed", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item owner level", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item owner spec", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item modifiers mask", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item context", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item bonus lists size", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): too many item bonus list IDs %u in |item command", iss.str().c_str(), numBonusListIDs);
        return false;
    }
    ...
    for (uint32 index = 0; index < numBonusListIDs; ++index)
    {
        ...
        if (!ReadInt32(iss, id))
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item bonus list id (index %u)", iss.str().c_str(), index);
            return false;
        }

        if (!sDB2Manager.GetItemBonusList(id))
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid item bonus list id %d in |item command", iss.str().c_str(), id);
            return false;
        }
    ...
    for (uint32 i = 0; i < MAX_ITEM_MODIFIERS; ++i)
    {
        if (modifiersMask & (1 << i))
        {
            ...
            if (!ReadInt32(iss, id))
            {
                sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item modifier id (index %u)", iss.str().c_str(), i);
                return false;
            }
    ...
    for (uint32 i = 0; i < MAX_ITEM_PROTO_SOCKETS; ++i)
    {
        ...
        if (HasValue(iss) && !ReadUInt32(iss, numBonusListIDs))
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item bonus lists size for gem %u", iss.str().c_str(), i);
            return false;
        }

        if (numBonusListIDs > maxBonusListIDs)
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): too many item bonus list IDs %u in |item command for gem %u", iss.str().c_str(), numBonusListIDs, i);
            return false;
        }
        ...
        for (uint32 index = 0; index < numBonusListIDs; ++index)
        {
            ...
            if (!ReadInt32(iss, id))
            {
                sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item bonus list id (index %u) for gem %u", iss.str().c_str(), index, i);
                return false;
            }

            if (!sDB2Manager.GetItemBonusList(id))
            {
                sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid item bonus list id %d in |item command for gem %u", iss.str().c_str(), id, i);
                return false;
            }
...
{
    std::stringstream ss;
    ss << _item->Name->Str[index];

    if (!(_item->Flags3 & ITEM_FLAG3_HIDE_NAME_SUFFIX))
        if (suffixStrings)
...
bool ItemChatLink::ValidateName(char* buffer, char const* context)
{
    ...
    for (uint8 index = LOCALE_enUS; index < MAX_LOCALES; ++index)
    {
        ...
        std::string localeItemName = std::string(_item->Name->Str[index]);
...
bool QuestChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): quest level %d is broken", iss.str().c_str(), _questLevel);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): _minLevel %d is broken", iss.str().c_str(), _minLevel);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): _maxLevel %d is broken", iss.str().c_str(), _maxLevel);
        return false;
    }
    return true;
}
...
bool SpellChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading spell unk", iss.str().c_str());
        return false;
    }
...
bool TradeChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading trade skillID", iss.str().c_str());
        return false;
    }
...
bool CurrencyChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading currency entry", iss.str().c_str());
        return false;
    }
    ...
    if (!currencyInfo)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid talent id %u in |currency command", iss.str().c_str(), _currencyId);
        return false;
    }

    if (_color != ItemQualityColors[currencyInfo->Quality])
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): linked currency has color %u, but user claims %u", iss.str().c_str(), ItemQualityColors[currencyInfo->Quality], _color);
        return false;
    }
...
bool BattlePetChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading battlepet species entry", iss.str().c_str());
        return false;
    }
    ...
    if (!speciesInfo)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid species id %u in |battlepet command", iss.str().c_str(), speciesID);
        return false;
    }
    ...
    for (uint8 index = 0; index < propsCount; ++index)
    {
        ...
        if (!ReadUInt32(iss, _data[index]))
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading battlepet property (%u)", iss.str().c_str(), index);
            return false;
        }
    }
    ...
    if (!ReadUInt32(iss, unk))
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading battlepet unk", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading battlepet unk1", iss.str().c_str());
        return false;
    }*/
...
bool GarrFollowerChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading garrfollower entry", iss.str().c_str());
        return false;
    }
    ...
    for (uint8 index = 0; index < propsCount; ++index)
    {
        ...
        if (!ReadUInt32(iss, _data[index]))
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading garrfollower property (%u)", iss.str().c_str(), index);
            return false;
        }
    }
...
bool GarrFollowerAbilityChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading garrfollowerability entry", iss.str().c_str());
        return false;
    }
    ...
    if (!abilityInfo)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid ability id %u in |garrfollowerability command", iss.str().c_str(), abilityID);
        return false;
    }
...
bool GarrMissionChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading garrmission entry", iss.str().c_str());
        return false;
    }
    ...
    if (!missionInfo)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid ability id %u in |garrmission command", iss.str().c_str(), missionID);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading garrmission DbID", iss.str().c_str());
        return false;
    }
...
bool InstanceLockChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading instancelock mapID", iss.str().c_str());
        return false;
    }
    ...
    if (!mapEntry)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid map id %u in |instancelock command", iss.str().c_str(), mapID);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading instancelock difficultyID", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading instancelock completedMask", iss.str().c_str());
        return false;
    }
...
bool KeystoneChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading keystone challengeID", iss.str().c_str());
        return false;
    }
    ...
    if (!challengeEntry)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid challengeID %u in |keystone command", iss.str().c_str(), challengeID);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading keystone challengeLevel", iss.str().c_str());
        return false;
    }

    if (challengeLevel > 30)
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got too big challengeLevel %u in |keystone command", iss.str().c_str(), challengeLevel);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading keystone affix1", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading keystone affix2", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading keystone affix3", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got too big affixes ID %u %u %u in |keystone command", iss.str().c_str(), affixes[0], affixes[1], affixes[2]);
        return false;
    }
    ...
    if (challengeLevel >= 4)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid affix1 %u in |keystone command", iss.str().c_str(), affixes[0]);
            return false;
        }
        ...
        if (challengeLevel >= 7)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid affix2 %u in |keystone command", iss.str().c_str(), affixes[1]);
                return false;
            }
            ...
            if (challengeLevel >= 10)
            {
                ...
                {
                    sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid affix3 %u in |keystone command", iss.str().c_str(), affixes[2]);
                    return false;
                }
            }
            else
            {
                ...
                if (affixes[2])
                {
                    sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): challenge level %u isn't matched of affixes %u %u %u in |keystone command", iss.str().c_str(), challengeLevel, affixes[0], affixes[1], affixes[2]);
                    return false;
                }
            }
        }
...
bool TransmogIllusionChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading transmogillusion enchantID", iss.str().c_str());
        return false;
    }
...
bool TransmogAppearanceChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading transmogappearance entry", iss.str().c_str());
        return false;
    }
...
bool JournalChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading journal typeID", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading journal entryID", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading journal difficultyMask", iss.str().c_str());
        return false;
    }
...
bool ArtifactPowerChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading apower powerID", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): got invalid powerID %u in |apower command", iss.str().c_str(), powerID);
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading apower unkRankValue", iss.str().c_str());
        return false;
    }
    ...
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading apower unkTierValue", iss.str().c_str());
        return false;
    }
