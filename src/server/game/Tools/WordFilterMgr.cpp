...
void WordFilterMgr::LoadLetterAnalogs()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING,">> Loaded %u (%u) letter analogs in %u ms", count, m_letterAnalogs.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void WordFilterMgr::LoadBadSentences()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 bad sentences. DB table `bad_sentences` is empty!");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u bad sentences in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void WordFilterMgr::LoadComplaints()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 report_complaints. DB table `report_complaints` is empty!");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u report_complaints in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
std::string WordFilterMgr::FindBadWord(std::string const& text, bool mail)
{
    ...
    for (BadWordMap::const_iterator it = m_badWords.begin(); it != m_badWords.end(); ++it)
    {
        ...
        {
            // sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Compare text %s _text %s *it %s", text.c_str(), boost::locale::conv::utf_to_utf<char>(_text).c_str(), boost::locale::conv::utf_to_utf<char>(*it).c_str());
    ...
    if(mail)
    {
        ...
        {
        //    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Compare %s and %s", boost::locale::conv::utf_to_utf<char>(*it).c_str(), boost::locale::conv::utf_to_utf<char>(_text).c_str());
            if (_text.find(*it) != std::wstring::npos)
                return boost::locale::conv::utf_to_utf<char>(*it);
        }
    }
...
void WordFilterMgr::GeneralFilterWstring(std::wstring& _text, LocaleConstant locale /*= LOCALE_enUS*/, bool* isValid /*= nullptr*/)
{
    ...
    for (uint8 i = 0; i < _text.size() && i < 50;)
    {
     ...
     /*   auto it = m_letterAnalogs.find(_text[i]);
        ...
        if (posItr == std::wstring::npos) // if this char not a-z а-я, then erase it
        {
            if ((locale == LOCALE_ruRU || locale == LOCALE_enUS) && isValid)
