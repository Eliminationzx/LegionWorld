...
{
    { "characters",                       DTT_CHARACTER, "`guid`, `account`, `name`, `slot`, `race`, `class`, `level`, `xp`, `money`, `skin`, `face`, `hairStyle`, `hairColor`, `facialStyle`, `blindfold`, `gender`, `tattoo`, `horn`, `playerFlags`, `position_x`, `position_y`, `position_z`, `map`, `instance_id`, `orientation`, `taximask`, `online`, `cinematic`, `totaltime`, `leveltime`, `logout_time`, `is_logout_resting`, `rest_bonus`, `trans_x`, `trans_y`, `trans_z`, `trans_o`, `transguid`, `extra_flags`, `at_login`, `zone`, `death_expire_time`, `taxi_path`, `totalKills`, `todayKills`, `yesterdayKills`, `chosenTitle`, `watchedFaction`, `drunk`, `health`, `mana`, `latency`, `activespec`, `specialization`, `exploredZones`, `equipmentCache`, `knownTitles`, `actionBars`, `currentpetnumber`, `petslot`, `grantableLevels`, `deleteInfos_Account`, `deleteInfos_Name`, `deleteDate`, `transfer`, `transfer_request`, `transfer_history`"},
  ...
  //  { "character_talent",                 DTT_CHAR_TABLE, "guid, talent, spec"},
    ...
    { "item_instance",                    DTT_ITEM,       "`guid`, `itemEntry`, `owner_guid`, `creatorGuid`, `giftCreatorGuid`, `count`, `duration`, `charges`, `flags`, `enchantments`, `randomPropertyId`, `durability`, `playedTime`, `text`, `upgradeId`, `battlePetSpeciesId`, `battlePetBreedData`, `battlePetLevel`, `battlePetDisplayId`, `bonusListIDs`, `randomPropertyType`"},
    ...
    while (!feof(fin))
    {
        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                ...
                gender = uint8(atol(GetNth(line, 16).c_str()));
                ...
                if (name == "")
                {
                    ...
                    if (result)
                        if (!ChangeNth(line, 40, "1"))       // characters.at_login set to "rename on login"
                            ROLLBACK(DUMP_FILE_BROKEN);
                }
                ...
                if (!ChangeNth(line, 62, null))             // characters.deleteInfos_Account
                    ROLLBACK(DUMP_FILE_BROKEN);
                if (!ChangeNth(line, 63, null))             // characters.deleteInfos_Name
                    ROLLBACK(DUMP_FILE_BROKEN);
            ...
            case DTT_CHAR_VS_FOLLOW:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_garrison_followers.guid line: '%s'!", line.c_str());
                    ROLLBACK(DUMP_FILE_BROKEN);
                }
                if (!ChangeNth(line, 1, lastfollowerid))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_garrison_followers.dbId update line: '%s'!", line.c_str());
                    ROLLBACK(DUMP_FILE_BROKEN);
                }
                break;
            }
    ...
    while (pos != std::string::npos)
    {
        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                if (!ChangeNth(line, 1, newguid))           // characters.guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.guid update line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }

                if (!ChangeNth(line, 2, chraccount))        // characters.account update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.account update line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                ...
                if (name == "")
                {
                    ...
                    if (result)
                        if (!ChangeNth(line, 40, "1"))       // characters.at_login set to "rename on login"
                        {
                            sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.at_login set to rename on login line: '%s'!", line.c_str());
                            return DUMP_FILE_BROKEN;
                        }
                }
                else if (!ChangeNth(line, 3, name.c_str())) // characters.name
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.name line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                ...
                if (!ChangeNth(line, 62, null))             // characters.deleteInfos_Account
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.deleteInfos_Account line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 63, null))             // characters.deleteInfos_Name
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.deleteInfos_Name line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 64, null))             // characters.deleteDate
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: characters.deleteDate line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_CHAR_TABLE:
            {
                if (!ChangeNth(line, 1, newguid))           // character_*.guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_QUEST_TABLE:
            {
                if (!ChangeNth(line, 1, newguid))           // character_*.guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_queststatus.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 2, chraccount))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_queststatus.account update line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_EQSET_TABLE:
            {
                if (!ChangeNth(line, 1, newguid))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_equipmentsets.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                ...
                if (!ChangeNth(line, 2, newSetGuid))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_equipmentsets.setguid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_INVENTORY:
            {
                if (!ChangeNth(line, 1, newguid))           // character_inventory.guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_inventory.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }

                if (!ChangeGuid(line, 2, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed(), true))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_inventory.bag line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeGuid(line, 4, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_inventory.item line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_VS_TABLE:
            {
                ...
                if (!ChangeNth(line, 1, newItemId))           // character_void_storage.itemId update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_void_storage.itemId line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 2, newguid))           // character_void_storage.playerGuid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_void_storage.playerGuid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }            // character_void_storage.itemGuid update
                if (!ChangeGuid(line, 9, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_void_storage.itemGuid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_MAIL:                                  // mail
            {
                if (!ChangeGuid(line, 1, mails, sObjectMgr->_mailId))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: mail.id line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 6, newguid))           // mail.receiver update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: mail.receiver line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_MAIL_ITEM:                             // mail_items
            {
                if (!ChangeGuid(line, 1, mails, sObjectMgr->_mailId))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: mail_items.id line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeGuid(line, 2, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: mail_items.item_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 3, newguid))           // mail_items.receiver
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: mail_items.receiver line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_ITEM:
            {
                ...
                if (!ChangeGuid(line, 1, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: item_instance.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 3, newguid))           // item_instance.owner_guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: item_instance.owner_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_ITEM_GIFT:
            {
                if (!ChangeNth(line, 1, newguid))           // character_gifts.guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_gifts.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeGuid(line, 2, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_gifts.item_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_DONA_TABLE:
            {
                if (!ChangeNth(line, 1, newguid))           // character_donate.owner_guid update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_donate.owner_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeGuid(line, 2, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_donate.itemguid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_PET:
            {
                ...
                if (!ChangeNth(line, 1, newpetid))          // character_pet.id update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_pet.id line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 3, newguid))           // character_pet.owner update
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_pet.owne line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
            ...
            case DTT_PET_TABLE:                             // pet_aura, pet_spell, pet_spell_cooldown
            {
                ...
                if (petids_iter == petids.end())             // couldn't find new inserted id
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: pet line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
            ...
            case DTT_LOGIN:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: store_history.char_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeGuid(line, 7, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: store_history.item_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_ITEM_TABLE:
            {
                if (!ChangeGuid(line, 1, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: item_instance_*.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_ARTI_TABLE:
            {
                ...
                if (!ChangeGuid(line, 1, items, sObjectMgr->GetGenerator<HighGuid::Item>()->GetNextAfterMaxUsed()))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: item_instance_artifact.itemGuid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                if (!ChangeNth(line, 2, newguid))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: item_instance_artifact.char_guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
            case DTT_CHAR_TRANSMOG:
            {
                if (!ChangeNth(line, 1, newguid))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_transmog_outfits.guid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                ...
                if (!ChangeNth(line, 2, newSetGuid))
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "LoadPlayerDump: character_transmog_outfits.setguid line: '%s'!", line.c_str());
                    return DUMP_FILE_BROKEN;
                }
                break;
            }
