...
LootStore::LootStore(std::string name, std::string entryName, bool ratesAllowed, std::string addColum): m_name(name), m_entryName(entryName), m_ratesAllowed(ratesAllowed), m_addColum(addColum)
{
...
LootStoreItem::LootStoreItem(uint32 _itemid, uint8 _type, float _chanceOrQuestChance, uint16 _lootmode, uint8 _group, int32 _mincountOrRef, uint32 _maxcount) : itemid(_itemid), type(_type), chance(fabs(_chanceOrQuestChance)), mincountOrRef(_mincountOrRef), maxcount(_maxcount),
lootmode(_lootmode), group(_group), needs_quest(_chanceOrQuestChance < 0)
{
}
// Checks if the entry (quest, non-quest, reference) takes it's chance (at loot generation)
// RATE_DROP_ITEMS is no longer used for all types of entries
bool LootStoreItem::Roll(bool rate, bool isDungeon /* = false*/) const
{
    // sLog->outDebug(LOG_FILTER_LOOT, "LootStoreItem::Roll chance %f rate %u mincountOrRef %i type %i itemid %u", chance, rate, mincountOrRef, type, itemid);
    ...
    if (type == LOOT_ITEM_TYPE_ITEM)
    {
        ...
        if (isDungeon && sWorld->getBoolConfig(RATE_DROP_DUNGEON_ONLY_X1))
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "LootItem::AddAllowedLooter ItemID %u player %s", item.ItemID, player->GetGUID().ToString().c_str());
...
void Loot::AddItem(LootStoreItem const & item)
{
    ...
    if (item.type != LOOT_ITEM_TYPE_CURRENCY && (item.needs_quest || items.size() < MAX_NR_LOOT_ITEMS))
    {
        ...
        if (pProto && pProto->Quality == ITEM_QUALITY_LEGENDARY && pProto->RequiredExpansion == EXPANSION_LEGION)
            generatedLoot.item.ItemBonus.BonusListIDs = sObjectMgr->GetItemBonusTree(generatedLoot.item.ItemID, 0, m_lootOwner->getLevel(), 0, 0);
        else if (_isEmissaryLoot)
        {
            ...
            generatedLoot.item.ItemBonus.BonusListIDs = sObjectMgr->GetItemBonusTree(generatedLoot.item.ItemID, _itemContext, m_lootOwner->getLevel(), _levelBonus, _challengeLevel);

        generatedLoot.item.ItemBonus.Context = _itemContext;
    }

    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::AddItem ItemID %i needs_quest %u personal %u isOnlyQuest %u chance %f", generatedLoot.item.ItemID, item.needs_quest, personal, isOnlyQuest, item.chance);
    ...
    if (item.needs_quest)
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::AddItem ItemID %i quest_items %u", generatedLoot.item.ItemID, quest_items.size());
    }
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool noGroup, bool noEmptyError, WorldObject const* lootFrom)
{
        ...
        if (uint32 instanceId = go->GetInstanceId())
            if (Scenario* progress = sScenarioMgr->GetScenario(instanceId))
                if (_challenge = progress->GetChallenge())
                    if (_challenge->_complete && !_challenge->_noLoot)
                        _itemContext = sChallengeMgr->GetLootTreeMod(_levelBonus, _challengeLevel, _challenge);

    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillLoot objEntry %i lootId %i isBoss %i DifficultyID %i personal %i isRareOrGo %u noGroup %u isOnlyQuest %u _itemContext %u store %s", objEntry, lootId, isBoss, _DifficultyID, personal, isRareOrGo, noGroup, isOnlyQuest, _itemContext, store.GetName());
    ...
    if (tab)
    {
        if (_isEmissaryLoot || _isTokenLoot || _isItemLoot)
    ...
    if (!noGroup && group)
    {
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillLoot !noGroup && group GetMembersCount %u", group->GetMembersCount());
    ...
    else if (isOnlyQuest && (creature && creature->CanShared() || isBoss))
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillLoot isOnlyQuest && creature && creature->CanShared size %u", savethreatlist->size());
    ...
    {
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillLoot else");
        FillNotNormalLootFor(lootOwner, true);
    }

    if (noGroup && personal || _challengeLevel && !isOploteChest) // Add looters to personal loot
...
void Loot::FillNotNormalLootFor(Player* player, bool presentAtLooting)
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillNotNormalLootFor plguid %u presentAtLooting %u PlayerQuestItems %u", plguid, presentAtLooting, PlayerQuestItems);
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillQuestLoot quest_items %u isOnlyQuest %u", quest_items.size(), isOnlyQuest);
    ...
    for (size_t i = 0; i < quest_items.size(); ++i)
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillQuestLoot freeforall %i is_blocked %u is_looted %u ItemID %u AllowedForPlayer %u unlootedCount %u", item.freeforall, item.is_blocked, item.is_looted, item.item.ItemID, AllowedForPlayer(player, item.item.ItemID, item.type, item.needs_quest), unlootedCount);
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillQuestLoot ql %u unlootedCount %u", ql->size(), unlootedCount);
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillNonQuestNonFFAConditionalLoot GetGUIDLow %u presentAtLooting %u GetLootMethod %i", player->GetGUIDLow(), presentAtLooting, player->GetGroup() ? player->GetGroup()->GetLootMethod() : -1);
    ...
    for (size_t i = 0; i < items.size(); ++i)
    {
        ...
        if (!item.is_looted && !item.freeforall && !item.currency && (AllowedForPlayer(player, item.item.ItemID, item.type, item.needs_quest, &item) || (item.follow_loot_rules && player->GetGroup() && ((player->GetGroup()->GetLootMethod() == MASTER_LOOT && player->GetGroup()->GetLooterGuid() == player->GetGUID()) || player->GetGroup()->GetLootMethod() != MASTER_LOOT))))
        {
            if (presentAtLooting)
            {
                ...
                // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillNonQuestNonFFAConditionalLoot ItemID %u allowToLooter %i", item.item.ItemID, allowToLooter);
                if (allowToLooter)
...
void Loot::FillPersonalLootFor(Player* lootOwner)
{
    ...
    {
        // sLog->outDebug(LOG_FILTER_LOOT, "LootItem::FillPersonalLootFor GetLootMethod %u", group->GetLootMethod());
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* player = itr->getSource())
            {
                ...
                for (size_t i = 0; i < items.size(); ++i)
                {
                    ...
                    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::FillPersonalLootFor freeforall %i is_blocked %u is_looted %u ItemID %u AllowedForPlayer %u is_underthreshold %u", item.freeforall, item.is_blocked, item.is_looted, item.item.ItemID, AllowedForPlayer(player, item.item.ItemID, item.type, item.needs_quest), item.is_underthreshold);
...
void Loot::generateMoneyLoot(uint32 minAmount, uint32 maxAmount, bool isDungeon /*=false*/)
{
    if (maxAmount > 0)
    {
        if (maxAmount <= minAmount)
            gold = uint32(maxAmount * (isDungeon && sWorld->getBoolConfig(RATE_DROP_DUNGEON_ONLY_X1) ? 1.0f : sWorld->getRate(RATE_DROP_MONEY)));
        else if ((maxAmount - minAmount) < 32700)
            gold = uint32(urand(minAmount, maxAmount) * (isDungeon && sWorld->getBoolConfig(RATE_DROP_DUNGEON_ONLY_X1) ? 1.0f : sWorld->getRate(RATE_DROP_MONEY)));
        else
            gold = uint32(urand(minAmount >> 8, maxAmount >> 8) * (isDungeon && sWorld->getBoolConfig(RATE_DROP_DUNGEON_ONLY_X1) ? 1.0f : sWorld->getRate(RATE_DROP_MONEY))) << 8;
    }
}
...
void Loot::BuildLootResponse(WorldPackets::Loot::LootResponse& packet, Player* viewer, PermissionTypes permission, ItemQualities threshold /*= ITEM_QUALITY_POOR*/) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::BuildLootResponse lootPlayerQuestItems size %i", lootPlayerQuestItems.size());

    QuestItemMap::const_iterator q_itr = lootPlayerQuestItems.find(viewer->GetGUIDLow());
    if (q_itr != lootPlayerQuestItems.end())
    {
        QuestItemList* q_list = q_itr->second;
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::BuildLootResponse QuestItemList size %i", q_list->size());
        ...
        for (QuestItemList::const_iterator qi = q_list->begin(); qi != q_list->end(); ++qi)
        {
            ...
            // sLog->outDebug(LOG_FILTER_LOOT, "Loot::BuildLootResponse canLoot %i personal %u LootListID %i is_looted %u ItemID %u", canLoot, personal, item.LootListID, qi->is_looted, item.item.ItemID);
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::AllowedForPlayer ItemID %i type %u", ItemID, type);
    ...
    if (type == LOOT_ITEM_TYPE_ITEM)
    {
        ...
        {
            if (!const_cast<Player*>(player)->HasSkill(pProto->RequiredSkill))
                return false;
            if (pProto->Effects.size() >= 2 && pProto->Effects[1])
            {
                ...
                if (SkillLineAbilityEntry const* skillline = sDB2Manager.GetSkillBySpell(pProto->Effects[1]->SpellID))
                {
                    if (skillline->SupercedesSpell && !const_cast<Player*>(player)->HasSpell(skillline->SupercedesSpell))
                        return false;
                    if (skillline->MinSkillLineRank > const_cast<Player*>(player)->GetSkillValue(pProto->RequiredSkill))
                        return false;
                }
            }
        }
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "Loot::AllowedForPlayer needs_quest %u ItemID %i HasQuestForItem %u", needs_quest, ItemID, player->HasQuestForItem(ItemID));
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "Loot::AllowedForPlayer ItemID %i allowed true", ItemID);
...
void LootTemplate::LootGroup::Process(Loot& loot, LootTemplate const* tab) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::Process EqualPossibleDrops %i ExplicitPossibleDrops %i", EqualPossibleDrops.size(), ExplicitPossibleDrops.size());
    ...
    while (!EqualPossibleDrops.empty())
    {
        ...
        if (item != nullptr)   // only add this item if roll succeeds and the mode matches
        {
            ...
            if (ItemTemplate const* _proto = sObjectMgr->GetItemTemplate(item->itemid))
            {
                ...
                if (_proto->Quality == ITEM_QUALITY_LEGENDARY) // Custom chance for Legendary item
                {
                    if(!roll_chance_f(item->chance) || _proto->RequiredExpansion == EXPANSION_LEGION)
                        duplicate = true;
                }
            }
            if(item->shared) //shared very low chance to and one item
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessItemLoot EqualChanced %u", EqualChanced.size());
...
void LootTemplate::LootGroup::ProcessWorld(Loot& loot, LootTemplate const* tab, bool ignore) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessWorld EqualPossibleDrops %u ignore %u", EqualPossibleDrops.size(), ignore);
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessWorld ItemPossibleDrops %u ignore %u", ItemPossibleDrops.size(), ignore);
    ...
    if (!ItemPossibleDrops.empty()) // If nothing selected yet - an item is taken from equal-chanced part
    {
        ...
        LootStoreItem* item = &*itr;
        loot.AddItem(*item);
        // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessWorld AddItem itemid %i", itr->itemid);
    }
}
...
bool LootTemplate::LootGroup::ProcessBossLoot(Loot& loot, LootTemplate const* tab) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessBossLoot EqualChanced %i ExplicitlyChanced %i", uiMaxAttempts, ExplicitlyChanced.size());
    ...
    for (LootStoreItemList::const_iterator i = EqualPossibleDrops.begin(); i != EqualPossibleDrops.end(); ++i)
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessBossLoot itemid %u chance %f lootmode %u", i->itemid, i->chance, i->lootmode);
...
bool LootTemplate::LootGroup::ProcessRareOrGoLoot(Loot& loot, LootTemplate const* tab) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot EqualPossibleDrops %i ExplicitlyChanced %i", uiMaxAttempts, ExplicitlyChanced.size());
    ...
    while (!EqualPossibleDrops.empty())
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot itemid %i type %u", item->itemid, item->type);
        ...
        {
            // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot lootmode %i _DifficultyMask %i", item->lootmode, loot._DifficultyMask);
            EqualPossibleDrops.erase(itr);
            continue;
        }
        ...
        if (ItemTemplate const* _proto = sObjectMgr->GetItemTemplate(item->itemid))
        {
            ...
            // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot lootmode %i ItemSpecExist %u itemid %i", item->lootmode, _proto->ItemSpecExist, item->itemid);
        }
        else
        {
            // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot AllowableClass %i", _proto->AllowableClass);
            EqualPossibleDrops.erase(itr);
            continue;
        }

        // sLog->outDebug(LOG_FILTER_LOOT, "LootGroup::ProcessRareOrGoLoot AddItem itemid %i", item->itemid);
...
void LootTemplate::AddEntry(LootStoreItem& item)
{
    ...
    else                                                    // Non-grouped entries and references are stored together
    {
        if (ItemTemplate const* _proto = sObjectMgr->GetItemTemplate(item.itemid))
        {
            bool isResurse = _proto->IsOtherDrops() || _proto->IsRecipe();
            if (_proto->ItemNameDescriptionID == 13219 && item.chance < 100 && !item.needs_quest)
            {
                uint32 group = 1;
            ...
            else if (_proto->Quality == ITEM_QUALITY_RARE && item.chance < 100 && !item.needs_quest && !isResurse)
            {
                uint32 group = 2;
                if (group >= PersonalGroups.size())
                    PersonalGroups.resize(group);
...
void LootTemplate::Process(Loot& loot, bool rate, uint8 groupId) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::Process isBoss %i _DifficultyMask %i loot.chance %u bonusLoot %i Entries %u AutoGroups %u Groups %u", loot.isBoss, loot._DifficultyMask, loot.chance, loot.bonusLoot, Entries.size(), AutoGroups.size(), Groups.size());
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (!i->Roll(rate, lootOwner->GetMap()->IsDungeon()))
            continue;                                         // Bad luck for the entry
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::Process mincountOrRef %i itemid %i chance %f", i->mincountOrRef, i->itemid, i->chance);
    }
...
void LootTemplate::ProcessBossLoot(Loot& loot) const
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessBossLoot isBoss %i canGetInstItem %i _DifficultyMask %i loot.chance %u bonusLoot %i", loot.isBoss, canGetInstItem, loot._DifficultyMask, loot.chance, loot.bonusLoot);
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessBossLoot itemid %u chance %f lootmode %u lootmode %u", i->itemid, i->chance, i->lootmode, i->lootmode);
    ...
    if (ItemTemplate const* _proto = sObjectMgr->GetItemTemplate(i->itemid))
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessBossLoot AddItem itemid %i", i->itemid);
    }
}
...
void LootTemplate::ProcessRareOrGoLoot(Loot& loot) const
{
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessRareOrGoLoot itemid %u chance %f lootmode %u", i->itemid, i->chance, i->lootmode);
        ...
        if (i->type == LOOT_ITEM_TYPE_ITEM)
        {
            if (ItemTemplate const* _proto = sObjectMgr->GetItemTemplate(i->itemid))
            {
                ...
                if (_proto->ItemNameDescriptionID == 13219 && i->chance < 100) // Only CA delete
                    continue;

                bool isResurse = _proto->IsOtherDrops() || _proto->IsRecipe();
                if (!isResurse && _proto->Quality == ITEM_QUALITY_RARE && i->chance < 100)
                    continue;

                if (loot.isRareNext && _proto->Quality > ITEM_QUALITY_RARE) // Prevent farm epic item
                    continue;
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessRareOrGoLoot AddItem itemid %i", i->itemid);
    }
...
// Rolls for every item in the template and adds the rolled items the the loot
void LootTemplate::ProcessWorld(Loot& loot, bool ignore) const
{
    // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessWorld _ExpansionID %u _DifficultyMask %i", loot._ExpansionID, loot._DifficultyMask);
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessWorld itemid %u chance %f lootmode %u _ExpansionID %u", i->itemid, i->chance, i->lootmode, loot._ExpansionID);
        ...
        if (i->mincountOrRef < 0 && i->type == LOOT_ITEM_TYPE_ITEM)              // References processing
        {
            ...
            for (uint32 loop = 0; loop < i->maxcount; ++loop)    // Ref multiplicator
                Referenced->ProcessWorld(loot, true);
        }
        else                                                  // Plain entries (not a reference, not grouped)
            loot.AddItem(*i);                                 // Chance is already checked, just add

        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessWorld AddItem itemid %i", i->itemid);
    }
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessLuck _ExpansionID %u _DifficultyMask %i entry %u", loot._ExpansionID, loot._DifficultyMask, entry);
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessLuck itemid %u chance %f lootmode %u _ExpansionID %u chance %f", i->itemid, i->chance, i->lootmode, loot._ExpansionID, chance);
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "LootTemplate::ProcessLuck AddItem itemid %i", i->itemid);
    }
}
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "ProcessItemLoot isBoss %i loot.chance %u bonusLoot %i", loot.isBoss, loot.chance, loot.bonusLoot);
    // sLog->outDebug(LOG_FILTER_LOOT, "ProcessItemLoot Entries %u AutoGroups %u PersonalGroups %u", Entries.size(), AutoGroups.size(), PersonalGroups.size());
    ...
    if (loot._isLegendaryLoot)
        loot.AddLegendaryItem(); // Add Legendary Item
...
void LootTemplate::ProcessOploteChest(Loot& loot) const
{
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (i->type == LOOT_ITEM_TYPE_ITEM)
        {
            if (ItemTemplate const* _proto = sObjectMgr->GetItemTemplate(i->itemid))
            {
                ...
                if(_proto->ItemId == 138019)
                    _delete = true;

                if (!_delete && (_proto->IsOtherDrops() || _proto->IsRecipe()))
                {
                    _delete = true;
                    if (_proto->ItemNameDescriptionID != 13219) // Only CA delete from loot
                        OtherPossibleDrops.push_back(*i);
                }
            }
            else
    ...
    LootStoreItem item = LootStoreItem(1501, LOOT_ITEM_TYPE_CURRENCY, 0.0f, 0, 0, 20, 20);
    loot.AddItem(item);
    ...
    if (const_cast<Player*>(lootOwner)->m_challengeKeyInfo.IsActive())
    {
...
{
    // sLog->outDebug(LOG_FILTER_LOOT, "ProcessChallengeChest Entries %u AutoGroups %u PersonalGroups %u", Entries.size(), AutoGroups.size(), PersonalGroups.size());
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "ProcessChallengeChest ItemPossibleDrops %i", ItemPossibleDrops.size());
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "ProcessChallengeChest OtherPossibleDrops %i", OtherPossibleDrops.size());

    // sLog->outDebug(LOG_FILTER_LOOT, "ProcessChallengeChest loot.items %i", loot.items.size());
    uint32 addCA = 0;
    if (_challenge->_complete && !_challenge->_noLoot)
    ...
    for (LootStoreItemList::const_iterator i = OtherPossibleDrops.begin(); i != OtherPossibleDrops.end(); ++i)
    {
        // sLog->outDebug(LOG_FILTER_LOOT, "ProcessChallengeChest OtherPossibleDrops itemid %i chance %f", i->itemid, i->chance);
        ...
        // sLog->outDebug(LOG_FILTER_LOOT, "ProcessChallengeChest AddItem itemid %i", i->itemid);
    }
...
{
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading World loot templates...");
    ...
    if (count)
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u World loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 World loot templates. DB table `fishing_loot_template` is empty");
}

void LoadLootTemplates_Luck()
{
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Luck loot templates...");
    ...
    if (count)
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Luck loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Luck loot templates. DB table `fishing_loot_template` is empty");
}

void LoadLootTemplates_Zone()
{
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Zone loot templates...");
    ...
    if (count)
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Zone loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    else
        sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Zone loot templates. DB table `fishing_loot_template` is empty");
}
...
void LootMgr::AddLoot(Loot* loot)
{
    ...
    // sLog->outDebug(LOG_FILTER_LOOT, "LootMgr::AddLoot guid %s size %i", loot->GetGUID().ToString().c_str(), m_Loots.size());
}
