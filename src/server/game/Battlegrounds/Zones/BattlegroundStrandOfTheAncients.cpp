...
void BattlegroundStrandOfTheAncients::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
...
void BattlegroundStrandOfTheAncients::CaptureGraveyard(BG_SA_Graveyards i, Player* Source)
{
    ...
    if (!sg)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundStrandOfTheAncients::CaptureGraveyard: non-existant GY entry: %u", BG_SA_GYEntries[i]);
        return;
    }
