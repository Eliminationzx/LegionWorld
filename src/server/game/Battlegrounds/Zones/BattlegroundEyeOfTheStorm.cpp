    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < _playersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!player)
                {
                    sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm:_CheckSomeoneJoinedPoint: Player (GUID: %u) not found!", _playersNearPoint[EY_POINTS_MAX][j].GetCounter());
                    ++j;
                    continue;
                }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < _playersNearPoint[i].size())
            {
                ...
                if (!player)
                {
                    sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm:_CheckSomeoneLeftPoint Player (GUID: %u) not found!", _playersNearPoint[i][j].GetCounter());
...
bool BattlegroundEyeOfTheStorm::SetupBattleground()
{
    ...
    for (int i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (!at)
        {
            sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm: Unknown trigger: %u", _pointsTrigger[i]);
            continue;
        }
        if (!AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3, Buff_Entries[0], at->Pos.X, at->Pos.Y, at->Pos.Z, 0.907571f, 0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY)
            ...
            sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm: Cannot spawn buff");
    }
...
void BattlegroundEyeOfTheStorm::EventPlayerClickedOnFlag(Player* Source, GameObject* object)
{
...
void BattlegroundEyeOfTheStorm::EventTeamCapturedPoint(Player* Source, uint32 Point)
{
    ...
    if (!sg || !AddSpiritGuide(Point, sg->Loc, Source->GetBGTeamId()))
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BatteGroundEY: Failed to spawn spirit guide! point: %u, team: %u, graveyard_id: %u", Point, Team, m_CapturingPointTypes[Point].GraveYardId);
...
WorldSafeLocsEntry const* BattlegroundEyeOfTheStorm::GetClosestGraveYard(Player* player)
{
    ...
    if (!entry)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm: Not found the main team graveyard. Graveyard system isn't working!");
        return nullptr;
    }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        if (_pointOwnedByTeam[i] == player->GetBGTeam() && _pointState[i] == EY_POINT_UNDER_CONTROL)
        {
            ...
            if (!entry)
                sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundEyeOfTheStorm: Not found graveyard: %u", m_CapturingPointTypes[i].GraveYardId);
            else
            {
