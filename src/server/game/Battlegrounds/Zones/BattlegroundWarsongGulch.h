...
public:
    ...
    uint8 GetFlagState(uint32 team) { return _flagState[MS::Battlegrounds::GetTeamIdByTeam(team)]; }
...
private:
    void UpdateFlagState(TeamId teamID, uint32 value, ObjectGuid flagKeeperGUID = ObjectGuid::Empty);
    void RespawnFlag(TeamId teamID, bool captured = false);
