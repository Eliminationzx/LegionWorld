        ...
        if (_updatePointsTimer <= diff)
        {
            for (uint8 i = 0; i < MAX_ORBS; ++i)
            {
                ...
                if (Player* player = ObjectAccessor::FindPlayer(GetBgMap(), guid))
                {
                    ...
                    m_TeamScores[teamID] += BG_KT_TickPoints[playerZone] * (IsBrawl() ? 2 : 1);
                    ...
                    UpdatePlayerScore(player, SCORE_ORB_SCORE, BG_KT_TickPoints[playerZone] * (IsBrawl() ? 2 : 1));
...
void BattlegroundKotmoguTemplate::EventPlayerClickedOnFlag(Player* source, GameObject* object)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
