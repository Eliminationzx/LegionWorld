...
void BattlegroundBattleForGilneas::_NodeOccupied(uint8 node, TeamId team)
{
    ...
    if (!AddSpiritGuide(node, BgBfgSpiritGuidePos[node], team))
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Failed to spawn spirit guide! point: %u, team: %u, ", node, MS::Battlegrounds::GetTeamByTeamId(team));
...
void BattlegroundBattleForGilneas::EventPlayerClickedOnFlag(Player* source, GameObject* /*object*/)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
