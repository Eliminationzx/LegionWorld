...
BG_AV_Nodes BattlegroundAlteracValley::GetNodeThroughObject(uint32 object)
{
    ...
    sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundAlteracValley: ERROR! GetPlace got a wrong object :(");
    ASSERT(false);
...
{ //this function is the counterpart to GetNodeThroughObject()
    ...
    sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundAlteracValley: Error! GetPlaceNode couldn't resolve node %i", node);
    ASSERT(false);
    return 0;
}
...
void BattlegroundAlteracValley::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
