...
void Arena::EndBattleground(uint32 winner)
{
    ...
    if (IsRated() && winner != WINNER_NONE && GetStatus() == STATUS_IN_PROGRESS)
    {
        uint8 coincidence = 0;
        std::vector<char*> IPmark;
        IPmark.push_back("!");
        IPmark.push_back("?");
        IPmark.push_back("*");
        ...
        for (auto& itr : m_nameList[ALLIANCE])
        {
            ...
            {
                if (j.IPChecked || j.PlayerName == itr.PlayerName)
                    continue;
                ...
                if (j.PlayerIP == itr.PlayerIP)
                {
                    j.IPChecked = true;
                    itr.IPChecked = true;
                    j.PlayerName += IPmark[coincidence];
                    attention = true;
                }
            }
            ...
            {
                if (!j.IPChecked && j.PlayerIP == itr.PlayerIP)
                {
                    j.IPChecked = true;
                    itr.IPChecked = true;
                    j.PlayerName += IPmark[coincidence];
            ...
            if (itr.IPChecked)
            {
                itr.PlayerName += IPmark[coincidence];
                coincidence++;
            }
        ...
        switch (GetJoinType())
        {
            ...
            case MS::Battlegrounds::JoinType::Arena2v2:
            {
                ...
                sLog->outArena("FINISH: Arena match Type: 2v2 --- Winner[%s]: old rating: %u --- Loser[%s]: old rating: %u --- Duration: %u min. %u sec. %s",
                               winnerTeam.c_str(), GetMatchmakerRating(winner), loserTeam.c_str(), GetMatchmakerRating(GetOtherTeam(winner)), _min, _sec, att.c_str());
                break;
            }
            case MS::Battlegrounds::JoinType::Arena3v3:
            {
                ...
                sLog->outArena("FINISH: Arena match Type: 3v3 --- Winner[%s]: old rating: %u --- Loser[%s]: old rating: %u --- Duration: %u min. %u sec. %s",
                               winnerTeam.c_str(), GetMatchmakerRating(winner), loserTeam.c_str(), GetMatchmakerRating(GetOtherTeam(winner)), _min, _sec, att.c_str());
                break;
            }
            default:
                sLog->outArena("match Type: %u --- Winner: old rating: %u  --- Loser: old rating: %u| DETAIL: Winner[%s] Loser[%s]",
