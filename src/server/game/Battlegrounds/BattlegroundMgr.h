...
struct BrawlData
{
    BrawlData(uint16 bgTypeId, uint32 holidayId, std::string name, std::string description, std::string description2, uint8 type);
    BrawlData() = default;
...
public:
    ...
    void BuildBattlegroundStatusNeedConfirmation(WorldPackets::Battleground::BattlefieldStatusNeedConfirmation* battlefieldStatus, Battleground* bg, Player* player, uint32 ticketId, uint32 joinTime, uint32 timeout, uint32 joinType);
    ...
    uint8 GetBrawlIternalGroup() const { return _brawlIternalGroup; }
    void SetBrawlIternalGroup(uint8 group) { _brawlIternalGroup = group; }
...
private:
    ...
    std::map<uint16, uint8> _selectionWeights[MS::Battlegrounds::IternalPvpTypes::Max - 1];
