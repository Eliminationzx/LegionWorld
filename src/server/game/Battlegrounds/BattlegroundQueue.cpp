...
bool BattlegroundQueue::SelectionPool::KickGroup(uint32 size)
{
    ...
    {
        if (abs(static_cast<int32>((*itr)->Players.size() - size)) <= 1)
        {
            groupToKick = itr;
            found = true;
        }
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, uint16 BgTypeId, PVPDifficultyEntry const* bracketEntry, uint8 JoinType, bool isRated, bool isPremade, WorldPackets::Battleground::IgnorMapInfo ignore, uint32 mmr, uint32 _team /*= 0*/)
{
    ...
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Adding Group to BattlegroundQueue bgTypeId : %u, bracketID : %u, bracket_type: %u, mmr: %i, index : %u", BgTypeId, bracketEntry->RangeIndex, bracket, ginfo->MatchmakerRating, index);
...
void BattlegroundQueue::RemovePlayerQueue(ObjectGuid guid, bool decreaseInvitedCount)
{
    ...
    if (group->IsInvitedToBGInstanceGUID && group->IsRated && decreaseInvitedCount)
        if (Player* player = ObjectAccessor::FindPlayer(guid))
            if (Bracket* bracket = player->getBracket(MS::Battlegrounds::GetBracketByJoinType(group->JoinType)))
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, uint32 side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        for (auto itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusNeedConfirmation(&battlefieldStatus, bg, player, queueSlot, player->GetBattlegroundQueueJoinTime(bgQueueTypeId), bg->IsArena() ? ARENA_INVITE_ACCEPT_WAIT_TIME : BG_INVITE_ACCEPT_WAIT_TIME, ginfo->JoinType);
...
bool BattlegroundQueue::CheckSkirmishForSameFaction(uint8 bracketID, uint32 minPlayersPerTeam)
{
    ...
    auto ginfo = m_SelectionPools[teamIndex].SelectedGroups.back();
    auto itr_team = _queuedGroups[bracketID][MS::Battlegrounds::QueueGroupTypes::NormalAlliance + teamIndex].begin();
    for (; itr_team != _queuedGroups[bracketID][MS::Battlegrounds::QueueGroupTypes::NormalAlliance + teamIndex].end(); ++itr_team)
        if (ginfo == *itr_team)
            break;

    if (itr_team == _queuedGroups[bracketID][MS::Battlegrounds::QueueGroupTypes::NormalAlliance + teamIndex].end())
        return false;

    auto itr_team2 = itr_team;
    ++itr_team2;
    for (; itr_team2 != _queuedGroups[bracketID][MS::Battlegrounds::QueueGroupTypes::NormalAlliance + teamIndex].end(); ++itr_team2)
    ...
    for (auto itr = m_SelectionPools[otherTeam].SelectedGroups.begin(); itr != m_SelectionPools[otherTeam].SelectedGroups.end(); ++itr)
    {
        ...
        auto itr2 = itr_team;
        ++itr2;
        for (; itr2 != _queuedGroups[bracketID][MS::Battlegrounds::QueueGroupTypes::NormalAlliance + teamIndex].end(); ++itr2)
        {
            if (*itr2 == *itr)
            {
                _queuedGroups[bracketID][MS::Battlegrounds::QueueGroupTypes::NormalAlliance + teamIndex].erase(itr2);
                break;
            }
        }
    }
...
bool BattlegroundQueue::SelectRatedTeams(uint32 bracket_id, GroupQueueInfo* & team1, GroupQueueInfo* & team2)
{
    ...
    for (std::vector<GroupQueueInfo*>::const_iterator itr = lowRatingTeams.begin(); itr != lowRatingTeams.end(); ++itr)
    {
        ...
        float mmrSteps = floor(float((time(nullptr) - selectedTeam->JoinTime) / 60)); // every 1 minute
...
uint16 BattlegroundQueue::GenerateRandomMap(uint16 bgTypeId)
{
    ...
    else if (bgTypeId == MS::Battlegrounds::BattlegroundTypeId::BrawlArenaAll) // temp... some hardcodes for generatind needed too - check for enabled brawl
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, uint16 bgTypeId, uint8 bracketID, uint8 joinType /*= 0*/, bool isRated /*= false*/, Roles role /*= ROLES_DEFAULT*/, uint8 bracket_MinLevel /*= 0*/)
{
    ...
    for (auto itr = sBattlegroundMgr->BGFreeSlotQueue.begin(); itr != sBattlegroundMgr->BGFreeSlotQueue.end(); itr = next)
    {
        ...
        if ((*itr)->IsBattleground() && ((*itr)->GetTypeID(true) == bgTypeId || bgTypeId == MS::Battlegrounds::BattlegroundTypeId::BattlegroundRandom && (*itr)->GetTypeID(true) != MS::Battlegrounds::BattlegroundTypeId::BattlegroundDeathMatch)
    ...
    if (!bracketEntry)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground: Update: bg bracket entry not found for map %u bracket id %u", bg_template->GetMapId(), bracketID);
        return;
    }
    ...
    if (!isRated)
    {
        if (CheckNormalMatch(bg_template, bracketID, MinPlayersPerTeam, MaxPlayersPerTeam) || bg_template->IsArena() && CheckSkirmishForSameFaction(bracketID, MinPlayersPerTeam))
        {
    ...
    else if (bg_template->IsArena() || bg_template->IsRBG())
    {
        ...
        if (!bg)
        {
            sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue::Update couldn't create bg instance for rated arena or bg match!");
            return;
        }
        ...
        if (bg->IsArena())
        {
            ...
            for (auto itr = aTeam->Players.begin(); itr != aTeam->Players.end(); ++itr)
            {
                if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                {
                    ...
                    pInfo.PlayerIP = player->GetSession()->GetRemoteAddress();
                    pInfo.IPChecked = false;
            ...
            switch (bg->GetJoinType())
            {
                case 1:
                {
                    sLog->outArena("START:  Arena match type: 1v1 for (%s) vs (%s).", team1.c_str(), team2.c_str());
                    break;
                }
                case 2:
                {
                    sLog->outArena("START:  Arena match type: 2v2 for (%s) vs (%s).", team1.c_str(), team2.c_str());
                    break;
                }
                case 3:
                {
                    sLog->outArena("START:  Arena match type: 3v3 for (%s) vs (%s).", team1.c_str(), team2.c_str());
                    break;
                }
                default:
                    sLog->outArena("Arena match type: %u for Team1Id: %u - Team2Id: %u started for (%s) vs (%s).", bg->GetJoinType(), bg->GetGroupIdByIndex(TEAM_ALLIANCE), bg->GetGroupIdByIndex(TEAM_HORDE), team1.c_str(), team2.c_str());;
                    break;
            }
        }
    }
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            if (bg && bg->GetJoinType() != MS::Battlegrounds::JoinType::Arena1v1)
                player->AddDelayedEvent(100, [player]() -> void { player->HandleArenaDeserter(); });
        }
    }
