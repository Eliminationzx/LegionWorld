...
inline void Battleground::_ProcessRessurect(uint32 diff)
{
    ...
    if (m_LastResurrectTime >= uint32(IsBrawl() ? RESURRECTION_INTERVAL_BRAWL : RESURRECTION_INTERVAL))
    {
...
Player* Battleground::GetPlayer(ObjectGuid guid, bool offlineRemove, char const* context)
{
    if (!offlineRemove)
    {
        ...
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::%s: player (GUID: %u) not found", context, guid.GetCounter());
    }
...
void Battleground::UpdateWorldState(uint32 variableID, uint32 value, bool hidden /*= false*/)
{
    ...
    //if (!sWorldStateMgr.FindTemplate(variableID))
    //{
    //    WorldStateTemplate wsTemplate(variableID, WorldStatesData::Types::Battlegound, 0, 0, value, 0);
    //    sWorldStateMgr.AddTemplate(variableID, wsTemplate);
    //}

    //sWorldStateMgr.SetWorldStateValueFor(GetBgMap(), variableID, value, hidden);
}
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for (auto const& itr : GetPlayers())
    {
        ...
        if (!IsWargame() && player->IsInvitedForBattlegroundQueueType(MS::Battlegrounds::BattlegroundQueueTypeId::BattlegroundRandom) || IsArena() || IsSkirmish() || IsRBG() || IsBG())
            PlayerReward(player, team == winner, bracket ? bracket->getRating() : 0);

        if (team == winner)
        {
            if (!player->HasWinToday(sBattlegroundMgr->GetPvpRewardType(this)))
...
void Battleground::PlayerReward(Player* player, bool isWinner, uint16 personalRating)
{
    if (!player)
        return;
    ...
    if (roll_chance_f(chance))
    {
        ...
        if (legendaryItemID)
        {
            ...
            {
                if (auto item = player->StoreNewItem(dest, legendaryItemID, true, Item::GenerateItemRandomPropertyId(legendaryItemID, playerSpecID)))
                {
                    sLog->outWarden("Player %s on map %u with killpoints %f and chance %f looted legendary item %u from source: PvP (rewardType %u)", player->GetSession()->GetPlayerName(false).c_str(), player->GetMapId(), player->m_killPoints, chance, legendaryItemID, uint8(type));
                    player->SendNewItem(item, 1, true, false, true);
    ...
    if ((IsArena() || IsRBG()) && !IsSkirmish())
    {
        ...
        //// Arena epic 885 ilvl, for 7.3.5 915
        if (personalRating >= 1400 && personalRating < 1600)
           levelBonus += 5;
        else if (personalRating >= 1600 && personalRating < 1800)
           levelBonus += 15;
        else if (personalRating >= 1800 && personalRating < 2000)
           levelBonus += 20;
        // Arena elit epic 910+ ilvl, for 7.3.5 940
        else if (personalRating >= 2000 && personalRating < 2200)
           levelBonus += 0;
        else if (personalRating >= 2200 && personalRating < 2400)
           levelBonus += 5;
        else if (personalRating >= 2400)
           levelBonus += 10;

        if (personalRating >= 2000)
        {
            itemsContainer = isAlliance ? reward->ItemsAElit : reward->ItemsHElit;
    ...
    if (!relicsContainer.empty())
    {
        ...
        {
            auto const& _proto = sObjectMgr->GetItemTemplate(itemIDr);
            if (!_proto)
                continue;

            if(!player->CanGetItemForLoot(_proto))
                continue;
    ...
    if (!itemsContainer.empty())
    {
        ...
        {
            auto const& _proto = sObjectMgr->GetItemTemplate(itemIDs);
            if (!_proto)
                continue;
            ...
            if (player->HasItemCount(itemIDs, 1, true)) // Prevent drop if item allready exist
            {
    ...
    std::vector<uint32> itemModifiers = sObjectMgr->GetItemBonusTree(itemId, m_Map->GetDifficultyLootItemContext(), playerLevel, 0, 0, needLevel);
...
void Battleground::RemovePlayerAtLeave(ObjectGuid guid, bool Transport, bool SendPacket)
{
    ...
    if (player)
    {
        ...
        //player->SetUInt32Value(PLAYER_FIELD_CURRENT_SPEC_ID, player->GetBGLastActiveSpec());
    ...
    if (participant)
    {
        ...
        if (IsBattleground() && !IsRBG() && !IsBrawl() && !IsWargame() && GetStatus() < STATUS_WAIT_LEAVE)
        {
...
void Battleground::AddPlayer(Player* player)
{
    ...
    bp.Role = static_cast<Roles>(player->GetSpecializationRoleMaskForGroup());
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/, GOState goState)
{
    ...
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::AddObject: cannot create gameobject (entry: %u) for BG (map: %u, instance id: %u)!", entry, m_MapId, m_InstanceID);
        delete go;
        return false;
    }
...
void Battleground::DoorClose(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::DoorClose: door gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, BgObjects[type].GetCounter(), m_MapId, m_InstanceID);
}
...
void Battleground::DoorOpen(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::DoorOpen: door gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, BgObjects[type].GetCounter(), m_MapId, m_InstanceID);
}
...
GameObject* Battleground::GetBGObject(uint32 type)
{
    ...
    if (!obj)
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::GetBGObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, BgObjects[type].GetCounter(), m_MapId, m_InstanceID);
    return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type)
{
    ...
    if (!creature)
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::GetBGCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, BgCreatures[type].GetCounter(), m_MapId, m_InstanceID);
    return creature;
}
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, uint32 teamval, float x, float y, float z, float o, uint32 respawntime, Transport* transport)
{
    ...
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::AddCreature: cannot create creature (entry: %u) for BG (map: %u, instance id: %u)!", entry, m_MapId, m_InstanceID);
        delete creature;
        return nullptr;
    }
    ...
    if (!cinfo)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::AddCreature: creature template (entry: %u) does not exist for BG (map: %u, instance id: %u)!", entry, m_MapId, m_InstanceID);
        delete creature;
        return nullptr;
    }
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::DelCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, BgCreatures[type].GetCounter(), m_MapId, m_InstanceID);
    BgCreatures[type].Clear();
    return false;
}
...
bool Battleground::DelObject(uint32 type)
{
    ...
    sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::DelObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, BgObjects[type].GetCounter(), m_MapId, m_InstanceID);
    BgObjects[type].Clear();
    return false;
}
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, uint32 team)
{
    ...
    sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::AddSpiritGuide: cannot create spirit guide (type: %u, entry: %u) for BG (map: %u, instance id: %u)!", type, entry, m_MapId, m_InstanceID);
    EndNow();
    return false;
}
...
void Battleground::HandleTriggerBuff(ObjectGuid go_guid)
{
    ...
    if (index < 0)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::HandleTriggerBuff: cannot find buff gameobject (GUID: %u, entry: %u, type: %u) in internal data for BG (map: %u, instance id: %u)!", go_guid.GetCounter(), obj->GetEntry(), obj->GetGoType(), m_MapId, m_InstanceID);
        return;
    }
...
uint32 Battleground::GetAlivePlayersCountByTeam(uint32 Team) const
{
    ...
    for (auto const& itr : GetPlayers())
        if (itr.second.Team == Team)
            if (Player* player = ObjectAccessor::FindPlayer(itr.first))
                if (player->isAlive() && player->GetShapeshiftForm() != FORM_SPIRITOFREDEMPTION && player->InArena())
                    ++count;
...
int32 Battleground::GetObjectType(ObjectGuid guid)
{
    ...
    sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::GetObjectType: player used gameobject (GUID: %u) which is not in internal data for BG (map: %u, instance id: %u), cheating?", guid.GetCounter(), m_MapId, m_InstanceID);
    return -1;
}
