...
BrawlData::BrawlData(uint16 bgTypeId, uint32 holidayId, std::string name, std::string description, std::string description2, uint8 type)
{
    BgTypeId = bgTypeId;
    HolidayId = holidayId;
    Name = name;
...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    for (uint16 i = MS::Battlegrounds::BattlegroundTypeId::None; i < MS::Battlegrounds::BattlegroundTypeId::Max; ++i)
    {
        ...
        for (; itr != _battlegrounds[i].end(); itr = next)
        {
            ...
            {
                RecursiveGuard _bg_lock(bg->m_bg_lock);
                _battlegrounds[i].erase(itr);
                if (!_clientBattlegroundIDs[i][bg->GetBracketId()].empty())
                    _clientBattlegroundIDs[i][bg->GetBracketId()].erase(bg->GetClientInstanceID());
...
void BattlegroundMgr::SendBattlegroundList(Player* player, ObjectGuid const& guid, uint16 bgTypeId)
{
    ...
    if (bgTypeId != MS::Battlegrounds::BattlegroundTypeId::ArenaAll && bgTypeId != MS::Battlegrounds::BattlegroundTypeId::BrawlArenaAll && bgTypeId != MS::Battlegrounds::BattlegroundTypeId::BattlegroundRatedEyeOfTheStorm)
...
void BattlegroundMgr::BuildBattlegroundStatusNeedConfirmation(WorldPackets::Battleground::BattlefieldStatusNeedConfirmation* battlefieldStatus, Battleground* bg, Player* player, uint32 ticketId, uint32 joinTime, uint32 timeout, uint32 joinType)
{
    ...
    battlefieldStatus->Role = player->GetSpecializationRole(); // temp hack
}
...
Battleground* BattlegroundMgr::CreateNewBattleground(uint16 bgTypeId, PVPDifficultyEntry const* bracketEntry, uint8 joinType, bool isRated, uint16 generatedType/*=bgTypeId*/, bool useTournamentRules /*= false*/, bool isWarGame /*= false*/)
{
    ...
    bool isRandom = bgData->IsArena || bgTypeId == MS::Battlegrounds::BattlegroundTypeId::BattlegroundRandom || bgTypeId == MS::Battlegrounds::BattlegroundTypeId::RatedBattleground;
    if (isRandom)
    {
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    do
    {
        ...
        {
            if (data.bgTypeId != MS::Battlegrounds::BattlegroundTypeId::BrawlArenaAll)
...
{
    if (iternalPvpType >= MS::Battlegrounds::IternalPvpTypes::Max - 1)
        return nullptr;
...
void BattlegroundMgr::LoadPvpRewards()
{
    ...
    */
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u PvpReward in %u ms", static_cast<uint32>(_pvpRewardsContainer.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
{
    //Name = namesEntry->HolidayName->Str[GetSessionDbLocaleIndex()];

    //_brawlTemplatesContainer[0] = BrawlData(0, 255, "PvP Brawl: Gravity Lapse", "", ""); // 239
    //_brawlTemplatesContainer[0] = BrawlData(MS::Battlegrounds::BattlegroundTypeId::BrawlBattlegroundSouthshoreVsTarrenMill, 256, "PvP Brawl: Southshore vs. Tarren Mill", "", "", MS::Battlegrounds::BrawlTypes::Battleground); // 238
    _brawlTemplatesContainer[0] = BrawlData(MS::Battlegrounds::BattlegroundTypeId::BrawlBattlegroundWarsongScramble, 257, "PvP Brawl: Warsong Scramble", "Поля боя 6 на 6", "– 6 игроков на команду – Новые режимы игры", MS::Battlegrounds::BrawlTypes::Battleground); // 240
    _brawlTemplatesContainer[1] = BrawlData(MS::Battlegrounds::BattlegroundTypeId::BrawlBattlegroundArathiBasinWinter, 262, "PvP Brawl: Arathi Blizzard", "Ледяная мощь", "- Захватите объекты - Видимость ограничена - Территория заморожена", MS::Battlegrounds::BrawlTypes::Battleground); // 240
    //_brawlTemplatesContainer[0] = BrawlData(MS::Battlegrounds::BattlegroundTypeId::BrawlBattleground6 or BrawlBattleground5 ?, 263, "PvP Brawl: Deepwind Dunk", "", "", MS::Battlegrounds::BrawlTypes::Battleground); // 240
    _brawlTemplatesContainer[2] = BrawlData(MS::Battlegrounds::BattlegroundTypeId::BrawlBattlegroundKotmoguTemplate, 264, "PvP Brawl: Temple of Hotmogu", "", "", MS::Battlegrounds::BrawlTypes::Battleground); // 240
    //_brawlTemplatesContainer[0] = BrawlData(0, 265, "PvP Brawl: Packed House", "", "", MS::Battlegrounds::BrawlTypes::Battleground); // 240
    //_brawlTemplatesContainer[0] = BrawlData(0, 267, "PvP Brawl: Deep Six", "Поля боя 6 на 6", "– 6 игроков на команду – Новые режимы игры", MS::Battlegrounds::BrawlTypes::Battleground); // 240
    //_brawlTemplatesContainer[0] = BrawlData(1428, 268, "PvP Brawl: Shado-Pan Showdown", "Арена 5 на 5", " – Арена 5 на 5 – Убейте босса - могу команды соперника, чтобы одержать победу – Откройте призванный сундук, чтобы подобрать усиление", MS::Battlegrounds::BrawlTypes::Lfg); // 240
