...
void PetBattleAbilityEffect::Kill(uint32 target)
{
    if (Flags & FailFlags)
    {
        // sLog->outDebug(LOG_FILTER_BATTLEPET, "PetBattleAbilityEffect::Kill BATTLEPET_STATE_Is_Dead");
    ...
    // sLog->outDebug(LOG_FILTER_BATTLEPET, "PetBattleAbilityEffect::Kill BATTLEPET_STATE_Special_ConsumedCorpse");
...
void PetBattleAbilityEffect::SelectTargets()
{
    if (EffectInfo->BattlePetEffectPropertiesID > MAX_PETBATTLE_EFFECT_TYPES)
        sLog->outError(LOG_FILTER_BATTLEPET, "Function: %s; Line: %u EffectInfo->effect > MAX_PETBATTLE_EFFECT_TYPES; effect: %u", __FUNCTION__, __LINE__, EffectInfo->BattlePetEffectPropertiesID);
