...
void WildBattlePetMgr::Load()
{
    ...
    do
    {
        ...
        if (mapID == -1)
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "WildBattlePetMgr::Load() no map id found for zone %u", zoneID);
            continue;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u species definitions.", count);
}
