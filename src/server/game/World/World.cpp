...
void World::LoadConfigSettings(bool reload)
{
    ...
    rate_values[RATE_DROP_DUNGEON_ONLY_X1] = sConfigMgr->GetBoolDefault("Rate.Drop.Dungeon", false);
    ...
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "InstanceUpdateInterval (%i) must be greater %u. Use this minimal value.", m_int_configs[CONFIG_INTERVAL_INSTANCE_UPDATE], MIN_MAP_UPDATE_DELAY);
    ...
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "PvpMapUpdateInterval (%i) must be greater %u. Use this minimal value.", m_int_configs[CONFIG_INTERVAL_PVP_MAP_UPDATE], 25);
    ...
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "ObjectUpdateInterval (%i) must be greater %u. Use this minimal value.", m_int_configs[CONFIG_INTERVAL_OBJECT_UPDATE], MIN_MAP_UPDATE_DELAY);
    ...
    if (reload)
    {
        ...
        if (val != m_int_configs[CONFIG_PORT_INSTANCE])
            sLog->outError(LOG_FILTER_SERVER_LOADING, "InstanceServerPort option can't be changed at worldserver.conf reload, using current value (%u).", m_int_configs[CONFIG_PORT_INSTANCE]);
    }
    else
    {
    ...
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "DemonHuntersPerRealm (%i) must be in range 0..12. Set to 1.", m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM]);
    ...
    if (int32 hotfixCacheId = sConfigMgr->GetIntDefault("HotfixCacheVersion", 0))
    {
        ...
        if (hotfixCacheId > 0)
        {
            ...
            sLog->outError(LOG_FILTER_SERVER_LOADING, "Hotfix cache version set to: %u", hotfixCacheId);
        }
        else
            sLog->outError(LOG_FILTER_SERVER_LOADING, "HotfixCacheVersion can't be negative %d, ignored.", hotfixCacheId);
    }
    ...
    m_int_configs[CONFIG_WORLD_QUEST_MIN_ITEMLEVEL]  = sConfigMgr->GetIntDefault("WorldQuest.ItemLEvel.Cap", 805);
    m_int_configs[CONFIG_WORLD_QUEST_ITEMLEVEL_CAP]  = sConfigMgr->GetIntDefault("WorldQuest.ItemLEvel.Cap", 855);
...
void World::SetInitialWorldSettings()
{
    ...
    sAchievPoolMgr->start(getIntConfig(CONFIG_NUMTHREADS));
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading World Rate Info...");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING,"Loading Bad Sentences...");
    sWordFilterMgr->LoadBadSentences();
    
    sLog->outInfo(LOG_FILTER_SERVER_LOADING,"Loading Complaints...");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Transport animations and rotations...");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Seamless Teleport Data...");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading DeathMatch Store Products...");
    ...
    m_timers[WUPDATE_WORLDSTATE].SetInterval(3 * IN_MILLISECONDS);
    m_timers[WUPDATE_CONTRIBUTION].SetInterval(15 * IN_MILLISECONDS);
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Update Challenge Key...");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Calculate next weekly reset time...");
    InitWeeklyResetTime();
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Calculate next banwave time...");
    InitBanWaveTime();
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_WORLDSTATE].OnTimerPassReset())
        sWorldStateMgr.Update(diff);

    if (m_timers[WUPDATE_CONTRIBUTION].OnTimerPassReset())
