...
namespace lfg
{
LFGMgr::LFGMgr() : m_QueueTimer(0), m_lfgProposalId(1), m_ShortageCheckTimer(0), m_options(sWorld->getIntConfig(CONFIG_LFG_OPTIONSMASK))
{
    new LFGPlayerScript();
    new LFGGroupScript();
}
...
void LFGMgr::LoadRewards()
{
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "Level %u specified for dungeon %u in table `lfg_dungeon_rewards` can never be reached!", maxLevel, dungeonId);
            maxLevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
        }

        if (!firstQuestId || !sQuestDataStore->GetQuestTemplate(firstQuestId))
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "First quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", firstQuestId, dungeonId);
            continue;
        }

        if (otherQuestId && !sQuestDataStore->GetQuestTemplate(otherQuestId))
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "Other quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", otherQuestId, dungeonId);
            otherQuestId = 0;
        }

        if (bonusQuestId && !sQuestDataStore->GetQuestTemplate(bonusQuestId))
        {
            sLog->outError(LOG_FILTER_SERVER_LOADING, "Bonus quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", bonusQuestId, dungeonId);
            bonusQuestId = 0;
        }
    ...
    for (auto& itr : LfgDungeonStore)
    {
        ...
        if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
        {
            if (AreaTriggerStruct const* at = sAreaTriggerDataStore->GetMapEntranceTrigger(dungeon.map))
            {
                ...
                sLog->outError(LOG_FILTER_LFG, "LFGMgr::LoadLFGDungeons: Failed to load teleport positions for dungeon %s, cant find areatrigger for map %u. dungeonId %u", dungeon.name.c_str(), dungeon.map, dungeon.id);
        }
    ...
    for (uint8 i = 0; i < ALL_TEAMS; i++)
        for (auto& itr : QueuesStore[i])
            if (uint8 newProposals = itr.second.FindGroups())
                sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::Update: Found %u new groups in queue %u team %u", newProposals, itr.first, i);
        ...
        for (auto it = dungeons.begin(); it != dungeons.end() && joinData.result == LFG_JOIN_OK; ++it)
        {
            ...
            switch (entry->dbc->TypeID)
            {
                ...
                case LFG_TYPE_RAID:
                    break;
                default:
                    sLog->outError(LOG_FILTER_LFG, "Wrong dungeon type %u for dungeon %u", entry->dbc->TypeID, *it);
                    joinData.result = LFG_JOIN_DUNGEON_INVALID;
                    break;
            }
...
void LFGMgr::LeaveLfg(ObjectGuid guid, uint32 queueId)
{
    ...
    sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::LeaveLfg: (%s) queueId %u", guid.ToString().c_str(), queueId);
...
void LFGMgr::UpdateRoleCheck(ObjectGuid gguid, ObjectGuid guid /* = 0 */, uint8 roles /* = PLAYER_ROLE_NONE */, uint8 partyIndex /*= 0*/)
{
    ...
    uint32 queueId = GetQueueId(*dungeons.begin() & 0xFFFFF);
...
void LFGMgr::MakeNewGroup(LfgProposal const& proposal)
{
    ...
    for (GuidList::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (dungeon->type == LFG_TYPE_RANDOM) player->AddDelayedEvent(10, [player]() -> void
        {
            if (player)
...
void LFGMgr::TeleportPlayer(Player* player, bool out, bool fromOpcode /*= false*/)
{
    ...
    if (!dungeon)
    {
        sLog->outDebug(LOG_FILTER_LFG, "TeleportPlayer: Player %s not in group/lfggroup or dungeon not found!", player->GetName());
    ...
    if (out)
    {
        sLog->outDebug(LOG_FILTER_LFG, "TeleportPlayer: Player %s is being teleported out. Current Map %u - Expected Map %u", player->GetName(), player->GetMapId(), uint32(dungeon->map));
    ...
    sLog->outDebug(LOG_FILTER_LFG, "TeleportPlayer: Player %s is being teleported in to map %u (x: %f, y: %f, z: %f) Result: %u", player->GetName(), dungeon->map, dungeon->x, dungeon->y, dungeon->z, error);
}
...
void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::FinishDungeon: [" UI64FMTD "] Finished dungeon %u but group queued for %u. Ignoring", gguid.GetCounter(), dungeonId, gDungeonId);
        return;
    }

    if (GetState(gguid, queueId) == LFG_STATE_FINISHED_DUNGEON) // Shouldn't happen. Do not reward multiple times
    {
        sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::FinishDungeon: [" UI64FMTD "] Already rewarded group. Ignoring", gguid.GetCounter());
        return;
    }
    ...
    for (auto guid : players)
    {
        ...
        if (!player || !player->IsInWorld())
        {
            sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::FinishDungeon: [" UI64FMTD "] not found in world", guid.GetCounter());
            continue;
        }
        ...
        if (!reward)
        {
            sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::FinishDungeon: [" UI64FMTD "] Don`t find reward for DungeonId %i player %u level", guid.GetCounter(), rDungeonId, player->getLevel());
            continue;
        }
        ...
        sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::FinishDungeon: [" UI64FMTD "] done dungeon %u, %s previously done.", player->GetGUID().GetCounter(), GetDungeon(gguid), done ? " " : " not");
        ...
        if (data.rdungeonEntry && data.sdungeonEntry)
        {
            ...
            if (data.reward->bonusQuestId)
            {
                if (CTARewardStore[guid] && player->GetGroup() && (player->GetGroup()->GetLfgRoles(guid) & ~PLAYER_ROLE_LEADER) == CTARewardStore[guid])
                    if (Quest const* quest = sQuestDataStore->GetQuestTemplate(data.reward->bonusQuestId))
...
LfgState LFGMgr::GetOldState(ObjectGuid guid, uint32 queueId)
{
    ...
    sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::GetOldState: %s = %u", guid.ToString().c_str(), state);
    return state;
}
...
void LFGMgr::SendLfgPlayerLockInfo(Player* player)
{
    ...
    for (auto const& dungeon : rewardableDungeons)
    {
        ...
        lfg::LFGDungeonData const* dungeonData = sLFGMgr->GetLFGDungeon(dungeon & 0xFFFFF);
        if (!dungeonData)
            continue;
        ...
        if (bonusQuest)
        {
            for (uint32 i = lfg::LFG_ROLE_SHORTAGE_RARE; i < lfg::LFG_ROLE_SHORTAGE_MAX; ++i)
            {
                switch (i)
                {
                case lfg::LFG_ROLE_SHORTAGE_RARE:
                {
                ...
                case lfg::LFG_ROLE_SHORTAGE_UNCOMMON:
                {
                    WorldPackets::LFG::ShortageReward Reward;
                    dungeonInfo.ShortageRewards.push_back(Reward);
                    break;
                }
                case lfg::LFG_ROLE_SHORTAGE_PLENTIFUL:
                {
                    WorldPackets::LFG::ShortageReward Reward;
                    dungeonInfo.ShortageRewards.push_back(Reward);
                    break;
...
void LFGMgr::RestoreState(ObjectGuid guid, char const* debugMsg, uint32 queueId)
{
    if (guid.IsParty())
    {
        ...
        if (sLog->ShouldLog(LOG_FILTER_LFG, LOG_LEVEL_DEBUG))
        {
            ...
            sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::RestoreState: Group: %s (%s) State: %s, oldState: %s", guid.ToString().c_str(), debugMsg, ps.c_str(), os.c_str());
        }
        data.RestoreState();
    }
    else
    {
        ...
        if (sLog->ShouldLog(LOG_FILTER_LFG, LOG_LEVEL_DEBUG))
        {
            std::string const& ps = GetStateString(data.GetState());
            std::string const& os = GetStateString(data.GetOldState());
            sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::RestoreState: Player: [" UI64FMTD "] (%s) State: %s, oldState: %s", guid.GetCounter(), debugMsg, ps.c_str(), os.c_str());
        }
        data.RestoreState();
    }
}
...
void LFGMgr::SetState(ObjectGuid guid, LfgState state, uint32 queueId)
{
    if (guid.IsParty())
    {
        ...
        if (sLog->ShouldLog(LOG_FILTER_LFG, LOG_LEVEL_TRACE))
        {
            ...
            std::string const& ps = GetStateString(data.GetState());
            std::string const& os = GetStateString(data.GetOldState());
            sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::SetState: Group: [" UI64FMTD "] newState: %s, previous: %s, oldState: %s", guid.GetCounter(), ns.c_str(), ps.c_str(), os.c_str());
        }
        data.SetState(state);
    }
    else
    {
        ...
        if (sLog->ShouldLog(LOG_FILTER_LFG, LOG_LEVEL_TRACE))
        {
            ...
            std::string const& ps = GetStateString(data.GetState());
            std::string const& os = GetStateString(data.GetOldState());
            sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::SetState: Player: [" UI64FMTD "] newState: %s, previous: %s, oldState: %s", guid.GetCounter(), ns.c_str(), ps.c_str(), os.c_str());
        }
        data.SetState(state);
    }
}
...
{
    sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::SetSelectedDungeons: %s Dungeons: %s", guid.ToString().c_str(), ConcatenateDungeons(dungeons).c_str());
...
{
    sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::RemovePlayerData: %s", guid.ToString().c_str());
...
{
    sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::SetGroup: %s queueId %u", guid.ToString().c_str(), queueId);
    PlayersStore[guid][queueId].SetGroup(group);
}

void LFGMgr::SetLfgGroup(ObjectGuid guid, ObjectGuid group, uint32 queueId)
{
    sLog->outDebug(LOG_FILTER_LFG, "LFGMgr::SetLfgGroup: %s queueId %u", guid.ToString().c_str(), queueId);
...
{
    if (!sLFGMgr->HasQueue(guid))
        return false;

    ObjectGuid gguid;

    for (auto& queue : PlayerDungeons[guid])
    {
        queueId = queue;
        if (ObjectGuid gguid = GetLfgGroup(guid, queueId))
