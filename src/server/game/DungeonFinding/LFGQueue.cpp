...
LfgCompatibility LFGQueue::FindNewGroups(GuidList& check, GuidList& all)
{
    ...
    // sLog->outDebug(LOG_FILTER_LFG, "LFGQueue::FindNewGroup2: (%s): %s - all(%s)", strGuids.c_str(), GetCompatibleString(compatibles), ConcatenateGuids(all).c_str());
...
LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_LFG, "LFGQueue::CheckCompatibility: (%s) Compatibles but not enough players(%u) (%u/%u)", strGuids.c_str(), numPlayers, minGroupSize, maxGroupSize);
