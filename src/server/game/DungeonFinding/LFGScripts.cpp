...
void LFGPlayerScript::OnMapChanged(Player* player)
{
    ...
    if (sLFGMgr->inLfgDungeonMap(player->GetGUID(), map->GetId(), map->GetDifficultyID(), queueId))
    {
        ...
        if (!group)
        {
            ...
            sLog->outError(LOG_FILTER_LFG, "LFGPlayerScript::OnMapChanged, Player %s (%u) is in LFG dungeon map but does not have a valid group! Teleporting to homebind.", player->GetName(), player->GetGUIDLow());
            return;
        }
