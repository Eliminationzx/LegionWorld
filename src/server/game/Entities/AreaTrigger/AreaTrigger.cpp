...
polygon(0), VisualID(0), maxCount(0), maxActiveTargets(0), isMoving(false), RePatch(false)
{
}
...
AreaTrigger::AreaTrigger() : WorldObject(false), _range(0.0f), m_CastItem(nullptr), m_aura(nullptr), _caster(nullptr), _duration(0),
_activationDelay(0), _updateDelay(0), _scaleDelay(0), _sequenceDelay(0), _sequenceStep(0), _liveTime(0), _radius(1.0f), _realEntry(0),
_reachedDestination(false), _lastSplineIndex(0), _movementTime(0), _nextMoveTime(0), _waitTime(0), _on_unload(false), _on_despawn(false), _on_remove(false),
_hitCount(1), _areaTriggerCylinder(false), _canMove(false), movespline(new Movement::MoveSpline()), m_spellInfo(nullptr), m_spell(nullptr)
{
...
{
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::RemoveFromWorld Entry: %u CustomEntry: %u", GetRealEntry(), GetCustomEntry());
#endif
...
bool AreaTrigger::CreateAreaTrigger(ObjectGuid::LowType guidlow, uint32 triggerEntry, Unit* caster, SpellInfo const* info, Position const& pos, Position const& posMove,
    Spell* spell /*= nullptr*/, ObjectGuid targetGuid /*= ObjectGuid::Empty*/, uint32 customEntry /*= 0*/, ObjectGuid castID /*= ObjectGuid::Empty*/)
{
    m_spellInfo = info;
    m_spell = spell;
    ...
    if (!caster->IsInWorld())
    {
        // sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::CreateAreaTrigger guid %u, GetEntry() %u IsInWorld %u", caster->GetGUIDLow(), caster->GetEntry(), caster->IsInWorld());
        return false;
    }

    // if (!caster->isAlive())
    // {
        // sLog->outError(LOG_FILTER_GENERAL, "AreaTrigger (spell %u) caster %s is dead ", info ? info->Id : 0, caster->ToString().c_str());
        // return false;
    // }
    ...
    if (!IsPositionValid())
    {
        sLog->outError(LOG_FILTER_GENERAL, "AreaTrigger (spell %u) not created. Invalid coordinates (X: %f Y: %f)", info ? info->Id : 0, GetPositionX(), GetPositionY());
        return false;
    }
...
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger (spell %u) coordinates (X: %f Y: %f) _actionInfo %u", info ? info->Id : 0, GetPositionX(), GetPositionY(), _actionInfo.size());
#endif
    ...
    int32 duration = info ? info->GetDuration(caster->GetSpawnMode()) : -1;
    if (!duration)
        duration = -1;
    ...
    else if (Transport* transport = caster->GetTransport())
    {
        float x, y, z, o;
    ...
    if (info)
    {
        ...
        SetUInt32Value(AREATRIGGER_FIELD_SPELL_FOR_VISUALS, info->Id);
...
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::Create AreaTrigger caster %s spellID %u spell rage %f dist %f dest - X:%f,Y:%f,Z:%f _nextMoveTime %i duration %i",
        caster->GetGUID().ToString().c_str(), info ? info->Id : 0, _radius, GetSpellInfo() ? GetSpellInfo()->GetMaxRange() : 0.0f, _spline.VerticesPoints.empty() ? 0.0f : _spline.VerticesPoints[0].x, _spline.VerticesPoints.empty() ? 0.0f : _spline.VerticesPoints[0].y, _spline.VerticesPoints.empty() ? 0.0f : _spline.VerticesPoints[0].z, _nextMoveTime, duration);
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::Create isMoving %i _range %f _spline.VerticesPoints %i moveType %i", isMoving(), _range, _spline.VerticesPoints.size(), atInfo.moveType);
#endif
...
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger end (spell %u) coordinates (X: %f Y: %f)", info ? info->Id : 0, GetPositionX(), GetPositionY());
    ...
    if (actionM & AT_ACTION_MOMENT_ENTER)
    {
        ...
        for (auto& unit : unitList)
        {
            if (!IsUnitAffected(unit->GetGUID()))
            {
                ...
                {
                    // sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::UpdateAffectedList !CheckValidateTargets GetGUID %s", unit->GetGUID().ToString().c_str());
                    continue;
                }
...
{
    return atInfo.TargetRollPitchYawX != 0.0f || atInfo.TargetRollPitchYawY != 0.0f;
}
...
{
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "ActionOnUpdate");
#endif
...
void AreaTrigger::DoAction(Unit* unit, ActionInfo& action)
{
    ...
    if (!_caster || !unit || !action.charges && action.action->maxCharges)
        return;

#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::DoAction caster %s unit %s type %u spellID %u, moment %u, targetFlags %u",
    ...
    switch (action.action->actionType)
    {
        ...
        {
            SetSphereScale(action.action->scaleStep, atInfo.updateDelay, false, action.action->scaleMin, action.action->scaleMax);
            break;
        }
            ...
            if (unit->ToPlayer())
                if (unit->GetForceGUID().IsEmpty())
                    if (!unit->HasAuraType(SPELL_AURA_DISABLE_MOVEMENT_FORCE))
                        dynamic_cast<Player*>(unit)->SendMovementForce(this, Position(atInfo.windX, atInfo.windY, atInfo.windZ), atInfo.windSpeed, atInfo.windType, true);
            break;
        }
        case AT_ACTION_TYPE_REMOVE_MOVEMENT_FORCE: // 7
        {
            if (unit->ToPlayer())
                dynamic_cast<Player*>(unit)->SendMovementForce(this, Position(atInfo.windX, atInfo.windY, atInfo.windZ), atInfo.windSpeed, atInfo.windType);
            break;
        }
    ...
    // sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::DoAction action _hitCount %i hitCount %i hitMaxCount %i hitType %i actionType %i Duration %u", _hitCount, action.hitCount, action.action->hitMaxCount, atInfo.hitType, action.action->actionType, GetDuration());
...
void AreaTrigger::DoActionLeave(ActionInfo& action)
{
    ...
    // do not process depleted actions
    if (!_caster || !action.charges && action.action->maxCharges)
        return;

#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::DoActionLeave caster %s type %u spellID %u, moment %u, targetFlags %u",
...
void AreaTrigger::InitSplines()
{
    ...
    #ifdef WIN32
        sLog->outDebug(LOG_FILTER_AREATRIGGER, "InitSplines TimeToTarget %i _liveTime %u RePatchSpeed %f Speed %f movespline %s", _spline.TimeToTarget, _liveTime, atInfo.RePatchSpeed, atInfo.Speed, movespline->ToString().c_str());
    #endif
...
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::UpdateSplinePosition %f %f %f %i  _reachedDestination %u", GetPositionX(), GetPositionY(), GetPositionZ(), _movementTime, _reachedDestination);
...
void AreaTrigger::UpdateRotation(uint32 diff)
{
    ...
    //sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::UpdateRotation _waitTime %i _movementTime %i _nextMoveTime %i diff %i o %f", _waitTime, _movementTime, _nextMoveTime, diff, GetOrientation());
...
bool AreaTrigger::IsInPolygon(Unit* unit)
{
    ...
    // sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::IsInPolygon x_source %f y_source %f angle %f dist %f x %f y %f pred_ptX %f pred_ptY %f pred_q %u GetGUID %s", x_source, y_source, angle, dist, x, y, pred_pt.x, pred_pt.y, pred_q, unit->GetGUID().ToString().c_str());
...
float AreaTrigger::CalculateRadiusPolygon()
{
    ...
    // sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::CalculateRadiusPolygon distance %f", distance);
...
bool AreaTrigger::CheckValidateTargets(Unit* unit, AreaTriggerActionMoment /*actionM*/)
{
    ...
    for (auto & itr : _actionInfo)
    {
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_FRIENDLY)
            if (!caster || !caster->IsFriendlyTo(unit) || unit->isTotem())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_FRIENDLY unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_HOSTILE)
            if (!caster || !caster->IsHostileTo(unit))
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_HOSTILE unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_VALIDATTACK)
            if (!caster || !caster->IsValidAttackTarget(unit) || unit->isTotem())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_VALIDATTACK unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_OWNER)
            if (unit->GetGUID() != GetCasterGUID())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_OWNER unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_PLAYER)
            if (!unit->IsPlayer())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_PLAYER unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_NOT_PET)
            if (unit->isPet())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_NOT_PET unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_NOT_FULL_HP)
            if (unit->IsFullHealth())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_NOT_FULL_HP unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_GROUP_OR_RAID)
            ...
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_GROUP_OR_RAID unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_NOT_AURA_TARGET)
            if (GetTargetGuid() && GetTargetGuid() == unit->GetGUID())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_NOT_AURA_TARGET unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAT_IN_FRONT)
            if (!HasInArc(static_cast<float>(M_PI), unit))
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAT_IN_FRONT unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_TARGET_IS_SUMMONER)
            if (Unit* summoner = _caster->GetAnyOwner())
                if (unit->GetGUID() != summoner->GetGUID())
                {
                    // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_TARGET_IS_SUMMONER unit %s", unit->GetGUID().ToString().c_str());
                    continue;
                }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_NOT_OWNER)
            if (unit->GetGUID() == GetCasterGUID())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_NOT_OWNER unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        if (action.action->targetFlags & AT_TARGET_FLAG_NPC_ENTRY)
            if (action.amount != unit->GetEntry() || unit->ToPlayer())
            {
                // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets AT_TARGET_FLAG_NPC_ENTRY unit %s", unit->GetGUID().ToString().c_str());
                continue;
            }
        ...
        {
            // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets aura > 0 unit %s", unit->GetGUID().ToString().c_str());
            continue;
        }
        if (action.action->hasAura < 0 && unit->HasAura(abs(action.action->hasAura))
        ...
        if (action.action->hasspell > 0 && !_caster->HasSpell(action.action->hasspell))
        {
            // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CheckValidateTargets hasspell > 0 unit %s", unit->GetGUID().ToString().c_str());
            continue;
        }
        if (action.action->hasspell < 0 && _caster->HasSpell(abs(action.action->hasspell)))
        {
...
void AreaTrigger::GetCollisionPosition(Position &_dest, float dist, float angle)
{
...
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "GetCollisionPosition coordinates (X: %f Y: %f) ", pos.m_positionX, pos.m_positionY, pos.m_positionZ);
#endif
    ...
    {
    #ifdef WIN32
        sLog->outDebug(LOG_FILTER_AREATRIGGER, "GetCollisionPosition coordinates !col  dest (X: %f Y: %f) ", destx, desty, destz);
    #endif
...
#ifdef WIN32
    sLog->outDebug(LOG_FILTER_AREATRIGGER, "GetCollisionPosition coordinates  dest (X: %f Y: %f) Z %f", destx, desty, destz, GetPositionZ());
#endif
...
void AreaTrigger::SetSphereScale(float mod, uint32 time, bool absolute_change /*=false*/, float scaleMin /*= 0.1f*/, float scaleMax /*= 100.0f*/)
{
    if (mod == 0.0f)
        return;
    ...
    switch (atInfo.moveType)
    {
        ...
        case AT_MOVE_TYPE_SPIRAL: // 2
        {
            ...
            for (float a = 0;; a += _nextAngle) // next angel 45
            {
                ...
                Position nextPos{ m_positionX - r * sin(rad), m_positionY + r * cos(rad), m_positionZ };
                _spline.VerticesPoints.emplace_back(nextPos.GetPositionX(), nextPos.GetPositionY(), nextPos.GetPositionZ());
        ...
        case AT_MOVE_TYPE_RE_PATH_LOS: // 8
        {
            ...
            #ifdef WIN32
                sLog->outDebug(LOG_FILTER_AREATRIGGER, "AT_MOVE_TYPE_RE_PATH_LOS _timeToTarget %i _dest (%f %f %f) %f",
        ...
        case AT_MOVE_TYPE_PART_PATH: // 9
        {
            if (_range != 0.0f)
            {
                ...
                if (GetDistance(posMove) > 0.0f && (posMove.GetPositionX() != 0.0f || posMove.GetPositionY() != 0.0f))
                {
                    float angleToMove = GetAngle(&posMove);
...
void AreaTrigger::ReCalculateSplinePosition()
{
    switch (atInfo.moveType)
    {
        ...
        case AT_MOVE_TYPE_RE_PATH_TO_TARGET:
        {
            ...
            #ifdef WIN32
            sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::UpdateMovement AT_MOVE_TYPE_RE_PATH size %i _timeToTarget %i dist %f", _spline.VerticesPoints.size(), _spline.TimeToTarget, dist);
                DebugVisualizePosition();
            #endif
            break;
        }
        case AT_MOVE_TYPE_RE_PATH_LOS: //8
        {
            ...
            #ifdef WIN32
            sLog->outDebug(LOG_FILTER_AREATRIGGER, "AreaTrigger::UpdateMovement AT_MOVE_TYPE_RE_PATH_LOS size %i _timeToTarget %i _dest (%f %f %f) %f",
...
void AreaTrigger::CalculateCyclicPosition(Position const& pos, Position const& /*posMove*/, Unit* caster)
{
    if (!atInfo.circleTemplate.CanLoop)
    {
        ...
        _CircleData->BlendFormRadius = atInfo.circleTemplate.Radius;
        ...
        _CircleData->TargetGuid = boost::in_place();
        _CircleData->TargetGuid = caster->GetGUID();
        return;
    }
    ...
    float radius = atInfo.circleTemplate.IsDinamicRadius ? target->GetDistance(pos) : atInfo.circleTemplate.Radius;
    ...
    // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CalculateCyclicPosition first calcLenght %f startAngle %f x %f y %f lenght %f", calcLenght, startAngle, x, y, lenght);
    ...
    while (lenght > calcLenght)
    {
        if (calcLenght + onePartLenght < lenght)
        {
            ...
            _spline.VerticesPoints.emplace_back(x, y, 0.0f);
            // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CalculateCyclicPosition calcLenght %f calcAngle %f x %f y %f", calcLenght, calcAngle, x, y);
        }
        else
        {
            ...
            y = posY + radius * std::sin(calcAngle + startAngle);
            _spline.VerticesPoints.emplace_back(x, y, 0.0f);
            // sLog->outDebug(LOG_FILTER_AREATRIGGER, "CalculateCyclicPosition last calcLenght %f calcAngle %f x %f y %f lasPart %f", calcLenght, calcAngle, x, y, lasPart);
        }
    }
    ...
    _CircleData->BlendFormRadius = radius;
    _CircleData->InitialAngle = startAngle;

    if (atInfo.circleTemplate.HasTarget)
    {
        _CircleData->TargetGuid = boost::in_place();
        _CircleData->TargetGuid = target->GetGUID();
    }
    if (atInfo.circleTemplate.HasCenterPoint)
    {
        _CircleData->CenterPoint = boost::in_place();
        _CircleData->CenterPoint = Position(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ());
    }
    InitSplines();
}
