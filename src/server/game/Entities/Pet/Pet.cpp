...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "LoadPetFromDB petentry %i, petnumber %i, ownerid %i slot %i m_currentPet %i", petentry, petnumber, ownerid, owner->m_currentSummonedSlot, owner->m_currentPetNumber);
...
void Pet::SavePetToDB(bool isDelete)
{
    ...
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "SavePetToDB petentry %i, petnumber %i, slotID %i ownerid %i GetNativeDisplayId %u", GetEntry(), m_charmInfo->GetPetNumber(), curentSlot, GetOwnerGUID().GetGUIDLow(), GetNativeDisplayId());
...
void Pet::Update(uint32 diff)
{
    ...
    Unit* owner = GetOwner();
    if (!owner || !owner->IsPlayer() || m_Update)
        return;
...
{
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Pet::CastPetAuras guid %u, apply %u, GetEntry() %u IsInWorld %u", GetGUIDLow(), apply, GetEntry(), IsInWorld());
    ...
    if (std::vector<PetAura> const* petSpell = sSpellMgr->GetPetAura(GetEntry()))
    {
        for (const auto& itr : *petSpell)
        {
            ...
            // sLog->outDebug(LOG_FILTER_PETS, "Pet::CastPetAuras PetAura bp0 %i, bp1 %i, bp2 %i, target %i spellId %i option %u", bp0, bp1, bp2, itr->target, itr->spellId, itr->option);
...
void Pet::CheckSpecialization()
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Pet::CheckSpecialization newSpecId %i m_specialization %i overrideSpec %i", newSpecId, m_specialization, overrideSpec);
