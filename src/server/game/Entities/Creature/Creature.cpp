...
{
    return pProto->Flags2 & ITEM_FLAG2_DONT_IGNORE_BUY_PRICE || !ExtendedCost || ExtendedCost >= 15000;
}
...
VendorItem const* VendorItemData::FindItemCostPair(uint32 item_id, uint32 extendedCost, uint8 type) const
{
    for (auto itr : m_items)
        if (itr->item == item_id && itr->ExtendedCost == extendedCost && itr->Type == type)
            return itr;
    return nullptr;
}
...
Creature::Creature(bool isWorldObject) : Unit(isWorldObject), MapObject(), lootForPickPocketed(false), lootForBody(false), CreatureSpells(nullptr), m_groupLootTimer(0), m_PlayerDamageReq(0), m_actionData{}, m_CanCallAssistance(false), m_callAssistanceText(0),
        ...
        if (MaxVisible && !isAnySummons())
            if (Map* mapInfo = GetMap())
                if (!mapInfo->IsMapUnload())
                    mapInfo->m_MaxVisibleList.insert(this);
    }
}
...
void Creature::RemoveFromWorld()
{
    if (IsInWorld())
    {
        ...
        if (mapInfo && !mapInfo->IsMapUnload()) // Don`t delete if map unload, this is autoclear when delete map
        {
            ...
            if (MaxVisible && !isAnySummons())
                mapInfo->m_MaxVisibleList.erase(this);
...
bool Creature::UpdateEntry(uint32 entry, uint32 team, const CreatureData* data)
{
    ...
    uint32 SandboxScalingID = cInfo->SandboxScalingID;
...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, int32 vehId, uint32 team, float x, float y, float z, float ang, const CreatureData* data)
{
    ...
    if (cinfo->Classification > CREATURE_CLASSIFICATION_ELITE)
    {
    ...
    switch (GetAffixState())
    {
        case 0:  // The boss and his servants will not get the affix
        {
            if (IsDungeonBoss())
            {
                disableAffix = true;
            ...
            {
                if (Creature* owner = GetAnyOwner()->ToCreature())
                    if (owner->IsDungeonBoss() || owner->IsAffixDisabled())
                        disableAffix = true;
            }
            break;
        }
...
SpellInfo const* Creature::reachWithSpellAttack(Unit* victim)
{
    ...
    for (auto spellID : m_templateSpells)
    {
        ...
        if (!spellInfo)
        {
            sLog->outError(LOG_FILTER_UNITS, "WORLD: unknown spell id %i", spellID);
            continue;
        }
...
bool Creature::_IsTargetAcceptable(const Unit* target) const
{
    ...
    if (IsFriendlyTo(target)
        ...
        || (m_vehicle && m_vehicle->GetBase() && (IsOnVehicle(target) || m_vehicle->GetBase()->IsOnVehicle(target))))
        return false;
...
void Creature::StopAttack(bool clearMove /*= false*/)
{
    SetReactState(REACT_PASSIVE);
    AttackStop();
    if (!clearMove)
...
bool Creature::LoadCreaturesAddon(bool /*reload*/)
{
    ...
    if (!cainfo->auras.empty())
    {
        for (auto aura : cainfo->auras)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Creature (GUID: %u entry: %u) has wrong spell %u defined in `auras` field.", GetGUIDLow(), GetEntry(), aura);
                continue;
            }
            ...
            AddAura(aura, this);
            sLog->outDebug(LOG_FILTER_UNITS, "Spell: %u added to creature (GUID: %u entry: %u GetGUID %s)", aura, GetGUIDLow(), GetEntry(), GetGUID().ToString().c_str());
        }
    }
...
{
    return (GetTrackingQuestID() || isWorldBoss() || (GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_PERSONAL_LOOT)) && GetCreatureTemplate()->RequiredExpansion > EXPANSION_WARLORDS_OF_DRAENOR;
}
