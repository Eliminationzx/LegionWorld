...
bool Conversation::CreateConversation(ObjectGuid::LowType guidlow, uint32 triggerEntry, Unit* caster, SpellInfo const* info, Position const& pos)
{
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "Conversation (spell %u) not created. Suggested coordinates (X: %f Y: %f). Error on add to map.", info ? info->Id : 0, GetPositionX(), GetPositionY());
        return false;
    }
...
void Conversation::Remove()
{
    ...
    {
        //Hack for Q: 40522
        //! ToDo: make AI system for conversation and link on remove event there.
        if (GetEntry() == 800)
        {
            if (FindMap())
            {
                if (Player* p = ObjectAccessor::FindPlayer(GetCasterGUID()))
                {
                    p->CastSpell(p, 198897, true);
                    p->CastSpell(p, 200282, true);
                }
            }
        }
        RemoveFromWorld();
...
void Conversation::BuildDynamicValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    *data << uint8(blockCount);
    ...
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Item::BuildDynamicValuesUpdate GetEntry %u flags %u blockCount %u visibleFlag %u maskPos %u _dynamicValuesCount %u _changesMask %u _fieldNotifyFlags %u BlockSize %u",
    ...
    else if(isCreature)
    {
        for (const auto& itr : *conversationCreature)
        {
            ...
            if (itr.creatureId)
            {
                ...
                if (!creature)
                {
                    sLog->outDebug(LOG_FILTER_GENERAL, "Conversation not created. Suggested coordinates (X: %f Y: %f). Can't find npc entry %u", GetPositionX(), GetPositionY(),
                                   itr.creatureId);
                    return false;
                }
