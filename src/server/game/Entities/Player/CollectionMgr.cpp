...
AppearanceErrors CollectionMgr::CanAddAppearance(ItemModifiedAppearanceEntry const* itemModifiedAppearance) const
{
    ...
    switch (itemTemplate->GetClass())
    {
        case ITEM_CLASS_WEAPON:
            if (!(_owner->GetWeaponProficiency() & (1 << itemTemplate->SubClass)))
        ...
        case ITEM_CLASS_ARMOR:
            switch (itemTemplate->GetInventoryType())
            {
                ...
                case INVTYPE_HANDS:
                case INVTYPE_ROBE:
                    if (itemTemplate->SubClass == ITEM_SUBCLASS_ARMOR_MISCELLANEOUS)
                        return AppearanceErrors::CANT_EQUIP;
                    break;
                default:
                    return AppearanceErrors::CANT_EQUIP;
    ...
    if (itemTemplate->Quality < ITEM_QUALITY_UNCOMMON)
        if (!(itemTemplate->Flags2 & ITEM_FLAG2_IGNORE_QUALITY_FOR_ITEM_VISUAL_SOURCE) || !(itemTemplate->Flags3 & ITEM_FLAG3_ACTS_AS_TRANSMOG_HIDDEN_VISUAL_OPTION))
            return AppearanceErrors::INVALID_ITEM_TYPE;
...
void CollectionMgr::LoadMountDefinitions()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 mount definitions. DB table `mount_definitions` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!sDB2Manager.GetMount(spellID))
        {
            sLog->outError(LOG_FILTER_SQL, "Mount spell %u defined in `mount_definitions` does not exists in Mount.db2, skipped", spellID);
            continue;
        }

        if (otherSpellId && !sDB2Manager.GetMount(otherSpellId))
        {
            sLog->outError(LOG_FILTER_SQL, "OtherSpell %u defined in `mount_definitions` for spellID %u does not exists in Mount.db2, skipped", otherSpellId, spellID);
            continue;
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u mount definitions in %u ms", uint32(_mountDefinitions.size()), GetMSTimeDiffToNow(oldMSTime));
}
