...
typedef cds::container::FeldmanHashSet< cds::gc::HP, Item*, ItemHashAccessor > ItemUpdateSet;
...
typedef std::shared_ptr<AchievementMgr<Player>> AchievementPtr;
typedef std::shared_ptr<Garrison> GarrisonPtr;
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        bool CheckInvisibleStatusRequirements();
       ...
       // bool BuyChatLogoByTokens(std::string const& logo, uint32 cost);
        ...
        float SpellCooldownModByRate(SpellInfo const* spellInfo);
        ...
        uint32 GetSpellCategoryChargesTimer(SpellCategoryEntry const* categoryEntry, SpellInfo const* spellInfo = nullptr) const;
        ...
        uint8 GetBGLastActiveSpec() const;
        void SaveBGLastSpecialization();
        ...
        void DeleteGarrison();
        GarrisonPtr GetGarrisonPtr() { return _garrison; }
        ...
        /***              ANTICHEAT                            ***/
        /*********************************************************/
        AnticheatMgr* GetAnticheatMgr() { return &_anticheatMgr; }
        const AnticheatMgr* GetAnticheatMgr() const { return &_anticheatMgr; }
    ...
    protected:
        ...
        */
        ...
        ItemUpdateSet m_itemUpdate;
        ItemUpdateSet m_itemDelete;
    ...
    public:
        ...
        std::set<std::pair<ObjectGuid, uint32>> AllArtifacts; // for found lost artifacts
    ...
    private:
        ...
        GarrisonPtr _garrison;
        std::unique_ptr<CollectionMgr> _collectionMgr;
        AnticheatMgr _anticheatMgr;
...
public:
    explicit TeleportEvent(Player* owner, uint32 _mapId, float _x, float _y, float _z, float _o, uint32 _params, bool _lfg, bool _scheduleDelayed, uint32 delay) :
        mapId(_mapId), params(_params), m_delay(delay), x(_x), y(_y), z(_z), o(_o), lfg(_lfg), scheduleDelayed(_scheduleDelayed), m_owner(owner) { }

    explicit TeleportEvent(Player* owner, WorldLocation const& loc, uint32 _params, bool _lfg, bool _scheduleDelayed, uint32 delay) :
        mapId(loc.GetMapId()), params(_params), m_delay(delay), x(loc.GetPositionX()), y(loc.GetPositionY()), z(loc.GetPositionZ()), o(loc.GetOrientation()), lfg(_lfg), scheduleDelayed(_scheduleDelayed), m_owner(owner) { }
...
class CompleteQuestEvent : public BasicEvent
{
    public:
        CompleteQuestEvent(Player& owner, uint32 questID) : BasicEvent(), m_owner(owner), m_questID(questID) {}
        virtual bool Execute(uint64, uint32);
