...
void PlayerTaxi::AppendTaximaskTo(WorldPackets::Taxi::ShowTaxiNodes& data, bool all, uint16 mapID)
{
    // TaxiMask taximaskTemp;
    // taximaskTemp.fill(0);
    // for (TaxiNodesEntry const* node : sTaxiNodesStore)
    // {
        // if (node->MapID == mapID)
        // {
            // uint16 field = (uint16)((node->ID - 1) / 8);
            // uint32 submask = 1 << ((node->ID - 1) % 8);
            // if (m_taximask[field] & submask)
                // taximaskTemp[field] |= submask;
        // }
    // }

    // std::map<uint8, uint8> taximask;
    // bool apply = false;

    // for (int16 i = TaxiMaskSize - 1; i >= 0; --i)
    // {
        // if (!apply && taximaskTemp[i] != 0)
            // apply = true;

        // if (apply && !all)
            // taximask[i] = taximaskTemp[i];
        // if (apply && all)
            // taximask[i] = sTaxiNodesMask[i];
    // }

    // data.Nodes = taximask;                  // known nodes
