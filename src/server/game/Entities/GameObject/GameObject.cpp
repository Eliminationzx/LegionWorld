...
void GameObject::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (map && !map->IsMapUnload()) // Don`t delete if map unload, this is autoclear when delete map
        {
            ...
            if (MaxVisible && !GetOwnerGUID())
                map->m_MaxVisibleList.erase(this);
...
bool GameObject::Create(ObjectGuid::LowType guidlow, uint32 name_id, Map* map, uint32 phaseMask, Position const& pos, G3D::Quat const& rotation, uint32 animprogress, GOState go_state, uint32 artKit, uint32 aid, GameObjectData const* data)
{
    ...
    if (!IsPositionValid())
    {
        sLog->outError(LOG_FILTER_GENERAL, "Gameobject (GUID: %u Entry: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", guidlow, name_id, pos.GetPositionX(), pos.GetPositionY());
        return false;
    }
    ...
    if (!goinfo)
    {
        sLog->outError(LOG_FILTER_SQL, "Gameobject (GUID: %u Entry: %u) not created: non-existing entry in `gameobject_template`. Map: %u (X: %f Y: %f Z: %f)", guidlow, name_id, map->GetId(), pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
        return false;
    }
...
void GameObject::Use(Unit* user)
{
    ...
    if (!spellInfo)
    {
        if (!user->IsPlayer() || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
            ...
            sLog->outError(LOG_FILTER_GENERAL, "WORLD: %u non-dbc spell was handled by OutdoorPvP", spellId);
        return;
    }
...
bool GameObject::_IsWithinDist(WorldObject const* obj, float dist2compare, bool) const
{
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
        if (_fieldNotifyFlags & flags[index] || (updateType == UPDATETYPE_VALUES ? _changesMask[index] : m_uint32Values[index]) && flags[index] & visibleFlag || index == GAMEOBJECT_FIELD_FLAGS && forcedFlags)
        {
            ...
            else if (index == GAMEOBJECT_FIELD_DISPLAY_ID)
            {
                ...
                {
                    if (GarrisonPtr g = target->GetGarrisonPtr())
