...
void EventObject::Update(uint32 /*p_time*/)
{
    ...
    for (GuidSet::iterator itr = affectedPlayers.begin(), next; itr != affectedPlayers.end(); itr = next)
    {
        ...
        if (!unit || unit->GetDistance(this) > eventTemplate->radius)
    ...
    for (auto player : playerList)
    {
        ...
        sScriptMgr->OnEventObject(player, this);
...
{
    /*if (!who || !eventTemplate || !who->IsPlayer()) // Now check only for player
        return;

    if (eventTemplate->radius <= 0.0f)
        return;

    if (!who->InSamePhase(this))
        return;

    if (Player* player = who->ToPlayer())
    {
        if (who->GetDistance(this) <= eventTemplate->radius)
        {
            if (ActivatedForPlayer(player))
                return;
            ...
            {
                if (eventTemplate->SpellID)
                    who->CastSpell(who, eventTemplate->SpellID, true);
        ...
        else
        {
            ...
            if (eventTemplate->RemoveSpell)
                player->RemoveAura(eventTemplate->SpellID);
        }
    }*/
}
...
bool EventObject::LoadEventObjectFromDB(ObjectGuid::LowType guid, Map* map)
{
    ...
    if (!data)
    {
        sLog->outError(LOG_FILTER_SQL, "EventObject (GUID: %u) not found in table `eventobject`, can't load. ", guid);
        return false;
    }
    ...
    if (!IsPositionValid())
    {
        sLog->outError(LOG_FILTER_GENERAL, "EventObject (EventObject %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", data->id, GetPositionX(), GetPositionY());
        return false;
    }
...
bool EventObject::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, float x, float y, float z, float ang, float radius, uint32 spell, uint32 worldsafe)
{
    ...
    if (!IsPositionValid())
    {
        sLog->outError(LOG_FILTER_UNITS, "EventObject::Create(): given coordinates for eventobject (guidlow %d, entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)", guidlow, entry, x, y, z, ang);
        return false;
    }
