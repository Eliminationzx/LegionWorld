...
void Vehicle::InstallAccessory(VehicleAccessory const* as)
{
    ...
    sLog->outDebug(LOG_FILTER_VEHICLES, "Vehicle (GuidLow: %u, DB Guid: %u, Entry %u): installing accessory (Entry: %u) on seat: %d RecAura %u SummonedType %u",
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (Player* player = unit->ToPlayer())
    {
        ...
        for (SeatMap::iterator extraSeat = Seats.begin(); extraSeat != Seats.end(); ++extraSeat)
            if (extraSeat->second.Passenger)
            {
                player->AddToExtraLook(extraSeat->second.Passenger);
                if (Player* passenger = ObjectAccessor::GetPlayer(*_me, extraSeat->second.Passenger))
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    if (Player* player = unit->ToPlayer())
    {
        ...
        for (SeatMap::iterator extraSeat = Seats.begin(); extraSeat != Seats.end(); ++extraSeat)
            if (extraSeat->second.Passenger)
            {
                player->RemoveFromExtraLook(extraSeat->second.Passenger);
                if (Player* passenger = ObjectAccessor::GetPlayer(*_me, extraSeat->second.Passenger))
...
void Vehicle::TeleportAccessory(uint32 zoneId)
{
    for (SeatMap::iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    {
        if (itr->second.Passenger)
        {
            if(Creature* passenger = ObjectAccessor::GetCreature(*_me, itr->second.Passenger))
            {
                passenger->m_Teleports = true;
                passenger->CleanupBeforeTeleport();
                passenger->ResetMap();
                itr->second.unit = passenger;
            }
            else if (Player* player = ObjectAccessor::GetPlayer(*_me, itr->second.Passenger))
            {
...
bool VehicleJoinEvent::Execute(uint64, uint32)
{
    ...
    // sLog->outDebug(LOG_FILTER_UWOW_CORE, "VehicleJoinEvent::Execute loc(%f %f %f) veSeat(%f %f %f)",
    ...
    if (offset.m_positionX == 0.0f && offset.m_positionY == 0.0f && offset.m_positionZ == 0.0f)
        if (VehicleAttachmentOffset const* attachmentOffset = sObjectMgr->GetVehicleAttachmentOffset(vehicle))
...
void VehicleJoinEvent::Abort(uint64)
{
    ...
    if (targetBase)
    {
        ...
        sLog->outDebug(LOG_FILTER_VEHICLES, "Passenger GuidLow: %u, Entry: %u, board on uninstalled vehicle SeatId: %d cancelled", Passenger->GetGUIDLow(), Passenger->GetEntry(), static_cast<int32>(Seat->first));
    else
