...
float Player::GetTotalStatValue(Stats stat)
{
    if (HasPvpStatsScalingEnabled())
    {
        ...
        float bonusStat = value - baseStat;
    ...
    float baseStat = GetCreateStat(stat);
    float value = baseStat + m_auraModifiersGroup[unitMod][BASE_VALUE];
...
{
    SendUpdateStat(USM_AGILITY | USM_INTELLECT | USM_STRENGTH | USM_STAMINA | USM_ARMOR);
...
void Player::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    else
    {
        ...
        if (Item* offhand = GetWeaponForAttack(OFF_ATTACK, true))
            if (CanDualWield() || offhand->GetTemplate()->Flags3 & ITEM_FLAG3_ALWAYS_ALLOW_DUAL_WIELD)
...
void Player::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage, float& maxDamage)
{
    ...
    if (HasPvpStatsScalingEnabled())
    {
        if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
        {
            if (ItemTemplate const* temp = pItem->GetTemplate())
            {
                ...
                weaponMinDamage = (temp->Delay / 1000.f) * pvpDamageTemp;
                weaponMaxDamage = (temp->Delay / 1000.f) * pvpDamageTemp;
            }
        }
    }
    else
    ...
    if (shapeshift && shapeshift->CombatRoundTime)
    {
        ...
        weaponMinDamage = weaponMinDamage / weaponSpeed * attackPowerMod;
        weaponMaxDamage = weaponMaxDamage / weaponSpeed * attackPowerMod;
    }
...
{
    m_modMeleeHitChance = 7.5f + static_cast<float>(GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE));
...
{
    m_modRangedHitChance = 7.5f + static_cast<float>(GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE));
    m_modRangedHitChance += GetRatingBonusValue(CR_HIT_RANGED);
}

void Player::UpdateSpellHitChances()
{
    m_modSpellHitChance = 15.0f + static_cast<float>(GetTotalAuraModifier(SPELL_AURA_MOD_SPELL_HIT_CHANCE));
...
void Unit::UpdatePowerRegen(uint32 power)
{
    ...
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Unit::UpdatePowerRegen RegenPeace valOutCOmbat %f, powerIndex %i, power %i", valOutCOmbat, powerIndex, power);
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Unit::UpdatePowerRegen RegenCombat valInCOmbat %f, powerIndex %i, power %i", valInCOmbat, powerIndex, power);
    // sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Unit::UpdatePowerRegen powerIndex %i, power %i, RegenCombat %f, RegenPeace %f", powerIndex, power, powerEntry->RegenCombat, powerEntry->RegenPeace);
}
