...
bool Item::Create(ObjectGuid::LowType const& guidlow, uint32 itemid, Player const* owner)
{
    ...
    DescriptionID = itemProto->ItemNameDescriptionID;
...
void Item::SaveToDB(SQLTransaction& trans)
{
    ...
    switch (uState)
    {
       ...
       //     if (GetOwner())
     //           sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 1 item  guid = %u, entry = %u, %s", guid, GetEntry(), GetOwner()->GetInfoForDonate().c_str());
...
bool Item::LoadFromDB(ObjectGuid::LowType const& guid, ObjectGuid const& owner_guid, Field* fields, uint32 entry, uint8 oLevel)
{
    ...
    {
        if (proto->Flags & ITEM_FLAG_REAL_DURATION)
        {
            if (proto->Duration != 0)
    ...
    {
        if ((v->Flags3 & ITEM_FLAG3_ITEM_CAN_BE_UPGRADED) != 0)
        {
...
void Item::DeleteFromDB(SQLTransaction& trans, ObjectGuid::LowType itemGuid)
{
 ...
 /*   stmt = CharacterDatabase.GetPreparedStatement(CHAR_ITEM_DONATE_SET_STATE);
    ...
    sLog->outDebug(LOG_FILTER_DONATE, "[Status] Status = 1 item guid = %u can't find any information", itemGuid);
...
ItemRandomEnchantmentId Item::GenerateItemRandomPropertyId(uint32 item_id, uint32 spec_id)
{
    ...
    if (!itemProto || (!itemProto->HasStats() && !(itemProto->RandomProperty || itemProto->RandomSuffix)))
        return{};
    ...
    if (itemProto->RandomProperty)
        return GetItemEnchantMod(itemProto->RandomProperty, ItemRandomEnchantmentType::Property, item_id, spec_id);
    return GetItemEnchantMod(itemProto->RandomSuffix, ItemRandomEnchantmentType::Suffix, item_id, spec_id);
}
...
bool Item::IsFitToSpellRequirements(SpellInfo const* spellInfo) const
{
    ...
    {
        if (isEnchantSpell && proto->Flags3 & ITEM_FLAG3_CAN_STORE_ENCHANTS)
            return true;
...
{
    return (GetTemplate()->Class == ITEM_CLASS_WEAPON || GetTemplate()->Class == ITEM_CLASS_ARMOR) && GetTemplate()->GetInventoryType() != INVTYPE_NON_EQUIP &&
...
uint32 Item::GetBuyPrice(bool &success)
{
    ...
    return static_cast<uint32>(qualityPrice->Data * proto->PriceVariance * proto->PriceRandomValue * typeFactor * baseFactor);
}
...
uint32 Item::GetSellPrice()
{
    ...
    ItemClassEntry const* classEntry = sDB2Manager.GetItemClassByOldEnum(proto->Class);
    if (!classEntry)
        return 0;
...
void Item::SetScaleIlvl(uint32 ilvl)
{
    m_scaleLvl = ilvl;
}
...
bool Item::CanTransmogrifyItemWithItem(Item const* item, ItemModifiedAppearanceEntry const* itemModifiedAppearance)
{
    ...
    if (source->Flags3 & ITEM_FLAG2_IGNORE_QUALITY_FOR_ITEM_VISUAL_SOURCE) // Invisible item don`t check
        return true;
...
uint32 Item::GetItemLevel(uint8 ownerLevel, bool isPvP) const
{
    ...
    if (owner)
    {
        if (owner->HasTournamentRules())
            if (stats->Flags3 & ITEM_FLAG3_PVP_TOURNAMENT_GEAR)
...
bool Item::CanBeDisenchanted()
{
    ...
    if (itemTemplate->Flags2 & ITEM_FLAG2_DISENCHANT_TO_LOOT_TABLE)
        return true;

    if (itemTemplate->Flags & (ITEM_FLAG_CONJURED | ITEM_FLAG_NO_DISENCHANT) || itemTemplate->Bonding == BIND_QUEST_ITEM)
        return false;

    if (itemTemplate->Area || itemTemplate->Map || itemTemplate->GetMaxStackSize() > 1)
        return false;

    if (!GetSellPrice() && !sDB2Manager.HasItemCurrencyCost(itemTemplate->ItemId))
        return false;
...
void BonusData::Initialize(ItemTemplate const* proto)
{
    ...
    Bonding = static_cast<ItemBondingType>(proto->Bonding);
    RelicType = -1;
    AppearanceModID = 0;
    if (ItemModifiedAppearanceEntry const* defaultAppearance = sDB2Manager.GetDefaultItemModifiedAppearance(proto->ItemId))
...
{
    //sLog->outDebug(LOG_FILTER_NETWORKIO, "Item::ApplyItemChildEquipment apply %u", apply);
...
void Item::UpgradeLegendary()
{
    ...
    if (GetItemLevel(GetOwnerLevel()) < 940) // Only for legendary item from 7.0.3
    {
        AddBonuses(3459);
        AddBonuses(3530); // Upgrade Legendary item to 940 ilevel
        // AddBonuses(3570); // Upgrade Legendary item to 970 ilevel
        // AddBonuses(3630); // Upgrade Legendary item to 1000 ilevel
        SetState(ITEM_CHANGED, GetOwner());
    }
}
