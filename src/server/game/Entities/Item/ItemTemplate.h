...
struct _ItemStat
{
    uint32  ItemStatType;
    int32   ItemStatValue;
    int32   StatPercentEditor;
    float   StatPercentageOfSocket;
};
...
struct ItemTemplate
{
    ...
    float  GetStatScalingFactor() const { return DmgVariance/*ExtendedData->DmgVariance*/; }
    ...
    int32 GetItemStatAllocation(uint32 index) const { ASSERT(index < MAX_ITEM_PROTO_STATS); return ItemStat[index].StatPercentEditor/*ExtendedData->StatPercentEditor[index]*/; }
    float GetItemStatSocketCostMultiplier(uint32 index) const { ASSERT(index < MAX_ITEM_PROTO_STATS); return ItemStat[index].StatPercentageOfSocket/*ExtendedData->StatPercentageOfSocket[index]*/; }
    uint8 GetSocketColor(uint8 index) const { ASSERT(index < MAX_ITEM_PROTO_SOCKETS); return SocketColor[index]/*ExtendedData->SocketColor[index]*/; }
    ...
    uint8 GetExpansion() const { return ExpansionID; }
