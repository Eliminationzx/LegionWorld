...
{
public:
    std::unordered_map<uint32, EnchStoreList> _data;
} static ItemBonusEnch;

class EnchantmentItemBonusStore
{
public:
    std::unordered_map<uint32, EnchStoreList> _data;
} static ItemBonusAdditionEnch;
...
void LoadRandomEnchantmentsTable()
{
        ...
        do
        {
            ...
            switch (type)
            {
                case ItemRandomEnchantmentType::Property:
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "Property %u used in `item_enchantment_template` by entry %u doesn't have exist in ItemRandomProperties.db2", ench, entry);
                        continue;
                    }
                    break;
                case ItemRandomEnchantmentType::Suffix:
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "Suffix %u used in `item_enchantment_template` by entry %u doesn't have exist in ItemRandomSuffix.db2", ench, entry);
                        continue;
                    }
                    break;
                case ItemRandomEnchantmentType::BonusList:
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "Bonus list %u used in `item_enchantment_template` by entry %u doesn't have exist in ItemBonus.db2", ench, entry);
                        continue;
                    }
                    break;
                default:
                    sLog->outError(LOG_FILTER_SQL, "Invalid random enchantment type specified in `item_enchantment_template` table for `entry` %u `ench` %u", entry, ench);
                    break;
            }

            if (chance < 0.000001f || chance > 100.0f)
            {
                sLog->outError(LOG_FILTER_SQL, "Random item enchantment for entry %u type %u ench %u has invalid chance %f", entry, uint32(type), ench, chance);
                continue;
            }
    ...
    //                                      0      1        2
    result = WorldDatabase.Query("SELECT ItemID, BonusID, chance FROM item_enchantment_bonus");
    if (result)
    {
        uint32 count = 0;
        ...
        do
        {
            ...
            if (chance < 0.000001f || chance > 100.0f)
            {
                sLog->outError(LOG_FILTER_SQL, "Random item enchantment for ItemID %u BonusID %u has invalid chance %f", ItemID, BonusID, chance);
                continue;
            }

            if (BonusID == 40 || BonusID == 41 || BonusID == 42 || BonusID == 43) // Stats
                ItemBonusAdditionEnch._data[ItemID].emplace_back(ItemRandomEnchantmentType::BonusListAddition, BonusID, 22.71235f);
            else
                ItemBonusEnch._data[ItemID].emplace_back(ItemRandomEnchantmentType::BonusList, BonusID, chance);

            ++count;
        } while (result->NextRow());

        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Item Enchantment Bonus definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Item Enchantment Bonus definitions. DB table `item_enchantment_bonus` is empty.");
}
...
ItemRandomEnchantmentId GetItemEnchantMod(int32 entry, ItemRandomEnchantmentType type, uint32 ItemID, uint32 spec_id)
{
    ...
    {
        auto tabItem = ItemBonusEnch._data.find(ItemID);
        if (tabItem != ItemBonusEnch._data.end())
            enchList = tabItem->second;
    }

    if (type == ItemRandomEnchantmentType::BonusListAddition)
    {
        auto tabItem = ItemBonusAdditionEnch._data.find(ItemID);
        if (tabItem != ItemBonusAdditionEnch._data.end())
            enchList = tabItem->second;
    ...
    if (enchList.empty())
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Item RandomProperty / RandomSuffix id #%u spec_id %u used in `item_template` but it does not have records in `item_enchantment_template` table.", entry, spec_id);
            return{};
        }
        enchList = tab->second;
    }
