...
struct Position
{
    ...
    void Relocate(float x, float y, float z, float orientation, float positionH);
    ...
    Position operator-(const Position& rkVector) const;
    ...
    bool IsPointInBox(Position centerBox, std::vector<Position> box, Position point) const;
...
public:
    ...
    explicit WorldLocation(uint32 _mapid, Position pos);
