...
WorldObject::~WorldObject()
{
    ...
    if (IsWorldObject() && m_currMap)
    {
        if (IsCorpse())
        {
            sLog->outFatal(LOG_FILTER_GENERAL, "Object::~Object Corpse guid=" UI64FMTD ", type=%d, entry=%u deleted but still in map!!", GetGUID().GetCounter(), ToCorpse()->GetType(), GetEntry());
            ASSERT(false);
        }
...
Object::~Object()
{
    if (IsInWorld())
    {
        sLog->outFatal(LOG_FILTER_GENERAL, "Object::~Object - guid=" UI64FMTD ", typeid=%d, entry=%u deleted but still in world!!", GetGUID().GetCounter(), GetTypeId(), GetEntry());
        if (isType(TYPEMASK_ITEM))
            sLog->outFatal(LOG_FILTER_GENERAL, "Item slot %u",ToItem()->GetSlot());
        //ASSERT(false);
    ...
    if (m_objectUpdated)
    {
        sLog->outFatal(LOG_FILTER_GENERAL, "Object::~Object - guid=" UI64FMTD ", typeid=%d, entry=%u deleted but still in update list!!", GetGUID().GetCounter(), GetTypeId(), GetEntry());
        //ASSERT(false);
    }
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "BuildCreateUpdateBlockForPlayer start UPDATEFLAG_SELF wrong flags %u for object GUID %s", flags, GetGUID().ToString().c_str());
        return;
    }
    if ((flags & UPDATEFLAG_HAS_WORLDEFFECTID) != 0 && !ToGameObject())
    {
        sLog->outError(LOG_FILTER_GENERAL, "BuildCreateUpdateBlockForPlayer start UPDATEFLAG_HAS_WORLDEFFECTID wrong flags %u for object GUID %s", flags, GetGUID().ToString().c_str());
        return;
    }
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "BuildCreateUpdateBlockForPlayer start UPDATEFLAG_ANIMKITS wrong flags %u for object GUID %s", flags, GetGUID().ToString().c_str());
        return;
    }
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "BuildCreateUpdateBlockForPlayer end UPDATEFLAG_SELF wrong flags %u for object GUID %s", flags, GetGUID().ToString().c_str());
        return;
    }
    if ((flags & UPDATEFLAG_HAS_WORLDEFFECTID) != 0 && !ToGameObject())
    {
        sLog->outError(LOG_FILTER_GENERAL, "BuildCreateUpdateBlockForPlayer end UPDATEFLAG_HAS_WORLDEFFECTID wrong flags %u for object GUID %s", flags, GetGUID().ToString().c_str());
        return;
    }
    if ((flags & UPDATEFLAG_ANIMKITS) != 0 && !ToWorldObject()->GetAIAnimKitId() && !ToWorldObject()->GetMovementAnimKitId() && !ToWorldObject()->GetMeleeAnimKitId())
    {
        sLog->outError(LOG_FILTER_GENERAL, "BuildCreateUpdateBlockForPlayer end UPDATEFLAG_ANIMKITS wrong flags %u for object GUID %s", flags, GetGUID().ToString().c_str());
        return;
    }
...
WorldObject::WorldObject(bool isWorldObject): LastUsedScriptID(0), m_name(""), m_isActive(false), m_isWorldObject(isWorldObject), m_zoneScript(nullptr), m_transport(nullptr), m_InstanceId(0), m_phaseMask(PHASEMASK_NORMAL), m_ignorePhaseIdCheck(false)
{
...
bool WorldObject::IsWithinDist(WorldObject const* obj, float dist2compare, bool is3D) const
{
    return obj && _IsWithinDist(obj, dist2compare, is3D);
}

bool WorldObject::IsWithinDistInMap(WorldObject const* obj, float dist2compare, bool is3D) const
{
...
bool WorldObject::_IsWithinDist(WorldObject const* obj, float dist2compare, bool is3D) const
{
    ...
    {
        if (auto cmodelData = sCreatureDisplayInfoStore.LookupEntry(obj->ToUnit()->GetDisplayId()))
            targetObjectSize = cmodelData->CreatureModelScale;
        else
            targetObjectSize = obj->IsPlayer() ? 1.0f : 1.5f;
    }

    float sizefactor = GetObjectSize() + targetObjectSize;
...
void WorldObject::Talk(uint32 textId, ChatMsg msgType, float textRange, WorldObject const* target)
{
    ...
    uint32 fSound = [bct, unit]() -> uint32
    {
        ...
        return gender == GENDER_MALE ? bct->SoundEntriesID[0] : bct->SoundEntriesID[1];
    }();
...
void WorldObject::MovePositionToTransportCollision(Position &pos, float dist, float angle)
{
    ...
    if (!transport)
    {
        // sLog->outFatal(LOG_FILTER_GENERAL, "WorldObject::MovePositionToTransportCollision invalid transport");
        return;
    }
    ...
    if (!_model)
    {
        // sLog->outFatal(LOG_FILTER_GENERAL, "WorldObject::MovePositionToTransportCollision invalid transport GameObjectModel");
        return;
    }
    ...
    if (!Trinity::IsValidMapCoord(destx, desty))
    {
        // sLog->outFatal(LOG_FILTER_GENERAL, "WorldObject::MovePositionToTransportCollision invalid coordinates X: %f and Y: %f were passed!", destx, desty);
        return;
    }
    ...
    if (destz == VMAP_INVALID_HEIGHT_VALUE)
    {
        sLog->outFatal(LOG_FILTER_GENERAL, "WorldObject::MovePositionToTransportCollision invalid destz: %f", destz);
        return;
    }
...
void WorldObject::MovePositionToCollisionBetween(Position &pos, float distMin, float distMax, float angle)
{
    ...
    if (!Trinity::IsValidMapCoord(destx, desty))
    {
        // sLog->outFatal(LOG_FILTER_GENERAL, "WorldObject::MovePositionToCollisionBetween invalid coordinates X: %f and Y: %f were passed!", destx, desty);
        return;
    }
