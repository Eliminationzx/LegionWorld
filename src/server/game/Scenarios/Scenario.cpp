...
Scenario::Scenario(Map* map, lfg::LFGDungeonData const* _dungeonData, Player* player, bool find) : m_achievementMgr(this)
{
    ...
    if (!_scenarioEntry)
    {
        ...
        // sLog->outDebug(LOG_FILTER_CHALLENGE, "Scenario::Scenario instanceId %u scenarioId %u dungeonData %u", instanceId, scenarioId, dungeonData->map);
        return;
    }
...
void Scenario::CreateChallenge(Player* player, Item* item)
{
    // sLog->outDebug(LOG_FILTER_CHALLENGE, "CreateChallenge player %u map %u", bool(player), bool(GetMap()));
    ...
    if (!group || !group->m_challengeEntry)
        return;

    _challenge = new Challenge(map, player, GetInstanceId(), this);

    // sLog->outDebug(LOG_FILTER_CHALLENGE, "CreateChallenge _challenge %u _canRun %u", bool(_challenge), bool(_challenge->_canRun));
    ...
    switch (group->m_challengeEntry->ID)
    {
    ...
    {
        // sLog->outDebug(LOG_FILTER_CHALLENGE, "CreateChallenge steps %u currentStep %u", steps.size(), currentStep);
        return;
    }
    ...
    // sLog->outError(LOG_FILTER_CHALLENGE, "%s %u, mapID: %u, scenarioID: %u", __FUNCTION__, __LINE__, map->GetId(), scenarioId);
}
...
uint8 Scenario::UpdateCurrentStep(bool loading)
{
    ...
    // sLog->outDebug(LOG_FILTER_CHALLENGE, "UpdateCurrentStep currentStep %u oldStep %u loading %u steps %u", currentStep, oldStep, loading, steps.size());
    ...
    // i_updateLock.unlock();
    //sLog->outDebug(LOG_FILTER_CHALLENGE, "UpdateCurrentStep currentStep %u oldStep %u loading %u", currentStep, oldStep, loading);
    return currentStep;
}
...
{
    // sLog->outDebug(LOG_FILTER_CHALLENGE, "Scenario::Reward bonus %u rewarded %u bonusRewarded %u rewardStep %u", bonus, rewarded, bonusRewarded, rewardStep);
    ...
    if (pvpMap)
    {
        pvpMap->ApplyOnEveryPlayerInZone([&](Player* player)
        {
    ...
    // sLog->outDebug(LOG_FILTER_CHALLENGE, "Reward Type %u", _scenarioEntry->Type);
...
void Scenario::SendCriteriaUpdate(CriteriaProgress const* progress)
{
    //sLog->outDebug(LOG_FILTER_CHALLENGE, "Scenario::SendCriteriaUpdate criteria %u Counter %u", progress->criteriaTree->criteria, progress->Counter);
