...
void GarrisonMgr::LoadMissionsRewards()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 mission reward. DB table `mission_reward` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u mission rewards in %u.", uint32(_garrMissionRewardByMissionID.size()), GetMSTimeDiffToNow(msTime));
}
...
void GarrisonMgr::LoadMissionsOwermaxRewards()
{
    ...
    //if (!result)
    //{
    //    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 mission owermax reward. DB table `mission_reward_owermax` is empty.");
    //    return;
    //}
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u mission owermax rewards in %u.", uint32(_garrMissionOwermaxRewardByMissionID.size()), GetMSTimeDiffToNow(msTime));
}
...
void GarrisonMgr::LoadMissionsQuestLink()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 mission quest reward. DB table `mission_quest_link` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Non-existing sGarrMissionStore missionID %u was referenced in `mission_quest_link`.", missionID);
            continue;
        }
        ...
        {
            sLog->outError(LOG_FILTER_SQL, "Non-existing quest %u was referenced in `mission_quest_link`.", quest_id);
            continue;
        }
        _quest_mission_link_store[quest_id] = missionID;
    } while (result->NextRow());

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u mission rewards in %u.", uint32(_quest_mission_link_store.size()), GetMSTimeDiffToNow(msTime));
}
...
void GarrisonMgr::LoadPlotFinalizeGOInfo()
{
    ...
    do
    {
        ...
        if (!goTemplate)
        {
            sLog->outError(LOG_FILTER_SQL, "Non-existing gameobject_template entry %u was referenced in `garrison_plot_finalize_info`.`hordeGameObjectId` for garrPlotInstanceId %u goID %u.", hordeGameObjectId, garrPlotInstanceId, hordeGameObjectId);
            continue;
        }

        if (goTemplate->type != GAMEOBJECT_TYPE_GOOBER)
        {
            sLog->outError(LOG_FILTER_SQL, "Invalid gameobject type %u (entry %u) was referenced in `garrison_plot_finalize_info`.`hordeGameObjectId` for garrPlotInstanceId %u.", goTemplate->type, hordeGameObjectId, garrPlotInstanceId);
            continue;
        }
        ...
        if (!goTemplate)
        {
            sLog->outError(LOG_FILTER_SQL, "Non-existing gameobject_template entry %u was referenced in `garrison_plot_finalize_info`.`allianceGameObjectId` for garrPlotInstanceId %u goID %u.", allianceGameObjectId, garrPlotInstanceId, allianceGameObjectId);
            continue;
        }

        if (goTemplate->type != GAMEOBJECT_TYPE_GOOBER)
        {
            sLog->outError(LOG_FILTER_SQL, "Invalid gameobject type %u (entry %u) was referenced in `garrison_plot_finalize_info`.`allianceGameObjectId` for garrPlotInstanceId %u.", goTemplate->type, allianceGameObjectId, garrPlotInstanceId);
            continue;
        }
