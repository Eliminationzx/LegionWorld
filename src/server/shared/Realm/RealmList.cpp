...
void RealmList::UpdateRealms(boost::system::error_code const& error)
{
    ...
    sLog->outTrace(LOG_FILTER_REALMLIST, "Updating Realm List...");
    ...
    if (result)
    {
        do
        {
            try
            {
                ...
                if (endPoint == end || ec)
                {
                    sLog->outError(LOG_FILTER_REALMLIST, "Could not resolve address %s for realm \"%s\" id %u", fields[2].GetString().c_str(), name.c_str(), realmId);
                    continue;
                }
                ...
                if (endPoint == end || ec)
                {
                    sLog->outError(LOG_FILTER_REALMLIST, "Could not resolve localAddress %s for realm \"%s\" id %u", fields[3].GetString().c_str(), name.c_str(), realmId);
                    continue;
                }
                ...
                if (endPoint == end || ec)
                {
                    sLog->outError(LOG_FILTER_REALMLIST, "Could not resolve localSubnetMask %s for realm \"%s\" id %u", fields[4].GetString().c_str(), name.c_str(), realmId);
                    continue;
                }
                ...
                if (!existingRealms.count(id))
                    sLog->outInfo(LOG_FILTER_REALMLIST, "Added realm \"%s\" at %s:%u.", name.c_str(), externalAddress.to_string().c_str(), port);
                else
                    sLog->outDebug(LOG_FILTER_REALMLIST, "Updating realm \"%s\" at %s:%u.", name.c_str(), externalAddress.to_string().c_str(), port);
    ...
    for (auto & existingRealm : existingRealms)
        sLog->outInfo(LOG_FILTER_REALMLIST, "Removed realm \"%s\".", existingRealm.second.c_str());
