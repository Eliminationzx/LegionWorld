...
uint32 ip2uint(char* ip)
{
    uint32 ip2uint = 0;

    uint32 D1 = atoi(strtok(ip, "."));
    uint32 D2 = atoi(strtok(NULL, "."));
    uint32 D3 = atoi(strtok(NULL, "."));
    uint32 D4 = atoi(strtok(NULL, ""));

    if (D1 > 255 || D2 > 255 || D3 > 255 || D4 > 255)
        return 0;

    ip2uint = D1 * 256 * 256 * 256 + D2 * 256 * 256 + D3 * 256 + D4;
