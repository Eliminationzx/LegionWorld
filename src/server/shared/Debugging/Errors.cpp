...
void DumpHandler(int signum)
{
    ...
    {
        std::this_thread::sleep_for(Milliseconds(60000)); // Waiting when gdb is dump this thread
        signal(signum, SIG_DFL);
    }
    else
    {
        m_worldCrashChecker = true;
        sLog->outError(LOG_FILTER_WORLDSERVER, "DumpHandler m_worldCrashChecker %u", m_worldCrashChecker);
        ...
        std::string buffer("gdb --pid " + std::to_string(GetPID()) + " -x crashed.gdb --batch>./errorlog/" + std::string(buf) + "_crashed.log 2>./errorlog/" + std::string(buf) + "_crashed.err");
        system(buffer.c_str());
        #endif
