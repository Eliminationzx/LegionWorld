...
void Log::EnableDBAppenders()
{
    for (auto & appender : appenders)
        if (appender.second && appender.second->getType() == APPENDER_DB)
            static_cast<AppenderDB *>(appender.second)->setEnable(true);
}
...
void Log::write(LogMessage* msg)
{
    ...
    auto logOperation = std::make_shared<LogOperation>(logger, msg);
...
void Log::LoadFromConfig()
{
    ...
    mapInfoFile = openLogFile("MapInfo.LogFile", NULL, "a");
