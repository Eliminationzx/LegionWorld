...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomPropertyType, ii.randomPropertyId, " \
        ...
        "im.fixedScalingLevel, im.artifactKnowledgeLevel, iia.xp, iia.artifactAppearanceId, iia.tier, ii.dungeonEncounterID, ii.contextID"
    ...
    PrepareStatement(CHAR_REP_BATTLEGROUND_RANDOM, "REPLACE INTO character_battleground_random (guid, bg, rbg, arena2v2, arena3v3, skirmish) VALUES (?,?,?,?,?,?)", CONNECTION_ASYNC);
    ...
    "health, mana, instance_id, activespec, specialization, exploredZones, equipmentCache, knownTitles, actionBars, "
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_RANDOMBG, "SELECT bg, rbg, arena2v2, arena3v3, skirmish FROM character_battleground_random WHERE guid = ?", CONNECTION_ASYNC);
    ...
    "latency, activespec, specialization, exploredZones, equipmentCache, knownTitles, actionBars, currentpetnumber, petslot, grantableLevels, created_time, killPoints) VALUES "
    "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    ...
    "watchedFaction=?,health=?,mana=?,latency=?,activespec=?,specialization=?,exploredZones=?,"
...
#define PETBATTLE_FULL_FIELDS "id, " PETBATTLE_FIELDS
    ...
    PrepareStatement(CHAR_INS_CHARACTER_GARRISON, "REPLACE INTO character_garrison (guid, siteLevelId, followerActivationsRemainingToday, lastResTaken, _MissionGen) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHARACTER_GARRISON, "UPDATE character_garrison SET siteLevelId = ? WHERE guid = ? AND siteLevelId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHARACTER_GARRISON, "DELETE FROM character_garrison WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHARACTER_GARRISON_ONLY, "DELETE FROM character_garrison WHERE guid = ? and siteLevelId in (5, 258, 444, 445, 6, 259)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHARACTER_GARRISON_BY_SITE, "DELETE FROM character_garrison WHERE guid = ? AND siteLevelId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_WORLD_STATE_BY_STATE_INSTANCE, "DELETE FROM worldstate_data WHERE VariableID = ? AND InstanceID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_WORLD_STAT, "INSERT INTO `worldstate_data` (`VariableID`, `InstanceID`, `Type`, `ConditionID`, `Flags`, `Value`, `RenewTime`) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
