...
bool SQLQueryHolder::SetQuery(size_t index, const char *sql)
{
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "Query index (%u) out of range (size: %u) for query: %s", uint32(index), static_cast<uint32>(m_queries.size()), sql);
        return false;
    }
...
bool SQLQueryHolder::SetPreparedQuery(size_t index, PreparedStatement* stmt)
{
    if (m_queries.size() <= index)
    {
        sLog->outError(LOG_FILTER_SQL, "Query index (%u) out of range (size: %u) for prepared statement", index, static_cast<uint32>(m_queries.size()));
        return false;
    }
