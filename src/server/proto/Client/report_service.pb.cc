...
void ReportService::SendReport(::bgs::protocol::report::v1::SendReportRequest const* request, std::function<void(::bgs::protocol::NoData const*)> responseCallback) {
  sLog->outDebug(LOG_FILTER_PROTOBUF, "%s Server called client method ReportService.SendReport(bgs.protocol.report.v1.SendReportRequest{ %s })",
...
void ReportService::SubmitReport(::bgs::protocol::report::v1::SubmitReportRequest const* request, std::function<void(::bgs::protocol::NoData const*)> responseCallback) {
  sLog->outDebug(LOG_FILTER_PROTOBUF, "%s Server called client method ReportService.SubmitReport(bgs.protocol.report.v1.SubmitReportRequest{ %s })",
    GetCallerInfo().c_str(), request->ShortDebugString().c_str());
  std::function<void(MessageBuffer)> callback = [responseCallback](MessageBuffer buffer) -> void {
    ::bgs::protocol::NoData response;
    if (response.ParseFromArray(buffer.GetReadPointer(), buffer.GetActiveSize()))
  ...
  switch(methodId) {
    case 1: {
      ...
      if (!request.ParseFromArray(buffer.GetReadPointer(), buffer.GetActiveSize())) {
        sLog->outDebug(LOG_FILTER_PROTOBUF, "%s Failed to parse request for ReportService.SendReport server method call.", GetCallerInfo().c_str());
      ...
      sLog->outDebug(LOG_FILTER_PROTOBUF, "%s Client called server method ReportService.SendReport(bgs.protocol.report.v1.SendReportRequest{ %s }) returned bgs.protocol.NoData{ %s } status %u.",
    ...
    case 2: {
      ...
      if (!request.ParseFromArray(buffer.GetReadPointer(), buffer.GetActiveSize())) {
        sLog->outDebug(LOG_FILTER_PROTOBUF, "%s Failed to parse request for ReportService.SubmitReport server method call.", GetCallerInfo().c_str());
      ...
      sLog->outDebug(LOG_FILTER_PROTOBUF, "%s Client called server method ReportService.SubmitReport(bgs.protocol.report.v1.SubmitReportRequest{ %s }) returned bgs.protocol.NoData{ %s } status %u.",
        GetCallerInfo().c_str(), request.ShortDebugString().c_str(), response.ShortDebugString().c_str(), status);
      if (!status)
...
uint32 ReportService::HandleSendReport(::bgs::protocol::report::v1::SendReportRequest const* request, ::bgs::protocol::NoData* response) {
  sLog->outError(LOG_FILTER_PROTOBUF, "%s Client tried to call not implemented method ReportService.SendReport({ %s })",
    GetCallerInfo().c_str(), request->ShortDebugString().c_str());
...
uint32 ReportService::HandleSubmitReport(::bgs::protocol::report::v1::SubmitReportRequest const* request, ::bgs::protocol::NoData* response) {
  sLog->outError(LOG_FILTER_PROTOBUF, "%s Client tried to call not implemented method ReportService.SubmitReport({ %s })",
    GetCallerInfo().c_str(), request->ShortDebugString().c_str());
  return ERROR_RPC_NOT_IMPLEMENTED;
}
